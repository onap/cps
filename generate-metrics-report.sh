...
# DESC: Generate report of prometheus metrics
# ARGS: None
# OUTS: None
function generate_report() {
  ...
  grep "_count" $TEMP_DIR/metrics-cps.txt | sed 's/_count//' | cut -f 1,2 >$TEMP_DIR/column1and2.txt
  grep "_sum" $TEMP_DIR/metrics-cps.txt | sed 's/_sum//' | cut -f 2 >$TEMP_DIR/column3.txt
  grep "_max" $TEMP_DIR/metrics-cps.txt | sed 's/_max//' | cut -f 2 >$TEMP_DIR/column4.txt

  # Combine columns into report.
  paste $TEMP_DIR/column{1and2,3,4}.txt >$TEMP_DIR/report.txt

  # Sort by Sum (column 3), descending.
  sort --numeric-sort --reverse --field-separator=$'\t' --key=3 $TEMP_DIR/report.txt >$TEMP_DIR/report-sorted.txt
