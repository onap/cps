...
This module is listed in the root aggregator `pom.xml` so the pre-built JAR is installed into the
local Maven repository as part of the normal `mvn install`. No code generation happens during the
normal build — sources are only generated when explicitly running the `generate-provmns-api` profile.
...
6. Commit the updated `pom.xml` files, the new JAR, and any changes to hand-written source files under `src/main/java/`. Do **not** commit generated sources from `target/`.
