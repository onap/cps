...
# Absolute path to this script's directory (provmns-api/)
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
# Absolute path to the repo root (parent of provmns-api/)
REPO_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"
# Extract the ProvMnS spec version from the module pom.xml (provmns.lib.version property)
PROVMNS_SPEC_VERSION=$(mvn help:evaluate -Dexpression=provmns.lib.version -q -DforceStdout -f "$SCRIPT_DIR/pom.xml")

echo ">>> Cleaning previous build artifacts..."
rm -rf "$SCRIPT_DIR/target"
rm -f "$SCRIPT_DIR/local-repo/org/onap/cps/provmns-api/${PROVMNS_SPEC_VERSION}/provmns-api-${PROVMNS_SPEC_VERSION}.jar"

echo ">>> Step 1: Generating sources from 3GPP spec and compiling to local-repo (version: $PROVMNS_SPEC_VERSION)..."
...
echo "    provmns-api/local-repo/org/onap/cps/provmns-api/${PROVMNS_SPEC_VERSION}/provmns-api-${PROVMNS_SPEC_VERSION}.jar"
