...
import static org.onap.cps.integration.DmiStubTestContainer.DMI_STUB_URL
...
class NcmpWriteSubJobSpec extends CpsIntegrationSpecBase {
    ...
    def 'Create a sub-job write request.'() {
        ...
        when: 'the data job service is called'
            dataJobService.writeDataJob(dataJobId, myDataJobMetadata, dataJobWriteRequest)
        and: 'the same method is called'
            def response = dataJobService.writeDataJob(dataJobId, myDataJobMetadata, dataJobWriteRequest)
        then: 'the result has the expected size and return type with the expected sub job id'
            assert response.size() == 1
            assert response[0].class == SubJobWriteResponse.class
            assert response[0].subJobId() == '2'
