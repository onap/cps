...
export function legacyBatchConsumeScenario() {
    try {
        const messages = legacyBatchEventReader.consume({ limit: 500, expectTimeout: true });
        const timestamp = (new Date()).toISOString();
        legacyBatchReadTrend.add(messages.length);
        console.log(`✅ At ${timestamp} consumed ${messages.length} messages by legacy batch read`);
    } catch (error) {
        console.error(`❌ Consume error (legacy batch read): ${error.message}`);
    }
}
