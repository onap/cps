...
# Function to create zip archive from collected logs
create_log_archive() {
  local service_name="$1"
  local temp_dir="$2"
  local zip_file="$3"
...
# Function to archive logs for a Docker service
archive_docker_service_logs() {
  local service_name="$1"
  local temp_dir="$LOG_DIR/temp_docker_${service_name}_$TIMESTAMP"
  local zip_file="$LOG_DIR/logs_docker_${service_name}_$TIMESTAMP.zip"
...
# Function to archive logs for a Kubernetes service
archive_k8s_service_logs() {
  local service_name="$1"
  local temp_dir="$LOG_DIR/temp_k8s_${service_name}_$TIMESTAMP"
  local zip_file="$LOG_DIR/logs_k8s_${service_name}_$TIMESTAMP.zip"

  echo "Processing service: $service_name"
  mkdir -p "$temp_dir"

  # Fetch pod logs for the service
...
case "$deploymentType" in
  "dockerHosts")
    ...
    for service_name in "${DOCKER_SERVICES_TO_BE_LOGGED[@]}"; do
      archive_docker_service_logs "$service_name"
    done
  ...
  "k8sHosts")
    ...
    for service_name in "${K8S_SERVICES_TO_BE_LOGGED[@]}"; do
      archive_k8s_service_logs "$service_name"
    done
