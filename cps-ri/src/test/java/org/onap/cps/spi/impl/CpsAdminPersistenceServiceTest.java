/*
 *  ============LICENSE_START=======================================================
 *  Copyright (C) 2020 Pantheon.tech
 *  ================================================================================
 *  Licensed under the Apache License, Version 2.0 (the "License");
 *  you may not use this file except in compliance with the License.
 *  You may obtain a copy of the License at
 *
 *        http://www.apache.org/licenses/LICENSE-2.0
 *  Unless required by applicable law or agreed to in writing, software
 *  distributed under the License is distributed on an "AS IS" BASIS,
 *  WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 *  See the License for the specific language governing permissions and
 *  limitations under the License.
 *
 *  SPDX-License-Identifier: Apache-2.0
 *  ============LICENSE_END=========================================================
 */

package org.onap.cps.spi.impl;

import static org.junit.Assert.assertEquals;
import static org.junit.Assert.assertNotNull;

import org.junit.Assert;
import org.junit.ClassRule;
import org.junit.Test;
import org.junit.runner.RunWith;
import org.onap.cps.DatabaseTestContainer;
import org.onap.cps.exceptions.CpsValidationException;
import org.onap.cps.exceptions.DataspaceNotFoundException;
import org.onap.cps.spi.CpsAdminPersistenceService;
import org.onap.cps.spi.entities.Anchor;
import org.onap.cps.spi.entities.Dataspace;
import org.onap.cps.spi.entities.SchemaSet;
import org.onap.cps.spi.repository.AnchorRepository;
import org.onap.cps.spi.repository.DataspaceRepository;
import org.onap.cps.spi.repository.SchemaSetRepository;
import org.onap.cps.spi.repository.YangResourceRepository;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.boot.test.context.SpringBootTest;
import org.springframework.test.context.jdbc.Sql;
import org.springframework.test.context.jdbc.SqlGroup;
import org.springframework.test.context.junit4.SpringRunner;

@RunWith(SpringRunner.class)
@SpringBootTest
public class CpsAdminPersistenceServiceTest {

    private static final String CLEAR_DATA = "/data/clear-all.sql";
    private static final String SET_DATA = "/data/anchor.sql";

    private static final String DATASPACE_NAME = "DATASPACE-001";
    private static final String DATASPACE_NAME_INVALID = "DATASPACE-X";
    private static final String SCHEMA_SET_NAME_CONFLICTING = "SCHEMA-SET-001";
    private static final String SCHEMA_SET_NAME_INVALID = "SCHEMA-SET-X";
    private static final String SCHEMA_SET_NAME = "SCHEMA-SET-002";
    private static final String ANCHOR_NAME_CONFLICTING = "ANCHOR-001";
    private static final String ANCHOR_NAME_NEW = "ANCHOR-002";

    @ClassRule
    public static DatabaseTestContainer databaseTestContainer = DatabaseTestContainer.getInstance();

    @Autowired
    private CpsAdminPersistenceService cpsAdminPersistenceService;

    @Autowired
    private AnchorRepository anchorRepository;

    @Autowired
    private DataspaceRepository dataspaceRepository;

    @Autowired
    private SchemaSetRepository schemaSetRepository;

    @Test(expected = DataspaceNotFoundException.class)
    @SqlGroup({@Sql(CLEAR_DATA), @Sql(SET_DATA)})
    public void testCreateAnchorAtInvalidDataspace() {
        cpsAdminPersistenceService.createAnchor(DATASPACE_NAME_INVALID, SCHEMA_SET_NAME, ANCHOR_NAME_NEW);
    }

    @Test(expected = CpsValidationException.class)
    @SqlGroup({@Sql(CLEAR_DATA), @Sql(SET_DATA)})
    public void testCreateAnchorToInvalidSchemaSet() {
        cpsAdminPersistenceService.createAnchor(DATASPACE_NAME, SCHEMA_SET_NAME_INVALID, ANCHOR_NAME_NEW);
    }

    @Test(expected = CpsValidationException.class)
    @SqlGroup({@Sql(CLEAR_DATA), @Sql(SET_DATA)})
    public void testCreateAnchorToSchemaSetHavingAnchor() {
        cpsAdminPersistenceService.createAnchor(DATASPACE_NAME, SCHEMA_SET_NAME_CONFLICTING, ANCHOR_NAME_NEW);
    }

    @Test(expected = CpsValidationException.class)
    @SqlGroup({@Sql(CLEAR_DATA), @Sql(SET_DATA)})
    public void testCreateAnchorWithNameConflict() {
        cpsAdminPersistenceService.createAnchor(DATASPACE_NAME, SCHEMA_SET_NAME, ANCHOR_NAME_CONFLICTING);
    }

    @Test
    @SqlGroup({@Sql(CLEAR_DATA), @Sql(SET_DATA)})
    public void testCreateAnchor() {
        cpsAdminPersistenceService.createAnchor(DATASPACE_NAME, SCHEMA_SET_NAME, ANCHOR_NAME_NEW);

        // validate anchor persisted
        final Dataspace dataspace = dataspaceRepository.findByName(DATASPACE_NAME).orElseThrow();
        final SchemaSet schemaSet = schemaSetRepository.findByDataspaceAndName(dataspace, SCHEMA_SET_NAME)
            .orElseThrow();
        final Anchor anchor = anchorRepository.findByDataspaceAndSchemaSet(dataspace, schemaSet).orElseThrow();

        assertNotNull(anchor.getId());
        assertEquals(ANCHOR_NAME_NEW, anchor.getName());
        assertEquals(DATASPACE_NAME, anchor.getDataspace().getName());
        assertEquals(SCHEMA_SET_NAME, anchor.getSchemaSet().getName());
    }

}
