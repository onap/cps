...
public class CpsDataPersistenceServiceTest {
    ...
    private static DataNode createDataNodeTree(final String... xpaths) {
        if (xpaths.length > 1) {
            ...
            return new DataNodeBuilder().withXpath(xpaths[0])
                .withChildDataNodes(ImmutableSet.of(childDataNode)).build();
        } else {
            return new DataNodeBuilder().withXpath(xpaths[0]).withChildDataNodes(Collections.emptySet()).build();
        }
    }
}
