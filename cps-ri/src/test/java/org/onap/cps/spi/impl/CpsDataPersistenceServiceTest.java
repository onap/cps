...
public class CpsDataPersistenceServiceTest {
    ...
    public void testGetFragmentsWithChildAndGrandChild() {
        final Fragment grandChildFragment = fragmentRepository.findAll().stream()
            .filter(fragment -> fragment.getXpath().contains(GRAND_CHILD_XPATH))
            .collect(onlyElement());

        final Fragment childFragment = grandChildFragment.getParentFragment();
    ...
    public void testStoreDataNodeWithChildrenAndGrandChildren() {
        ...
        final Fragment grandChildFragment = fragmentRepository.findAll().stream()
            .filter(fragment -> fragment.getXpath().contains(GRAND_CHILD_XPATH_NEW))
            .collect(onlyElement());
