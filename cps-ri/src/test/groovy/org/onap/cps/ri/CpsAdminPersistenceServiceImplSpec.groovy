    ...
    def 'Get paginated anchors for a dataspace with #scenario.'() {
        given: 'the repository returns anchors for the paginated query'
            if (requestedSchemaSetNames == null) {
                ...
                mockAnchorRepository.getByDataspaceIdAndSchemaSetNameIn(dataspaceEntity, requestedSchemaSetNames, _ as PageRequest) >> [anchorEntity]
            }
        when: 'anchors are retrieved with pagination (page 1, size 2)'
            def result = objectUnderTest.getAnchors('some-dataspace', requestedSchemaSetNames, new PaginationOption(1, 2))
        then: 'the expected number of anchors is returned'
            assert result.size() == expectedSize
        where: 'the following schema set name combinations are used'
            scenario                 | requestedSchemaSetNames || expectedSize
            'no schema set names'    | null                    || 2
            'with schema set names'  | schemaSetNames          || 1
    }
