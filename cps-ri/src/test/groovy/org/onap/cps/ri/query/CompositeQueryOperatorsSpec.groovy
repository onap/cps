...
class CompositeQueryOperatorsSpec extends Specification {
    ...
    static final Long FRAGMENT_ID_1 = 1L
    static final Long FRAGMENT_ID_2 = 2L
    static final Long FRAGMENT_ID_3 = 3L
    def mockAnchorEntity = Mock(AnchorEntity)

    static def objectUnderTest = CompositeQueryOperators

    def fragmentEntities = [(FRAGMENT_ID_1): Mock(FragmentEntity), (FRAGMENT_ID_2): Mock(FragmentEntity), (FRAGMENT_ID_3): Mock(FragmentEntity)]
    ...
    def 'Resolve operator from string for #scenario.'() {
        ...
        where: 'the following variations of operators operators are used'
    ...
    def 'Resolve operator from #scenario string.'() {
        ...
        where: 'the following case-variations of operators operators are used'
    ...
    def 'requiresAllConditions flag is correct for each operator.'() {
        ...
        where: 'the following operators operators are used'
    ...
    def 'AND operator with overlapping fragments returns intersection without duplicates.'() {
        ...
        and: 'evaluator returns overlapping sets'
            def evaluator = Mock(CompositeQueryOperators.ConditionEvaluator)
            evaluator.evaluate(condition1, fragmentEntities, mockAnchorEntity) >> ([FRAGMENT_ID_1, FRAGMENT_ID_2] as Set)
            evaluator.evaluate(condition2, fragmentEntities, mockAnchorEntity) >> ([FRAGMENT_ID_2, FRAGMENT_ID_3] as Set)
        when: 'AND operator is evaluated'
    ...
    def 'AND operator returns empty set when #scenario.'() {
        ...
        then: 'an empty set is returned because not all conditions matched'
            result.isEmpty()
        where: 'the following combinations of condition are evaluated'
            scenario                      | resultOfCondition1     | resultOfCondition2
            'the conditions are disjoint' | [FRAGMENT_ID_1] as Set | [FRAGMENT_ID_2] as Set
            'one condition is empty'      | [FRAGMENT_ID_1] as Set | [] as Set
    ...
    def 'OR operator returns #scenario.'() {
        ...
        where: 'the following combinations of condition are used'
            scenario                                            | resultOfCondition1       | resultOfCondition2       | expectedResponse
            'union of all condition, when all conditions match' | ([FRAGMENT_ID_1] as Set) | ([FRAGMENT_ID_2] as Set) | [FRAGMENT_ID_1, FRAGMENT_ID_2] as Set
            'matched IDs when only one condition matches'       | ([FRAGMENT_ID_1] as Set) | ([] as Set)              | [FRAGMENT_ID_1] as Set
