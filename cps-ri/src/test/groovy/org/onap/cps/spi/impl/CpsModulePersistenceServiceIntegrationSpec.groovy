...
class CpsModulePersistenceServiceIntegrationSpec extends CpsPersistenceSpecBase {
    ...
    @Sql([CLEAR_DATA, SET_DATA])
    def 'Retrieving all schema set from dataspace'() {
        when: 'the schema set resources from dataspace is retrieved'
            def result = objectUnderTest.getSchemaSetsFromDataspace(DATASPACE_NAME)
        then: 'the correct resources are returned'
