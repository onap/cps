...
class CpsDataPersistenceServiceDeletePerfTest extends CpsPersistencePerfSpecBase {
    ...
    def 'Delete 5 children with grandchildren'() {
        given: 'a list of xpaths to delete'
            def xpathsToDelete = (1..5).collect {
                "${PERF_TEST_PARENT}/perf-test-child-${it}".toString()
            }
        when: 'child nodes are deleted'
            stopWatch.start()
            xpathsToDelete.each {
                objectUnderTest.deleteDataNode(PERF_DATASPACE, PERF_ANCHOR, it)
            }
            stopWatch.stop()
    ...
    def 'Batch delete 250 grandchildren (that have no descendants)'() {
        given: 'a list of xpaths to delete'
            def xpathsToDelete = []
        ...
        then: 'delete duration is under 200 milliseconds'
            recordAndAssertPerformance('Batch delete 250 grandchildren', 200, deleteDurationInMillis)
    }
    ...
    def 'Delete 5 whole lists'() {
        ...
        when: 'lists are deleted'
            stopWatch.start()
            xpathsToDelete.each {
                objectUnderTest.deleteListDataNode(PERF_DATASPACE, PERF_ANCHOR, it)
            }
            stopWatch.stop()
            def deleteDurationInMillis = stopWatch.getTotalTimeMillis()
        then: 'delete duration is under 1200 milliseconds'
            recordAndAssertPerformance('Delete 5 whole lists', 1200, deleteDurationInMillis)
    }
    ...
    def 'Delete 10 list elements'() {
        given: 'a list of xpaths to delete'
            def xpathsToDelete = (1..10).collect {
    ...
    def 'Batch delete 250 list elements'() {
        given: 'a list of xpaths to delete'
            def xpathsToDelete = []
            for (int childIndex = 0; childIndex < 5; childIndex++) {
                xpathsToDelete.addAll((1..50).collect {
        ...
        when: 'list elements are deleted'
            ...
            def deleteDurationInMillis = stopWatch.getTotalTimeMillis()
        then: 'delete duration is under 200 milliseconds'
            recordAndAssertPerformance('Batch delete 250 lists elements', 200, deleteDurationInMillis)
    }
    ...
    def 'Delete 1 large data node'() {
        ...
        then: 'delete duration is under 250 milliseconds'
            recordAndAssertPerformance('Delete one large node', 1, deleteDurationInMillis)
    }
