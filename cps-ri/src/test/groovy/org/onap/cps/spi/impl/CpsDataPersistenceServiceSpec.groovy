...
class CpsDataPersistenceServiceSpec extends CpsPersistenceSpecBase {
    ...
    def 'Cps Path query with and without descendants using : #type.'() {
        ...
        where: 'the following data is used'
            type                                    | cpsPath        | includeDescendantsOption | expectedNumberOfChildNodes
            'ends with and omit descendants'        | '///child-202' | OMIT_DESCENDANTS         | 0
            'ends with and include all descendants' | '///child-202' | INCLUDE_ALL_DESCENDANTS  | 1
    }
