...
class CpsToDataNodePerfSpec extends CpsPersistenceSpecBase {
    ...
    def 'Get data node by xpath with all descendants with many children'() {
        ...
        then : 'setup duration is under 20 seconds'
            assert setupDurationInMillis < 20000 // will be adjusted after CI build fails
        and: 'read duration is under x seconds'
            assert readDurationInMillis < 0 // setting to 0 to measure response
