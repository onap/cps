...
class CpsAdminPersistenceServiceSpec extends CpsPersistenceSpecBase {
    ...
    def 'Get all anchors for an #scenario.'() {
        ...
        then: 'an exception is thrown'
            thrown(expectedException)
        where: 'the following data is used'
            scenario                                                   | ModuleNames                           | dataspaceName   || expectedException
            'existing module in an unknown dataspace'                  | ['MODULE-NAME-001']                   | 'DATASPACE-123' || DataspaceNotFoundException
            'unknown module in an existing dataspace'                  | ['does-not-exist']                    | 'DATASPACE-001' || ModuleNamesNotFoundException
            'unknown module and known module in an existing dataspace' | ['MODULE-NAME-001', 'does-not-exist'] | 'DATASPACE-001' || ModuleNamesNotFoundException
    }
