...
class CpsAdminPersistenceServiceSpec extends CpsPersistenceSpecBase {
    ...
    def 'Create and retrieve a new anchor and new schema set from new modules and existing modules.'() {
        given: 'anchor name, map of new modules, a list of existing modules, module reference and a valid dataspace'
            def anchorName = 'someCmHandleId'
            ...
            cpsModulePersistenceService.storeSchemaSet(NEW_DATASPACE_NAME, "someSchemaSetName", mapOfExistingModule)
        when: 'a new anchor and new schema set is created from these new modules and existing modules'
            objectUnderTest.createAnchorFromModules(anchorName, mapOfNewModules, listOfExistingModulesModuleReference)
        then: 'that anchor and the schema set can be retrieved'
            def anchor = objectUnderTest.getAnchor(NEW_DATASPACE_NAME, anchorName)
            anchor.name == anchorName
            anchor.dataspaceName == NEW_DATASPACE_NAME
            anchor.schemaSetName == anchorName
            def expectedYangResourcesMapAfterAnchorHasBeenCreated = mapOfNewModules + mapOfExistingModule
            def actualYangResourcesMapAfterAnchorHasBeenCreated =
                    cpsModulePersistenceService.getYangSchemaSetResources(NEW_DATASPACE_NAME, anchorName)
