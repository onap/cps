...
import org.onap.cps.cpspath.parser.InvalidPathException
...
class CpsDataPersistenceServiceIntegrationSpec extends CpsPersistenceSpecBase {
    ...
    def 'Cps Path query with syntax error throws a CPS Path Exception.'() {
        when: 'trying to execute a query with a syntax (parsing) error'
            objectUnderTest.getDataNode(DATASPACE_NAME, ANCHOR_FOR_DATA_NODES_WITH_LEAVES, 'cpsPath that cannot be parsed' , OMIT_DESCENDANTS)
    ...
    def 'Delete data node with #scenario.'() {
        ...
        where: 'the following parameters were used'
            ...
            'invalid list element'                          | '/parent-206/child-206/grand-child-206@key="A"]' | InvalidPathException
    }
