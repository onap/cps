...
class CpsDataPersistenceServiceIntegrationSpec extends CpsPersistenceSpecBase {
    ...
    static final String CM_HANDLE_WITH_ADVISED_STATE = '/dmi-registry/cm-handles[@id="PNFDemo5"]'
    ...
    def 'Get data node with null xpath.'() {
        ...
        then: 'cm-handle data node with an advised state is returned with no descendants'
            assert result.xpath == CM_HANDLE_WITH_ADVISED_STATE
        and: 'expected leaves'
            assert result.childDataNodes.size() == 0
            assertLeavesMaps(result.leaves, expectedLeavesByXpathMap[CM_HANDLE_WITH_ADVISED_STATE])
    }
