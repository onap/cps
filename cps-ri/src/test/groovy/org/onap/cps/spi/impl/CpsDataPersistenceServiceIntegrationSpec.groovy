...
class CpsDataPersistenceServiceIntegrationSpec extends CpsPersistenceSpecBase {
    ...
    def 'Add multiple new list elements including an element with a child datanode.'() {
        given: 'two new child data nodes for an existing parent'
        ...
        and: 'a grand child data node'
            def grandChildDataNode = buildDataNode('/parent-201/child-204[@key="NEW1"]/grand-child-204[@key2="NEW1-CHILD"]', [leave:'value'], [])
            listElements[0].childDataNodes = [grandChildDataNode]
        when: 'the new data nodes (list elements) are added to existing parent node'
        ...
        and: 'the grand child node is also added to existing parent'
            ...
            def grandChildListElement = fragmentRepository.findByDataspaceAndAnchorAndXpath(dataspaceEntity, anchorEntity, grandChildDataNode.xpath)
            assert grandChildListElement.isPresent()
    }
