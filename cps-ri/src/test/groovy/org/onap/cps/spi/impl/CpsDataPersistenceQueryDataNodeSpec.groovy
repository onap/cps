...
class CpsDataPersistenceQueryDataNodeSpec extends CpsPersistenceSpecBase {
    ...
    @Unroll
    @Sql([CLEAR_DATA, SET_DATA])
    def 'Cps Path query error scenario for %scenario.'() {
        when: 'a query is executed to get a data node by the given cps path'
            def result = objectUnderTest.queryDataNodes(DATASPACE_NAME, ANCHOR_FOR_DATA_NODES_WITH_LEAVES, cpsPath, OMIT_DESCENDANTS)
        ...
        where: 'the following data is used'
            scenario                                                | cpsPath
            'one of the descendant list attributes missing a value' | '//child-202[@common-leaf-name-int=5 and @another-attribute"]'
            'one descendant list attribute separated by or'         | '//child-202[@common-leaf-name-int=5 or @common-leaf-name="common-leaf value"]'
            'an ancestor value that does not exist'                 | '//books/ancestor::ancestorDoesNotExist'
    }
    ...
    def 'Query for attribute by cps path of type ancestor with #scenario.'() {
        when: 'the given cps path is parsed'
            def result = objectUnderTest.queryDataNodes(DATASPACE_NAME, ANCHOR_NAME3, cpsPath, FetchDescendantsOption.INCLUDE_ALL_DESCENDANTS)
        ...
        where: 'the following data is used'
            scenario             | cpsPath                        || expectedXPaths
            'multiple ancestors' | '//books/ancestor::categories' || ['/bookstore/categories[@name="SciFi"]', '/bookstore/categories[@name="kids"]']
            'root ancestor'      | '//books/ancestor::bookstore'  || '/bookstore'
    }
}
