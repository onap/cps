...
class CpsDataPersistenceQueryDataNodeSpec extends CpsPersistenceSpecBase {
    ...
    def 'Cps Path query using descendant anywhere with %scenario '() {
        ...
        and: 'the xpath of the retrieved data node is as expected'
            for(int i = 0; i<result.size(); i++) {
                result[i].getXpath() == expectedXPath[i]
            }
        where: 'the following data is used'
            scenario                                  | cpsPath             || expectedResultSize || expectedXPath
            'fully unique descendant name'            | '//grand-child-202' || 1                  || ['/parent-200/child-202/grand-child-202']
            'descendant name match end of other node' | '//child-202'       || 2                  || ['/parent-200/child-202','/parent-201/child-202']
    }
    ...
    def 'Cps Path query using descendant anywhere ends with yang list containing %scenario '() {
        ...
        where: 'the following data is used'
            scenario                       | cpsPath                                                                          || expectedResultSize || expectedXPath
            'one attribute'                | '//child-202[@common-leaf-name-int=5]'                                           ||  2                 || ['/parent-200/child-202','/parent-201/child-202']
            'more than one attribute'      | '//child-202[@common-leaf-name-int=5 and @common-leaf-name="common-leaf value"]' ||  1                 || ['/parent-200/child-202']
            'attributes reversed in order' | '//child-202[@common-leaf-name="common-leaf value" and @common-leaf-name-int=5]' ||  1                 || ['/parent-200/child-202']
    }
