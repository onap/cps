...
class CpsDataPersistenceQueryDataNodeSpec extends CpsPersistenceSpecBase {
    ...
    def 'Cps Path query for single leaf value with type: #type.'() {
        ...
        where: 'the following data is used'
            ...
            'String and no descendants' | '/shops/shop[@id=1]/categories[@code=1]/book[@title="Dune"]' | FetchDescendantsOption.OMIT_DESCENDANTS        || 0
            'Integer and descendants'   | '/shops/shop[@id=1]/categories[@code=1]/book[@price=15]'     | FetchDescendantsOption.INCLUDE_ALL_DESCENDANTS || 1
    }
    ...
    def 'Cps Path query using descendant anywhere and #type (further) descendants.'() {
        ...
        where: 'the following data is used'
            ...
            'omit'    | FetchDescendantsOption.OMIT_DESCENDANTS        || 0
            'include' | FetchDescendantsOption.INCLUDE_ALL_DESCENDANTS || 1
    }
    ...
    def 'Cps Path query using descendant anywhere with #scenario condition(s) for a container element.'() {
        ...
        where: 'the following data is used'
            ...
            'trailing "and" is ignored' | '//author[@FirstName="Joe" and]'                   || ['/shops/shop[@id=1]/categories[@code=1]/book/author[@FirstName="Joe" and @Surname="Bloggs"]', '/shops/shop[@id=1]/categories[@code=2]/book/author[@FirstName="Joe" and @Surname="Smith"]']
    ...
    def 'Cps Path query error scenario using descendant anywhere ends with yang list containing %scenario '() {
        when: 'a query is executed to get a data node by the given cps path'
            objectUnderTest.queryDataNodes(DATASPACE_NAME, ANCHOR_FOR_DATA_NODES_WITH_LEAVES, cpsPath, FetchDescendantsOption.OMIT_DESCENDANTS)
        then: 'exception is thrown'
            thrown(CpsPathException)
        where: 'the following data is used'
            scenario                             | cpsPath
            'one of the leaf without value'      | '//categories[@code=1 or @name=]'
    ...
    def 'Query for attribute by cps path of type ancestor with #scenario.'() {
        ...
        where: 'the following data is used'
            ...
            'ancestor with parent'                      | '//book/ancestor::shop[@id=1]/categories[@code=2]' || ['/shops/shop[@id=1]/categories[@code=2]']
