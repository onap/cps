...
class CpsPathQueryDetailsSpec extends Specification {
    ...
    def 'getXpathPrefixAndLeafNameAndLeafValue with CpsPathException.'()
    {   given: 'an invalid cps path'
        ...
        when: 'a query is executed to get a fragment by the given cps path'
            objectUnderTest.getXpathPrefixAndLeafNameAndLeafValue(invalidCpsPath)
        then: 'a CpsPathException is thrown'
            thrown(CpsPathException)
    }

    def 'getXpathPrefixAndLeafNameAndLeafValue with valid cps path : #scenario.'()
    {   when: 'a query is executed to get a fragment by the given cps path'
            def result = objectUnderTest.getXpathPrefixAndLeafNameAndLeafValue(cpsPath)
