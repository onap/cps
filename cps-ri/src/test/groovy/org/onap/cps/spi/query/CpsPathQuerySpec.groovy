...
class CpsPathQuerySpec extends Specification {
    ...
    @Unroll
    def 'Parse cps path of type ends with a yang list containing #scenario.'() {
        when: 'the given cps path is parsed'
            def result = objectUnderTest.createFrom(cpsPath)
        then: 'the query has the right type'
            result.cpsPathQueryType == CpsPathQueryType.XPATH_HAS_DESCENDANT_WITH_LEAF_VALUE
        and: 'the right ends with parameters are set'
            result.descendantName == expectedEndsWithValue
            result.getAlternatingLeafValueList().size()/2 == expectedCountOfLeaves
        where: 'the following data is used'
            scenario                   | cpsPath                                            || expectedEndsWithValue || expectedCountOfLeaves
            'one attribute'            | '//child[@common-leaf-name-int=5]'                 || 'child'               ||  1
            'more than one attribute'  | '//child[@int-leaf=5 and @leaf-name="leaf-value"]' || 'child'               ||  2
    }
