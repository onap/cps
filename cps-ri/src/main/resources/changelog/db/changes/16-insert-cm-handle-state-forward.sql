INSERT INTO fragment(
  xpath, attributes, anchor_id, parent_id, 
  dataspace_id, schema_node_id
) 
SELECT 
  ...
  to_jsonb(
    concat(
      ...
      to_char(
        now(), 'YYYY-MM-DD"T"HH24:MI:SS.MSTZHTZM'
      ), 
...
SELECT 
  concat(
    cmHandlesStates.xpath, '/datastores'
  ), 
  to_jsonb('{}' :: json), 
...
SELECT 
  concat(
    cmHandlesDatastores.xpath, '/operational'
  ), 
  to_jsonb(
    concat(
