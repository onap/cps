    ...
    public String createAnchor(final Anchor anchor) {
        ...
        try {
            final Dataspace dataspace =
                    dataspaceRepository.findByName(dataspaceName).orElseThrow(() -> new DataspaceNotFoundException(
                            "Dataspace " + dataspaceName + " does not exist."));
            final String namespace = anchor.getNamespace();
            final String revision = anchor.getRevision();
            final Module module =
                    moduleRepository.findByDataspaceAndNamespaceAndRevision(dataspace,
                            namespace, revision).orElseThrow(() -> new CpsNotFoundException("Cannot find module",
                            String.format("Module with dataspace %s, revision %s does not exist in namespace %s.",
