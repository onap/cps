...
public class CpsModulePersistenceServiceImpl implements CpsModulePersistenceService {
    ...
    @Override
    public List<SchemaSet> getSchemaSetsByDataspaceName(final String dataspaceName) {
        final List<SchemaSet> schemaSets = new ArrayList<>();
        final List<SchemaSetEntity> schemaSetEntities = schemaSetRepository.findAll();
        for (final SchemaSetEntity schemaSetEntity : schemaSetEntities) {
            if (schemaSetEntity.getDataspace().getName().equals(dataspaceName)) {
                final SchemaSet schemaSet = new SchemaSet();
                schemaSet.setName(schemaSetEntity.getName());
                schemaSet.setDataspaceName(schemaSetEntity.getDataspace().getName());
