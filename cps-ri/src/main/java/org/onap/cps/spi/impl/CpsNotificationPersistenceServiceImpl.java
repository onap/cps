...
public class CpsNotificationPersistenceServiceImpl implements CpsNotificationPersistenceService {
    ...
    /*@Override
    public void subscribeNotificationForDataspace(String dataspaceName) {
        final AnchorEntity anchorEntity = getAnchorEntity(NOTIFICATION_DATASPACE_NAME, NOTIFICATION_ANCHOR_NAME);
        final FragmentEntity fragmentEntity = new FragmentEntity();
        FragmentEntity ParentEntity = fragmentRepository.findByAnchorAndXpath(anchorEntity, "/dataspaces").get();
        fragmentEntity.setXpath("/dataspaces/dataspace[@name='"+dataspaceName+"']");
        fragmentEntity.setAttributes("{\"name\":"+dataspaceName+",\"subscriptionIds\": [\"0\"]");
        fragmentEntity.setParentId(ParentEntity.getId());
