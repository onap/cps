...
public class CpsCmHandlerQueryServiceImpl implements CpsCmHandlerQueryService {
    ...
    public Collection<DataNode> queryCmHandles(final CmHandleQueryParameters cmHandleQueryParameters) {
        ...
        final boolean firstQuery = moduleNameQuery(cmHandleQueryParameters, amalgamatedQueryResult);

        publicPropertyQuery(cmHandleQueryParameters, amalgamatedQueryResult, firstQuery);

        return amalgamatedQueryResult;
    }

    private void publicPropertyQuery(final CmHandleQueryParameters cmHandleQueryParameters,
                                     final Collection<DataNode> amalgamatedQueryResult, boolean firstQuery) {
        for (final Map.Entry<String, String> entry :
            ...
            final Collection<DataNode> singleConditionQueryResult = cpsDataPersistenceService.queryDataNodes(
    ...
    private boolean moduleNameQuery(final CmHandleQueryParameters cmHandleQueryParameters,
                                    final Collection<DataNode> amalgamatedQueryResult) {
        if (!getModuleNames(cmHandleQueryParameters.getCmHandleQueryRestParameters()).isEmpty()) {
            final Collection<Anchor> anchors = cpsAdminPersistenceService.queryAnchors("NFP-Operational",
                    getModuleNames(cmHandleQueryParameters.getCmHandleQueryRestParameters()));
            anchors.forEach(anchor -> {
                final List<DataNode> dataNodes = cpsDataPersistenceService.queryDataNodes("NCMP-Admin",
                        "ncmp-dmi-registry", "//cm-handles[@id='" + anchor.getName() + "']",
                        INCLUDE_ALL_DESCENDANTS);
                amalgamatedQueryResult.addAll(dataNodes);
            });
            return false;
        }
        return true;
    ...
    private List<String> getModuleNames(final List<ConditionProperties> conditionProperties) {
        ...
        getConditions(conditionProperties, MODULE_QUERY_NAME).forEach(
                conditionProperty -> {
    ...
    private Collection<DataNode> getAllCmHandles() {
        return cpsDataPersistenceService.queryDataNodes("NCMP-Admin", "ncmp-dmi-registry",
                "//public-properties/ancestor::cm-handles", INCLUDE_ALL_DESCENDANTS);
    }
}
