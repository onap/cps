...
public interface FragmentRepository extends JpaRepository<FragmentEntity, Long> {
    ...
    @Query(value = "SELECT * FROM FRAGMENT WHERE (anchor_id = :anchor) AND xpath LIKE (:xpath%) AND attributes @>"
        + " jsonb_build_object(:leafName , :leafValue)", nativeQuery = true)
    List<FragmentEntity> getByAnchorAndXpathAndLeafAttributes(@Param("anchor")int anchorId, @Param("xpath")
        String xpathPrefix, @Param("leafName")String leafName, @Param("leafValue")String  leafValue);
}