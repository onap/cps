...
@NoArgsConstructor
@AllArgsConstructor
public class CpsPathQueryDetails {
     ...
     */
    public static CpsPathQueryDetails getXpathPrefixAndLeafNameAndLeafValue(final String cpsPath) {
        ...
        if (matcher.matches()) {
            ...
            return ((!leafValue.contains("[a-zA-Z]+")) && leafValue.length() > 2)
                ? new CpsPathQueryDetails(xpathPrefix, leafName, leafValue) :
                new CpsPathQueryDetails(xpathPrefix, leafName, Integer.valueOf(leafValue));
        } else {
