...
public class FragmentPersistenceServiceImpl implements FragmentPersistenceService {
    ...
    public String createAnchor(final AnchorDetails anchorDetails) {
        final Dataspace dataspace = findDatasapce(anchorDetails);
        final ModuleEntity module = findModule(anchorDetails, dataspace);
        ...
        try {
            ...
            throw new CpsValidationException("Duplication Error",
                String.format(
                    "Anchor with the dataspace %s, namespace %s and revision %s already exists.",
                    dataspace.getName(), anchorDetails.getNamespace(),
                    anchorDetails.getRevision()));
        }
    }

    private ModuleEntity findModule(final AnchorDetails anchorDetails, final Dataspace dataspace)
        ...
        if (null == module) {
            throw new CpsNotFoundException("Not Found",
                String.format(
                    "Module with dataspace %s, revision %s and namespace %s does not exist.",
                    dataspace.getName(), anchorDetails.getNamespace(),
                    anchorDetails.getRevision()));
        }
        return module;
    ...
    private Dataspace findDatasapce(final AnchorDetails anchorDetails) throws CpsNotFoundException {
