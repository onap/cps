    ...
    private Collection<FragmentEntity> getAllFragmentsByXpath(final String dataspaceName, final String anchorName,
        ...
        if (isRootXpath(xpath)) {
            ...
            if (normalizedXpath != null) {
                if (FetchDescendantsOption.OMIT_DESCENDANTS.equals(fetchDescendantsOption)) {
                    return fragmentRepository.getAllByDataspaceAndAnchorAndXpath(dataspaceEntity, anchorEntity,
                            normalizedXpath);
                } else {
                    return buildFragmentEntitiesFromFragmentExtracts(anchorEntity, normalizedXpath);
                }
            }
            throw new DataNodeNotFoundException(dataspaceEntity.getName(), anchorEntity.getName(), xpath);
        }
