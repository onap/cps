...
public class CpsDataPersistenceServiceImpl implements CpsDataPersistenceService {
    ...
    private List<Map<String, Object>> getDeltaBetweenDataNodes(final Collection<DataNode> dataNodes1,
                                                               final Collection<DataNode> dataNodes2) {


        final List<Map<String, Object>> deltaReport = new ArrayList<>();

        final Map<String, DataNode> xpathToDataNodes2 = new HashMap<>();
        for (final DataNode dataNode : dataNodes2) {
            xpathToDataNodes2.put(dataNode.getXpath(), dataNode);
        }

        for (final DataNode dataNode : dataNodes1) {
            final String xpath = dataNode.getXpath();
            final DataNode dataNodeWithXpath = xpathToDataNodes2.get(xpath);
            if (dataNodeWithXpath == null) {
                ...
                if (!dataNodeWithXpath.getLeaves().isEmpty()) {
                    final Map<String, Serializable> leavesInDataNode1 = dataNode.getLeaves();
                    final Map<String, Serializable> leavesInDataNode2 = dataNodeWithXpath.getLeaves();
                    final var leafNameToUpdatedLeaves = getUpdatedLeaves(leavesInDataNode1, leavesInDataNode2);
                ...
                if (!dataNodeWithXpath.getChildDataNodes().isEmpty()) {
                    final List<Map<String, Object>> deltaBetweenChildDataNodes =
                            getDeltaBetweenDataNodes(dataNode.getChildDataNodes(),
                                    dataNodeWithXpath.getChildDataNodes());
