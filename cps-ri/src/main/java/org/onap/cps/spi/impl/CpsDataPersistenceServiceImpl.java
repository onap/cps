...
public class CpsDataPersistenceServiceImpl implements CpsDataPersistenceService {
    ...
    public void storeDataNodes(final String dataspaceName, final String anchorName,
        ...
        try {
            ...
            log.warn("Exception occurred : {} , While saving : {} ", exception, fragmentEntities.size());
            storeDataNode(dataspaceName, anchorName, dataNodes);
        }
    }

    //TODO: Can be made private
    @Override
    public void storeDataNode(final String dataspaceName, final String anchorName,
                              final Collection<DataNode> dataNodes) {
        final DataspaceEntity dataspaceEntity = dataspaceRepository.getByName(dataspaceName);
        final AnchorEntity anchorEntity = anchorRepository.getByDataspaceAndName(dataspaceEntity, anchorName);
        final Collection<String> failedXpaths = new HashSet<>();
