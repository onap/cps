        ...
        if (isRootXpath(xpath)) {
            ...
            if (FetchDescendantsOption.OMIT_DESCENDANTS.equals(fetchDescendantsOption)) {
                ...
                final List<FragmentExtract> fragmentExtracts =
                    fragmentRepository.findByAnchorIdAndPathPrefix(anchorEntity.getId(), normalizedXpath);
                log.debug("Fetched {} fragment entities by anchor and cps path.", fragmentExtracts.size());
                fragmentEntity = FragmentEntityArranger.toFragmentEntityTree(fragmentExtracts);
            }
