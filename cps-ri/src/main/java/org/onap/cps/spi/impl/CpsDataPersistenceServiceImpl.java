...
public class CpsDataPersistenceServiceImpl implements CpsDataPersistenceService {
    ...
    private static Set<String> processAncestorXpath(final List<FragmentEntity> fragmentEntities,
        ...
        for (final FragmentEntity fragmentEntity : fragmentEntities) {
            final Matcher ancestorMatcher = ancestorPattern.matcher(fragmentEntity.getXpath());
            final Matcher ancestorRootmatcher = ancestorRootPattern.matcher(fragmentEntity.getXpath());
            if (ancestorMatcher.matches()) {
                ancestorXpath.add(ancestorMatcher.group(1));
            } else if (ancestorRootmatcher.matches()) {
                ancestorXpath.add(ancestorRootmatcher.group(1));
            } else {
