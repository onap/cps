    ...
    private List<Map<String, Object>> getDeltaBetweenDataNodes(final Collection<DataNode> referenceDataNodes,
        ...
        for (final DataNode referenceDataNode : referenceDataNodes) {
            ...
            if (comparandDataNode == null) {
                deltaReport.add(getDeltaReportEntities(REMOVE_ACTION, xpath, null));
            } else {
                if (!referenceDataNode.getLeaves().isEmpty() && !comparandDataNode.getLeaves().isEmpty()) {
                    ...
                    if (!leafNameToUpdatedLeaves.isEmpty()) {
                        deltaReport.add(getDeltaReportEntities(UPDATE_ACTION, xpath, leafNameToUpdatedLeaves));
                    }
                } else if (referenceDataNode.getLeaves().isEmpty() && !comparandDataNode.getLeaves().isEmpty()) {
                    deltaReport.add(getDeltaReportEntities(UPDATE_ACTION, xpath, comparandDataNode.getLeaves()));
                } else if (!referenceDataNode.getLeaves().isEmpty()) {
                    deltaReport.add(getDeltaReportEntities(UPDATE_ACTION, xpath, comparandDataNode.getLeaves()));
                } else {
                    continue;
                }

                deltaReport.addAll(getDeltaBetweenDataNodes(referenceDataNode.getChildDataNodes(),
        ...
        for (final DataNode comparandDataNode: comparandDataNodes) {
            ...
            if (referenceDataNode == null) {
                if (!comparandDataNode.getLeaves().isEmpty() && !comparandDataNode.getChildDataNodes().isEmpty()) {
                    deltaReport.add(getDeltaReportEntities(ADD_ACTION, comparandDataNode.getXpath(),
                            comparandDataNode.getLeaves()));
                    final List<Map<String, Object>> deltaBetweenChildDataNodes =
                            getDeltaBetweenDataNodes(Collections.emptyList(), comparandDataNode.getChildDataNodes());
                    ...
                    deltaReport.add(getDeltaReportEntities(ADD_ACTION, comparandDataNode.getXpath(), null));
                }
            }
        }
