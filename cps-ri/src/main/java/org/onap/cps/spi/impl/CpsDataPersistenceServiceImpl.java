...
public class CpsDataPersistenceServiceImpl implements CpsDataPersistenceService {
    ...
    public List<DataNode> queryDataNodesAcrossAnchors(final String dataspaceName, final String cpsPath,
        ...
        List<Long> anchorIds = Collections.EMPTY_LIST;
        if (isQueryForPagination(paginationOption)) {
        ...
        if (cpsPathQuery.hasAncestorAxis()) {
            ...
            if (anchorIds.isEmpty()) {
                ...
                fragmentEntities = fragmentRepository.findByDataspaceIdAnchorIdsAndXpathIn(dataspaceEntity.getId(),
                        ancestorXpaths.toArray(new String[0]), anchorIds.toArray(new Integer[0]));
            }
    ...
    private List<Long> getAnchorIDsForPagination(final DataspaceEntity dataspaceEntity, final CpsPathQuery cpsPathQuery,
                                                 final PaginationOption paginationOption) {
        return fragmentRepository.findAnchorIdsForPagination(dataspaceEntity, cpsPathQuery,
                paginationOption.getPageIndex(), paginationOption.getPageSize());
    }

    private boolean isQueryForPagination(final PaginationOption paginationOption) {
        return paginationOption != null
                && paginationOption.getPageIndex() != null && paginationOption.getPageIndex() > 0
                && paginationOption.getPageSize() != null && paginationOption.getPageSize() > 0;
    }
    ...
    @Override
    public Integer queryTotalAnchorsForDataspaceAndCpsPath(final String dataspaceName, final String cpsPath) {
        final DataspaceEntity dataspaceEntity = dataspaceRepository.getByName(dataspaceName);
        final CpsPathQuery cpsPathQuery;
        try {
            cpsPathQuery = CpsPathUtil.getCpsPathQuery(cpsPath);
        ...
        final List<Long> anchorIdList = getAnchorIDsForPagination(dataspaceEntity, cpsPathQuery,
                new PaginationOption(-1, -1));
