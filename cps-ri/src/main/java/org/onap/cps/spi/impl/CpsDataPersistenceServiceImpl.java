...
public class CpsDataPersistenceServiceImpl implements CpsDataPersistenceService {
    ...
    public List<DataNode> queryDataNodes(final String dataspaceName, final String anchorName, final String cpsPath,
        ...
        if (canUseRegexQuickFind(fetchDescendantsOption, cpsPathQuery)) {
            return (anchorEntity == ALL_ANCHORS) ?
                getDataNodesUsingRegexQuickFindAcrossAnchors(fetchDescendantsOption, dataspaceEntity, cpsPathQuery) :
                getDataNodesUsingRegexQuickFind(fetchDescendantsOption, anchorEntity, cpsPathQuery);
        }
    ...
    private List<DataNode> getDataNodesUsingRegexQuickFind(final FetchDescendantsOption fetchDescendantsOption,
        ...
        final List<FragmentExtract> fragmentExtracts = fragmentRepository.quickFindWithDescendants(anchorEntity.getId(), xpathRegex);
    ...
    private List<DataNode> getDataNodesUsingRegexQuickFindAcrossAnchors(
        ...
        final List<FragmentExtract> fragmentExtracts = fragmentRepository
                .quickFindWithDescendantsAcrossAnchor(xpathRegex);//TODO: change for given dataspace
        final Map<Long, AnchorEntity> fragmentExtractAnchorMap = getFragmentExtractAnchorMap(fragmentExtracts);
        fragmentEntities = FragmentEntityArranger.toFragmentEntityTreesAcrossAnchors(fragmentExtracts, fragmentExtractAnchorMap);
        if (cpsPathQuery.hasAncestorAxis()) {
            fragmentEntities = getAncestorFragmentEntitiesAcrossAnchors(dataspaceEntity, cpsPathQuery, fragmentEntities);
        }
        return createDataNodesFromFragmentEntities(fetchDescendantsOption, fragmentEntities);
    }

    private Map<Long, AnchorEntity> getFragmentExtractAnchorMap(List<FragmentExtract> fragmentExtracts){
        final Map<Long, AnchorEntity> fragmentEntityAnchorMap = new HashMap<>();
        fragmentExtracts.forEach(fragmentExtract -> {
            AnchorEntity anchorEntity = anchorRepository.getById(Math.toIntExact(fragmentExtract.getAnchorId()));
