...
public class ModelPersistenceServiceImpl implements ModelPersistenceService {
    ...
    public void storeModuleSet(final ModuleSet moduleSet) {
        Dataspace dataspace = dataspaceRepository.findByName(moduleSet.getDataspace())
        ...
        Set<YangFileEntity> yangFiles = persistYangFiles(moduleSet.getYangFiles());
        ModuleSetEntity moduleSetEntity = new ModuleSetEntity();
    ...
    @Override
    public List<ModuleSet> getAllModuleSets(String dataspaceName) {
        // not implemented yet
        return null;
    }

    @Override
    public ModuleSet getModuleSet(String dataspaceName, String moduleSetName) {
        // not implemented yet
        return null;
    }

    private Set<YangFileEntity> persistYangFiles(Set<YangFile> yangFiles) {
        Map<String, YangFile> checksumMap = yangFiles.stream().collect(Collectors.toMap(
            yangFile -> DigestUtils.md5DigestAsHex(yangFile.getContent().getBytes()),
            yangFile -> yangFile));

        List<YangFileEntity> existingYangFiles = yangFileRepository.findAllByChecksumIn(checksumMap.keySet());
        existingYangFiles.forEach(yangFile -> checksumMap.remove(yangFile.getChecksum()));

        List<YangFileEntity> newYangFiles = checksumMap.entrySet().stream().map(entry -> {
            YangFileEntity entity = new YangFileEntity();
        ...
        Set<YangFileEntity> result = new HashSet<>();
