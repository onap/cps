...
import static org.onap.cps.ri.query.CompositeQueryOperators.getNormalizedOperator;
...
public class CompositeQueryProcessor {
     ...
     *
     * @param anchorEntity           the anchorEntity entity to query against
     ...
     */
    public Collection<DataNode> processCompositeQuery(final AnchorEntity anchorEntity,
        ...
        final List<FragmentEntity> baseFragments = fragmentRepositoryCpsPathQuery
            .findByAnchorAndCpsPath(anchorEntity, baseCpsPath, 0);
        ...
        final Set<Long> matchingIds = applyConditionsOnFlatMap(compositeQuery, flatFragmentEntities, anchorEntity);
    ...
    private Set<Long> applyConditionsOnFlatMap(final CompositeQuery compositeQuery,
        ...
        final CompositeQueryOperators normalizedOperator = getNormalizedOperator(compositeQuery.getOperator());
    ...
    private Set<Long> evaluateCondition(final CompositeQuery compositeQuery,
        ...
        final List<FragmentEntity> fragmentEntities = fragmentRepositoryCpsPathQuery
            .findByAnchorAndCpsPath(anchorEntity, cpsPathQuery, 0);
        ...
        if (compositeQuery.getConditions() != null && !compositeQuery.getConditions().isEmpty()) {
            ...
            final Set<Long> nestedMatches = applyConditionsOnFlatMap(compositeQuery, scopedFragments, anchorEntity);
    ...
    private Map<Long, FragmentEntity> getScopedFragmentsFromFlatMap(final Set<Long> parentIds,
        ...
        final Map<Long, FragmentEntity> scopedFragments = new HashMap<>();
        for (final Long descendantId : descendantIds) {
            final FragmentEntity fragment = flattenFragmentEntities.get(descendantId);
            if (fragment != null) {
                scopedFragments.put(descendantId, fragment);
            }
        }
