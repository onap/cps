...
public interface AnchorRepository extends JpaRepository<AnchorEntity, Long> {
    ...
    Page<AnchorEntity> findAllByDataspace(DataspaceEntity dataspaceEntity, Pageable pageable);

    Collection<AnchorEntity> findAllBySchemaSet(SchemaSetEntity schemaSetEntity);

    @Query(value = "SELECT anchor.* FROM anchor"
                  + " JOIN schema_set ON anchor.schema_set_id = schema_set.id"
                  + " WHERE anchor.dataspace_id = :dataspaceId AND schema_set.name IN (:schemaSetNames)",
        countQuery = "SELECT count(anchor.id) FROM anchor"
                  + " JOIN schema_set ON anchor.schema_set_id = schema_set.id"
                  + " WHERE anchor.dataspace_id = :dataspaceId AND schema_set.name IN (:schemaSetNames)",
        nativeQuery = true)
    ...
    @Query(value = "SELECT anchor.* FROM anchor"
        + " LEFT OUTER JOIN schema_set ON anchor.schema_set_id = schema_set.id"
        + " WHERE anchor.dataspace_id = :dataspaceId AND schema_set.name IN (:schemaSetNames)",
        nativeQuery = true)
    ...
    default Collection<AnchorEntity> getByDataspace(DataspaceEntity dataspaceEntity, Pageable pageable) {
        return findAllByDataspace(dataspaceEntity, pageable).getContent();
    }
}
