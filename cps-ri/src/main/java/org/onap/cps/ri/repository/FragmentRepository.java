...
public interface FragmentRepository extends JpaRepository<FragmentEntity, Long>, FragmentRepositoryCpsPathQuery,
    ...
    // --- Composite delta queries (single query returning removed + added + updated) ---
    ...
    @Query(value = "SELECT COALESCE(s.xpath, t.xpath) AS xpath, "
        ...
        + "FROM (SELECT * FROM fragment WHERE anchor_id = :sourceAnchorId) s "
        + "FULL OUTER JOIN (SELECT * FROM fragment WHERE anchor_id = :targetAnchorId) t "
        + "ON s.xpath = t.xpath "
    ...
    // --- Convenience default methods for composite queries ---

    default List<DeltaProjection> findAllDeltaFragments(final long sourceAnchorId, final long targetAnchorId,
                                                        final String xpath) {
