...
class DataSyncWatchdogSpec extends Specification {
    ...
    def objectUnderTest = new DataSyncWatchdog(mockInventoryPersistence, mockCpsDataService, mockModuleOperationUtils, mockDataSyncSemaphores)
    ...
    def 'Data Sync for Cm Handle State in READY and Operational Sync State in UNSYNCHRONIZED.'() {
        ...
        and: 'we have the module and root nodes to form the options field'
            mockModuleOperationUtils.getModuleAndRootNodes('cm-handle-1') >> ['some-module-1:some-root-node']
            mockModuleOperationUtils.getModuleAndRootNodes('cm-handle-2') >> ['some-module-2:some-root-node']
    ...
    def 'Data sync handles exception during resource data retrieval.'() {
        ...
        and: 'semaphore map allows processing'
            mockDataSyncSemaphores.putIfAbsent('cm-handle-1', false, _, _) >> null
        and: 'module operations returns module and root nodes'
            mockModuleOperationUtils.getModuleAndRootNodes('cm-handle-1') >> ['some-module-1:some-root-node', 'some-module-2:some-root-node']
        when: 'data sync poll is executed'
            objectUnderTest.executeUnSynchronizedReadyCmHandleForInitialDataSync()
