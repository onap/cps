...
class PolicyExecutorSpec extends Specification {
    ...
    def 'Permission check logging.'() {
        ...
        then: 'correct details are logged '
            ...
            assert getLogEntry(4).contains('my credentials')
            assert getLogEntry(5).contains('cm_patch')
            assert getLogEntry(6).contains('fdn1')
            assert getLogEntry(7).contains('ch-1')
            assert getLogEntry(8).contains('my resource')
            assert getLogEntry(9).contains('my change')
    }
