...
class DmiPluginUrlValidatorSpec extends Specification {
    ...
    def 'Validation rejects invalid scheme.'() {
        ...
        where: 'the following URLs have invalid schemes'
            scenario           | url
            'ftp scheme'       | 'ftp://some-host'
            'file scheme'      | 'file:///etc/passwd'
            'no scheme'        | 'just-a-hostname'
    }

    def 'Validation checks all DMI plugin URL fields.'() {
        given: 'a registration with an invalid URL in the #fieldName field'
            def registration = new DmiPluginRegistration()
            registration."$fieldName" = 'ftp://some-host'
        when: 'validation is performed'
            objectUnderTest.validateDmiPluginUrls(registration)
        then: 'a DataValidationException is thrown'
            thrown(DataValidationException)
    ...
    def 'Validation with allowed URL pattern rejects non-matching URLs.'() {
        given: 'a validator with a pattern allowing only cluster-local URLs'
            def restrictedValidator = new DmiPluginUrlValidator('^https?://[a-zA-Z0-9\\-\\.]+\\.svc\\.cluster\\.local(:\\d+)?(/.*)?$')
    ...
    def 'Validation with allowed URL pattern accepts matching URLs.'() {
        given: 'a validator with a pattern allowing only cluster-local URLs'
            def restrictedValidator = new DmiPluginUrlValidator('^https?://[a-zA-Z0-9\\-\\.]+\\.svc\\.cluster\\.local(:\\d+)?(/.*)?$')
    ...
    def 'Validation with no pattern configured allows all valid URLs.'() {
