...
class EventProducerSimulatingFailureOnFirstAttempt extends EventProducer {
    ...
    private static final int FAIL_ON_TENTH_EVENT = 10
    ...
    void sendCloudEventBatch(String topicName, List<Map.Entry<String, CloudEvent>> events) {
        if (!hasFailed && events.size() > FAIL_ON_TENTH_EVENT) {
            hasFailed = true
            // collect futures and replace one with a failed future
            def futures = events.withIndex().collect { event, eventIndex ->
                def future = cloudEventKafkaTemplateForExactlyOnceSemantics.send(topicName, event.key, event.value)
                if (eventIndex == FAIL_ON_TENTH_EVENT - 1) {
                    return createFailedFuture()
                }
                return future
            }
            // join all futures (including one failed)
