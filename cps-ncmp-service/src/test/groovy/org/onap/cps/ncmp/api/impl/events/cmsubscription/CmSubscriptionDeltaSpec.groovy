...
class CmSubscriptionDeltaSpec extends Specification {
    ...
    def 'Find Delta of given list of predicates'() {
        given: 'A list of predicates'
            def predicateList = [new CmSubscriptionPredicate(['ch-1','ch-2'].toSet(), new ScopeFilter(DatastoreType.PASSTHROUGH_OPERATIONAL,['a/1/','b/2'].toSet()))]
        ...
        then: 'verify correct delta is returned'
            ...
            assert result[0].targetFilter[0] == 'ch-2'
            assert result[0].scopeFilter.xpathFilters[0] == 'b/2'
