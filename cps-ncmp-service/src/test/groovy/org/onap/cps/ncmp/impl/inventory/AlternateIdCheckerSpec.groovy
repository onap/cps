...
class AlternateIdCheckerSpec extends Specification {
    ...
    def setup() {
        def returnedCmHandlePerAlternateIdMap = [:]
            returnedCmHandlePerAlternateIdMap.put('', 'dont matter')
            returnedCmHandlePerAlternateIdMap.put('fdn1', 'ch-1')
            returnedCmHandlePerAlternateIdMap.put('fdn2', 'ch-2')
        mockCmHandleIdPerAlternateId.getAll(_) >> returnedCmHandlePerAlternateIdMap
    }
    ...
    def 'Check a batch of created cm handles with #scenario.'() {
        ...
        where: 'the following alternate ids are used'
            ...
            'one already used alternate id'   | 'fdn1'   | 'newFdn' || ['ch-1']
            'two already used alternate ids'  | 'fdn1'   | 'fdn2'   || ['ch-1', 'ch-2']
