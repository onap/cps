...
class NetworkCmProxyCmHandleQueryServiceSpec extends Specification {
    ...
    def 'Query untrustworthy cm handles.'() {
        given: 'a trust level condition property'
            ...
            def trustLevelConditionProperties = createConditionProperties('cmHandleWithTrustLevel', [['trustLevel': 'COMPLETE']])
        ...
        then: 'the query for untrustworthy cm handles is called'
            1 * cmHandleQueries.queryUntrustworthyCmHandles() >> { ['CMHandle1','CMHandle2','CMHandle3','CMHandle4'] as Set }
        and: 'the expected untrustworthy cm handles are returned successfully'
            assert result.size() == 4
            assert result.containsAll(['CMHandle1','CMHandle2','CMHandle3','CMHandle4'])
    }
