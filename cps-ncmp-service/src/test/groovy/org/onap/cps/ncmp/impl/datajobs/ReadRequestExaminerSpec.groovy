...
class ReadRequestExaminerSpec extends Specification {
    ...
    def 'Classify selectors as broadcast for #scenario'() {
        ...
        where: 'the following selectors are used'
            ...
            'multiple broadcast selectors via OR'  | '/SubNetwork[id="SN1"] OR //ManagedElement'            || ['/SubNetwork[id="SN1"]', '//ManagedElement']
    }
    ...
    def 'Classify selector into DMI group when target is READY'() {
        ...
        then: 'the selector is grouped under the correct DMI service and producer key'
            ...
            assert result.error().isEmpty()
            assert result.dmiSelectors().size() == 1
            def producerKeyMap = result.dmiSelectors().get('dmi-plugin-1')
            assert producerKeyMap.values().flatten() == ['/ManagedElement[id="ME1"]']
    }
    ...
    def 'Classify multiple selectors into same DMI group'() {
        ...
        then: 'both selectors are grouped under the same DMI and producer key'
            def producerKeyMap = result.dmiSelectors().get('dmi-plugin-1')
            assert producerKeyMap.values().flatten().size() == 2
    }
    ...
    def 'Classify selectors into different DMI groups'() {
        ...
        then: 'selectors are grouped under different DMI services'
            assert result.dmiSelectors().size() == 2
            assert result.dmiSelectors().containsKey('dmi-plugin-1')
            assert result.dmiSelectors().containsKey('dmi-plugin-2')
    }

    def 'Classify selector as error when #scenario'() {
        given: 'alternate id matcher behaviour'
            if (throwsException) {
        ...
        where: 'the following scenarios apply'
            scenario                  | throwsException | cmHandleState
            'target not found'        | true            | null
            'target not READY'        | false           | CmHandleState.LOCKED
            'target state is null'    | false           | null
    }
    ...
    def 'Classify contains selector matching multiple CM handles on same DMI'() {
        ...
        when: 'a contains selector is classified'
            def result = objectUnderTest.classifySelectors('/ManagedElement[contains(id, "Ireland")]')
        then: 'the selector appears once under the single DMI group'
            def selectors = result.dmiSelectors().get('dmi-plugin-1').values().flatten()
            assert selectors.count { it == '/ManagedElement[contains(id, "Ireland")]' } == 2
    }
    ...
    def 'Classify contains selector as error when matched CM handle is not READY'() {
    ...
    def 'Classify mixed broadcast, DMI, and error selectors'() {
        given: 'a READY cm handle for ME1'
            def yangModelCmHandle = createYangModelCmHandle('ch-1', 'dmi-plugin-1', 'producer-A', CmHandleState.READY)
            mockAlternateIdMatcher.getCmHandleIdByLongestMatchingAlternateId('/ManagedElement=ME1', '/') >> 'ch-1'
            mockInventoryPersistence.getYangModelCmHandle('ch-1') >> yangModelCmHandle
        and: 'no match for ME_UNKNOWN'
            mockAlternateIdMatcher.getCmHandleIdByLongestMatchingAlternateId('/ManagedElement=ME_UNKNOWN', '/') >> { throw new NoAlternateIdMatchFoundException('test') }
        when: 'mixed selectors are classified'
            def result = objectUnderTest.classifySelectors('//ManagedElement\n/ManagedElement[id="ME1"]\n/ManagedElement[id="ME_UNKNOWN"]')
        then: 'selectors are correctly distributed'
            assert result.broadcast() == ['//ManagedElement']
            assert result.dmiSelectors().containsKey('dmi-plugin-1')
            assert result.error() == ['/ManagedElement[id="ME_UNKNOWN"]']
    }
