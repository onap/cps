...
class ExactlyOnceSemanticsKafkaConfigSpec extends Specification {
    ...
    def 'Error handler is configured with EventBatchSendException as retryable.'() {
        given: 'the exception matcher from the error handler'
            def exceptionMatcher = getErrorHandlerField('exceptionMatcher')
        expect: 'EventBatchSendException is classified as retryable'
            exceptionMatcher.match(new EventBatchSendException('some error', 'some details', null)) == true
        and: 'ClassCastException is classified as non-retryable'
            exceptionMatcher.match(new ClassCastException('some error')) == false
    }

    def 'Error handler recoverer logs discarded record at error and debug levels.'() {
        given: 'a consumer record and a non-retryable exception'
            def consumerRecord = new ConsumerRecord('my-topic', 3, 42L, 'key', null)
        ...
        then: 'an error log message contains the topic, partition, offset and exception message'
            ...
            assert errorLog.formattedMessage.contains('3')
            assert errorLog.formattedMessage.contains('42')
        ...
        and: 'a debug log message contains the full stack trace'
            ...
            assert debugLog.formattedMessage.contains('3')
            assert debugLog.formattedMessage.contains('42')
            assert debugLog.throwableProxy != null
    }

    def getErrorHandlerField(String fieldName) {
