...
class InventoryPersistenceSpec extends Specification {
    ...
    @Shared
    def childDataNodesForCmHandleWithState = [new DataNode(xpath: "/dmi-registry/cm-handles[@id='some-cm-handle']/state", leaves: ['cm-handle-state': CmHandleState.ADVISED])]
    ...
    def 'Get a Cm Handle Composite State'() {
        given: 'a valid cm handle id'
            def cmHandleId = 'Some-Cm-Handle'
            def dataNode = new DataNode(leaves: ['cm-handle-state': CmHandleState.ADVISED])
    ...
    def 'Get Cm Handles By State and Cm-Handle Id'() {
        ...
        and: 'cps data service returns a list of data nodes'
            mockCpsDataPersistenceService.queryDataNodes('NCMP-Admin', 'ncmp-dmi-registry',
                '//cm-handles[@id=\'some-cm-handle\']/state[@cm-handle-state="'+ Event.CmhandleState.READY.name()+'"]/ancestor::cm-handles', OMIT_DESCENDANTS) >> sampleDataNodes
    ...
    def 'Retrieve cm handle by cps path '() {
        given: 'a cm handle state to query based on the cps path'
            def cmHandleDataNode = new DataNode(xpath: 'xpath', leaves: ['cm-handle-state': Event.CmhandleState.LOCKED])
