...
class DataJobServiceImplSpec extends Specification {
    ...
    def 'Write data job request.'() {
        ...
        and: 'the DMI Plugin returns a response'
            ...
            mockDmiRestClient.postOperationWithJsonData('my-dmi-service-name/dmi/v1/writeJob/some-job-id', _ as String, OperationType.CREATE, NO_AUTH) >> response
        when: 'read/write data job request is processed'
        ...
        then: 'the data job id is correctly logged'
            ...
            1 * mockDmiRestClient.postOperationWithJsonData('my-dmi-service-name/dmi/v1/writeJob/some-job-id', _, OperationType.CREATE, NO_AUTH)
    }
