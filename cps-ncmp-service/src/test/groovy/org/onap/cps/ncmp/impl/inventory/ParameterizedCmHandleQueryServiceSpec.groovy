...
class ParameterizedCmHandleQueryServiceSpec extends Specification {
    ...
    def 'Query cm handle ids with cpsPath.'() {
        ...
        and: 'the query get the cm handle datanodes excluding all descendants returns a datanode'
            cmHandleQueries.queryCmHandleAncestorsByCpsPath('/some/cps/path', FetchDescendantsOption.OMIT_DESCENDANTS) >> [new DataNode(leaves: ['id':'some-cmhandle-id', 'alternateId':'some-alternate-id'])]
    ...
    def static createDataNodeList(dataNodeIds) {
        def dataNodes =[]
        dataNodeIds.each{ dataNodes << new DataNode(xpath: "/dmi-registry/cm-handles[@id='${it}']", leaves: ['id':it, 'alternateId':'alt-' + it]) }
        return dataNodes
    }
}
