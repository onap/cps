...
class DmiRestClientSpec extends Specification {
    ...
    def 'Dmi trust level is determined by spring boot health status'() {
        given: 'a health check response with #springBootHealthStatus'
            ...
            ((ObjectNode) jsonNode).put('status', springBootHealthStatus)
            mockRestTemplate.getForObject(*_) >> {jsonNode}
        when: 'get trust level of the dmi plugin'
            def result = objectUnderTest.getDmiPluginTrustLevel('some url')
        then: 'the correct trust level is returned'
            result == expectedTrustLevel
        where: 'the following health status are used'
            springBootHealthStatus || expectedTrustLevel
    ...
    def 'Failing to get dmi plugin health status #scenario'() {
        ...
        when: 'attempt to get trust level of the dmi plugin'
            def result = objectUnderTest.getDmiPluginTrustLevel('some url')
        then: 'trust level will be NONE'
