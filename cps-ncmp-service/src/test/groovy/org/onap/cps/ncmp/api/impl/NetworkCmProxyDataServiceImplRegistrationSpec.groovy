...
<<<<<<< HEAD   (3ccd62 Code coverage: Update YANG schema set using moduleSetTag)
import org.onap.cps.ncmp.api.models.UpgradedCmHandles
=======
import jakarta.validation.constraints.Null
>>>>>>> CHANGE (c82bc5 Initial Registration with trustLevel on NCMP)
...
class NetworkCmProxyDataServiceImplRegistrationSpec extends Specification {
    ...
    def 'Create CM-Handle Successfully: #scenario.'() {
        given: 'a registration without cm-handle properties'
            ...
            dmiPluginRegistration.createdCmHandles = [new NcmpServiceCmHandle(cmHandleId: 'cmhandle', dmiProperties: dmiProperties, publicProperties: publicProperties, trustLevel: trustLevel)]
        ...
        and: 'trustLevel is set for the created cm-handle'
            1 * mockTrustLevelPerCmHandle.put('cmhandle', expectedTrustLevel)
        where:
            scenario                          | dmiProperties            | publicProperties  || trustLevel          || expectedDmiProperties                      | expectedPublicProperties                     | expectedTrustLevel
            'with dmi & public properties'    | ['dmi-key': 'dmi-value'] | [:]               || TrustLevel.COMPLETE || '[{"name":"dmi-key","value":"dmi-value"}]' | '[{"name":"trustLevel","value":"COMPLETE"}]' | TrustLevel.COMPLETE
            'with only public properties'     | [:]                      | [:]               || TrustLevel.NONE     || '[]'                                       | '[{"name":"trustLevel","value":"NONE"}]'     | TrustLevel.NONE
            'with only dmi properties'        | ['dmi-key': 'dmi-value'] | [:]               || null                || '[{"name":"dmi-key","value":"dmi-value"}]' | '[]'                                         | TrustLevel.COMPLETE
            'without dmi & public properties' | [:]                      | [:]               || null                || '[]'                                       | '[]'                                         | TrustLevel.COMPLETE
    }
    ...
    def getObjectUnderTest() {
        return Spy(new NetworkCmProxyDataServiceImpl(spiedJsonObjectMapper, mockDmiDataOperations,
                ...
                mockModuleSyncStartedOnCmHandles, mockTrustLevelPerCmHandle))
    }
