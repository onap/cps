...
class NetworkCmProxyDataServiceImplRegistrationSpec extends Specification {
    ...
    def 'Add CM-Handle to trustLevelPerCmHandle Successfully: #scenario.'() {
        given: 'a registration with trustLevel and populated cache'
            def dmiPluginRegistration = new DmiPluginRegistration(dmiPlugin: 'my-server')
            dmiPluginRegistration.createdCmHandles = [new NcmpServiceCmHandle(cmHandleId: 'cmhandle1', registrationTrustLevel: TrustLevel.NONE),
                                                      new NcmpServiceCmHandle(cmHandleId: cmHandleId, registrationTrustLevel: trustLevel)]
        when: 'registration is updated'
            def response = objectUnderTest.updateDmiRegistrationAndSyncModule(dmiPluginRegistration)
        then: 'a successful response is received'
            response.createdCmHandles.size() == createdCmHandles
        and: 'trustLevel is set for the created cm-handle'
            trustLevelPerCmHandle.get(cmHandleId) == expectedTrustLevel
        where:
            scenario                                         | cmHandleId  | trustLevel          | createdCmHandles || expectedCmHandleId | expectedTrustLevel
            'with valid cmHandleId and trustLevel'           | 'cmhandle2' | TrustLevel.COMPLETE | 2                || 'cmhandle2'        | TrustLevel.COMPLETE
            'with duplicate cmHandleId with null trustLevel' | 'cmhandle1' | null                | 1                || 'cmhandle1'        | TrustLevel.NONE
            'with cmHandleId with null trustLevel'           | 'cmhandle2' | null                | 2                || 'cmhandle1'        | TrustLevel.COMPLETE
    }
