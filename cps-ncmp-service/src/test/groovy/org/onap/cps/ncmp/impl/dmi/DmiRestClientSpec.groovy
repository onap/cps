...
class DmiRestClientSpec extends Specification {
    ...
    def 'DMI PATCH Operation for DMI Data Service '() {
        given: 'the parameters for the DMI web client'
            ...
            def mockRequestBodySpec = Mock(WebClient.RequestBodySpec)
        ...
        and: 'the response from the DMI'
            mockDataServicesWebClient.patch() >> mockRequestBodyUriSpec
            mockRequestBodyUriSpec.uri(_, _) >> mockRequestBodySpec
            mockRequestBodySpec.headers(_) >> mockRequestBodySpec
            mockRequestBodySpec.body(_) >> mockRequestHeadersSpec
            mockRequestHeadersSpec.retrieve() >> mockResponseSpec
            mockResponseSpec.toEntity(Object.class) >> Mono.just(responseEntity)
        when: 'PATCH operation is invoked for Data Service'
            objectUnderTest.synchronousPatchOperationWithJsonData(DATA, urlTemplateParameters, 'some json', DELETE)
