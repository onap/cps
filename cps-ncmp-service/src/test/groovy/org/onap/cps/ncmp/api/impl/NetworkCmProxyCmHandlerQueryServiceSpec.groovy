...
class NetworkCmProxyCmHandlerQueryServiceSpec extends Specification {
    ...
    def 'Retrieve cm handles with public properties when #scenario.'() {
        ...
        then: 'the correct expected cm handles are returned'
            returnedCmHandlesJustIds.stream().map(d -> d.cmHandleId).collect(Collectors.toSet()) == expectedCmHandleIds as Set
    ...
    def 'Retrieve cm handles when the query is empty.'() {
        ...
        when: 'the service is invoked'
            ...
            def returnedCmHandlesWithData = objectUnderTest.queryCmHandles(cmHandleQueryParameters, false)
        then: 'the correct expected cm handles are returned'
            returnedCmHandlesJustIds.stream().map(d -> d.cmHandleId).collect(Collectors.toSet()) == ['PNFDemo', 'PNFDemo2', 'PNFDemo3', 'PNFDemo4'] as Set
            returnedCmHandlesWithData.stream().map(d -> d.cmHandleId).collect(Collectors.toSet()) == ['PNFDemo', 'PNFDemo2', 'PNFDemo3', 'PNFDemo4'] as Set
    }
