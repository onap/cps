...
class ModelLoaderSpec extends Specification {
    ...
    def 'Create schema set fails due to AlreadyDefined exception'() {
        ...
        and: 'the admin service to create an anchor set is called once'
            10 * mockCpsAdminService.createAnchor(SUBSCRIPTION_DATASPACE_NAME, SUBSCRIPTION_SCHEMASET_NAME, SUBSCRIPTION_ANCHOR_NAME)
    }
    ...
    def 'Create schema set fails due to any other exception'() {
        ...
        and: 'the admin service to create an anchor set is called up to 10 times due to retrying'
            10 * mockCpsAdminService.createAnchor(SUBSCRIPTION_DATASPACE_NAME, SUBSCRIPTION_SCHEMASET_NAME, SUBSCRIPTION_ANCHOR_NAME)
    }

    def 'Create anchor fails due to exception'() {
