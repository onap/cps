...
class SubscriptionModelsLoaderSpec extends Specification {
    ...
    def yangResourceToContentMapForPreviousModel
    def yangResourceToContentMapForCurrentModel
    def logger = (Logger) LoggerFactory.getLogger(objectUnderTest.class)
    def loggingListAppender

    void setup() {
        yangResourceToContentMapForPreviousModel = objectUnderTest.createYangResourceToContentMap(PREVIOUS_SUBSCRIPTION_MODEL.getModelFileName())
        yangResourceToContentMapForCurrentModel = objectUnderTest.createYangResourceToContentMap(CURRENT_SUBSCRIPTION_MODEL.getModelFileName())
    ...
    def 'Onboard a subscription model via application ready event.'() {
        ...
        then: 'the module service to create schema set is called once for each model (current and previous)'
            1 * mockCpsModuleService.createSchemaSet(NCMP_DATASPACE_NAME, 'subscriptions', yangResourceToContentMapForPreviousModel)
            1 * mockCpsModuleService.createSchemaSet(NCMP_DATASPACE_NAME, 'subscriptions', yangResourceToContentMapForCurrentModel)
