...
class DmiRegistryPersistenceSpec extends Specification {

    def spiedJsonObjectMapper = Spy(new JsonObjectMapper(new ObjectMapper()))

    def mockCpsDataService = Mock(CpsDataService)

    def objectUnderTest = new DmiRegistryPersistence(spiedJsonObjectMapper, mockCpsDataService)
    ...
    def 'Save Lock Reason and Details'() {
        ...
        when: 'save lock reason and details is invoked'
            objectUnderTest.saveLockReasonAndDetails(cmHandleId, LockReason.LOCKED_MISBEHAVING, 'some lock reason details')
