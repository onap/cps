...
class AlternateIdMatcherSpec extends Specification {
    ...
    def 'Get cm handle id from a cm handle reference.' () {
        given: 'cmHandleIdPerAlternateId cache contains the given reference'
            def returnedCacheValue = 'someValue'
            mockCmHandleIdPerAlternateId.get('someExistingKey') >> returnedCacheValue
        when: 'getting a cm handle id from the reference'
            def result = objectUnderTest.getCmHandleId('someExistingKey')
        then: 'the expected cm handle id is returned'
            assert result == returnedCacheValue
    }
