...
class AlternateIdMatcherSpec extends Specification {
    ...
    def 'Get cm handle id when id exists in cache.'() {
        given: 'expected cm handle id from reference'
            def expectedCmHandleId = 'someCmHandleId'
        and: 'cmHandleIdPerAlternateId cache returns #expectedResult'
            mockCmHandleIdPerAlternateId.get('existingId') >> expectedCmHandleId
        when: 'getting a cm handle id from the reference'
            def result = objectUnderTest.getCmHandleId('existingId')
