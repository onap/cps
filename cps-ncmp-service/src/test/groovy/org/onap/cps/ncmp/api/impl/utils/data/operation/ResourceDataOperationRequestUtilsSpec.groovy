...
class ResourceDataOperationRequestUtilsSpec extends MessagingBaseSpec {
    ...
    def 'Process one data operation request with #serviceName and Module Set Tag set.'() {
        ...
        and: 'converted to a json node'
            ...
            def cmHanndlesInRequestBody = jsonObjectMapper.convertToJsonNode(dmiDataOperationRequestBody).get(0).get('cmHandles')
        then: 'it contains the correct operation details'
            assert cmHanndlesInRequestBody.size() == 1
            assert cmHanndlesInRequestBody.get(0).get('id').asText() == 'ch1-dmi1'
            assert cmHanndlesInRequestBody.get(0).get('moduleSetTag').asText() == 'module-set-tag1'
    }
