...
class TrustLevelCacheConfigSpec extends Specification {
    ...
    def cleanupSpec() {
        Hazelcast.getHazelcastInstanceByName('cps-and-ncmp-hz-instance-test-config').shutdown()
    }
    ...
    def 'Hazelcast cache for trust level per dmi plugin'() {
        ...
        and: 'Dmi Plugin Trust Level Cache is present'
            assert Hazelcast.allHazelcastInstances.name.contains('cps-and-ncmp-hz-instance-test-config')
    }
    ...
    def 'Trust level cache configurations: #scenario'() {
        given: 'retrieving the common cache config'
            def cacheConfig = Hazelcast.getHazelcastInstanceByName('cps-and-ncmp-hz-instance-test-config').config
    ...
    def 'Verify deployment network configs for Distributed Caches'() {
        given: 'the Trust Level Per Dmi Plugin Cache config'
            def trustLevelDmiPerPluginCacheConfig = Hazelcast.getHazelcastInstanceByName('cps-and-ncmp-hz-instance-test-config').config.networkConfig
    ...
    def 'Verify deployment network configs for Cm Handle Distributed Caches'() {
        given: 'the Trust Level Per Cm Handle Cache config'
            def trustLevelPerCmHandlePluginCacheConfig = Hazelcast.getHazelcastInstanceByName('cps-and-ncmp-hz-instance-test-config').config.networkConfig
        expect: 'system created instance with correct config'
