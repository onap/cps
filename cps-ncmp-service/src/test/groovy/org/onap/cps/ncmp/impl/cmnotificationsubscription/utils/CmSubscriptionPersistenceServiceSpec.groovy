...
class CmSubscriptionPersistenceServiceSpec extends Specification {
    ...
    def 'Check ongoing cm data job subscription #scenario'() {
        given: 'a valid cm data job subscription query'
            def cpsPathQuery = CPS_PATH_QUERY_FOR_CM_DATA_JOB_SUBSCRIPTION_WITH_ALTERNATE_ID_AND_DATATYPE.formatted('altId1', 'dataType1')
        ...
        when: 'we check for an ongoing cm data job subscription'
            def response = objectUnderTest.isOngoingCmDataJobSubscription('dataType1', 'altId1')
    ...
    def 'Checking uniqueness of incoming subscription ID'() {
        given: 'a cps path with a data job subscription ID for querying'
            def cpsPathQuery = CPS_PATH_QUERY_FOR_CM_DATA_JOB_SUBSCRIPTION_WITH_DATA_JOB_ID.formatted('mySubId')
        ...
        when: 'a data job subscription id is tested for uniqueness'
            def result = objectUnderTest.isUniqueDataJobSubscriptionId('mySubId')
    ...
    def 'Get all nodes for subscription id'() {
        given: 'the query service returns nodes for subscription id'
            ...
            def cmDataJobSubscriptionIdCpsPath = CPS_PATH_QUERY_FOR_CM_DATA_JOB_SUBSCRIPTION_WITH_DATA_JOB_ID.formatted('mySubId')
        ...
        when: 'retrieving all nodes for data job subscription id'
            def result = objectUnderTest.getAllNodesForDataJobSubscriptionId('mySubId')
    ...
    def 'Add new subscriber to an ongoing cm notification data job subscription: data type and alternate id exists'() {
        given: 'a valid cm subscription path query'
            def cmSubscriptionCpsPathQuery = CPS_PATH_QUERY_FOR_CM_DATA_JOB_SUBSCRIPTION_WITH_ALTERNATE_ID_AND_DATATYPE.formatted('altId1', 'dataType1')
        and: 'a dataNode exists for the given cps path query'
            mockCpsQueryService.queryDataNodes('NCMP-Admin', 'cm-data-job-subscriptions',
                cmSubscriptionCpsPathQuery, OMIT_DESCENDANTS) >> [new DataNode(leaves: ['dataJobId': ['existingId'], 'dataTypeId': 'dataType1', 'alternateId': 'altId1'])]
        and: 'updated cm data job subscription details as json'
            def subscriptionDetailsAsJson = objectUnderTest.getSubscriptionDetailsAsJson(['existingId', 'newSubId'], 'dataType1', 'altId1')
        when: 'the method to add/update cm notification subscription is called'
            objectUnderTest.addCmDataJobSubscription('dataType1', 'altId1', 'newSubId')
    ...
    def 'Add new cm notification data job subscription for data type and alternate Id'() {
        ...
        and: 'data job subscription details is mapped as JSON'
            ...
            def subscriptionAsJson = objectUnderTest.getSubscriptionDetailsAsJson(dataJobSubscriptionIds, 'dataType1', 'altId1')
        when: 'the method to add/update cm notification subscription is called'
            objectUnderTest.addCmDataJobSubscription('dataType1', 'altId1', 'newSubId')
    ...
    def 'Remove subscriber from a list of an ongoing cm notification subscription'() {
        given: 'a subscription exists when queried'
            def cmSubscriptionCpsPathQuery = CPS_PATH_QUERY_FOR_CM_DATA_JOB_SUBSCRIPTION_WITH_ALTERNATE_ID_AND_DATATYPE.formatted('altId1', 'dataType1')
            mockCpsQueryService.queryDataNodes('NCMP-Admin', 'cm-data-job-subscriptions',
                cmSubscriptionCpsPathQuery, OMIT_DESCENDANTS) >> [new DataNode(leaves: ['dataJobId': ['existingId','subIdToRemove'], 'dataTypeId': 'dataType1', 'alternateId': 'altId1'])]
        and: 'updated cm data job subscription details as json'
        ...
        when: 'the subscriber is removed'
            objectUnderTest.removeCmDataJobSubscription('dataType1', 'altId1','subIdToRemove')
    ...
    def 'Removing last ongoing subscription for datastore and cmhandle and xpath'() {
        ...
        and: 'a cps path with alternate id and data type for deleting a node'
            def cmDataJobSubscriptionCpsPath = CPS_PATH_QUERY_FOR_CM_DATA_JOB_SUBSCRIPTION_WITH_ALTERNATE_ID_AND_DATATYPE.formatted('last-alt-id', 'last-data-type')
        ...
        then: 'the data job subscription with empty subscribers list is removed'
            1 * mockCpsDataService.deleteDataNode('NCMP-Admin', 'cm-data-job-subscriptions', cmDataJobSubscriptionCpsPath, _)
    }
