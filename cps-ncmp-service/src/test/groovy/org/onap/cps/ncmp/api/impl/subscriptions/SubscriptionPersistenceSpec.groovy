...
class SubscriptionPersistenceSpec extends Specification {
    ...
    private static final String SUBSCRIPTION_REGISTRY_XPATH = "/subscription-registry/subscription[@clientID='some-client-id' and @subscriptionName='some-subscription-name']/predicates";
   ...
   def 'save a subscription event as yang model into db' () {
       given: 'a black data node that exist in db'
    ...
    def 'add or replace cm handle list element into db' () {
        ...
        then: 'the cpsDataService save non-existing cm handle with the correct data'
            1 * mockCpsDataService.saveListElements(SUBSCRIPTION_DATASPACE_NAME, SUBSCRIPTION_ANCHOR_NAME,
                SUBSCRIPTION_REGISTRY_XPATH, '{"targetCmHandles":[{"cmHandleId":"cmhandle2","status":"PENDING"}]}',
                NO_TIMESTAMP)
        and: 'the cpsDataService update existing cm handle with the correct data'
            1 * mockCpsDataService.updateNodeLeaves(SUBSCRIPTION_DATASPACE_NAME, SUBSCRIPTION_ANCHOR_NAME,
                SUBSCRIPTION_REGISTRY_XPATH, '{"targetCmHandles":[{"cmHandleId":"cmhandle1","status":"PENDING"}]}',
                NO_TIMESTAMP)
    }
