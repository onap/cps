...
class DmiSubJobRequestHandlerSpec extends Specification {
    ...
    def 'Send a sub-job request to the DMI Plugin.'() {
        given: 'a data job id, metadata and a map of producer keys and write operations to create a request'
            ...
            def dmi3gppWriteOperation = new DmiWriteOperation('fdn-1', 'create', 'module-set-tag', null, 'some-operation-id', ['dmi-service-name':'my-dmi-service'])
            def dmi3ggpWriteOperationsPerProducerKey = [new ProducerKey('my-dmi-service', 'my-data-producer-identifier'): [dmi3gppWriteOperation]]
        when: 'sending request to DMI invoked'
            objectUnderTest.sendRequestsToDmi(dataJobId, dataJobMetadata, dmi3ggpWriteOperationsPerProducerKey)
        then: 'the dmi rest client is called'
            1 * mockDmiRestClient.postOperationWithJsonData(RequiredDmiService.DATA, _, _, OperationType.CREATE, NO_AUTH)
    }
}
