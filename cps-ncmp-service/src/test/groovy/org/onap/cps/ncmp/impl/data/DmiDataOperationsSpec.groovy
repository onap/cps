...
class DmiDataOperationsSpec extends DmiOperationsBaseSpec {
    ...
    def 'Execute (async) data operation from DMI service.'() {
        ...
        and: 'a positive response from DMI service when it is called with valid request parameters'
            ...
            def expectedUrlTemplateWithVariables = new UrlTemplateParameters("someServiceName/dmi/v1/data?requestId={requestId}&topic={topic}", ['requestId': 'requestId', 'topic': 'my-topic-name'])
    ...
    def 'call get all resource data.'() {
        ...
        and: 'a positive response from DMI service when it is called with the expected parameters'
            ...
            def expectedTemplateWithVariables = new UrlTemplateParameters("someServiceName/dmi/v1/ch/{cmHandleId}/data/ds/{datastore}?resourceIdentifier={resourceIdentifier}", ['resourceIdentifier': '/', 'datastore': 'ncmp-datastore:passthrough-operational', 'cmHandleId': cmHandleId])
    ...
    def 'Write data for pass-through:running datastore in DMI.'() {
        ...
        and: 'a positive response from DMI service when it is called with the expected parameters'
            def expectedUrlTemplateParameters = new UrlTemplateParameters("someServiceName/dmi/v1/ch/{cmHandleId}/data/ds/{datastore}?resourceIdentifier={resourceIdentifier}", ['resourceIdentifier': resourceIdentifier, 'datastore': 'ncmp-datastore:passthrough-running', 'cmHandleId': cmHandleId])
    ...
    def getExpectedUrlTemplateWithVariables(expectedOptions, expectedDataStore) {
        ...
        def expectedUrlTemplate = "$dmiServiceName/dmi/v1/ch/{cmHandleId}/data/ds/{datastore}?resourceIdentifier={resourceIdentifier}" + (includeOptions ? "&options={options}" : "")
