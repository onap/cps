...
class DmiDataOperationsSpec extends DmiOperationsBaseSpec {
    ...
    def 'call get resource data for #expectedDataStore from DMI without topic #scenario.'() {
        ...
        when: 'get resource data is invoked'
            def cmResourceAddress = new CmResourceAddress(expectedDataStore.datastoreName, DmiOperationsBaseSpec.cmHandleId, DmiOperationsBaseSpec.resourceIdentifier)
    ...
    def 'Execute (async) data operation from DMI service.'() {
        ...
        and: 'a positive response from DMI service when it is called with valid request parameters'
            ...
            def expectedTemplateWithVariables = new UrlTemplateParameters('someServiceName/{dmiBasePath}/v1/data?requestId={requestId}&topic={topic}', ['requestId': 'requestId', 'topic': 'my-topic-name', 'dmiBasePath': dmiBasePath])
    ...
    def 'call get all resource data.'() {
        ...
        and: 'a positive response from DMI service when it is called with the expected parameters'
            ...
            def expectedTemplateWithVariables = new UrlTemplateParameters('someServiceName/{dmiBasePath}/v1/ch/{cmHandleId}/data/ds/{datastore}?resourceIdentifier={resourceIdentifier}', ['resourceIdentifier': '/', 'datastore': 'ncmp-datastore:passthrough-operational', 'dmiBasePath': dmiBasePath, 'cmHandleId': cmHandleId])
    ...
    def 'Write data for pass-through:running datastore in DMI.'() {
        ...
        and: 'a positive response from DMI service when it is called with the expected parameters'
            def expectedTemplateWithVariables = new UrlTemplateParameters('someServiceName/{dmiBasePath}/v1/ch/{cmHandleId}/data/ds/{datastore}?resourceIdentifier={resourceIdentifier}', ['resourceIdentifier': resourceIdentifier, 'datastore': 'ncmp-datastore:passthrough-running', 'dmiBasePath': dmiBasePath, 'cmHandleId': cmHandleId])
    ...
    def getExpectedUriTemplateWithVariables(expectedOptions, expectedDataStore) {
        ...
        def expectedUriTemplate = "$dmiServiceName/{dmiBasePath}/v1/ch/{cmHandleId}/data/ds/{datastore}?resourceIdentifier={resourceIdentifier}" + (includeOptions ? "&options={options}" : "")
        def uriVariables = ['resourceIdentifier': resourceIdentifier, 'datastore': expectedDataStore.datastoreName, 'dmiBasePath': dmiBasePath, 'cmHandleId': cmHandleId] + (includeOptions ? ['options': expectedOptions] : [:])
        return new UrlTemplateParameters(expectedUriTemplate, uriVariables)
    }
}
