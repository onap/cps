...
@SpringBootTest(classes = [TrustLevelDmiPluginCacheConfig])
class TrustLevelDmiPluginCacheConfigSpec extends Specification {
    ...
    def 'Verify Trust Level Per Dmi Plugin Cache for basic hazelcast map operations'() {
        when: 'the key inserted into Trust Level Per Dmi Plugin Cache'
            trustLevelPerDmiPlugin.put('some-dmi', TrustLevel.COMPLETE)
            trustLevelPerDmiPlugin.put('some-other-dmi', TrustLevel.NONE)
        then: 'the keys are being retrieved from Trust Level Per Dmi Plugin Cache'
            def resultForDmi = trustLevelPerDmiPlugin.get('some-dmi')
            def resultForOtherDmi = trustLevelPerDmiPlugin.get('some-other-dmi')
        and: 'the entry is present in the map as expected'
            assert TrustLevel.COMPLETE == resultForDmi
            assert TrustLevel.NONE == resultForOtherDmi
    }
    ...
    def 'Verify network config'() {
        given: 'Synchronization config object and test configuration'
            def objectUnderTest = new TrustLevelDmiPluginCacheConfig()
