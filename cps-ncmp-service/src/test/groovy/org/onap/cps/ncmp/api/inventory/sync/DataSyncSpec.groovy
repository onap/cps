...
class DataSyncSpec extends Specification {
    ...
    def 'Schedule Data Sync for Cm Handle State in READY and Operational Sync State in UNSYNCHRONIZED'() {
        ...
        then: 'the inventory persistence cm handle returns a composite state for the first cm handle'
            1 * mockInventoryPersistence.getCmHandleState('some-cm-handle-1') >> getCompositeState('some-cm-handle-1')
        ...
        and: 'the first cm handle operational sync state is updated from "UNSYNCHRONIZED" to "SYNCHRONIZED"'
            1 * mockInventoryPersistence.saveCmHandleState('some-cm-handle-1', getCompositeState('some-cm-handle-1'))
        then: 'the inventory persistence cm handle returns a composite state for the first cm handle'
            1 * mockInventoryPersistence.getCmHandleState('some-cm-handle-2') >> getCompositeState('some-cm-handle-2')
        and: 'the sync util returns first resource data'
        ...
        and: 'the second cm handle operational sync state is updated from "UNSYNCHRONIZED" to "SYNCHRONIZED"'
            1 * mockInventoryPersistence.saveCmHandleState('some-cm-handle-2', getCompositeState('some-cm-handle-2'))
    }

    def createSampleYangModelCmHandle(cmHandleId) {
        def compositeState = getCompositeState(cmHandleId)
        return new YangModelCmHandle(id: cmHandleId, compositeState: compositeState)
    }

    def getCompositeState(cmHandleId) {
