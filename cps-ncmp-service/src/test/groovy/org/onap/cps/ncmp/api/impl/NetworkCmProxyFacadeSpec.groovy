...
class NetworkCmProxyFacadeSpec extends Specification {
    ...
    def 'Execute Data Operation for CM Handles [delegation].'() {
        ...
        expect: 'the response form the handler'
             objectUnderTest.executeDataOperationForCmHandles('topic', dataOperationRequest, 'authorization') == responseFromHandler
    }
    ...
    def 'Query Resource Data for cm handle [delegation].'() {
        ...
        expect: 'the response form the handler'
            objectUnderTest.queryResourceDataForCmHandle('ch-1','some cps path',true) == responseFromHandler
    }
    ...
    def 'Update resource data for pass-through running from dmi.'() {
