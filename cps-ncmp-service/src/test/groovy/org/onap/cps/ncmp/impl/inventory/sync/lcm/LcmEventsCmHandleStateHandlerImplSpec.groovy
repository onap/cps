...
class LcmEventsCmHandleStateHandlerImplSpec extends Specification {
    ...
    def 'Update and Publish Events on State Change from non-existing to ADVISED'() {
        ...
        and: 'a log entry is written'
            assert logger.list[0].formattedMessage == "${cmHandleId} is now in ADVISED state"
    }
    ...
    def 'Update and Publish Events on State Change to READY'() {
        ...
        and: 'event service is called to publish event'
            1 * mockLcmEventsService.publishLcmEvent(cmHandleId, _, _)
        and: 'a log entry is written'
            assert logger.list[0].formattedMessage == "${cmHandleId} is now in READY state"
    }
    ...
    def 'No state change and no event to be published'() {
        ...
        then: 'no changes are persisted'
            ...
            0 * mockInventoryPersistence.saveCmHandleStateBatch(_)
    ...
    def 'Batch of new cm handles provided'() {
        ...
        and: 'two log entries are written'
            assert logger.list[0].formattedMessage == 'cmhandle1 is now in ADVISED state'
            assert logger.list[1].formattedMessage == 'cmhandle2 is now in ADVISED state'
    }
    ...
    def 'Batch of existing cm handles is updated'() {
        ...
        and: 'event service is called to publish events'
            2 * mockLcmEventsService.publishLcmEvent(_, _, _)
        and: 'two log entries are written'
            assert logger.list[0].formattedMessage == 'cmhandle1 is now in READY state'
            assert logger.list[1].formattedMessage == 'cmhandle2 is now in DELETING state'
    }
    ...
    def 'Batch of existing cm handles is deleted'() {
        ...
        and: 'event service is called to publish events'
            2 * mockLcmEventsService.publishLcmEvent(_, _, _)
        and: 'two log entries are written'
            assert logger.list[0].formattedMessage == 'cmhandle1 is now in DELETED state'
            assert logger.list[1].formattedMessage == 'cmhandle2 is now in DELETED state'
    }
