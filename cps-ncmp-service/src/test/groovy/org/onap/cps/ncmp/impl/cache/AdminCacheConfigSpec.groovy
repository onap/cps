...
class AdminCacheConfigSpec extends Specification {
    ...
    @Autowired
    @Qualifier(AdminCacheConfig.CM_HANDLES_BY_STATE)
    ...
    def 'Hazelcast cache for cm handle by state gauge'() {
        ...
        and: 'Hazelcast cache instance for cm handle by state is present'
            assert Hazelcast.getHazelcastInstanceByName('cps-and-ncmp-hazelcast-instance-test-config').getMap(AdminCacheConfig.CM_HANDLES_BY_STATE) != null
        and: 'the cache already contains 5 entries for each state'
            def cmHandleByState =  Hazelcast.getHazelcastInstanceByName('cps-and-ncmp-hazelcast-instance-test-config').getMap(AdminCacheConfig.CM_HANDLES_BY_STATE)
