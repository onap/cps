...
class OpenTelemetryConfigPropertiesSpec extends Specification {
    ...
    def 'Default values are correctly set'() {
        expect: 'validate that the default values are set correctly'
            ...
            assert openTelemetryConfigProperties.exporter.endpoint == 'exporter-test-url'
            assert openTelemetryConfigProperties.sampler.jaegerRemote.endpoint == 'jaeger-Remote-test-url'
            assert !openTelemetryConfigProperties.enabled
            assert openTelemetryConfigProperties.excludedTaskNames == ['some.task.name', 'another.task.name']
    }
}
