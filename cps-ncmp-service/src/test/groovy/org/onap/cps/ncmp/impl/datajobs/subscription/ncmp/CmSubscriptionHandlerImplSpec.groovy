...
class CmSubscriptionHandlerImplSpec extends Specification {
    ...
    def 'Update subscription status to ACCEPTED: #scenario'() {
        ...
        and: 'alternate id matcher always returns a cm handle id'
            mockAlternateIdMatcher.getCmHandleId(_) >> 'someCmHandleId'
        and: 'the inventory persistence service returns a yang model with a dmi service name'
        ...
        when: 'the method to update subscription status is called with status=ACCEPTED'
            objectUnderTest.updateSubscriptionStatus(mySubscriptionId, myDmi, ACCEPTED)
        then: 'the persistence service to update subscription status is called for every dmi service name matched'
            expectedCallsToPersistenceService * mockCmSubscriptionPersistenceService.updateStatus('/myDataNodeSelector[id=""]', ACCEPTED)
        where: 'the following data are used'
            scenario                                                                |dmiName        || expectedCallsToPersistenceService
            'given dmi name matches resolved dmi name for data node selector'       |'myDmi'        || 1
            'given dmi name DOES NOT match resolved dmi name for data node selector'| 'someOtherDmi'|| 0
    }
