...
class CmSubscriptionHandlerImplSpec extends Specification {
    ...
    def 'Process subscription CREATE request for new target [non existing]'() {
        given: 'relevant subscription details'
            ...
            def dataJobExtraAttr = new DataSelector(notificationTypes: notificationTypes, notificationFilter: notificationFilter)
        ...
        when: 'the method to process subscription create request is called'
            objectUnderTest.processSubscriptionCreate(dataJobExtraAttr, mySubId, myDataNodeSelectors)
    ...
    def 'Process subscription CREATE request for overlapping targets [non existing & existing]'() {
        ...
        when: 'the method to process subscription create request is called'
            objectUnderTest.processSubscriptionCreate(dataJobExtraAttr, myNewSubId, myDataNodeSelectors)
        then: 'the persistence service is called'
        ...
        where: 'following data are used'
            ...
            'new target overlaps with REJECTED targets'        | ['/existingDataNodeSelector','/newDataNodeSelector']|| 1
            'new target overlaps with UNKNOWN targets'         | ['/existingDataNodeSelector','/newDataNodeSelector']|| 1
            'new target does not overlap with existing targets'| ['/newDataNodeSelector']                            || 1
    }
