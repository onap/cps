...
class SyncUtilsSpec extends Specification{
    ...
    def static stateDataNodes = [new DataNodeBuilder()
                                         ...
                                         .withLeaves(['reason': 'LOCKED-MISBEHAVING', 'details': 'lock details']).build(),
                                 new DataNodeBuilder()
    ...
    def 'Get all locked Cm-Handle where Lock Reason is LOCKED-MISBEHAVING cm handle #scenario'() {
        given: 'the cps (persistence service) returns a collection of data nodes'
            mockCpsDataPersistenceService.queryDataNodes(CM_HANDLE_DATASPACE,
                    CM_HANDLE_ANCHOR, "//lock-reason[@reason=\"LOCKED-MISBEHAVING\"]/ancestor::cm-handles",
                    FetchDescendantsOption.OMIT_DESCENDANTS) >> dataNodeCollection
