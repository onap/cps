...
class SyncUtilsSpec extends Specification{
    ...
    def 'Retry Locked Cm-Handle where the last update time is #scenario'() {
        when: 'retry locked cm handle is invoked'
            def result = objectUnderTest.retryLockedCmHandle(new CompositeStateBuilder()
                .withLockReason(LockReasonCategory.LOCKED_MISBEHAVING, 'test')
                .withLastUpdatedTime(lastUpdateTime).build())
        then: 'result returns #expectedResult'
            result == expectedResult
        where:
            scenario                     | lastUpdateTime                  || expectedResult
            'is greater than one minute' | '1900-01-01T00:00:00.000+0100'  || true
            'is less than one minute'    | formattedDateAndTime            || false
    }
