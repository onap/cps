...
class SyncUtilsSpec extends Specification{
    ...
    def 'Update cm handle state'() {
        ...
        when: 'update cm handle state is called and cm handle state is updated to READY'
            objectUnderTest.updateCmHandleState(yangModelCmHandle)
        then: 'update data node leaves is invoked with the correct params'
    ...
    def 'Update cm handle state from ADVISED to LOCKED'() {
        ...
        when: 'lock cm handle state is called'
            objectUnderTest.lockCmHandleState(yangModelCmHandle, LockReasonEnum.LOCKED_MISBEHAVING, 'some lock reason details')
