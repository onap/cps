...
class PolicyExecutorSpec extends Specification {
    ...
    def 'Build policy executor patch operation details from ProvMnS request parameters where #scenario.'() {
        given: 'a provMnsRequestParameter and a patchItem list'
            def path = new RequestPathParameters(uriLdnFirstPart: 'someUriLdnFirstPart', className: 'classNameInUri', id: 'someId')
        ...
        then: 'the result contain 2 operations of the correct types in the correct order'
    ...
    def 'Build policy executor patch operation details from ProvMnS request parameters where the path #scenario'() {
        given: 'a requestParameter and a patchItem list'
            ...
            def pathItems = [new PatchItem(op: 'REPLACE', 'path':"someUriLdnFirstPart${suffix}", value: value)]
        when: 'patch operation details are created'
            def result = objectUnderTest.buildPatchOperationDetails(path, pathItems)
        then: 'the result has the correct type and populated fields'
            assert result instanceof PatchOperationsDetails
            assert result.operations.size() == 1
            assert result.operations[0]['targetIdentifier'] == 'someUriLdnFirstPart'
            assert result.operations[0]['changeRequest'].size() == 1
            assert result.operations[0]['changeRequest'].keySet()[0] == 'someClassName'
            assert result.operations[0]['changeRequest']['someClassName'][0]['attributes'] != null
        and: 'the attributes structure of the result is correct'
            assert jsonObjectMapper.asJsonString(result).contains(expectedJsonStringResult)
        where:
            scenario             | suffix                         | value         || expectedJsonStringResult
            'contains a #'       | '#/attributes/simpleAttribute' | 1             || '"attributes":{"simpleAttribute":1}}'
            'does not contain #' | ''                             | getResource() || '"attributes":["someAttribute1:someValue1","someAttribute2:someValue2"]'
    }
    ...
    def 'Build an attribute map with different depths of hierarchy with #scenario.'() {
        ...
        then: 'the map depth is equal to the expected number of attributes'
            assert hierarchyMap.toString().contains(expectedAttributesOutput)
        where:
            scenario                  | path                                                               || expectedAttributesOutput
            'set a simple attribute'  | 'someUriLdnFirstPart#/attributes/simpleAttribute'                  || '[simpleAttribute:123]'
            'set a complex attribute' | 'someUriLdnFirstPart#/attributes/complexAttribute/simpleAttribute' || '[complexAttribute:[simpleAttribute:123]]'
    }
