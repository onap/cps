...
class SubscriptionEventResponseOutcomeSpec extends DataNodeBaseSpec {
    ...
    def 'Check cm handle id to status map to see if it is a full outcome response'() {
        ...
        where: 'the following values are used'
            ...
            'The map contains PENDING status'                 | ['CMHanlde1': SubscriptionStatus.PENDING] as Map                                            || false
            'The map contains ACCEPTED status'                | ['CMHanlde1': SubscriptionStatus.ACCEPTED] as Map                                           || true
            'The map contains REJECTED status'                | ['CMHanlde1': SubscriptionStatus.REJECTED] as Map                                           || true
            'The map contains PENDING and ACCEPTED statuses'  | ['CMHanlde1': SubscriptionStatus.PENDING,'CMHanlde2': SubscriptionStatus.ACCEPTED] as Map   || false
            'The map contains REJECTED and ACCEPTED statuses' | ['CMHanlde1': SubscriptionStatus.REJECTED,'CMHanlde2': SubscriptionStatus.ACCEPTED] as Map  || true
            'The map contains PENDING and REJECTED statuses'  | ['CMHanlde1': SubscriptionStatus.PENDING,'CMHanlde2': SubscriptionStatus.REJECTED] as Map   || false
    }
