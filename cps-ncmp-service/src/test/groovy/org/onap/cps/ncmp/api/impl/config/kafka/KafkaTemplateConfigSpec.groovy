...
class KafkaTemplateConfigSpec extends Specification {
    ...
    def 'Verify kafka template serializer and deserializer configuration for #eventType.'() {
        ...
        and: 'verify event key and value deserializer'
            assert kafkaTemplateInstance.properties['consumerFactory'].configs['spring.deserializer.value.delegate.class'].asType(String.class).contains(springDeserializerValue.getCanonicalName())
        where: 'the following event type is used'
            eventType      | kafkaTemplateInstance    || beanName                   | valueSerializer      | springDeserializerValue
