...
class CmSubscriptionComparatorSpec extends Specification {
    ...
    def objectUnderTest = new CmSubscriptionComparator(mockCmNotificationSubscriptionPersistenceService)

    def 'Find Delta of given list of predicates'() {
        given: 'A list of predicates'
            def predicateList = [new DmiCmSubscriptionPredicate(['ch-1', 'ch-2'].toSet(), DatastoreType.PASSTHROUGH_OPERATIONAL, ['a/1/', 'b/2'].toSet())]
        ...
        when: 'getDelta is called'
            def result = objectUnderTest.getNewDmiCmNotificationSubscriptionPredicates(predicateList)
    ...
    def 'Find Delta of given list of predicates when it is an ongoing Cm Subscription'() {
        given: 'A list of predicates'
            def predicateList = [new DmiCmSubscriptionPredicate(['ch-1'].toSet(), DatastoreType.PASSTHROUGH_OPERATIONAL, ['a/1/'].toSet())]
