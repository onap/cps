...
class TrustLevelManagerSpec extends Specification {
    ...
    def 'Initial cm handle registration with a cm handle that is not trusted'() {
        ...
        then: 'notification is sent'
            1 * mockAttributeValueChangeEventPublisher.publishAttributeValueChangeEvent(*_)
    }

    def 'Trust level updated'() {
        given: 'a cm handle: with trusted'
    ...
    def 'Trust level updated with the different trust factor'() {
        given: 'a cm handle: wit not trusted'
        ...
        and: 'cm handle in the cache is trusted'
            assert trustLevelPerCmHandle.get('ch-1') == TrustLevel.COMPLETE
    }
