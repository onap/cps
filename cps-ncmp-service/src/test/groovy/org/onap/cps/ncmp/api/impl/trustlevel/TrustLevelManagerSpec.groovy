...
class TrustLevelManagerSpec extends Specification {
    ...
    def 'Initial cm handle registration'() {
        given: 'two cm handles: one with no trustlevel and one not trusted'
        ...
        then: 'notifications sent for not trusted only'
            1 * mockAttributeValueChangeEventPublisher.publishAttributeValueChangeEvent('ch-2', 'trustLevel', null, 'NONE')
        and: 'both cm handles are in the cache: one with trusted and the other not trusted'
            assert trustLevelPerCmHandle.get('ch-1') == TrustLevel.COMPLETE
            assert trustLevelPerCmHandle.get('ch-2') == TrustLevel.NONE
    }

    def 'Initial cm handle registration with a cm handle that is trusted'() {
        given: 'the cache has been initialised and "knows" about my cm handles'
            trustLevelPerCmHandle.put('ch-1', TrustLevel.COMPLETE)
        and: 'a cm handle that is trusted'
            def cmHandleModelsToBeCreated = ['ch-1': TrustLevel.COMPLETE]
        when: 'the initial registration handled'
            objectUnderTest.handleInitialRegistrationOfTrustLevels(cmHandleModelsToBeCreated)
    ...
    def 'Update trust level'() {
        given: 'a cm handle with not trusted '
            trustLevelPerCmHandle.put('ch-1', TrustLevel.NONE)
        when: 'the update handled with trusted cm handle'
        ...
        and: 'the cm handle in the cache with trusted'
            assert trustLevelPerCmHandle.get('ch-1') == TrustLevel.COMPLETE
    }

    def 'Update trust level with the same trust value'() {
        given: 'a cm handle with not trusted'
            trustLevelPerCmHandle.put('ch-1', TrustLevel.NONE)
