...
import static CmDataJobSubscriptionPersistenceService.DATA_JOB_SUBSCRIPTION_DATA_NODE_SELECTOR
import static CmDataJobSubscriptionPersistenceService.CPS_PATH_TEMPLATE_FOR_SUBSCRIPTION_WITH_DATA_JOB_ID
import static CmDataJobSubscriptionPersistenceService.CPS_PATH_TEMPLATE_FOR_SUBSCRIPTION_WITH_STATUS_FOR_DATA_JOB_ID
...
class CmSubscriptionPersistenceServiceSpec extends Specification {
    ...
    def 'Check cm data job subscription details has at least one subscriber #scenario'() {
        given: 'a valid cm data job subscription query'
            def cpsPathQuery = DATA_JOB_SUBSCRIPTION_DATA_NODE_SELECTOR.formatted('/myDataNodeSelector')
    ...
    def 'Get all nodes for subscription id with status'() {
        given: 'the query service returns nodes for subscription id'
            ...
            def cmDataJobSubscriptionIdCpsPath = CPS_PATH_TEMPLATE_FOR_SUBSCRIPTION_WITH_STATUS_FOR_DATA_JOB_ID.formatted('UNKNOWN', 'id1')
        ...
        when: 'retrieving all nodes for data job subscription id'
            def result = objectUnderTest.getDataNodesForSubscription('id1', UNKNOWN)
        then: 'the result returns correct number of datanodes'
            assert result.size() == 1
        and: 'the attribute of the data nodes is as expected'
            assert result.iterator().next().leaves.dataNodeSelector == expectedDataNode.leaves.dataNodeSelector
            assert result.iterator().next().leaves.status == expectedDataNode.leaves.status
            assert result.iterator().next().leaves.dataNode == expectedDataNode.leaves.dataNode
    }

    def 'Get all data node selector(s) with status REJECTED'() {
        given: 'the query service returns nodes for subscription id'
            def expectedDataNode = new DataNode(leaves: ['datajobId': ['id1'], 'dataNodeSelector': '/rejectedDataNodeSelector', 'status': 'REJECTED'])
            def queryServiceResponse = [expectedDataNode].asCollection()
            def cmDataJobSubscriptionIdCpsPath = CPS_PATH_TEMPLATE_FOR_SUBSCRIPTION_WITH_STATUS_FOR_DATA_JOB_ID.formatted('REJECTED', 'id1')
            1 * mockCpsQueryService.queryDataNodes('NCMP-Admin', 'cm-data-job-subscriptions', cmDataJobSubscriptionIdCpsPath, OMIT_DESCENDANTS) >> queryServiceResponse
        when: 'retrieving all nodes for data job subscription id'
            def result = objectUnderTest.getRejectedDataNodeSelectors('id1')
        then: 'the result returns correct number of data node selectors'
            assert result.size() == 1
        and: 'the content of the result is as expected'
            assert result.iterator().next() == '/rejectedDataNodeSelector'
    }
    ...
    def 'Add subscription for a data node selector that have no subscriptions yet.'() {
        given: 'a valid cm data job subscription path query'
            ...
            def query = DATA_JOB_SUBSCRIPTION_DATA_NODE_SELECTOR.formatted(dataNodeSelector)
        ...
        and: 'data job subscription details is mapped as JSON'
            ...
            def subscriptionAsJson = objectUnderTest.getSubscriptionDetailsAsJson(dataNodeSelector, subscriptionIds, 'UNKNOWN')
        when: 'the method to add cm notification subscription is called'
            objectUnderTest.addSubscription(dataNodeSelector, 'newSubId')
    ...
    def 'Add subscription for a data node selector that already have subscription(s).'() {
        ...
        and: 'updated cm data job subscription details as json'
            ...
            def subscriptionDetailsAsJson = objectUnderTest.getSubscriptionDetailsAsJson(dataNodeSelector, newListOfSubscriptionIds, 'ACCEPTED')
        when: 'the method to add cm notification subscription is called'
            objectUnderTest.addSubscription(dataNodeSelector, 'newSubId')
