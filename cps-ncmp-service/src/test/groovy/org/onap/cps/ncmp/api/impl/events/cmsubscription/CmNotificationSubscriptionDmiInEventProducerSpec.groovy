...
class CmNotificationSubscriptionDmiInEventProducerSpec extends Specification {
    ...
    def 'Create and Publish Cm Notification Subscription DMI In Event'() {
        given: 'a cm subscription for a dmi plugin'
            def cmSubscriptionId = 'test-subscription-id'
        ...
        when: 'the event is published'
            objectUnderTest.publishCmNotificationSubscriptionDmiInEvent(cmSubscriptionId, dmiPluginName, eventType, cmNotificationSubscriptionDmiInEvent)
                ...
                args ->
                    {
                        ...
                        assert args[1] == cmSubscriptionId
                        def cmNotificationSubscriptionDmiInEventAsCloudEvent = (args[2] as CloudEvent)
                        assert cmNotificationSubscriptionDmiInEventAsCloudEvent.getExtension('correlationid') == cmSubscriptionId + '#' + dmiPluginName
