...
class DmiCmNotificationSubscriptionCacheHandlerSpec extends MessagingBaseSpec {
    ...
    def testCache = [:]
    def dmiCmNotificationSubscriptionDetailsPerDmi = [:]
    def objectUnderTest = new DmiCmNotificationSubscriptionCacheHandler(testCache, mockInventoryPersistence, dmiCmNotificationSubscriptionDetailsPerDmi)
    ...
    def 'Create map for DMI cm notification subscription per DMI service name'() {
        ...
        and: 'the target cmHandles for each predicate is correct'
            assert resultMapForDmi1.dmiCmNotificationSubscriptionPredicates[0].targetCmHandleIds == ['ch1']
            assert resultMapForDmi1.dmiCmNotificationSubscriptionPredicates[1].targetCmHandleIds == ['ch3']

            assert resultMapForDmi2.dmiCmNotificationSubscriptionPredicates[0].targetCmHandleIds == ['ch2']
            assert resultMapForDmi2.dmiCmNotificationSubscriptionPredicates[1].targetCmHandleIds == ['ch4']
        and: 'the list of xpath for each is correct'
            assert resultMapForDmi1.dmiCmNotificationSubscriptionPredicates[0].xpaths
                    && resultMapForDmi2.dmiCmNotificationSubscriptionPredicates[0].xpaths == ['/x1/y1','x2/y2']

            assert resultMapForDmi1.dmiCmNotificationSubscriptionPredicates[1].xpaths
                    && resultMapForDmi2.dmiCmNotificationSubscriptionPredicates[1].xpaths == ['/x3/y3','x4/y4']
    }
