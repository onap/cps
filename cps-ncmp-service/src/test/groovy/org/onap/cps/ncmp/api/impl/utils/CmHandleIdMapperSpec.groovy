...
class CmHandleIdMapperSpec extends Specification {
    ...
    def setup() {
        ...
        objectUnderTest.addMapping('my cmhandle id', 'alternate id')
    }
    ...
    def 'Checking entries in the cache.'() {
        expect: 'the alternate id can be converted to cmhandle id'
            assert objectUnderTest.alternateIdToCmHandleId('alternate id') == 'my cmhandle id'
        and: 'the cmhandle id can be converted to alternate id'
            assert objectUnderTest.cmHandleIdToAlternateId('my cmhandle id') == 'alternate id'
    }
    ...
    def 'Cannot update existing alternate id.'() {
        ...
        and: 'a warning is logged with the original alternate id'
            ...
            assert lastLoggingEvent.formattedMessage.contains('alternate id')
    }

    def 'Update existing alternate id with the same value.'() {
        given: 'update an existing alternate id with the same value'
            objectUnderTest.addMapping('my cmhandle id', 'alternate id')
        and: 'one message was logged about the initialization'
            def lastLoggingEvent = logger.list[0]
            assert lastLoggingEvent.level == Level.INFO
            assert lastLoggingEvent.formattedMessage.contains('Alternate ID cache initialized')
    }
