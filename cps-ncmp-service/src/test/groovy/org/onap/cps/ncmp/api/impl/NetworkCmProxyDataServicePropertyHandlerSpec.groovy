    ...
    def '#scenario error leads to #exception when we try to update cmHandle'() {
        ...
        and: 'it has expected error details'
            with(response.get(0)) {
                ...
                assert it.ncmpResponseCode == expectedError
                assert it.errorText == expectedErrorText
            }
        where:
        scenario                   | cmHandleId               | exception                                                                                           || expectedError        | expectedErrorText
        'Cm Handle does not exist' | 'cmHandleId'             | new DataNodeNotFoundException(NCMP_DATASPACE_NAME, NCMP_DMI_REGISTRY_ANCHOR)                        || CM_HANDLES_NOT_FOUND | 'cm-handle does not exist'
    ...
    def 'Multiple update operations in a single request'() {
        ...
        and: 'the 2nd request failed with correct error code'
            with(cmHandleResponseList.get(1)) {
                assert it.status == Status.FAILURE
                assert it.cmHandle == cmHandleId
                assert it.ncmpResponseCode == CM_HANDLES_NOT_FOUND
