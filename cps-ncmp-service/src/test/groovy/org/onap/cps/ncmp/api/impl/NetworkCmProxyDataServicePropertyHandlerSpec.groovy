...
class NetworkCmProxyDataServicePropertyHandlerSpec extends Specification {
    ...
    def 'Update CM Handle Public Properties: #scenario'() {
        ...
        then: 'the replace list method is called with correct params'
            1 * mockCpsDataService.replaceListContent(dataspaceName, anchorName, cmHandleXpath, _, noTimeStamp) >> { args ->
                {
                    assert args[3].leaves == convertToProperties(expectedPropertiesAfterUpdate)
                }
            }
    ...
    def 'Update DMI Properties: #scenario'() {
        ...
        where: 'following DMI properties updates are made'
            ...
            'property added'                  | ['newAdditionalProp1': 'add-value'] || [['additionalProp1': 'additionalValue1'], ['additionalProp2': 'additionalValue2'], ['newAdditionalProp1': 'add-value']]
            'property updated'                | ['additionalProp1': 'newValue']     || [['additionalProp2': 'additionalValue2'], ['additionalProp1': 'newValue']]
            'property removed'                | ['additionalProp1': null]           || [['additionalProp2': 'additionalValue2']]
            'property ignored(value is null)' | ['new-prop': null]                  || [['additionalProp1': 'additionalValue1'], ['additionalProp2': 'additionalValue2']]
    }

    def 'Update CM Handle Properties(all property to remove): #scenario'() {
        given: 'the CPS service return a CM handle'
            mockCpsDataService.getDataNode(dataspaceName, anchorName, cmHandleXpath, FetchDescendantsOption.INCLUDE_ALL_DESCENDANTS) >> cmHandleDataNode
        and: 'an update cm handle request with public properties updates'
            def cmHandleUpdateRequest = [new CmHandle(cmHandleID: cmHandleId, publicProperties: updatedPublicProperties)]
        ...
        then: 'the replace list method is not called'
            expectedCallsToReplaceListContent * mockCpsDataService.replaceListContent(*_)
        then: 'delete data node will be called'
            expectedCallsToDeleteDataNode * mockCpsDataService.deleteDataNode(dataspaceName, anchorName, _, noTimeStamp) >> { arg ->
                {
                    arg[2].contains("@name='publicProp")
                }
            }
        where: 'following public properties updates are made'
            scenario               | updatedPublicProperties                    || expectedPropertiesAfterUpdate | expectedCallsToReplaceListContent | expectedCallsToDeleteDataNode
            'all property removed' | ['publicProp3': null, 'publicProp4': null] || [[]]                          | 0                                 | 2
    }
    ...
    def convertToProperties(expectedPropertiesAfterUpdateAsMap) {
        ...
        expectedPropertiesAfterUpdateAsMap.forEach(property -> property.forEach((key, val) -> {
