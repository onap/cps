...
class CmSubscriptionNcmpInEventConsumerSpec extends MessagingBaseSpec {
    ...
    def 'Consume, persist and forward valid CMSubscription create message'() {
        given: 'a cmsubscription event'
            ...
            def testCloudEventData = CloudEventBuilder.v1()
                .withData(objectMapper.writeValueAsBytes(testEventData))
                .withId('subscriptionCreated')
                .withType('subscriptionCreated')
                .withSource(URI.create('some-resource'))
                .withExtension('correlationid', 'test-cmhandle1').build()
            def consumerRecord = new ConsumerRecord<String, CloudEvent>('topic-name', 0, 0, 'event-key', testCloudEventData)
        ...
        then: 'the event is mapped to a yangModelSubscription'
            numberOfTimesToPersist * mockCmSubscriptionNcmpInEventMapper.toYangModelCmDataSubscriptionEvent(testEventData) >> yangModelCmDataSubscriptionEvent
        where: 'given values are used'
        scenario                                          | isNotificationEnabled | isModelLoaderEnabled || numberOfTimesToPersist
        'Both model loader and notification are enabled'  |    true               |      true            ||        1
        'Both model loader and notification are disabled' |    false              |      false           ||        0
        'Model loader enabled and notification  disabled' |    false              |      true            ||        1
        'Model loader disabled and notification enabled'  |    true               |      false           ||        0
    }
