...
import org.onap.cps.ncmp.events.async.DataOperationEventV1
...
class NcmpAsyncDataOperationEventConsumerSpec extends MessagingBaseSpec {
    ...
    @SpringBean
    EventsPublisher asyncDataOperationEventPublisher = new EventsPublisher<DataOperationEventV1>(kafkaTemplate)
    ...
    @Autowired
    RecordFilterStrategy<String, DataOperationEventV1> recordFilterStrategy

    def kafkaConsumer = new KafkaConsumer<>(consumerConfigProperties('test'))
    def static clientTopic = 'client-topic'
    def static dataOperationType = 'org.onap.cps.ncmp.events.async.DataOperationEventV1'
    ...
    def 'Consume and publish event to client specified topic'() {
        ...
        then: 'verifying consumed event operationID is same as published event operationID'
            ...
            def operationIdOut = jsonObjectMapper.convertJsonString((String)consumerRecordOut.value(), DataOperationEventV1.class).data.responses[0].operationId
    ...
    def createConsumerRecord(eventTypeAsString) {
        ...
        def testEventSent = jsonObjectMapper.convertJsonString(jsonData, DataOperationEventV1.class)
