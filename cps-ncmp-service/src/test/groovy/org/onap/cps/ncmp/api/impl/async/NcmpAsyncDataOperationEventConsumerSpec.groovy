...
@SpringBootTest(classes = [EventsPublisher, NcmpAsyncDataOperationEventConsumer, DataOperationRecordFilterStrategy,JsonObjectMapper,
                ObjectMapper])
@Testcontainers
@DirtiesContext
class NcmpAsyncDataOperationEventConsumerSpec extends MessagingBaseSpec {
    ...
    def static dataOperationEventType = 'org.onap.cps.ncmp.events.async.DataOperationResponseEventV1'
    ...
    def 'Consume and publish event to client specified topic'() {
        ...
        and: 'consumer record for data operation event'
            def consumerRecordIn = createConsumerRecord(dataOperationEventType)
    ...
    def 'Filter an event with type #eventType'() {
        ...
        where: 'filter the event based on the eventType #eventType'
            ...
            'not filtered(the consumer will see the event)' | dataOperationEventType || false
