...
class InventoryPersistenceImplSpec extends Specification {
    ...
    def 'Getting cm handle data node for #scenario'() {
        given: 'expected xPath to get cmHandle data node'
            def expectedXPath = '/dmi-registry/cm-handles[@alternate-id=\'alternate id\']'
        and: 'query service is invoked once with expected xpath'
            mockCmHandleQueries.queryNcmpRegistryByCpsPath(expectedXPath, OMIT_DESCENDANTS) >> dataNode
        when: 'getting the cm handle data node'
            def thrownException
            def result
            try {
                result = objectUnderTest.getCmHandleDataNodeByAlternateId('alternate id')
            } catch (DataNodeNotFoundException dataNodeNotFoundException) {
                thrownException = dataNodeNotFoundException
            }
        then: 'result as expected'
            assert result == expectedResult
            assert thrownException == expectedException
        where: 'following data nodes is used'
            scenario   | dataNode         || expectedResult | expectedException
            'existing' | [new DataNode()] || new DataNode() | null
            //'none'     | []               || null           | new DataNodeNotFoundException('','')
    }
