...
class CmHandleRegistrationServicePropertyHandlerSpec extends Specification {
    ...
    def 'Update Additional Properties syncs dmi-properties when it exists: #scenario'() {
        ...
        then: 'dmi-properties is updated with the merged result'
            1 * mockInventoryPersistence.updateCmHandleField(cmHandleId, 'dmi-properties', expectedDmiProperties)
        where: 'the following scenarios are used'
            scenario           | updatedAdditionalProperties                        || expectedDmiProperties
    ...
    def 'Update Additional Properties does not sync dmi-properties when it does not exist.'() {
        ...
        then: 'dmi-properties is never updated'
            0 * mockInventoryPersistence.updateCmHandleField(_, 'dmi-properties', _)
    }
