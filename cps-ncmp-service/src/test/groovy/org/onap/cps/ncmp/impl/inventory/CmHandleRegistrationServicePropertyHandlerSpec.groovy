...
import org.onap.cps.ncmp.impl.inventory.sync.lcm.LcmEventsCmHandleStateHandlerAsyncHelper
...
class CmHandleRegistrationServicePropertyHandlerSpec extends Specification {
    ...
    def mockLcmEventsCmHandleStateHandlerAsyncHelper = Mock(LcmEventsCmHandleStateHandlerAsyncHelper)

    def objectUnderTest = new CmHandleRegistrationServicePropertyHandler(mockInventoryPersistence, mockCpsDataService, jsonObjectMapper, mockAlternateIdChecker, mockCmHandleIdPerAlternateId, mockLcmEventsCmHandleStateHandlerAsyncHelper)
    ...
    def 'Update CM Handle data producer identifier from #scenario'() {
        ...
        and: 'LCM event is sent asynchronously'
            1 * mockLcmEventsCmHandleStateHandlerAsyncHelper.sendLcmEventAsynchronously(_, _) >> { args ->
        ...
        and: 'correct information is logged'
            ...
            assert loggingEvents.any { it.level == Level.DEBUG && it.formattedMessage.contains('initially set') }
    ...
    def 'Update CM Handle data producer identifier with same value'() {
        ...
        and: 'LCM event is not sent'
            0 * mockLcmEventsCmHandleStateHandlerAsyncHelper.sendLcmEventAsynchronously(*_)
        and: 'debug information is logged'
            def lastLoggingEvent = logger.list[0]
            assert lastLoggingEvent.level == Level.DEBUG
            assert lastLoggingEvent.formattedMessage.contains('dataProducerIdentifier for cmHandle cmHandleId is already set to same-data-producer-id')
    }
    ...
    def 'Update CM Handle data producer identifier from existing to new value'() {
        ...
        and: 'correct information is logged'
            def loggingEvents = logger.list
            assert loggingEvents.any {
                it.level == Level.DEBUG &&
                    it.formattedMessage.contains('updated from oldDataProducerIdentifier to newDataProducerIdentifier')
            }
    }
    ...
    def createYangModelCmHandle(cmHandleId, dataProducerIdentifier) {
        ...
        yangModelCmHandle.dmiDataServiceName = 'some-dmi-plugin'
        yangModelCmHandle.dataProducerIdentifier = dataProducerIdentifier
        yangModelCmHandle.additionalProperties = []
        yangModelCmHandle.publicProperties = []
