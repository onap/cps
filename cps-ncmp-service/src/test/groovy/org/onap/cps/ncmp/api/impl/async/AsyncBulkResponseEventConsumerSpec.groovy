...
class AsyncBulkResponseEventConsumerSpec extends MessagingBaseSpec {
    ...
    @SpringBean
    EventsPublisher bulkResponseEventPublisher = new EventsPublisher<AsyncBulkResponseEvent>(kafkaTemplate);
    ...
    def 'Consume and forward valid message'() {
        ...
        def jsonData = TestUtils.getResourceFileContent('bulkResponseEvent.json')
        def testEventSent = jsonObjectMapper.convertJsonString(jsonData, BulkResponseEvent.class)
        ...
        and: 'consumed published eventCorrelationId is the same as sent eventCorrelationId'
        def record = records.iterator().next()
        assert testEventSent.eventCorrelationId.equalsIgnoreCase(jsonObjectMapper.convertJsonString(record.value(),
                BulkResponseEvent).getEventCorrelationId())
    }
}
