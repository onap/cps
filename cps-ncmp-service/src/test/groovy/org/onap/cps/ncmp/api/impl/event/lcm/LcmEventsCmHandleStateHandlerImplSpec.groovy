...
class LcmEventsCmHandleStateHandlerImplSpec extends Specification {
    ...
    def 'New Cm Handles and Publish Events on State Change in Batch'() {
    ...
    def 'Update Existing Cm Handles and Publish Events on State Change in Batch'() {
        given: 'Cm Handle Batch represented as Map of YangModelCmHandle and target cm handle state'
    ...
    def setupBatch(type) {
        ...
        def yangModelCmHandle1
        def yangModelCmHandle2

        if ('NEW' == type) {
            yangModelCmHandle1 = new YangModelCmHandle(id: 'cmhandle1', dmiProperties: [], publicProperties: [])
            yangModelCmHandle2 = new YangModelCmHandle(id: 'cmhandle2', dmiProperties: [], publicProperties: [])
            return [(yangModelCmHandle1): ADVISED, (yangModelCmHandle2): ADVISED]
        }

        if ('UPDATE' == type) {
            yangModelCmHandle1 = new YangModelCmHandle(id: 'cmhandle1', compositeState: new CompositeState(cmHandleState: ADVISED), dmiProperties: [], publicProperties: [])
            yangModelCmHandle2 = new YangModelCmHandle(id: 'cmhandle2', compositeState: new CompositeState(cmHandleState: READY), dmiProperties: [], publicProperties: [])
