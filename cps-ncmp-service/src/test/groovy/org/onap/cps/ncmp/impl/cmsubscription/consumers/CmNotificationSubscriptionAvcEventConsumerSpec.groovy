...
@SpringBootTest(classes = [EventsPublisher, CmNotificationSubscriptionAvcEventConsumer, ObjectMapper, JsonObjectMapper])
@Testcontainers
@DirtiesContext
class CmNotificationSubscriptionAvcEventConsumerSpec extends MessagingBaseSpec {

    @SpringBean
    EventsPublisher eventsPublisher = new EventsPublisher<CloudEvent>(legacyEventKafkaTemplate, cloudEventKafkaTemplate)

    @SpringBean
    CmNotificationSubscriptionAvcEventConsumer acvEventConsumer = new CmNotificationSubscriptionAvcEventConsumer(eventsPublisher)
