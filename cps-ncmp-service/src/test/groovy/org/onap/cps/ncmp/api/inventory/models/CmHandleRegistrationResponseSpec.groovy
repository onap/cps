...
class CmHandleRegistrationResponseSpec extends Specification {
    ...
    def 'Conflicting cm-handle Registration with multiple responses with an unexpected xpath.'() {
    ...
    def 'Conflicting cm-handle registration based on cm handle id and registration error'() {
        when: 'the conflict response is created with "alternate id already associated" error code for 1 cm handle'
            def cmHandleRegistrationResponses =
                    CmHandleRegistrationResponse.createConflictResponses(['ch 1'], ALTERNATE_ID_ALREADY_ASSOCIATED)
