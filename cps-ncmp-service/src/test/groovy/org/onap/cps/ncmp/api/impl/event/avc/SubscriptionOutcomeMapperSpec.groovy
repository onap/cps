...
class SubscriptionOutcomeMapperSpec extends Specification {
    ...
    def 'Map subscription event response to subscription event outcome'() {
        ...
        then: 'the resulting subscription event outcome contains the correct clientId'
            assert result.event.subscription.clientID == "SCO-9989752"
        and: 'subscription name'
            assert result.event.subscription.name == "cm-subscription-001"
        and: 'predicate targets rejected'
            assert result.event.predicates.rejectedTargets == ["CMHandle2"]
        and: 'predicate targets accepted'
            assert result.event.predicates.acceptedTargets == ["CMHandle1"]
        and: 'predicate targets pending'
            assert result.event.predicates.pendingTargets == []
    }
}