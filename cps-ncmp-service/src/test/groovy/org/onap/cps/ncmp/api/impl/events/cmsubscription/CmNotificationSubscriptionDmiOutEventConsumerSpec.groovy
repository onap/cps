...
class CmNotificationSubscriptionDmiOutEventConsumerSpec extends MessagingBaseSpec {
    ...
    def 'Consume a valid CM Notification Subscription Event and perform correct actions base on status'() {
        ...
        when: 'the event is consumed'
            objectUnderTest.consumeCmNotificationSubscriptionDmiOutEvent(consumerRecord)
        then: 'correct methods have been called'
            expectedCacheCalls * mockDmiCmNotificationSubscriptionCacheHandler.updateDmiCmNotificationSubscriptionStatusPerDmi(_,_,_)
            expectedPersistenceCalls * mockDmiCmNotificationSubscriptionCacheHandler.persistIntoDatabasePerDmi(_,_)
