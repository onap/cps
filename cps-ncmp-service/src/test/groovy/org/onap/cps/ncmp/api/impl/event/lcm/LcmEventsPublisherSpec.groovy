...
class LcmEventsPublisherSpec extends MessagingSpec {
    ...
    def 'Produce and Consume Lcm Event'() {
        ...
        and: 'we have an expected LcmEvent'
            def expectedJsonString = TestUtils.getResourceFileContent('expectedLcmEvent.json')
            def expectedNcmpEvent = jsonObjectMapper.convertJsonString(expectedJsonString, LcmEvent.class)
        ...
        and: 'record matches the expected event'
            assert expectedNcmpEvent == jsonObjectMapper.convertJsonString(record.value, LcmEvent.class)
    }
}
