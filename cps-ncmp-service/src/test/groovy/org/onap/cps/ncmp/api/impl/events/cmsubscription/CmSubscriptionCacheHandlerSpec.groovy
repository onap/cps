...
class CmSubscriptionCacheHandlerSpec extends MessagingBaseSpec {
    ...
    def setup() {
        ...
        yangModelCmHandle1.setId("CMHandle1")
        yangModelCmHandle1.setDmiServiceName("dmi-1")

        yangModelCmHandle2.setId("CMHandle2")
        yangModelCmHandle2.setDmiServiceName("dmi-2")

        yangModelCmHandle3.setId("CMHandle3")
        yangModelCmHandle3.setDmiServiceName("dmi-2")

        mockInventoryPersistence.getYangModelCmHandles(['CMHandle1', 'CMHandle2', 'CMHandle3'])
                >> [yangModelCmHandle1, yangModelCmHandle2, yangModelCmHandle3]
    }
    ...
    def 'Create map for cache object by DMI service name'() {
        ...
        then: 'the method to group cm handles by DMI service name is called only once'
            1 * objectUnderTest.groupTargetCmHandlesByDmi(requestPredicate)
        ...
        and: 'the size of predicates in each object is correct'
            cacheObjectForDmi1.cmSubscriptionPredicates.size() == 1
            cacheObjectForDmi2.cmSubscriptionPredicates.size() == 1
        ...
        and: 'the target cmHandles for each is correct '
            cacheObjectForDmi1.cmSubscriptionPredicates.get(0).getTargetFilter() == ["CMHandle1"]
            cacheObjectForDmi2.cmSubscriptionPredicates.get(0).getTargetFilter() == ["CMHandle2", "CMHandle3"]
        and: 'the list of xpath for each is correct'
            ...
            cacheObjectForDmi2.cmSubscriptionPredicates.get(0).getScopeFilter()
                    .getXpathFilters() == ["/x1/y1","/x2/y2"]
    }
    ...
    def 'Get map for cm handle IDs by DMI service name'() {
        ...
        and: 'the values in the map is as expected'
            mapOfCMHandleIDsByDmi.get("dmi-1") == ["CMHandle1"]
            mapOfCMHandleIDsByDmi.get("dmi-2") == ["CMHandle2", "CMHandle3"]
    }
}