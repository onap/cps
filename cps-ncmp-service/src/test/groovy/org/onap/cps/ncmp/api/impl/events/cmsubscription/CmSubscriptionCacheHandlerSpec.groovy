...
class CmSubscriptionCacheHandlerSpec extends MessagingBaseSpec {
    ...
    def 'Create map for cache object by DMI service name'() {
        ...
        and: 'the size of predicates in each object is correct'
            assert cacheObjectForDmi1.cmSubscriptionPredicates.size() == 2
            assert cacheObjectForDmi2.cmSubscriptionPredicates.size() == 2
        and: 'the subscription status in each object is correct'
            assert cacheObjectForDmi1.cmSubscriptionStatus.toString() == "PENDING"
            assert cacheObjectForDmi2.cmSubscriptionStatus.toString() == "PENDING"
        and: 'the target cmHandles for each predicate is correct'
            assert cacheObjectForDmi1.cmSubscriptionPredicates.get(0).getTargetFilter() == ["ch1"]
            assert cacheObjectForDmi1.cmSubscriptionPredicates.get(1).getTargetFilter() == ["ch3"]

            assert cacheObjectForDmi2.cmSubscriptionPredicates.get(0).getTargetFilter() == ["ch2"]
            assert cacheObjectForDmi2.cmSubscriptionPredicates.get(1).getTargetFilter() == ["ch4"]
        and: 'the list of xpath for each is correct'
            assert cacheObjectForDmi1.cmSubscriptionPredicates.get(0).getScopeFilter()
                    .getXpathFilters() && cacheObjectForDmi2.cmSubscriptionPredicates.get(0).getScopeFilter()
                    .getXpathFilters() == ["/x1/y1","x2/y2"]

            assert cacheObjectForDmi1.cmSubscriptionPredicates.get(1).getScopeFilter()
