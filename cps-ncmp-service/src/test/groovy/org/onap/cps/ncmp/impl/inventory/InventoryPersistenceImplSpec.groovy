...
class InventoryPersistenceImplSpec extends Specification {
    ...
    def 'Retrieve CmHandle without properties (top-level leaves only).'() {
        ...
        then: 'the result has the correct id and service names'
            assert result.id == cmHandleId
            assert result.dmiServiceName == 'common service name'
            assert result.dmiDataServiceName == 'data service name'
            assert result.dmiModelServiceName == 'model service name'
        and: 'no additional or public properties are returned'
            assert result.additionalProperties.isEmpty()
            assert result.publicProperties.isEmpty()
        and: 'the CM Handle ID is validated'
            1 * mockCpsValidator.validateNameCharacters(cmHandleId)
    }
