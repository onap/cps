...
class InventoryPersistenceImplSpec extends Specification {
    ...
    def "YangModelCmHandles are not returned for invalid cm handle ids"() {
        ...
        and: 'a cm handle is returned for a valid cm handle id'
            def dataNodes = [new DataNode(xpath: xpath, leaves: ['id': cmHandleId])]
            mockCpsDataService.getDataNodesForMultipleXpaths(NCMP_DATASPACE_NAME, NCMP_DMI_REGISTRY_ANCHOR, [xpath] , INCLUDE_ALL_DESCENDANTS) >> dataNodes
        when: 'retrieving the yang modelled cm handles'
            def results = objectUnderTest.getYangModelCmHandles(['Invalid Cm Handle Id', cmHandleId])
