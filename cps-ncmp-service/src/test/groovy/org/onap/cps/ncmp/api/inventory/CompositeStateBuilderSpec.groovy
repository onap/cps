...
class CompositeStateBuilderSpec extends Specification {
    ...
    def static stateDataNodes = [new DataNodeBuilder().withXpath("/dmi-registry/cm-handles[@id='${cmHandleId}']/state/lock-reason")
                                         .withLeaves(['reason': LockReasonCategory.LOCKED_MISBEHAVING.name(), 'details': 'lock details']).build(),
                                 new DataNodeBuilder().withXpath("/dmi-registry/cm-handles[@id='${cmHandleId}']/state/datastores")
                                            .withChildDataNodes(Arrays.asList(new DataNodeBuilder()
                                                    ...
                                                    .withLeaves(['sync-state': SyncState.UNSYNCHRONIZED.name()]).build())).build()]
    def static cmHandleDataNode = new DataNode(xpath: cmHandleXpath, childDataNodes: stateDataNodes, leaves: ['cm-handle-state': CmHandleState.ADVISED.name()])
    ...
    def "Build composite state from DataNode "() {
        given: "a Data Node "
            new DataNode(leaves: ['cm-handle-state': CmHandleState.ADVISED.name()])
