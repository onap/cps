...
class ModuleSyncWatchdogSpec extends Specification {
    ...
    def 'Schedule a Cm-Handle Sync for ADVISED Cm-Handles with #scenario'() {
        given: 'cm handles in an advised state and a data sync state'
            ...
            objectUnderTest.isGlobalCacheConfigured = dataSyncEnabled
        ...
        where:
            scenario                   | dataSyncEnabled  || expectedDataStoreSyncState
            'data sync enabled'        | true             || DataStoreSyncState.UNSYNCHRONIZED
            'data sync is not enabled' | false            || DataStoreSyncState.NONE_REQUESTED
    }
