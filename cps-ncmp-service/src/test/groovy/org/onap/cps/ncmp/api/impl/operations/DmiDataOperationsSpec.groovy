...
class DmiDataOperationsSpec extends DmiOperationsBaseSpec {
    ...
    def 'Execute (async) data operation from DMI service.'() {
        ...
        and: 'a positive response from DMI service when it is called with valid request parameters'
            ...
            def expectedTemplateWithVariables = new UriTemplateParameters('someServiceName/{dmiBasePath}/v1/data?requestId={requestId}&topic={topic}', ['requestId': 'requestId', 'topic': 'my-topic-name', 'dmiBasePath': dmiBasePath])
    ...
    def 'call get all resource data.'() {
        ...
        and: 'a positive response from DMI service when it is called with the expected parameters'
            ...
            def expectedTemplateWithVariables = new UriTemplateParameters('someServiceName/{dmiBasePath}/v1/ch/{cmHandleId}/data/ds/{datastore}?resourceIdentifier={resourceIdentifier}', ['resourceIdentifier': '/', 'datastore': 'ncmp-datastore:passthrough-operational', 'dmiBasePath': dmiBasePath, 'cmHandleId': cmHandleId])
    ...
    def 'Write data for pass-through:running datastore in DMI.'() {
        ...
        and: 'a positive response from DMI service when it is called with the expected parameters'
            def expectedTemplateWithVariables = new UriTemplateParameters('someServiceName/{dmiBasePath}/v1/ch/{cmHandleId}/data/ds/{datastore}?resourceIdentifier={resourceIdentifier}', ['resourceIdentifier': resourceIdentifier, 'datastore': 'ncmp-datastore:passthrough-running', 'dmiBasePath': dmiBasePath, 'cmHandleId': cmHandleId])
    ...
    def getExpectedUriTemplateWithVariables(expectedOptions, expectedDataStore) {
        def expectedUriTemplate = (expectedOptions == null || expectedOptions == '')
                ? dmiServiceName + '/{dmiBasePath}/v1/ch/{cmHandleId}/data/ds/{datastore}?resourceIdentifier={resourceIdentifier}'
                : dmiServiceName + '/{dmiBasePath}/v1/ch/{cmHandleId}/data/ds/{datastore}?resourceIdentifier={resourceIdentifier}&options={options}'
        def uriVariables = (expectedOptions == null || expectedOptions == '')
                ? ['resourceIdentifier': resourceIdentifier, 'datastore': expectedDataStore.datastoreName, 'dmiBasePath': dmiBasePath, 'cmHandleId': cmHandleId]
                : ['resourceIdentifier': resourceIdentifier, 'datastore': expectedDataStore.datastoreName, 'dmiBasePath': dmiBasePath, 'options': expectedOptions, 'cmHandleId': cmHandleId]
