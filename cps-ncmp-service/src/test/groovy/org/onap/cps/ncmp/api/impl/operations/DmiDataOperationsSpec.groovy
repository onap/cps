...
class DmiDataOperationsSpec extends DmiOperationsBaseSpec {
    ...
    def 'Execute (async) data operation from DMI service for #scenario.'() {
        ...
        and: 'map data operation cloud event to data operation event'
            def dataOperationResponseEvent = CloudEventUtils.mapData(actualDataOperationCloudEvent, PojoCloudEventDataMapper.from(new ObjectMapper(), DataOperationEvent.class)).getValue()
        and: 'the event contains the expected error details'
            def eventDataValue = extractDataValue(dataOperationResponseEvent)
    ...
    def extractDataValue(dataOperationResponseEvent) {
        return dataOperationResponseEvent.data.responses[0]
    }
}
