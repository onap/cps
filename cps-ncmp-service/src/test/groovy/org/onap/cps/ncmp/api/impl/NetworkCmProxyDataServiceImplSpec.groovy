...
class NetworkCmProxyDataServiceImplSpec extends Specification {
    ...
    def 'Set Cm Handle Data Sync Enabled Flag where data sync flag is  #scenario'() {
        given: 'an existing cm handle composite state'
            def compositeState = new CompositeState(cmHandleState: CmHandleState.READY, dataSyncEnabled: intialDataSyncEnabledFlag,
        ...
        then: 'the data sync enabled flag is set to true'
        ...
        and: 'the inventory persistence service to update node leaves is called with the correct values'
            expectedNumberOfInvocations * mockInventoryPersistence.updateNodeLeaves('/dmi-registry/cm-handles[@id=\'some-cm-handle-id\']',
                '{"state":{"cm-handle-state":"READY","data-sync-enabled":' +
                    dataSyncEnabledFlag + ',"datastores":{"operational":{"sync-state":"' +
                    expectedDataStoreSyncState.toString() + '"}}}}')
        where: 'the following data sync enabled flag is used'
            scenario                                              | dataSyncEnabledFlag | intialDataSyncEnabledFlag | initialDataSyncState               || expectedDataStoreSyncState         | expectedNumberOfInvocations
            'enabled'                                             | true                | false                     | DataStoreSyncState.NONE_REQUESTED  || DataStoreSyncState.UNSYNCHRONIZED  | 1
            'disabled'                                            | false               | true                      | DataStoreSyncState.UNSYNCHRONIZED  || DataStoreSyncState.NONE_REQUESTED  | 1
            'disabled where sync-state is currently SYNCHRONIZED' | false               | true                      | DataStoreSyncState.SYNCHRONIZED    || DataStoreSyncState.NONE_REQUESTED  | 1
            'is set to existing flag state'                       | true                | true                      | DataStoreSyncState.UNSYNCHRONIZED  || DataStoreSyncState.UNSYNCHRONIZED  | 0
    }
