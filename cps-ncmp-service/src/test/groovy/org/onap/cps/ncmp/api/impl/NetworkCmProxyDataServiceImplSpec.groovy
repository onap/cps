...
class NetworkCmProxyDataServiceImplSpec extends Specification {
    ...
    def dataNode = new DataNode(leaves: ['id': 'cm-handle', 'dmi-service-name': 'testDmiService'])
    ...
    def 'Schedule a Cm-Handle Sync for ADVISED Cm-Handles'() {
        given: 'a cm handle with an advised state'
            ...
            def cmHandles = ['cm-handle'] as Set
        when: 'a sync is scheduled'
            objectUnderTest.scheduleCmHandleSync()
        then: 'cm handles are queried'
            2 * mockCpsAdminService.queryCmHandles(_) >> ['cm-handle']
        and: 'cm handles with an advised state are queried'
            1 * mockCpsAdminService.queryAdvisedCmHandle(cmHandles) >> dataNode
        and: 'a cm handle is retrieved with an advised state'
            1 * mockYangModelCmHandleRetriever.getYangModelCmHandle('cm-handle') >> yangModelCmHandle
        and: 'no further cm handles are retrieved with an advised state'
            1 * mockCpsAdminService.queryAdvisedCmHandle(cmHandles) >> null
