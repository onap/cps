...
class NetworkCmProxyDataServiceImplSpec extends Specification {
    ...
    def 'Get cm handle details for a given cm handle.'() {
        given: 'an existing yang model cm handle'
        ...
        and: 'the retriever is invoked with the correct params and returns a yang model cm handle'
            1 * mockYangModelCmHandleRetriever.retrieveYangModelCmHandleDmiServiceNameAndDmiProperties('Some-Cm-Handle') >> yangModelCmHandle
        when: 'getting cm handle details for a given cm handle id'
            def result = objectUnderTest.getNcmpServiceCmHandle('Some-Cm-Handle')
        then: 'the result is returned with the correct parameters'
