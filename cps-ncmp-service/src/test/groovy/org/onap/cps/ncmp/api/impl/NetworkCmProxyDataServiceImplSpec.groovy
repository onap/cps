...
import org.onap.cps.ncmp.api.models.datajob.RestProtocolParameter
...
class NetworkCmProxyDataServiceImplSpec extends Specification {
    ...
    def 'process data job #operation  request.'() {
        ...
        when: 'data job request is processed'
            def response = objectUnderTest.processDataJob(operation + '_some-job-id', dataJobRequest)
    ...
    def getWriteOrReadOperationRequest(operation) {
        def restProtocolParameter = new RestProtocolParameter(destination: 'client-topic', dataAcceptType: ' application/vnd.3gpp.object-tree-hierarchical+json', dataContentType: 'application/3gpp-json-patch+json')
        if (operation == 'write') {
            return new WriteOperation(restProtocolParameter: restProtocolParameter, path: 'some/write/path', op: 'add', operationId: 1, value: 'some-value')
        }
        return new ReadOperation(restProtocolParameter: restProtocolParameter, path: 'some/read/path', op: 'read', operationId: 2, attributes: ['some-attrib-1', 'some-attrib-2'], fields: ['some-field-1', 'some-field-2'], scopeType: ScopeType.BASE_NTH_LEVEL, scopeLevel: 4)
    }
}
