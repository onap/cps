...
class ModuleSyncTasksSpec extends Specification {
    ...
    def 'Reset failed CM Handles #scenario.'() {
        ...
        then: 'updated to state "ADVISED" from "READY" is called as often as there are cm handles ready for retry'
            expectedNumberOfInvocationsToSaveCmHandleState * mockLcmEventsCmHandleStateHandler.updateCmHandleStateBatch(expectedCmHandleStatePerCmHandle)
        and: 'after reset performed size of in progress map'
            assert moduleSyncStartedOnCmHandles.size() == expectedNumberOfInvocationsToRemoveCmHandleState
        where:
            scenario                        | isReadyForRetry | expectedNumberOfInvocationsToRemoveCmHandleState || expectedNumberOfInvocationsToSaveCmHandleState
            'retry locked cm handle'        | [true, false]   | 0                                                || 1
            'do not retry locked cm handle' | [false, false]  | 1                                                || 0
    }
    ...
    def 'Module Sync ADVISED cm handle without entry in progress map.'() {
        ...
        and: 'the entry for other cm handle is still in the progress map'
            0 * moduleSyncStartedOnCmHandles.remove('other-cm-handle')
    }
