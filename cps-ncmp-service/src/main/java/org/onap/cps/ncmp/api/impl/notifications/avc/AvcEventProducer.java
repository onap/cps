...
public class AvcEventProducer {
     ...
     *
     * @param avcEvent message payload
     */
    public void sendMessage(AvcEvent avcEvent) {
        // generate new event id while keeping other data
        avcEvent = avcEventMapper.toOutgoingAvcEvent(avcEvent);
        log.debug("Forwarding AVC event {} to topic {} ", avcEvent.getEventId(), "cm-events");
        kafkaTemplate.send("cm-events", avcEvent.getEventId(), avcEvent);
    }
}
