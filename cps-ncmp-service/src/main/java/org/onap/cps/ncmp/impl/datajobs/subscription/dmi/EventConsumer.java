...
public class EventConsumer {
    ...
    public void consumeDmiOutEvent(final ConsumerRecord<String, CloudEvent> dmiOutEventAsConsumerRecord) {
        ...
        if (dmiOutEvent != null && correlationId != null) {
            final String subscriptionId = correlationId.split(CORRELATION_ID_SEPARATOR)[0];
            final String dmiPluginName = correlationId.split(CORRELATION_ID_SEPARATOR)[1];

            if (eventType.equals("subscriptionCreateResponse")) {
                final CmSubscriptionStatus status = getStatus(dmiOutEvent);
                if (status == ACCEPTED) {
                    cmSubscriptionHandler.updateSubscriptionStatus(subscriptionId, dmiPluginName, status);
                }
            }
        }
    }

    private CmSubscriptionStatus getStatus(final DataJobSubscriptionDmiOutEvent dmiOutEvent) {
