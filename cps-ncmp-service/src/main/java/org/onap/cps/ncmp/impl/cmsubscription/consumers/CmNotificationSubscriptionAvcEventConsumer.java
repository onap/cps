...
public class CmNotificationSubscriptionAvcEventConsumer {
     ...
     *
     * @param cmNotificationSubscriptionAvcEventConsumerRecord Incoming raw consumer record
     */
    @KafkaListener(topics = "${app.dmi.cm-events.topic}",
            containerFactory = "cloudEventConcurrentKafkaListenerContainerFactory")
    public void consumeAndForward(
            final ConsumerRecord<String, CloudEvent> cmNotificationSubscriptionAvcEventConsumerRecord) {
        log.debug("Consuming AVC event {} ...", cmNotificationSubscriptionAvcEventConsumerRecord.value());
        final String newEventId = UUID.randomUUID().toString();
        final CloudEvent outgoingAvcEvent =
                CloudEventBuilder.from(cmNotificationSubscriptionAvcEventConsumerRecord.value()).withId(newEventId)
                        .build();
