...
public class ModuleSyncWatchdog {
    ...
    private Collection<String> prepareNextBatch() {
        ...
        if (skippedCount > 0) {
            log.warn("{} of {} candidates already in progress by other instance",
                    skippedCount, nextBatchCandidates.size());
        }
        if (nextBatch.isEmpty() && !nextBatchCandidates.isEmpty()) {
            log.warn("All {} candidates blocked by in-progress map. In-progress map size: {}",
                    nextBatchCandidates.size(), moduleSyncStartedOnCmHandles.size());
        }
