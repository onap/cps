...
public class ParameterHelper {
    ...
    private static final Pattern PROVMNS_PATTERNS = Pattern.compile(
        PROVMNS_ACTIONS_PATH + "|" + PROVMNS_BASE_PATH);
    ...
    public static ActionRequestParameters extractActionRequestParameters(final HttpServletRequest httpServletRequest) {
        final String fullPath = getFullPath(httpServletRequest);
        isValidProvMnSExtensionsPath(fullPath, httpServletRequest.getMethod());
    ...
    private static void isValidProvMnSExtensionsPath(final String path, final String httpMethodName) {
    ...
    private static String getFullPath(final HttpServletRequest httpServletRequest) {
        ...
        final Matcher matcher = PROVMNS_PATTERNS.matcher(uriPath);
        if (matcher.find()) {
            final String matchedPath = matcher.group(1) != null ? matcher.group(1) : matcher.group(2);
            return "/" + matchedPath;
        } else {
