    ...
    private void sendDataOperationRequestToDmiService(final String dataOperationResourceUrl,
        ...
        try {
            dmiRestClient.postOperationWithJsonData(dataOperationResourceUrl, dmiDataOperationRequestAsJsonString, READ,
                    authorization);
        } catch (final HttpClientRequestException ex) {
            handleTaskCompletionException(ex, dataOperationResourceUrl, dmiDataOperationRequestBodies);
        }
    }

    private void handleTaskCompletionException(final HttpClientRequestException httpClientRequestException,
        ...
        dmiDataOperationRequestBodies.forEach(dmiDataOperationRequestBody -> {
            ...
            cmHandleIdsPerResponseCodesPerOperation.add(dmiDataOperationRequestBody,
                    Map.of(NcmpResponseStatus.lookup(httpClientRequestException.getNcmpResponseStatus(), UNKNOWN_ERROR),
                            cmHandleIds));
        });
