...
public class DmiCacheHandler {
    ...
    public void removeAcceptedAndRejectedDmiCmNotificationSubscriptionEntries(final String subscriptionId) {
        final Map<String, DmiCmSubscriptionDetails> dmiCmNotificationSubscriptionsPerDmi =
                cmNotificationSubscriptionCache.get(subscriptionId);
        final Map<String, DmiCmSubscriptionDetails> updatedDmiCmNotificationSubscriptionsPerDmi =
     ...
     */
    public Map<String, DmiCmSubscriptionDetails> createDmiCmNotificationSubscriptionsPerDmi(
            final List<Predicate> predicates) {
        final Map<String, DmiCmSubscriptionDetails> dmiCmNotificationSubscriptionDetailsPerDmi =
                new HashMap<>();
    ...
    public void updateDmiCmNotificationSubscriptionStatusPerDmi(final String subscriptionId,
        ...
        final Map<String, DmiCmSubscriptionDetails> dmiCmNotificationSubscriptionDetailsPerDmi =
                cmNotificationSubscriptionCache.get(subscriptionId);
        dmiCmNotificationSubscriptionDetailsPerDmi.get(dmiServiceName).setCmSubscriptionStatus(status);
    ...
    public void persistIntoDatabasePerDmi(final String subscriptionId, final String dmiServiceName) {
        final List<DmiCmSubscriptionPredicate> dmiCmSubscriptionPredicateList =
                cmNotificationSubscriptionCache.get(subscriptionId).get(dmiServiceName)
                        .getDmiCmSubscriptionPredicates();
        for (final DmiCmSubscriptionPredicate dmiCmSubscriptionPredicate : dmiCmSubscriptionPredicateList) {
            ...
            for (final String cmHandle: cmHandles) {
                for (final String xpath: xpaths) {
                    cmSubscriptionPersistenceService.addCmNotificationSubscription(datastoreType, cmHandle,
    ...
    public void removeFromDatabasePerDmi(final String subscriptionId, final String dmiServiceName) {
        ...
        for (final DmiCmSubscriptionPredicate dmiCmSubscriptionPredicate : dmiCmSubscriptionPredicateList) {
            ...
            for (final String cmHandle: cmHandles) {
                for (final String xpath: xpaths) {
                    cmSubscriptionPersistenceService.removeCmNotificationSubscription(datastoreType,
    ...
    private boolean isAcceptedOrRejected(
            final DmiCmSubscriptionDetails dmiCmNotificationSubscription) {
        return dmiCmNotificationSubscription.getCmSubscriptionStatus().toString().equals("ACCEPTED")
                || dmiCmNotificationSubscription.getCmSubscriptionStatus().toString().equals("REJECTED");
    }
}
