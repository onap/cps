...
public class NcmpEventsCmHandleStateHandlerImpl implements NcmpEventsCmHandleStateHandler {
    ...
    private void updateToSpecifiedCmHandleState(final YangModelCmHandle yangModelCmHandle,
        ...
        if (ADVISED == targetCmHandleState) {
            if (yangModelCmHandle.getCompositeState().getCmHandleState() == LOCKED) {
                setCompositeStateToAdvisedAndRetainOldLockReasonDetails().accept(yangModelCmHandle.getCompositeState());
                inventoryPersistence.saveCmHandleState(yangModelCmHandle.getId(),
                        yangModelCmHandle.getCompositeState());
            } else {
                setCompositeStateToAdvised().accept(yangModelCmHandle.getCompositeState());
        ...
        if (READY == targetCmHandleState) {
            setCompositeStateToReadyWithInitialDataStoreSyncState().accept(yangModelCmHandle.getCompositeState());
            inventoryPersistence.saveCmHandleState(yangModelCmHandle.getId(), yangModelCmHandle.getCompositeState());
        }

        if (LOCKED == targetCmHandleState) {
            setCompositeStateToLocked().accept(yangModelCmHandle.getCompositeState());
            inventoryPersistence.saveCmHandleState(yangModelCmHandle.getId(), yangModelCmHandle.getCompositeState());
        }
    ...
    private void publishNcmpEvent(final YangModelCmHandle yangModelCmHandle) {
        final NcmpServiceCmHandle ncmpServiceCmHandle = yangModelCmHandleToNcmpServiceCmHandle(yangModelCmHandle);
    ...
    private NcmpServiceCmHandle yangModelCmHandleToNcmpServiceCmHandle(final YangModelCmHandle yangModelCmHandle) {
