...
public class ModuleSyncTasks {
    ...
    /**
     * Perform module sync on a baths of cm handles.
     *
     * @param cmHandlesAsDataNodes the batch of cm handles to perform module sync on
    ...
    public CompletableFuture<Void> performModuleSync(final Collection<DataNode> cmHandlesAsDataNodes) {
        ...
        updateCmHandlesStateBatch(cmHandleStates);
    ...
    public CompletableFuture<Void> resetFailedCmHandles(final List<YangModelCmHandle> failedCmHandles) {
        final Map<YangModelCmHandle, CmHandleState> cmHandleStates = new HashMap<>(failedCmHandles.size());
        for (final YangModelCmHandle failedCmHandle : failedCmHandles) {
            ...
            if (isReadyForRetry) {
                ...
                cmHandleStates.put(failedCmHandle, CmHandleState.ADVISED);
            }
        }
        updateCmHandlesStateBatch(cmHandleStates);
        return COMPLETED_FUTURE;
    ...
    private void updateCmHandlesStateBatch(final Map<YangModelCmHandle, CmHandleState> cmHandleStates) {
        // To be refactored as part of CPS-1231; Use state-save-batch capability (depends sub-task12, 13)
        for (final Map.Entry<YangModelCmHandle, CmHandleState> entry : cmHandleStates.entrySet()) {
