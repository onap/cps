...
public class CmAvcEventConsumer {
    ...
    private static final String CLOUD_EVENT_SOURCE_SYSTEM_HEADER_NAME = "ce_source";
     ...
     * If event is coming from ONAP-DMI-PLUGIN then the event will also be processed and the cps cache will be updated.
     *
    ...
    private void processCmAvcEventChanges(final ConsumerRecord<String, CloudEvent> cmAvcEventAsConsumerRecord) {
        ...
        final CompositeState cmHandleState = inventoryPersistence.getCmHandleState(cmHandleId);
        if (Boolean.TRUE.equals(cmHandleState.getDataSyncEnabled())) {
    ...
    private boolean isEventFromOnapDmiPlugin(final Headers headers) {
        final String sourceSystem = KafkaHeaders.getParsedKafkaHeader(headers, CLOUD_EVENT_SOURCE_SYSTEM_HEADER_NAME);
