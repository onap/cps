...
public class AsyncTaskExecutor {
    ...
    private static final int ASYNC_TASK_PARALLELISM_LEVEL = 30;
    ...
    public void executeTask(final Supplier<Object> taskSupplier, final int timeOutInMillis) {
        log.info("Async task timeOutInMillis : {}", timeOutInMillis);
        CompletableFuture.supplyAsync(taskSupplier::get, Executors.newWorkStealingPool(ASYNC_TASK_PARALLELISM_LEVEL))
    ...
    private void handleTaskCompletion(final Object response, final Throwable throwable) {
        if (throwable == null) {
            log.info("Async task response : {}", response);
        } else {
