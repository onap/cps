 ...
 */
@Component
public class ClientRequestMetricsTagCustomizer extends DefaultClientRequestObservationConvention 
     ...
     *
     * @param context the client request observation context
     * @return KeyValues containing the modified URI tag if applicable
     */
    protected KeyValues additionalTags(final ClientRequestObservationContext context) {
        final String uriTemplate = context.getUriTemplate();
        final String provMnsApiPath = provMnsBasePath + "/v1/";
        
        if (uriTemplate != null && uriTemplate.contains(provMnsApiPath)) {
            final String queryParams = extractQueryParameters(uriTemplate);
            final String maskedUri = provMnsApiPath + "{fdn}" + queryParams;
