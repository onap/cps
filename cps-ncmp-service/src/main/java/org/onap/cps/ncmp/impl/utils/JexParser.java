...
public class JexParser {
    ...
    private static final Pattern LOCATION_SEGMENT_PATTERN =
            Pattern.compile("(.*)\\[id=\\\"(.*)\\\"]");
    ...
    public static List<String> extractFdnsFromLocationPaths(final String jsonExpression) {
        ...
        final Stream<String> locationPaths = Arrays.stream(lines)
                .map(String::trim)
                .filter(locationPath -> !locationPath.startsWith(COMMENT_PREFIX));
    ...
    /**
     * Returns FDN from a JSON expression.
    ...
    private static Optional<String> extractFdnPrefix(final String locationPath) {
        ...
        final ArrayList<String> locationPathSegmentsAsList = new ArrayList<>(Arrays.asList(locationPathSegments));
        locationPathSegmentsAsList.remove(0); //ignore root
        final StringBuilder fdnBuilder = new StringBuilder();
        for (final String locationPathSegment : locationPathSegmentsAsList) {
