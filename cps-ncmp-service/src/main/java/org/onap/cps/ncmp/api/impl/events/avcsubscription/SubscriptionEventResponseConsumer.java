...
public class SubscriptionEventResponseConsumer {
    ...
    public void consumeSubscriptionEventResponse(
        ...
        if (createOutcomeResponse && isAllCmHandlesAccepted(clientId, subscriptionName)) {
            forwardedSubscriptionEventCache.remove(subscriptionEventId);
        }
    }

    private boolean isAllCmHandlesAccepted(final String clientId, final String subscriptionName) {
        final Collection<DataNode> dataNodeSubscription = subscriptionPersistence.getDataNodesForMultipleXpaths(
