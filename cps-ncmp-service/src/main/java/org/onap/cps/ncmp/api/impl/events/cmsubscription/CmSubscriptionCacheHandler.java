...
public class CmSubscriptionCacheHandler {
    ...
    public Map<String, CmSubscriptionCacheObject> getAllCmSubscriptionCacheObjectByDmiMap(
        ...
        for (final Predicate requestPredicate : cmSubscriptionRequestPredicates) {
            ...
            for (final Map.Entry<String, List<String>> targetCmHandlesByDmi: targetCmHandlesByDmiMap.entrySet()) {
                ...
                final CmSubscriptionPredicate cmSubscriptionPredicate =
                        createCmSubscriptionPredicate(requestPredicate);
                cmSubscriptionPredicate.setTargetFilter(targetCmHandles);
                cmSubscriptionCacheObject.setCmSubscriptionPredicates(new ArrayList<>());

                if (dmiToCmSubscriptionCacheObjectMap.containsKey(dmiServiceName)) {
                    dmiToCmSubscriptionCacheObjectMap.get(dmiServiceName).getCmSubscriptionPredicates()
                            .add(cmSubscriptionPredicate);
                } else {
    ...
    private CmSubscriptionPredicate createCmSubscriptionPredicate(final Predicate requestPredicate) {
        ...
        return cmSubscriptionPredicate;
    }
}