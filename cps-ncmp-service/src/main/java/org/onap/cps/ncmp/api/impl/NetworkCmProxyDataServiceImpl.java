...
public class NetworkCmProxyDataServiceImpl implements NetworkCmProxyDataService {
    ...
    protected void modelSync(final String cmHandle) {
        final var modulesForCmHandle = callDmi(cmHandle, "modules");

        final List<ModuleReference> modulesFromDmiForCmHandle = getModuleReferences(modulesForCmHandle);

        final var knownModulesInCps = cpsModuleService.getAllYangResourcesModuleReferences();

        final List<ModuleReference> modulesNotAlreadyInCps = new ArrayList<>();
        for (final ModuleReference moduleFromDmiForCmHandle : modulesFromDmiForCmHandle) {
            if (!knownModulesInCps.contains(moduleFromDmiForCmHandle)) {
                modulesNotAlreadyInCps.add(moduleFromDmiForCmHandle);
            }
        }

        final Map<String, String> newYangResourcesModuleNameToContentMap =
            getYangResources(cmHandle, modulesNotAlreadyInCps);

        cpsModuleService.createSchemaSetFromModules(NCMP_DATASPACE_NAME, cmHandle,
            newYangResourcesModuleNameToContentMap, modulesFromDmiForCmHandle);
    }

    private Map<String, String> getYangResources(final String cmHandle,
                                                 final List<ModuleReference> modulesNotAlreadyInCps) {
        final var moduleResourcesAsJsonString = callDmi(cmHandle, "moduleResources");
    ...
    private YangResource toYangResource(final JsonObject yangResourceAsJson) {
        final var name = yangResourceAsJson.get("name").getAsString();
        final var revision = yangResourceAsJson.get("revision").getAsString();
        final var yangSource = yangResourceAsJson.get("yang-source").getAsString();
    ...
    private List<ModuleReference> getModuleReferences(final ResponseEntity<String> response) {
        ...
        for (final JsonElement module : modules) {
            final ModuleReference moduleReference = toModuleReference((JsonObject) module);
            modulesFromDmiForCmHandle.add(moduleReference);
        }
        return modulesFromDmiForCmHandle;
    }

    private ModuleReference toModuleReference(final JsonObject module) {
        final var moduleReference = new ModuleReference();
        moduleReference.setName(module.get("identifier").getAsString());
        moduleReference.setNamespace(module.get("namespace").getAsString());
        moduleReference.setRevision(module.get("version").getAsString());
