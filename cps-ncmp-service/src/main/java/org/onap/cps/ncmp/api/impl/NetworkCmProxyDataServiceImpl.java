...
public class NetworkCmProxyDataServiceImpl implements NetworkCmProxyDataService {
    ...
    public void createResourceDataPassThroughRunningForCmHandle(final @NotNull String cmHandle,
        ...
        final Map<String, String> cmHandlePropertyMap = getCmHandlePropertyMap(cmHandlePropertiesList);
        final var dmiRequestBodyObject = GenericRequestBody.builder()
                .operation(GenericRequestBody.OperationEnum.CREATE)
                .dataType(contentType)
                .data(requestBody)
                .cmHandleProperties(cmHandlePropertyMap)
                .build();
    ...
    private String prepareOperationBody(final GenericRequestBody requetBodyObject) {
        try {
            return objectMapper.writeValueAsString(requetBodyObject);
    ...
    private Map<String, String> getCmHandlePropertyMap(final Collection<DataNode> cmHandlePropertyList) {
        if (cmHandlePropertyList == null || cmHandlePropertyList.size() == 0) {
            return null;
        }
        final Map<String, String> cmHandlePropertyMap = new LinkedHashMap<>();
        for (final var node: cmHandlePropertyList) {
            cmHandlePropertyMap.put(String.valueOf(node.getLeaves().get("name")),
                    String.valueOf(node.getLeaves().get("value")));
        }
        return cmHandlePropertyMap;
    }
    ...
    private String getGenericRequestBody(final DataNode cmHandleDataNode) {
        final Collection<DataNode> cmHandlePropertyList = cmHandleDataNode.getChildDataNodes();
        final Map<String, String> cmHandlePropertyMap = getCmHandlePropertyMap(cmHandlePropertyList);
    ...
    private PersistenceCmHandle toPersistenceCmHandle(final String dmiPluginServiceName,
                                                      final CmHandle cmHandle) {
        final PersistenceCmHandle persistenceCmHandle = new PersistenceCmHandle();
        persistenceCmHandle.setDmiServiceName(dmiPluginServiceName);
