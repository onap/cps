...
public class NetworkCmProxyDataServiceImpl implements NetworkCmProxyDataService {
    ...
    protected List<CmHandleRegistrationResponse> parseAndRemoveCmHandlesInDmiRegistration(
        ...
        for (final String cmHandle : tobeRemovedCmHandles) {
            try {
                ...
                publishCmHandleStateTriggerForDeletingAndDeleted(cmHandle);
    ...
    private void publishCmHandleStateTriggerForDeletingAndDeleted(final String cmHandleId) {
        log.debug("Moving the cmHandleId : {} state from ANY_TO_DELETING", cmHandleId);
        ncmpEventsStateHandler.publishNcmpEventForStateTransition(cmHandleId, ANY_TO_DELETING);
        log.debug("Moving the cmHandleId : {} state from DELETING_TO_DELETED", cmHandleId);
        ncmpEventsStateHandler.publishNcmpEventForStateTransition(cmHandleId, DELETING_TO_DELETED);
    }
}
