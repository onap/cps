...
public class InventoryPersistenceImpl extends NcmpPersistenceImpl implements InventoryPersistence {
    ...
    public void cmHandleBulkMigrate(
            final List<CmHandleMigrationDetail> cmHandleUpdateDetailsPerLeaf) {
        if (cmHandleUpdateDetailsPerLeaf.isEmpty()) {
            return;
        }
        final List<Map<String, String>> cmHandlesList = new ArrayList<>(cmHandleUpdateDetailsPerLeaf.size());
        for (final CmHandleMigrationDetail data : cmHandleUpdateDetailsPerLeaf) {
            final Map<String, String> cmHandleData = new HashMap<>();
    ...
    private void updateCmHandleLeaves(final List<Map<String, String>> cmHandleUpdateDetailsPerLeaf) {
        cpsDataService.updateNodeLeaves(
                NCMP_DATASPACE_NAME,
                NCMP_DMI_REGISTRY_ANCHOR,
                NCMP_DMI_REGISTRY_PARENT,
                jsonObjectMapper.asJsonString(Map.of("cm-handles", cmHandleUpdateDetailsPerLeaf)),
                OffsetDateTime.now(),
                ContentType.JSON);
    }
