...
public class InventoryPersistenceImpl extends NcmpPersistenceImpl implements InventoryPersistence {
     ...
     */
    @Override
    public void setAndUpdateCmHandleField(final String cmHandleId, final String fieldName,
    ...
    public void updateCmHandleFields(final Map<String, String> cmHandleIdToValueMap, final String fieldName) {
        final Map<String, Object> jsonForUpdate = new HashMap<>(); //rename
        final List<Map<String, String>> cmHandlesList = new ArrayList<>();
        ...
        for (final Map.Entry<String, String> entry : cmHandleIdToValueMap.entrySet()) {
            ...
            log.debug("Adding {} update for cmHandle {} to {}", fieldName, entry.getKey(), entry.getValue());
        }
        jsonForUpdate.put("cm-handles", cmHandlesList);
        cpsDataService.updateNodeLeaves(
                NCMP_DATASPACE_NAME,
                NCMP_DMI_REGISTRY_ANCHOR,
                NCMP_DMI_REGISTRY_PARENT,
                jsonObjectMapper.asJsonString(jsonForUpdate),
                OffsetDateTime.now(),
                ContentType.JSON);
    }
