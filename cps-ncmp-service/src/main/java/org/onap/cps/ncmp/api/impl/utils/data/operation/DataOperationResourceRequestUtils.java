...
import org.onap.cps.ncmp.api.impl.events.async.DataOperationEventCreator;
...
public class DataOperationResourceRequestUtils {
     ...
     */
    public static Map<String, List<DmiDataOperation>> processPerOperationInBatchRequest(
            final String topicParamInQuery,
            final String requestId,
            final DataOperationRequest dataOperationRequestIn,
            final Collection<YangModelCmHandle> yangModelCmHandles) {

        final Map<String, List<DmiDataOperation>> dmiBatchOperationsOutPerDmiServiceName = new HashMap<>();
        final Map<String, Map<DataOperationResponse, List<String>>> cmHandleIdsPerOperationIdByResponseCode
                = new HashMap<>();
        ...
        for (final DataOperationDefinition dataOperationDefinitionIn :
            ...
            populateCmHandleIdsPerOperationIdByResponseCode(cmHandleIdsPerOperationIdByResponseCode,
                    dataOperationDefinitionIn.getOperationId(), DataOperationResponse.CODE_100, nonExistingCmHandleIds);
            populateCmHandleIdsPerOperationIdByResponseCode(cmHandleIdsPerOperationIdByResponseCode,
                    dataOperationDefinitionIn.getOperationId(), DataOperationResponse.CODE_101, nonReadyCmHandleIds);
        }
        if (!cmHandleIdsPerOperationIdByResponseCode.isEmpty()) {
            publishErrorMessageToClientTopic(topicParamInQuery, requestId, cmHandleIdsPerOperationIdByResponseCode);
        }
        return dmiBatchOperationsOutPerDmiServiceName;
    }

    @Async
    private static void publishErrorMessageToClientTopic(final String topicParamInQuery,
                                                         final String requestId,
                                                         final Map<String, Map<DataOperationResponse, List<String>>>
                                                                 cmHandleIdsPerOperationIdByResponseCode) {
        final CloudEvent dataOperationCloudEvent = DataOperationEventCreator.createDataOperationEvent(topicParamInQuery,
                requestId, cmHandleIdsPerOperationIdByResponseCode);
        final EventsPublisher<CloudEvent> eventsPublisher = CpsApplicationContext.getCpsBean(EventsPublisher.class);
        eventsPublisher.publishCloudEvent(topicParamInQuery, requestId, dataOperationCloudEvent);
    }
    ...
    private static void populateCmHandleIdsPerOperationIdByResponseCode(final Map<String, Map<DataOperationResponse,
            List<String>>> cmHandleIdsPerOperationIdByResponseCode,
                                                                        final String operationId,
                                                                        final DataOperationResponse
                                                                                dataOperationResponse,
                                                                        final List<String> cmHandleIds) {
        if (!cmHandleIds.isEmpty()) {
            cmHandleIdsPerOperationIdByResponseCode.put(operationId, Map.of(dataOperationResponse, cmHandleIds));
        }
    }
}
