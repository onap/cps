 ...
 * Based on event type create the LcmEvent payload.
 */
@Slf4j
...
public class LcmEventsCreatorHelper {
    ...
    private static LcmEventsCreator.CmHandleValuesHolder determineUpdateEventValues(
        ...
        if (isDataSyncEnabledFlagChanged(targetNcmpServiceCmHandle, existingNcmpServiceCmHandle)) {
        ...
        if (isCmHandleStateChanged(targetNcmpServiceCmHandle, existingNcmpServiceCmHandle)) {
            setCmHandleStateChangeUsingTargetAndExistingCmHandle(targetNcmpServiceCmHandle, existingNcmpServiceCmHandle,
                    cmHandleValuesHolder);
        }

        if (!isPublicCmHandlePropertiesEqual(targetNcmpServiceCmHandle.getPublicProperties(),
