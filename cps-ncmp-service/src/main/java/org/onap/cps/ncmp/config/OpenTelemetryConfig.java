    ...
    private ObservationPredicate observationPredicate(final PathMatcher pathMatcher) {
        return (observationName, context) -> {
            if (context instanceof ServerRequestObservationContext observationContext) {
                ...
                return !getExcludedTaskNamesAsList().contains(observationName);
            }
        };
    }

    private List<String> getExcludedTaskNamesAsList() {
        return Arrays.asList(excludedTaskNames.split("\\s*,\\s*"));
    }
}
