...
public class DmiServiceUrlBuilder extends UriComponentsBuilder {
     ...
     * @return a UrlTemplateParameters instance containing the complete URL template and URI variables
     */
    public UrlTemplateParameters createUrlTemplateParameters(final String dmiServiceName, final String dmiBasePath) {
        pathSegment("{dmiBasePath}")
            .pathSegment(VERSION_SEGMENT);

        final Map<String, Object> urlTemplateVariables = new HashMap<>();
        urlTemplateVariables.put("dmiBasePath", dmiBasePath);
        ...
        final String urlTemplate = dmiServiceName + build().toUriString();
