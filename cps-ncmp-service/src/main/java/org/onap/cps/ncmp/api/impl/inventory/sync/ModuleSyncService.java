...
public class ModuleSyncService {
    ...
    public void syncAndCreateOrUpgradeSchemaSetAndAnchor(final YangModelCmHandle yangModelCmHandle) {
        ...
        final boolean inUpgrade = ModuleSyncOrUpgradeUtils.isInUpgradeOrUpgradeFailed(compositeState);

        if (inUpgrade) {
            moduleSetTag = ModuleSyncOrUpgradeUtils.getLockedCompositeStateDetails(compositeState.getLockReason())
                    .get(ModuleSyncOrUpgradeUtils.MODULE_SET_TAG_KEY);
        } else {
