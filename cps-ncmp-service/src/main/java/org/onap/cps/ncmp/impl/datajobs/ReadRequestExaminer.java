...
public class ReadRequestExaminer {
    ...
    /**
     * Classifies selectors from a data node selector string into broadcast, per-DMI, and error groups.
     *
    ...
    public ClassifiedSelectors classifySelectors(final String dataNodeSelector) {
        ...
        final Map<String, Map<ProducerKey, List<String>>> selectorsPerDmi = new HashMap<>();
        ...
        for (final String selector : selectors) {
            if (isBroadcast(selector)) {
                ...
                resolveSelector(selector, selectorsPerDmi, errorSelectors);
            }
        }

        return new ClassifiedSelectors(broadcastSelectors, selectorsPerDmi, errorSelectors);
    }

    private void resolveSelector(final String selector,
                                 final Map<String, Map<ProducerKey, List<String>>> selectorsPerDmi,
                                 final List<String> errorSelectors) {
        final Optional<String> containsValue = JexParser.extractContainsValue(selector);
        if (containsValue.isPresent()) {
            resolveContainsSelector(selector, containsValue.get(), selectorsPerDmi, errorSelectors);
            return;
        }
        ...
        try {
            ...
            addToPerDmiIfReady(selector, cmHandleId, selectorsPerDmi, errorSelectors);
        } catch (final NoAlternateIdMatchFoundException exception) {
            errorSelectors.add(selector);
        }
    }

    private void resolveContainsSelector(final String selector,
                                         final String searchTerm,
                                         final Map<String, Map<ProducerKey, List<String>>> selectorsPerDmi,
                                         final List<String> errorSelectors) {
        final Collection<String> cmHandleIds =
                alternateIdMatcher.getCmHandleIdsByAlternateIdContaining(searchTerm);
    ...
    private static boolean isBroadcast(final String selector) {
        return selector.contains(DEEP_SEARCH) || !SPECIFIC_NODE_ID_PATTERN.matcher(selector).matches();
    }

    public record ClassifiedSelectors(List<String> broadcast,
