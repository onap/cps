...
public class ReadRequestExaminer {
    ...
    public ClassifiedSelectors classifySelectors(final String dataNodeSelector) {
        ...
        for (final String selector : selectors) {
            if (isBroadcast(selector)) {
                ...
                resolveSelector(selector, selectorsPerDmi, notReadySelectors, errorSelectors);
            }
        }

        return new ClassifiedSelectors(broadcastSelectors, selectorsPerDmi, notReadySelectors, errorSelectors);
    }
    ...
    private void resolveSelector(final String selector,
        ...
        final Optional<String> containsValue = JexParser.extractSearchTerm(selector);
        if (containsValue.isPresent()) {
            resolveContainsSelector(selector, containsValue.get(), selectorsPerDmi, notReadySelectors, errorSelectors);
            return;
        }
        ...
        try {
            ...
            addToPerDmiIfReady(selector, cmHandleId, selectorsPerDmi, notReadySelectors);
