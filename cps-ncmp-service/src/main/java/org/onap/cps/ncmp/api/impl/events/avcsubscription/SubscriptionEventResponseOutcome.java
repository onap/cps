...
public class SubscriptionEventResponseOutcome {
    ...
    private SubscriptionEventOutcome formSubscriptionOutcomeMessage(
        ...
        return fromSubscriptionEventResponse(subscriptionEventResponse,
                isFullOutcomeResponse(cmHandleIdToStatusAndDetailsAsMap));
    }
    ...
    private boolean isFullOutcomeResponse(final Map<String, Map<String, String>> cmHandleIdToStatusAndDetailsAsMap) {
        for (final Map<String, String> statusAndDetailsMap : cmHandleIdToStatusAndDetailsAsMap.values()) {
            final String status = statusAndDetailsMap.get("status");
    ...
    private SubscriptionEventOutcome fromSubscriptionEventResponse(
        ...
        if (isFullOutcomeResponse) {
            subscriptionEventOutcome.getData().setStatusCode(Integer.parseInt(NcmpEventResponseCode
                    .SUCCESSFULLY_APPLIED_SUBSCRIPTION.getStatusCode()));
            subscriptionEventOutcome.getData().setStatusMessage(NcmpEventResponseCode
                    .SUCCESSFULLY_APPLIED_SUBSCRIPTION.getStatusMessage());
        } else {
            subscriptionEventOutcome.getData().setStatusCode(Integer.parseInt(NcmpEventResponseCode
                    .PARTIALLY_APPLIED_SUBSCRIPTION.getStatusCode()));
            subscriptionEventOutcome.getData().setStatusMessage(NcmpEventResponseCode
                    .PARTIALLY_APPLIED_SUBSCRIPTION.getStatusMessage());
        }
