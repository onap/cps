...
public class LcmEventsCmHandleStateHandlerImpl implements LcmEventsCmHandleStateHandler {
    ...
    public void updateCmHandleState(final YangModelCmHandle yangModelCmHandle,
        ...
        if (compositeState != null && compositeState.getCmHandleState() == targetCmHandleState) {
            ...
            final NcmpServiceCmHandleStateHolder ncmpServiceCmHandleStateHolder =
                    updateAndGetCmHandleDetails(yangModelCmHandle, targetCmHandleState);
            publishLcmEvent(ncmpServiceCmHandleStateHolder);
        }

    }

    private NcmpServiceCmHandleStateHolder updateAndGetCmHandleDetails(final YangModelCmHandle yangModelCmHandle,
            final CmHandleState targetCmHandleState) {
        final NcmpServiceCmHandleStateHolder ncmpServiceCmHandleStateHolder = new NcmpServiceCmHandleStateHolder();
        ncmpServiceCmHandleStateHolder.setExistingNcmpServiceCmHandle(toNcmpServiceCmHandle(yangModelCmHandle));

        updateToSpecifiedCmHandleState(yangModelCmHandle, targetCmHandleState);

        ncmpServiceCmHandleStateHolder.setTargetNcmpServiceCmHandle(
                YangDataConverter.convertYangModelCmHandleToNcmpServiceCmHandle(yangModelCmHandle));
        return ncmpServiceCmHandleStateHolder;
    ...
    private void publishLcmEvent(final NcmpServiceCmHandleStateHolder ncmpServiceCmHandleStateHolder) {
        final String cmHandleId = ncmpServiceCmHandleStateHolder.getTargetNcmpServiceCmHandle().getCmHandleId();
        final LcmEvent lcmEvent = lcmEventsCreator.populateLcmEvent(cmHandleId,
                ncmpServiceCmHandleStateHolder.getTargetNcmpServiceCmHandle(),
                ncmpServiceCmHandleStateHolder.getExistingNcmpServiceCmHandle());
    ...
    @Setter
    @NoArgsConstructor
    static class NcmpServiceCmHandleStateHolder {
