...
public class DmiPluginUrlValidator {
    ...
    private void validateUrl(final String fieldName, final String url) {
        ...
        try {
            ...
            throw createDataValidationException(fieldName, "URL is not valid: " + url);
        }
        if (allowedUrlPattern != null) {
            if (!allowedUrlPattern.matcher(url).matches()) {
                throw createDataValidationException(fieldName, "URL does not match the allowed pattern: " + url);
            }
        } else {
            ...
            if (uri.getHost() == null) {
                throw createDataValidationException(fieldName, "URL must contain a valid host: " + url);
            }
        }
    }

    private DataValidationException createDataValidationException(final String fieldName, final String errorDetails) {
        return new DataValidationException(String.format("Invalid DMI plugin URL for %s", fieldName), errorDetails);
    }
}
