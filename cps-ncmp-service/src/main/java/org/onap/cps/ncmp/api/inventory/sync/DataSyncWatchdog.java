...
public class DataSyncWatchdog {
     ...
     * 'UNSYNCHRONIZED'.
     */
    @Scheduled(fixedDelayString = "${timers.cm-handle-data-sync.sleep-time-ms:3000}")
    public void executeUnSynchronizedReadyCmHandlePoll() {
        ...
        while (unSynchronizedReadyCmHandle != null) {
            ...
            final CompositeState compositeState = inventoryPersistence
                    .getCmHandleState(unSynchronizedReadyCmHandle.getId());
            final Optional<String> resourceData = syncUtils.getResourceData(unSynchronizedReadyCmHandle.getId());
            if (!resourceData.isEmpty()) {
                cpsDataService.saveData("NFP-Operational", unSynchronizedReadyCmHandle.getId(),
                        resourceData.get(), OffsetDateTime.now());
            }
            ...
            inventoryPersistence.saveCmHandleState(unSynchronizedReadyCmHandle.getId(), compositeState);
