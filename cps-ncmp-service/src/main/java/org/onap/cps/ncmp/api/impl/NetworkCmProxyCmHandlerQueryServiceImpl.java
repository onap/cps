...
public class NetworkCmProxyCmHandlerQueryServiceImpl implements NetworkCmProxyCmHandlerQueryService {
        ...
        if (!getModuleNames(cmHandleQueryServiceParameters.getCmHandleQueryParameters()).isEmpty()) {
            ...
            if (justIds) {
                anchorNames.forEach(anchorName ->
                    createDataNodeFromAnchorName(amalgamatedQueryResultIdentifiers, amalgamatedQueryResults, anchorName)
                );
            } else {
    ...
    private Collection<DataNode> getAllCmHandles(final boolean justIds) {
        if (justIds) {
            ...
            final Map<DataNodeIdentifier, DataNode> amalgamatedQueryResults = new HashMap<>();
            anchorNames.forEach(anchorName ->
                createDataNodeFromAnchorName(new HashSet<>(), amalgamatedQueryResults, anchorName)
            );
            return amalgamatedQueryResults.values();
        } else {
    ...
    private void createDataNodeFromAnchorName(final Collection<DataNodeIdentifier> amalgamatedQueryResultIdentifiers,
                                              final Map<DataNodeIdentifier, DataNode> amalgamatedQueryResults,
                                              final String anchorName) {
        final DataNodeIdentifier dataNodeIdentifier = new DataNodeIdentifier();
        dataNodeIdentifier.setXpath("/dmi-registry/cm-handles[@id='" + anchorName + "']");
        final DataNode dataNode = jsonObjectMapper.convertToValueType(dataNodeIdentifier, DataNode.class);
        dataNode.setLeaves(Collections.singletonMap("id", anchorName));
