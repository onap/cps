...
public class NetworkCmProxyCmHandlerQueryServiceImpl implements NetworkCmProxyCmHandlerQueryService {
    ...
    public Set<NcmpServiceCmHandle> queryCmHandles(
        ...
        final boolean firstQuery = publicPropertyQuery(cmHandleQueryServiceParameters, cmHandleIds,
                cmHandleIdToNcmpServiceCmHandle);
        moduleNameQuery(cmHandleQueryServiceParameters, cmHandleIds, cmHandleIdToNcmpServiceCmHandle, firstQuery);

        final Set<NcmpServiceCmHandle> queryResult = new HashSet<>();
        cmHandleIds.forEach(cmHandleId -> queryResult.add(cmHandleIdToNcmpServiceCmHandle.get(cmHandleId)));
    ...
    private boolean publicPropertyQuery(final CmHandleQueryServiceParameters cmHandleQueryServiceParameters,
                                     ...
                                     final Map<String, NcmpServiceCmHandle> cmHandleIdToNcmpServiceCmHandle) {
        ...
        for (final Map.Entry<String, String> entry :
            ...
            if (firstQuery) {
                firstQuery = false;
                collectDataNodesToNcmpServiceCmHandles(cmHandleIdToNcmpServiceCmHandle, dataNodes, cmHandleIds);
            } else {
                final Collection<String> singleConditionQueryCmHandleIds = new ArrayList<>();
                collectDataNodesToNcmpServiceCmHandles(cmHandleIdToNcmpServiceCmHandle, dataNodes,
                        singleConditionQueryCmHandleIds);
    ...
    private void moduleNameQuery(final CmHandleQueryServiceParameters cmHandleQueryServiceParameters,
        ...
        if (!getModuleNames(cmHandleQueryServiceParameters.getCmHandleQueryParameters()).isEmpty()) {
            ...
            if (firstQuery && !cmHandleIdsByModuleName.isEmpty()) {
    ...
    private void collectDataNodesToNcmpServiceCmHandles(
            final Map<String, NcmpServiceCmHandle> cmHandleIdToNcmpServiceCmHandle,
            final Collection<DataNode> dataNodes, final Collection<String> cmHandleIds) {
