...
public class NetworkCmProxyCmHandlerQueryServiceImpl implements NetworkCmProxyCmHandlerQueryService {
    ...
    public Set<NcmpServiceCmHandle> queryCmHandles(
        ...
        final boolean firstQuery = moduleNameQuery(cmHandleQueryServiceParameters, cmHandleIds, cmHandles, false);

        publicPropertyQuery(cmHandleQueryServiceParameters, cmHandleIds, cmHandles, firstQuery);
    ...
    public Set<String> queryCmHandleIds(
        ...
        final boolean firstQuery = moduleNameQuery(cmHandleQueryServiceParameters, cmHandleIds, cmHandles, true);
        ...
        if (!getModuleNames(cmHandleQueryServiceParameters.getCmHandleQueryParameters()).isEmpty()) {
            ...
            if (searchForIds) {
                anchorNames.forEach(anchorName ->
                        createNcmpServiceCmHandleFromAnchorName(cmHandleIds, cmHandles, anchorName)
                );
            } else {
    ...
    private Set<NcmpServiceCmHandle> getAllCmHandles() {
        return getDataNodes("/dmi-registry/cm-handles").stream()
    ...
    private void createNcmpServiceCmHandleFromAnchorName(final Collection<String> cmHandleIds,
                                              final Map<String, NcmpServiceCmHandle> cmHandles,
                                              final String anchorName) {
        final NcmpServiceCmHandle ncmpServiceCmHandle = new NcmpServiceCmHandle();
        ncmpServiceCmHandle.setCmHandleId(anchorName);
        cmHandleIds.add(anchorName);
        cmHandles.put(anchorName, ncmpServiceCmHandle);
    }
