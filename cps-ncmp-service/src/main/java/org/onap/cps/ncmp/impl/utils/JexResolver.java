...
public class JexResolver {
    ...
    public static List<String> resolveAlternateIds(final String JexBasicExpressionWithManyPaths) {
        ...
        String[] validPaths = JexBasicExpressionWithManyPaths.split("\\R");

        Stream<String> trimmedLines = Arrays.stream(validPaths)
                .map(String::trim);

        Stream<String> validLines = trimmedLines
                .filter(line -> !line.isEmpty())
                .filter(line -> !line.startsWith("&&"));

        Stream<String> fdns = validLines
                .map(JexResolver::resolveFdnFromSinglePath)
                .flatMap(Optional::stream);

        Stream<String> alternateIds = fdns
     ...
     *
     * @param singlePathJex A single JEX path.
     * @return Optional containing resolved FDN if found; empty otherwise.
     */
    private static Optional<String> resolveFdnFromSinglePath(final String singlePathJex) {
        if (!singlePathJex.startsWith("/")) {
            return Optional.empty();

        }
        final String[] parts = singlePathJex.split("/");
