...
public class CmDataJobSubscriptionPersistenceService {
    ...
    private static final String DATA_JOB_ID = "dataJobId";
    ...
    public Collection<String> getSubscriptionIds(final String dataNodeSelector) {
        ...
        return (Collection<String>) existingNodes.iterator().next().getLeaves().get(DATA_JOB_ID);
    }
    ...
    public void delete(final String subscriptionId, final String dataNodeSelector) {
        ...
        if (!subscriptionIds.remove(subscriptionId)) {
            log.warn("SubscriptionId={} not found under {}={}", subscriptionId, DATA_NODE_SELECTOR, dataNodeSelector);
            return;
        }

        if (subscriptionIds.isEmpty()) {
            log.info("Last subscriber removed for {}={}, setting status as UNKNOWN", DATA_NODE_SELECTOR,
                    dataNodeSelector);
            updateSubscriptionDetails(dataNodeSelector, Collections.emptyList(), UNKNOWN.name());
        } else {
            log.info("Removed subscriber {} from {}={}, remaining subscribers={}", DATA_NODE_SELECTOR,
    ...
    private String createSubscriptionDetailsAsJson(final String dataNodeSelector,
        ...
        final Map<String, Serializable> subscriptionDetailsAsMap =
            Map.of(DATA_NODE_SELECTOR, dataNodeSelector,
                    DATA_JOB_ID, (Serializable) subscriptionIds,
