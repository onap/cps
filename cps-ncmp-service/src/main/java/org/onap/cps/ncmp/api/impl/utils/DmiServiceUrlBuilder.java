...
public class DmiServiceUrlBuilder {
    ...
    public String getDmiDatastoreUrl(final MultiValueMap<String, String> queryParams,
        ...
        final UriComponentsBuilder uriComponentsBuilder = getCmHandleUrl()
                .pathSegment(DmiRegistryConstants.DATA_PATH)
                .pathSegment(DmiRegistryConstants.DS)
                .pathSegment(DmiRegistryConstants.DATA_STORE)
    ...
    public UriComponentsBuilder getCmHandleUrl() {
        return UriComponentsBuilder.newInstance()
                .path(DmiRegistryConstants.DMI_SERVICE_NAME)
                .pathSegment(DmiRegistryConstants.DMI_BASE_PATH)
                .pathSegment(DmiRegistryConstants.V1)
                .pathSegment(DmiRegistryConstants.CH)
                .pathSegment(DmiRegistryConstants.CM_HANDLE);
    }
    ...
    public Map<String, Object> populateUriVariables(final YangModelCmHandle yangModelCmHandle,
        ...
        uriVariables.put(DmiRegistryConstants.DMI_SERVICE_NAME_URI_VAR_NAME,
                yangModelCmHandle.resolveDmiServiceName(DATA));
        uriVariables.put(DmiRegistryConstants.DMI_BASE_PATH_URI_VAR_NAME, Strings.isNotEmpty(dmiBasePath)
                ? dmiBasePath : DmiRegistryConstants.DEFAULT_DMI_BASE_PATH);
        uriVariables.put(DmiRegistryConstants.CM_HANDLE_URI_VAR_NAME, cmHandle);
        uriVariables.put(DmiRegistryConstants.DATA_STORE_URI_VAR_NAME, dataStore.getValue());
        return uriVariables;
    }
    ...
    public MultiValueMap<String, String> populateQueryParams(final String resourceId,
        ...
        getQueryParamConsumer().accept(DmiRegistryConstants.RESOURCE_IDENTIFIER_QUERY_PARAM_NAME,
                resourceId, queryParams);
        getQueryParamConsumer().accept(DmiRegistryConstants.OPTIONS_QUERY_PARAM_NAME, optionsParamInQuery, queryParams);
        if (Strings.isNotEmpty(topicParamInQuery)) {
            getQueryParamConsumer().accept(DmiRegistryConstants.TOPIC_QUERY_PARAM_NAME, topicParamInQuery, queryParams);
        }
        return queryParams;
    }
