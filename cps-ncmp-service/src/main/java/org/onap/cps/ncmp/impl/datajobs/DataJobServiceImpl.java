...
public class DataJobServiceImpl implements DataJobService {
    ...
    private void logJsonRepresentation(final String description, final Object object) {
        ...
        log.info("{} (JSON): {}", description, objectAsJsonString);
    }
    ...
    private int countTotalOpsInDmiWriteOperations(final Map<ProducerKey, List<DmiWriteOperation>>
                                                          dmiWriteOperationsPerProducerKey) {
        return dmiWriteOperationsPerProducerKey.values().stream()
                .mapToInt(List::size)
                .sum();
    }
}
