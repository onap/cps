...
public class DataJobServiceImpl implements DataJobService {
    ...
    public void writeDataJob(final String dataJobId, final DataJobMetadata dataJobMetadata,
        ...
        producerKeyMap.forEach((key, value) -> {
            final String dmiResourceUrl = DmiServiceUrlBuilder.newInstance()
                                                        .pathSegment("writeJob")
                                                        .variablePathSegment("requestId", dataJobId)
                                                        .build(key.dmiServiceName(), dmiProperties.getDmiBasePath());
            final ResponseEntity<Object> response = dmiRestClient.postOperationWithJsonData(dmiResourceUrl,
                    jsonObjectMapper.asJsonString(value), OperationType.CREATE, NO_AUTH_HEADER);
            log.info("Sub job write response: {}", response);
        });
    }
