...
public class InventoryModelLoader extends AbstractModelLoader {
    ...
    private static final Pattern SCHEMA_SET_NAME_PATTERN =
            Pattern.compile("^(?<module>[a-zA-Z0-9\\-]+)-(?<revision>\\d{4}-\\d{2}-\\d{2})$");
    ...
    public void onboardOrUpgradeModel() {
        if (newRevisionEnabled) {
            if (doesAnchorExist(NCMP_DATASPACE_NAME, NCMP_DMI_REGISTRY_ANCHOR)) {
                ...
                installInventoryModel(NEW_INVENTORY_SCHEMA_SET_NAME);
            }
        } else {
            installInventoryModel(PREVIOUS_SCHEMA_SET_NAME);
        }
    ...
    private static String toYangFileName(final String schemaSetName) {
        final Matcher schemaSetNameMatcher = SCHEMA_SET_NAME_PATTERN.matcher(schemaSetName);
        schemaSetNameMatcher.matches();
        return schemaSetNameMatcher.group("module") + "@"
                + schemaSetNameMatcher.group("revision") + ".yang";
    }
}
