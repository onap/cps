...
import org.onap.cps.ncmp.api.models.BatchOperationDetails;
...
public class ResourceDataBatchRequestUtils {
     ...
     * @param requestedCmHandleIds     uri param map as key (placeholder),value pair
     */
    public static void extractAllDistinctCmHandleIds(final ResourceDataBatchRequest resourceDataBatchRequest,
                                                     final Set<String> requestedCmHandleIds) {
        resourceDataBatchRequest.getOperations().stream().filter(batchOperationDetails ->
                        !batchOperationDetails.getCmHandleIds().isEmpty())
                .forEach(batchOperationDetails -> requestedCmHandleIds.addAll(batchOperationDetails.getCmHandleIds()));
    }
    ...
    public static Map<String, List<DmiBatchRequestBody>> populateGroupsOutPerDmiServiceName(
        ...
        resourceDataBatchRequest.getOperations().forEach(batchOperationDetails -> {

            final DmiBatchRequestBody.DmiBatchRequestBodyBuilder dmiBatchRequestBodyBuilder
                    = getDmiBatchRequestBodyBuilder(batchOperationDetails);

            final List<String> nonEmptyCmHandleIds = batchOperationDetails.getCmHandleIds().stream()
    ...
    private static void setDmiBatchRequestBodyPerServiceName(final Map<String, List<DmiBatchRequestBody>>
        ...
        groupsOutPerDmiServiceName.get(dmiServiceName).stream()
                ...
                .ifPresentOrElse(dmiBatchRequestBody ->
                                dmiBatchRequestBody.getCmHandles().put(requestedCmHandleId, cmHandleProperties),
                        () -> {
    ...
    private static void setCmHandleProperties(final DmiBatchRequestBody.DmiBatchRequestBodyBuilder
        ...
        dmiBatchRequestBodyBuilder.cmHandles(cmHandleIdWithProperties);
    }

    private static boolean isAnyCmHandleIdAlreadyAddedIntoDmiBatchRequestBody(final List<String> nonEmptyCmHandleIds,
                                                                              final DmiBatchRequestBody
                                                                                     dmiBatchRequestBody) {
        return dmiBatchRequestBody.getCmHandles().keySet().stream().anyMatch(nonEmptyCmHandleIds::contains);
    }

    private static DmiBatchRequestBody.DmiBatchRequestBodyBuilder getDmiBatchRequestBodyBuilder(
            final BatchOperationDetails
                    batchOperationDetails) {
        return DmiBatchRequestBody.builder().operation(OperationType.fromOperationName(
                        batchOperationDetails.getOperation()))
                .datastore(DatastoreType.fromDatastoreName(batchOperationDetails.getDatastore()).getDatastoreName())
                .options(batchOperationDetails.getOptions())
                .resourceIdentifier(batchOperationDetails.getResourceIdentifier());
    }
}
