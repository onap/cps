...
public class ModuleSyncTasks {
    ...
    private CmHandleState processCmHandle(final YangModelCmHandle yangModelCmHandle) {
        ...
        try {
            ...
            if (isNoLongerLockable(yangModelCmHandle.getId())) {
    ...
    /**
     * Determine whether a CM handle should NOT be locked after a local sync failure. This is the case when another
     * instance already synced it to READY (so locking would overwrite a good state and emit a wrong event), or when
     * the CM handle no longer exists (it was deleted during sync).
     *
    ...
    private boolean isNoLongerLockable(final String cmHandleId) {
        try {
            ...
            log.info("CM handle '{}' no longer exists; lockg/retry not applicable", cmHandleId);
            return true;
        }
    }
