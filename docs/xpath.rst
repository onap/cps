...
In CPS , Xpath is a parameter used in several APIs, which allows us to query and parse JSON and XML data efficiently.
...
- JSON data and XML data are node trees
    - 'root element' is the top element of these trees

Relationships
=============

- Every element has a parent, not every element has a children
- Ancestor: a node's parent, parent's parent , parent's parent's parents …
    - In the given sample model and data, the root element 'shops' is the ancestor of every other element
