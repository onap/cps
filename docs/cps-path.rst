...
Each cpsPath expression need to start with an 'absolute' or 'descendant' xpath.
...
**Syntax**: ``<xpath> '[' @<leaf-name1> '=' <leaf-value1> ( ' and ' @<leaf-name> '=' <leaf-value> )* ']'``
  - ``xpath``: Absolute or descendant or xpath to the (list) node which elements will queried.
...
**Limitations**
  ...
  - Only string and integer values are supported, boolean and float values are not supported.
  - Since CPS cannot return individual leaves it will always return the container with all its leaves. Ancestor-axis can de used to specify a parent higher up the tree.
  - When querying a leaf value (instead of leaf-list) it is better, more performant to use use a text value condition use @<leaf-name> as described above.
