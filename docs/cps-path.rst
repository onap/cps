...
**Limitations**
  ...
  - Multiple attributes can only be combined using ``and`` ,``or``, Multiple AND 's , Multiple OR's.
  - Bracketing is not supported.
  - When mixing ``and/or`` operators the order of execution is not guaranteed and therefore the result is unpredicted. Although new error will be reported this mixing of operator is not recommended.
