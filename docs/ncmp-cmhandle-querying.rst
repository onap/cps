...
- ncmp/v1/ch/id-searches Returns all CM Handles which match the query properties provided. Gives a JSON payload of the **ids** of all matching CM Handles.

The only thing separating these endpoints is the JSON returned. Both can be queried with the same request body. If no matching CM Handles are found an empty array is returned.
...
Request Body example using all available query criteria:
