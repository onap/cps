...
- The Delta feature brings in two new functionalities:

    - Ability to find the delta between the configurations stored in two anchors within the same dataspace.
    - Ability to find the delta between configuration stored under an anchor against an incoming JSON payload.
...
The Delta Report is based on the RFC 9144, which defines a set of parameters to be included in the Delta Report. In CPS only the relevant parameters defined in RFC 9144 are used. These include:
    ...
    - **source-data:** this parameter is added to the delta report when a data node is deleted or updated, this represents the source/reference data against which the delta is being generated. In case of newly added data node this fiels is not included in the delta report.
