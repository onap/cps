...
All conditions in a ``conditions`` array are combined using the ``operator`` of the
containing composite query. They can therefore be read as a parenthesized logical
expression:

  - ``operator: "and"`` combines the conditions as ``(condition-1 and condition-2)``.
  - ``operator: "or"`` combines the conditions as ``(condition-1 or condition-2)``.

Nested ``conditions`` arrays remain grouped and are evaluated within the nodes selected
by their parent condition. For example, the query below can be understood as:

``category=SciFi and (price=5 or label=classic)``

The parentheses are important: the price and label conditions are combined with ``or``
before the result is combined with the category condition using ``and``.

Example
=======

Using the sample bookstore model and data described in :ref:`CPS Path<path>`, the following composite query
returns all books in the ``SciFi`` category that are either priced at 5 or tagged with
the label "classic":
