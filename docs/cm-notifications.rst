...
Two consumer modes are available, controlled by the ``batch-enabled`` boolean property (``true`` or ``false``).

Single-Record Mode (Default: ``batch-enabled: false``)
-----------------------------
...
Batch Mode with Exactly-Once Semantics (``batch-enabled: true``)
---------------------------------------
