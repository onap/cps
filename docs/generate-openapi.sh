...
# local development environments) generation proceeds without the guard;
# concurrent runs could then race, but that is not expected in CI or on
# ReadTheDocs.
LOCK_FILE="$SCRIPT_DIR/.openapi-generator.lock"
if command -v flock > /dev/null 2>&1; then
    exec 9> "$LOCK_FILE"
    # Fail fast rather than block indefinitely if the lock is not acquired.
    flock -w 300 9 || { echo "Timed out waiting for generation lock" >&2; exit 1; }
else
    echo "WARNING: flock not available; running without concurrency guard" >&2
fi
