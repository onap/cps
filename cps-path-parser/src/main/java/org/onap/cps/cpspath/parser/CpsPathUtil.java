...
public class CpsPathUtil {
    ...
    // Single source of truth for a valid key condition after getNormalizedXpath.
    // Matches on quote boundaries so ']' and ' and ' inside values are handled correctly.
    private static final String CONDITION_REGEX = "@[^=]+='(?:[^']|'')*'|@[^=]+=\\d+";

    // Extracts individual conditions from a predicate body.
    private static final Pattern CONDITION_PATTERN = Pattern.compile(CONDITION_REGEX);

    // Finds full [...] predicate blocks. Built from CONDITION_REGEX so bracket
    // detection is also quote-aware — no [^]]+ fragility.
