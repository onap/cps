...
public class CpsPathBuilder extends CpsPathBaseListener {
    ...
    public void exitLeafCondition(final LeafConditionContext ctx) {
        ...
        final String[] operatorValues = booleanOperators.toArray(new String[0]);
        for (final String operatorValue : operatorValues) {
            getBooleanOperator(ctx, comparisonValue, lastCharacter, operatorValue);
        }
    }

    private void getBooleanOperator(final LeafConditionContext ctx, final Object comparisonValue,
