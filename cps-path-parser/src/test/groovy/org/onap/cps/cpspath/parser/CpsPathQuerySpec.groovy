...
class CpsPathQuerySpec extends Specification {
    ...
    def 'Parse cps path to form the Normalized cps path containing #scenario.'() {
        ...
        'parent & child with more than one attribute'         | '/parent/child[@key1=1 and @key2="abc" and @key3=1]/child2' || "/parent/child[@key1='1' and @key2='abc' and @key3='1']/child2"
    }
    ...
    def 'Parse cps path having OR operator containing #scenario.'() {
        when: 'the given cps path is parsed'
        def result = CpsPathUtil.getCpsPathQuery(cpsPath)
        then: 'the query has the right normalized xpath type'
        assert result.normalizedXpath == expectedNormalizedXPath
        ...
        'parent & child with multiple  attributes'    | '/parent/child[@key1=1 or @key2="abc" or @key3=1]/child2' || "/parent/child[@key1='1' or @key2='abc' or @key3='1']/child2"
    }

    def 'Parse cps path that ends with a yang list containing #scenario and having or operator '() {
        when: 'the given cps path is parsed'
        def result = CpsPathQuery.createFrom(cpsPath)
        then: 'the query has the right xpath type'
        result.cpsPathPrefixType == DESCENDANT
