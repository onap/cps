...
${jsonMatchingPublicProperties}             {"publicCmHandleProperties": {"Contact" : "newemailforstore@bookstore.com"}}
${jsonMissingPropertyPublicProperties}      {"publicCmHandleProperties": { "" : "doesnt matter"}}
...
Throw 400 when Structure of Request is Incorrect
    ${uri}=              Set Variable       ${ncmpBasePath}/data/ch/searches
    ${headers}=          Create Dictionary  Content-Type=application/json   Authorization=${auth}
    ${response}=         POST On Session    CPS_URL   ${uri}   headers=${headers}   data=${jsonMissingPropertyPublicProperties}    expected_status=400
