...
while [ "$TICKER" -le "$TIME_OUT" ]; do
	...
	RESPONSE=$(curl --location --request GET 'http://'$1'/manage/health/liveness')
...
ROBOT_VARIABLES="-v CPS_HOST:$LOCAL_IP -v CPS_PORT:$CPS_CORE_PORT -v DMI_HOST:$LOCAL_IP -v DMI_PORT:$DMI_PORT -v CPS_CORE_MANAGEMENT_PORT:$CPS_CORE_MANAGEMENT_PORT -v DATADIR:$WORKSPACE/data"