...
import org.onap.cps.integration.base.NcmpRegistryPerformanceSpecBase

class CmHandleQueryPerformanceSpec extends NcmpRegistryPerformanceSpecBase {
    ...
    def 'Query CM Handle IDs by a property name and value.'() {
        when: 'a cps-path query on name-value pair is performed (without getting descendants)'
            ...
            def dataNodes = cpsQueryService.queryDataNodes(PERFORMANCE_TEST_DATASPACE, REGISTRY_ANCHOR, cpsPath, OMIT_DESCENDANTS)
        ...
        and: 'get single get is executed to get all the parent objects and their descendants'
            def result = cpsDataService.getDataNodesForMultipleXpaths(PERFORMANCE_TEST_DATASPACE, REGISTRY_ANCHOR, xpaths, INCLUDE_ALL_DESCENDANTS)
            stopWatch.stop()
            def durationInMillis = stopWatch.getTotalTimeMillis()
        then: 'the operations are performed within 1,000 ms'
            recordAndAssertPerformance("CpsPath Registry attributes Query", 1000, durationInMillis)
    ...
    def 'Multiple get limitation: 32,764 (~ 2^15) xpaths.'() {
        ...
        when: 'get single get is executed to get all the parent objects and their descendants'
            cpsDataService.getDataNodesForMultipleXpaths(PERFORMANCE_TEST_DATASPACE, REGISTRY_ANCHOR, xpaths, INCLUDE_ALL_DESCENDANTS)
        then: 'the operation fails because the limit is exceeded'
