...
class CmSubscriptionSpec extends CpsIntegrationSpecBase {
    ...
    def cleanup() {
        ...
        deregisterCmHandles('dmi-1', ['cmHandle1', 'cmHandle2', 'cmHandle99'])
    ...
    def 'Delete subscription removes last subscriber.'() {
        given: 'an existing subscription with only one data node selector'
            def dataNodeSelector = '/parent[id=\\\"99\\\"]'
        and: 'a subscription created and accepted by DMI'
            ...
            sendSubscriptionRequest(subscriptionTopic, 'lastDataJobId', createEventPayload)
        and: 'data nodes is persisted '
            ...
            assert dataNodes.iterator().next().leaves.dataNodeSelector == '/parent[id="99"]'
        when: 'a delete event is received for the subscription'
            ...
            sendSubscriptionRequest(subscriptionTopic, 'lastDataJobId', deleteEventPayload)
    ...
    def 'Delete subscription removes one of multiple subscribers.'() {
        given: 'a data node with multiple subscriptions that match registered CM handles'
            def dataNodeSelector = '/parent[id=\\\"1\\\"]'
        ...
        and: 'the remaining subscription is still present'
            def remainingSubscription = cpsQueryService.queryDataNodes('NCMP-Admin', 'cm-data-job-subscriptions',
                    "//subscription/dataJobId[text()='id-remaining']", DIRECT_CHILDREN_ONLY)
            assert !remainingSubscription.isEmpty()
    ...
    def registerCmHandlesForSubscriptions() {
        ...
        registerCmHandle('dmi-1', 'cmHandle99', '', '/parent=99')
    }
