...
class ModuleSyncWatchdogWithModuleSetTagSpec extends CpsIntegrationSpecBase {
    ...
    def cleanup() {
        deregisterSequenceOfCmHandles(DMI1_URL, CM_HANDLES_PER_TAG)
    }

    def 'CPS-2478 Module Sync inefficiencies'() {
        ...
        and: 'this takes about 1.2 second'
            ...
            assert dmiModuleRetrievalTimer.totalTime(TimeUnit.MILLISECONDS) < 1500
        ...
        and: '1 schema set stored in db created for each cm handle'
            ...
            //TODO Toine, check why this is almost always 1 sort...
            assert dbSchemaSetStorageTimer.count() >= TOTAL_CM_HANDLES - 1
        and: 'this takes over 7 seconds'
            assert dbSchemaSetStorageTimer.totalTime(TimeUnit.MILLISECONDS) > 7000
    }
