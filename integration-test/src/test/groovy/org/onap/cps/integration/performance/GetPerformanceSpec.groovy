...
package org.onap.cps.integration.performance

import org.onap.cps.integration.base.PerformanceSpecBase
import org.onap.cps.spi.FetchDescendantsOption

class GetPerformanceSpec extends PerformanceSpecBase {
    ...
    def 'Read complete data trees from multiple anchors using #scenario.'() {
        when: 'get data nodes for 10 anchors'
            stopWatch.start()
            (1..10).each {
                objectUnderTest.getDataNodes(PERFORMANCE_TEST_DATASPACE, "anchor${it}", xpath, FetchDescendantsOption.INCLUDE_ALL_DESCENDANTS)
            }
            stopWatch.stop()
