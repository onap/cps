...
class CpsAdminServiceIntegrationSpec extends CpsIntegrationSpecBase {
    ...
    def 'Update anchor schema set.'() {
        when: 'a new schema set with bookstore yang model is created'
        def bookstoreYangModelAsString = readResourceDataFile('bookstore/bookstore.yang')
        cpsModuleService.createSchemaSet(GENERAL_TEST_DATASPACE, 'newSchemaSet', [bookstore: bookstoreYangModelAsString])
        then: 'an anchor with new schema set is created'
        objectUnderTest.createAnchor(GENERAL_TEST_DATASPACE, 'newSchemaSet', 'anchor4')
        and: 'a updated schema set with bookstore yang model is created'
        def updatedBookstoreYangModelAsString = readResourceDataFile('bookstore/bookstore.yang')
        cpsModuleService.createSchemaSet(GENERAL_TEST_DATASPACE, 'updatedSchemaSet', [bookstore: updatedBookstoreYangModelAsString])
