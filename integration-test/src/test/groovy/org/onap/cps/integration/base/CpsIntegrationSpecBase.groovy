...
abstract class CpsIntegrationSpecBase extends Specification {
    ...
    def getLatestConsumerRecordsWithPollDurationInMs(numberOfRecordsToRead, pollDurationInMs) {
        def consumerRecords = []
        def retryAttempts = 10
        while (consumerRecords.size() < numberOfRecordsToRead) {
            retryAttempts--
            consumerRecords.addAll(kafkaConsumer.poll(Duration.ofMillis(pollDurationInMs)))
            if (retryAttempts == 0)
                break
        }
