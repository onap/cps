...
class ResourceMeterPerfTest extends Specification {
    ...
    def 'ResourceMeter accurately measures duration'() {
        when: 'we measure how long a known operation takes'
            resourceMeter.start()
            TimeUnit.SECONDS.sleep(durationToSleepInSeconds as long)
            resourceMeter.stop()
        then: 'ResourceMeter reports a duration within 10ms of the expected duration'
            assert resourceMeter.getTotalTimeInSeconds() >= durationToSleepInSeconds
            assert resourceMeter.getTotalTimeInSeconds() <= durationToSleepInSeconds + 0.01
        where:
            durationToSleepInSeconds << [1, 2, 3]
    }
    ...
    def 'ResourceMeter reports memory usage when allocating a large byte array'() {
        ...
        and: 'some memory is allocated'
            byte[] array = new byte[megabytesToAllocate * MEGABYTE]
        and: 'the resource meter is stopped'
            resourceMeter.stop()
        then: 'the reported memory usage is close to the amount of memory allocated'
            assert resourceMeter.getTotalMemoryUsageInMB() >= megabytesToAllocate
            assert resourceMeter.getTotalMemoryUsageInMB() <= megabytesToAllocate + 5
        where:
            megabytesToAllocate << [25, 50, 100, 200]
    }
    ...
    def 'ResourceMeter measures memory increase only during measurement'() {
        ...
        when: 'memory is allocated during measurement'
            resourceMeter.start()
            byte[] arrayDuring = new byte[megabytesToAllocate * 1_000_000]
            resourceMeter.stop()
        ...
        then: 'the reported memory usage is close to the amount allocated DURING measurement'
            ...
            assert resourceMeter.getTotalMemoryUsageInMB() <= megabytesToAllocate + 5
        where:
            megabytesToAllocate << [20, 40, 100, 200]
    }
