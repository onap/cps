...
class AnchorServiceIntegrationSpec extends FunctionalSpecBase {
    ...
    def 'Get anchors returns all anchors when #scenario.'() {
        ...
        then: 'all 3 anchors are returned (no-pagination)'
    ...
    def 'Get anchors with pagination returns correct page for #scenario.'() {
        when: 'getAnchors is called with the given schema set names and page parameters'
        ...
        where: 'the following pagination and schema set name combinations are used'
            scenario                          | schemaSetNames       | pageIndex | pageSize || expectedSize
            'no schema set name, page 1 of 2' | null                 | 1         | 2        || 2
            'no schema set name, page 2 of 2' | null                 | 2         | 2        || 1
            'schema set name, page 1'         | 'bookstoreSchemaSet' | 1         | 2        || 2
            'schema set name, page 2'         | 'bookstoreSchemaSet' | 2         | 1        || 1
    }
    ...
    def 'Get anchors filtered by schema set names (no pagination) returns correct count.'() {
        ...
        where: 'the following schema set filters are used'
            scenario              | schemaSetNames                      || expectedSize
    ...
    def 'Get anchors with for invalid pagination: #scenario.'() {
        when: 'getAnchors is called with invalid pagination values'
            objectUnderTest.getAnchors(GENERAL_TEST_DATASPACE, schemaSetNames, invalidPageIndex, invalidPageSize)
        ...
        where: 'the following invalid pagination values and optional schema set filters are used'
            scenario                         | schemaSetNames       | invalidPageIndex | invalidPageSize
            'negative pageIndex'             | null                 | -1               | 4
            'negative pageSize'              | null                 | 1                | -1
            'negative pageSize and index'    | null                 | -1               | -1
            'zero pageIndex'                 | null                 | 0                | 4
            'zero pageSize'                  | null                 | 1                | 0
            'pagesize and index set to zero' | null                 | 0                | 0
            'schema set, negative pageIndex' | 'bookstoreSchemaSet' | -1               | 4
            'schema set, negative pageSize'  | 'bookstoreSchemaSet' | 1                | -1
            'schema set, both negative'      | 'bookstoreSchemaSet' | -1               | -1
            'schema set, zero pageIndex'     | 'bookstoreSchemaSet' | 0                | 4
            'schema set, zero pageSize'      | 'bookstoreSchemaSet' | 1                | 0
    }
