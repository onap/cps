...
class DataServiceIntegrationSpec extends FunctionalSpecBase {
    ...
    def 'Replace data node at root xpath on an anchor with no existing data.'() {
        ...
        when: 'replace is performed at the root xpath with a full data tree'
            ...
            def dataWasCreated = objectUnderTest.updateDataNodeAndDescendants(FUNCTIONAL_TEST_DATASPACE_1, 'emptyAnchorForPutTest', '/', json, now, ContentType.JSON)
        then: 'the method reports that data was created (not replaced)'
            assert dataWasCreated
