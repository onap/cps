...
class FunctionalSpecBase extends CpsIntegrationSpecBase {
    ...
    def static PARENT_LIST_TEST_DATASPACE = 'parentlistTestDataspace'
    def static NUMBER_OF_ANCHORS_PER_DATASPACE_WITH_PARENTLIST_DATA = 1
    def static PARENT_LIST_TEST_ANCHOR = 'parentlistTestAnchor1'
    ...
    def setupParentListInfraStructure() {
        cpsAdminService.createDataspace(PARENT_LIST_TEST_DATASPACE)
        def parentlistYangModelAsString = readResourceDataFile('parent-list/parent-list.yang')
        cpsModuleService.createSchemaSet(PARENT_LIST_TEST_DATASPACE , PARENT_LIST_SCHEMA_SET, [parentlist: parentlistYangModelAsString])
    }

    def addParentListData() {
        addAnchorsWithData(NUMBER_OF_ANCHORS_PER_DATASPACE_WITH_PARENTLIST_DATA, PARENT_LIST_TEST_DATASPACE, PARENT_LIST_SCHEMA_SET, 'parentlistTestAnchor', parentlistJsonData)
    }
