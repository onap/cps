...
class UpdatePerfTest extends CpsPerfTestBase {
    ...
    def 'Update 1 data node with descendants'() {
        ...
        then: 'update completes within expected time and memory used is within limit'
            recordAndAssertPerformance('Update 1 data node', 600, updateDurationInMillis, 200, resourceMeter.getTotalMemoryUsageInMB())
    }
    ...
    def 'Batch update 100 data nodes with descendants'() {
        ...
        when: 'the fragment entities are updated by the data nodes'
            ...
            def updateDurationInMillis = resourceMeter.getTotalTimeMillis()
        then: 'update completes within expected time and memory used is within limit'
            recordAndAssertPerformance('Update 100 data nodes', TimeUnit.SECONDS.toMillis(30), updateDurationInMillis, 800, resourceMeter.getTotalMemoryUsageInMB())
    }
    ...
    def 'Update leaves for 1 data node (twice)'() {
        ...
        when: 'update is performed for leaves'
            ...
            def updateDurationInMillis = resourceMeter.getTotalTimeMillis()
        then: 'update completes within expected time and memory used is within limit'
            recordAndAssertPerformance('Update leaves for 1 data node', 500, updateDurationInMillis, 300, resourceMeter.getTotalMemoryUsageInMB())
    }
    ...
    def 'Batch update leaves for 100 data nodes (twice)'() {
        ...
        when: 'update is performed for leaves'
            ...
            def updateDurationInMillis = resourceMeter.getTotalTimeMillis()
        then: 'update completes within expected time and memory used is within limit'
            recordAndAssertPerformance('Batch update leaves for 100 data nodes', TimeUnit.SECONDS.toMillis(1), updateDurationInMillis, 300, resourceMeter.getTotalMemoryUsageInMB())
    }
