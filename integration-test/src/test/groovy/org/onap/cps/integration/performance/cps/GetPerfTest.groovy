...
class GetPerfTest extends CpsPerfTestBase {
    ...
    def 'Read top-level node with #scenario.'() {
        ...
        then: 'all data is read within #durationLimit ms and memory used is within limit'
            recordAndAssertPerformance("Read datatrees with ${scenario}", durationLimit, durationInMillis, memoryLimit, resourceMeter.getTotalMemoryUsageInMB())
    ...
    def 'Read data trees for multiple xpaths'() {
        ...
        and: 'all data is read within expected time and memory used is within limit'
            recordAndAssertPerformance("Read datatrees for multiple xpaths", TimeUnit.SECONDS.toMillis(3) , durationInMillis, 200, resourceMeter.getTotalMemoryUsageInMB())
    }
    ...
    def 'Read for multiple xpaths to non-existing datanodes'() {
        ...
        and: 'the operation completes within within expected time'
            recordAndAssertPerformance("Read non-existing xpaths", 10, durationInMillis, 2, resourceMeter.getTotalMemoryUsageInMB())
    }
    ...
    def 'Read complete data trees using #scenario.'() {
        ...
        then: 'all data is read within expected time and memory used is within limit'
            recordAndAssertPerformance("Read datatrees using ${scenario}", durationLimit, durationInMillis, memoryLimit, resourceMeter.getTotalMemoryUsageInMB())
