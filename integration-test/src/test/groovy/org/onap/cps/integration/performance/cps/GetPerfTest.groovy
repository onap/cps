...
class GetPerfTest extends CpsPerfTestBase {
    ...
    def 'Read top-level node with #scenario.'() {
        ...
        and: 'all data is read within #durationLimit ms'
            recordAndAssertPerformance("Read datatrees with ${scenario}", durationLimit, durationInMillis, memoryUsed)
    ...
    def 'Read data trees for multiple xpaths'() {
        ...
        and: 'all data is read within expected time'
            recordAndAssertPerformance("Read datatrees for multiple xpaths", TimeUnit.SECONDS.toMillis(3) , durationInMillis, memoryUsed)
    }
    ...
    def 'Read for multiple xpaths to non-existing datanodes'() {
        ...
        and: 'the operation completes within within expected time'
            recordAndAssertPerformance("Read non-existing xpaths", 10, durationInMillis, memoryUsed)
    }
    ...
    def 'Read complete data trees using #scenario.'() {
        ...
        then: 'memory used is within #peakMemoryUsage'
            ...
            assert memoryUsed <= memoryLimit
        and: 'all data is read within expected time'
            recordAndAssertPerformance("Read datatrees using ${scenario}", durationLimit, durationInMillis, memoryUsed)
