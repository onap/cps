...
class DeltaPerfTest extends CpsPerfTestBase{
    ...
    def jsonPayload = generateModifiedOpenRoadData(200, 100, 100)
    ...
    def 'Get delta between an anchor and JSON payload with grouping enabled'() {
        ...
        where: 'the following parameters are used'
            ...
            'no descendants'     | OMIT_DESCENDANTS        || 3.5
            'direct descendants' | DIRECT_CHILDREN_ONLY    || 4.0
            'all descendants'    | INCLUDE_ALL_DESCENDANTS || 22.0
    }
    ...
    def 'Get delta between an anchor and JSON payload with grouping disabled'() {
        when: 'attempt to get delta between an anchor and JSON payload with grouping disabled'
            ...
            print(durationInSeconds)
        then: 'the delta is returned and operation completes within expected time'
            recordAndAssertResourceUsage('CPS:Get delta between an anchor and JSON payload with grouping disabled', 17, durationInSeconds, resourceMeter.getTotalMemoryUsageInMB())
    }
    ...
    def 'Apply delta report to an anchor'() {
        ...
        then: 'the delta is applied and operation completes within expected time'
            recordAndAssertResourceUsage('CPS:Apply delta report to an anchor', 6.5, durationInSeconds, resourceMeter.getTotalMemoryUsageInMB())
    }
}
