...
class CpsDeltaServicePerfTest extends CpsPerfTestBase{
    ...
    def 'Get delta between an anchor and JSON payload with grouping disabled'() {
        when: 'attempt to get delta between an anchor and JSON payload with grouping disabled'
            resourceMeter.start()
            def jsonPayload = generateOpenRoadData(200)
            def deltaReport = objectUnderTest.getDeltaByDataspaceAnchorAndPayload(CPS_PERFORMANCE_TEST_DATASPACE, 'openroadm1', '/openroadm-devices', Collections.emptyMap(), jsonPayload, INCLUDE_ALL_DESCENDANTS, false)
            resourceMeter.stop()
        print(deltaReport.size())
            def durationInSeconds = resourceMeter.getTotalTimeInSeconds()
        then: 'the delta is returned and operation completes within expected time'
            recordAndAssertResourceUsage('CPS:Get delta between an anchor and JSON payload with grouping disabled', 3.5, durationInSeconds, resourceMeter.getTotalMemoryUsageInMB())
    }
}
