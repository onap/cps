...
class CmHandleQueryPerfTest extends NcmpPerfTestBase {
    ...
    def 'JVM warmup.'() {
        ...
        then: 'resource usage is as expected'
            recordAndAssertResourceUsage('NCMP:JVM warmup for CmHandleQueryPerfTest', 60, resourceMeter.totalTimeInSeconds, resourceMeter.totalMemoryUsageInMB)
    }
    ...
    def 'CM-handle is looked up by alternate id.'() {
        ...
        then: 'all alternate ids are resolved correctly'
            assert count == 100
        and: 'average performance is as expected'
            recordAndAssertResourceUsage('NCMP:Look up CM-handle by alternate-id', 1.75, resourceMeter.totalTimeInSeconds, resourceMeter.totalMemoryUsageInMB, REFERENCE_GRAPH)
    }
    ...
    def 'A batch of CM-handles is looked up by alternate id.'() {
        ...
        then: 'average performance is as expected'
            recordAndAssertResourceUsage('NCMP:Batch look up CM-handle by alternate-id', 0.4, resourceMeter.totalTimeInSeconds, resourceMeter.totalMemoryUsageInMB)
    }
    ...
    def 'Find any CM-handle given moduleSetTag when there are 20K READY handles with same moduleSetTag.'() {
        ...
        then: 'average performance is as expected'
            ...
            recordAndAssertResourceUsage('NCMP:Look up CM-handles by module-set-tag', 0.25, averageResponseTime, resourceMeter.totalMemoryUsageInMB)
    }
