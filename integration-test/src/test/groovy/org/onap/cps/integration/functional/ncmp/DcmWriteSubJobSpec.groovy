...
class DcmWriteSubJobSpec extends CpsIntegrationSpecBase {
    ...
    def 'Create write data job and validate sub-job routing'() {
        ...
        when: 'a POST request is made to the write job test endpoint'
            ...
            .andReturn()
        then: 'the response contains 2 sub-jobs, grouped by their DMI plugin'
            def response = parseJsonResponse(mvcResult.response.contentAsString)
            assert response.size() == 2
            assert response.every {
                it.subJobId == 'some sub job id' &&
                it.dataProducerId == 'some data producer id' &&
                (it.dmiServiceName.startsWith('http://kubernetes') || it.dmiServiceName.startsWith('http://localhost'))
            }
        and: 'DMI 1 received the correct job details'
            def dmi1SubJobs = dmiDispatcher1.receivedSubJobs['?destination=d1']?.data
            assert dmi1SubJobs != null
        ...
        and: 'DMI 2 received the correct job details'
            ...
            assert dmi2SubJobs != null
            assert dmi2SubJobs.size() == 1
            assert dmi2SubJobs[0].path == 'p3'
    }

    def parseJsonResponse(String jsonResponse) {
