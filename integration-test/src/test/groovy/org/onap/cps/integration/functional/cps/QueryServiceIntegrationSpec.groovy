...
class QueryServiceIntegrationSpec extends FunctionalSpecBase {
    ...
    def 'Query data leaf with type #leafType using CPS path.'() {
        when: 'query data leaf for bookstore container'
        def results = objectUnderTest.queryDataLeaf(FUNCTIONAL_TEST_DATASPACE_1, BOOKSTORE_ANCHOR_1,
                '//books[@title="Matilda" or @title="Good Omens"]/@' + leafName, leafType)
        then: 'the result contains the expected leaf values'
            assert results == expectedResults.toSet()
        where:
    ...
    def 'Query data leaf using CPS path with ancestor axis.'() {
        when: 'query data leaf for bookstore container'
            def result = objectUnderTest.queryDataLeaf(FUNCTIONAL_TEST_DATASPACE_1, BOOKSTORE_ANCHOR_1,
                    '//books[@title="Matilda" or @title="Good Omens"]/ancestor::categories/@name', String.class)
        then: 'the result contains the expected leaf values'
            assert result == ['Children', 'Comedy'].toSet()
    }
