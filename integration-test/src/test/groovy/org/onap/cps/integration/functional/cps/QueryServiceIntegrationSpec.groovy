...
class QueryServiceIntegrationSpec extends FunctionalSpecBase {
    ...
    def 'Query data leaf using CPS path for #scenario.'() {
        when: 'query data leaf for bookstore container'
            def result = objectUnderTest.queryDataLeaf(FUNCTIONAL_TEST_DATASPACE_1, BOOKSTORE_ANCHOR_1, cpsPath, Object.class, -1)
    ...
    def 'Query data leaf with type #leafType using CPS path.'() {
        ...
        when: 'query data leaf for bookstore container'
            def results = objectUnderTest.queryDataLeaf(FUNCTIONAL_TEST_DATASPACE_1, BOOKSTORE_ANCHOR_1, cpsPath, leafType, -1)
    ...
    def 'Query data leaf using CPS path with ancestor axis.'() {
        ...
        when: 'query data leaf for bookstore container'
            def result = objectUnderTest.queryDataLeaf(FUNCTIONAL_TEST_DATASPACE_1, BOOKSTORE_ANCHOR_1, cpsPath, String.class, -1)
        then: 'the result contains the expected leaf values'
    ...
    def 'Attempt to query data leaf without specifying leaf name gives an error.'() {
        ...
        when: 'query data leaf is called without attribute axis in cps path'
            objectUnderTest.queryDataLeaf(FUNCTIONAL_TEST_DATASPACE_1, BOOKSTORE_ANCHOR_1, cpsPathWithoutAttributeAxis, String.class, -1)
