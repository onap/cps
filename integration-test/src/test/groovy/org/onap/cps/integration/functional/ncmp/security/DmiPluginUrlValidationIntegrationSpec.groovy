...
class DmiPluginUrlValidationIntegrationSpec extends CpsIntegrationSpecBase {
    ...
    def 'DMI registration rejected by allowed URL pattern.'() {
        given: 'a registration with a URL not matching the configured pattern'
            def cmHandleToCreate = new NcmpServiceCmHandle(cmHandleId: 'ch-pattern-reject')
            def registration = new DmiPluginRegistration(dmiPlugin: 'http://not-allowed-host:8080', createdCmHandles: [cmHandleToCreate])
        when: 'the registration is submitted'
            objectUnderTest.updateDmiRegistration(registration)
        then: 'a DataValidationException is thrown'
            thrown(DataValidationException)
