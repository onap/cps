...
class DeletePerfTest extends CpsPerfTestBase {
    ...
    def 'Create test data (please note, subsequent tests depend on this running first).'() {
        ...
        then: 'setup duration is within expected time and memory used is within limit'
            recordAndAssertPerformance('Delete test setup', TimeUnit.SECONDS.toMillis(200), setupDurationInMillis, 200, resourceMeter.getTotalMemoryUsedMB())
    }
    ...
    def 'Delete 100 container nodes'() {
        ...
        then: 'delete duration is within expected time and memory used is within limit'
            recordAndAssertPerformance('Delete 100 containers', TimeUnit.SECONDS.toMillis(2), deleteDurationInMillis, 30, resourceMeter.getTotalMemoryUsedMB())
    }
    ...
    def 'Batch delete 100 container nodes'() {
        ...
        when: 'child nodes are deleted'
            ...
            def deleteDurationInMillis = resourceMeter.getTotalTimeMillis()
        then: 'delete duration is within expected time and memory used is within limit'
            recordAndAssertPerformance('Batch delete 100 containers', 500, deleteDurationInMillis, 5, resourceMeter.getTotalMemoryUsedMB())
    }
    ...
    def 'Delete 100 list elements'() {
        ...
        when: 'list elements are deleted'
            ...
            def deleteDurationInMillis = resourceMeter.getTotalTimeMillis()
        then: 'delete duration is within expected time and memory used is within limit'
            recordAndAssertPerformance('Delete 100 lists elements', TimeUnit.SECONDS.toMillis(2), deleteDurationInMillis, 20, resourceMeter.getTotalMemoryUsedMB())
    }
    ...
    def 'Batch delete 100 list elements'() {
        ...
        when: 'list elements are deleted'
            ...
            def deleteDurationInMillis = resourceMeter.getTotalTimeMillis()
        then: 'delete duration is within expected time and memory used is within limit'
            recordAndAssertPerformance('Batch delete 100 lists elements', 500, deleteDurationInMillis, 2, resourceMeter.getTotalMemoryUsedMB())
    }
    ...
    def 'Delete 100 whole lists'() {
        ...
        when: 'lists are deleted'
            ...
            def deleteDurationInMillis = resourceMeter.getTotalTimeMillis()
        then: 'delete duration is within expected time and memory used is within limit'
            recordAndAssertPerformance('Delete 100 whole lists', TimeUnit.SECONDS.toMillis(5), deleteDurationInMillis, 30, resourceMeter.getTotalMemoryUsedMB())
    }
    ...
    def 'Batch delete 100 whole lists'() {
        ...
        when: 'lists are deleted'
            ...
            def deleteDurationInMillis = resourceMeter.getTotalTimeMillis()
        then: 'delete duration is within expected time and memory used is within limit'
            recordAndAssertPerformance('Batch delete 100 whole lists', TimeUnit.SECONDS.toMillis(4), deleteDurationInMillis, 5, resourceMeter.getTotalMemoryUsedMB())
    }
    ...
    def 'Delete 1 large data node'() {
        when: 'parent node is deleted'
            ...
            def deleteDurationInMillis = resourceMeter.getTotalTimeMillis()
        then: 'delete duration is within expected time and memory used is within limit'
            recordAndAssertPerformance('Delete one large node', TimeUnit.SECONDS.toMillis(2), deleteDurationInMillis, 2, resourceMeter.getTotalMemoryUsedMB())
    }
    ...
    def 'Delete root node with many descendants'() {
        when: 'root node is deleted'
            ...
            def deleteDurationInMillis = resourceMeter.getTotalTimeMillis()
        then: 'delete duration is within expected time and memory used is within limit'
            recordAndAssertPerformance('Delete root node', TimeUnit.SECONDS.toMillis(2), deleteDurationInMillis, 2, resourceMeter.getTotalMemoryUsedMB())
    }
    ...
    def 'Delete data nodes for an anchor'() {
        when: 'data nodes are deleted'
            ...
            def deleteDurationInMillis = resourceMeter.getTotalTimeMillis()
        then: 'delete duration is within expected time and memory used is within limit'
            recordAndAssertPerformance('Delete data nodes for anchor', TimeUnit.SECONDS.toMillis(2), deleteDurationInMillis, 2, resourceMeter.getTotalMemoryUsedMB())
    }
    ...
    def 'Batch delete 100 non-existing nodes'() {
        ...
        when: 'child nodes are deleted'
            ...
            def deleteDurationInMillis = resourceMeter.getTotalTimeMillis()
        then: 'delete duration is within expected time and memory used is within limit'
            recordAndAssertPerformance('Batch delete 100 non-existing', TimeUnit.SECONDS.toMillis(7), deleteDurationInMillis, 5, resourceMeter.getTotalMemoryUsedMB())
    }
    ...
    def 'Clean up test data'() {
        ...
        when: 'data nodes are deleted'
            ...
            def deleteDurationInMillis = resourceMeter.getTotalTimeMillis()
        then: 'delete duration is within expected time and memory used is within limit'
            recordAndAssertPerformance('Delete test cleanup', TimeUnit.SECONDS.toMillis(10), deleteDurationInMillis, 2, resourceMeter.getTotalMemoryUsedMB())
    }
