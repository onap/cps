...
class QueryPerfTest extends CpsPerfTestBase {
    ...
    def 'Query complete data trees with #scenario.'() {
        ...
        and: 'all data is read #expectedDuration seconds with a margin of 100%'
            recordAndAssertResourceUsage('CPS:', "Query 1 anchor ${scenario}", expectedDuration, durationInSeconds, memoryLimit, resourceMeter.getTotalMemoryUsageInMB(), false)
    ...
    def 'Query complete data trees across all anchors with #scenario.'() {
        ...
        then: 'the expected number of nodes is returned'
            assert countDataNodesInTree(result) == expectedNumberOfDataNodes
        and: 'all data is read #expectedDuration seconds with a margin of 100%'
            recordAndAssertResourceUsage('CPS:', "Query across anchors ${scenario}", expectedDuration, durationInSeconds, memoryLimit, resourceMeter.getTotalMemoryUsageInMB(), false)
        where: 'the following parameters are used'
            scenario                     | cpspath                                                             || expectedDuration | memoryLimit | expectedNumberOfDataNodes
    ...
    def 'Query with leaf condition and #scenario.'() {
        ...
        then: 'the expected number of nodes is returned'
            assert countDataNodesInTree(result) == expectedNumberOfDataNodes
        and: 'all data is read #expectedDuration seconds with a margin of 100%'
            recordAndAssertResourceUsage('CPS:', "Query with ${scenario}", expectedDuration, durationInSeconds, memoryLimit, resourceMeter.getTotalMemoryUsageInMB(), false)
        where: 'the following parameters are used'
    ...
    def 'Query ancestors with #scenario.'() {
        ...
        then: 'the expected number of nodes is returned'
            assert countDataNodesInTree(result) == expectedNumberOfDataNodes
        and: 'all data is read #expectedDuration seconds with a margin of 100%'
            recordAndAssertResourceUsage('CPS:', "Query ancestors with ${scenario}", expectedDuration, durationInSeconds, memoryLimit, resourceMeter.getTotalMemoryUsageInMB(), false)
        where: 'the following parameters are used'
            scenario             | fetchDescendantsOption  || expectedDuration | memoryLimit | expectedNumberOfDataNodes
    ...
    def 'Query data leaf with #scenario.'() {
        ...
        and: 'all data is read #expectedDuration seconds with a margin of 100%'
            recordAndAssertResourceUsage('CPS:', "Query data leaf ${scenario}", expectedDuration, durationInSeconds, memoryLimit, resourceMeter.getTotalMemoryUsageInMB(), false)
        where: 'the following parameters are used'
