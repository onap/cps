...
class QueryPerfTest extends CpsPerfTestBase {
    ...
    def 'Query complete data trees across all anchors with #scenario.'() {
        ...
        where: 'the following queries are used'
            ...
            'ancestors'                  | '//openroadm-device/ancestor::openroadm-devices'                    || 7500          | 2151 * 5
    ...
    def 'Query with #scenario.'() {
        ...
        where: 'the following queries are used'
            ...
            'omit descendants'             | OMIT_DESCENDANTS        | 'openroadm1' | '//openroadm-device[@ne-state="inservice"]'                             || 100           | 25
            ...
            'ancestors omit descendants'   | OMIT_DESCENDANTS        | 'openroadm4' | '//openroadm-device[@ne-state="inservice"]/ancestor::openroadm-devices' || 100           | 1
            'ancestors direct descendants' | DIRECT_CHILDREN_ONLY    | 'openroadm5' | '//openroadm-device[@ne-state="inservice"]/ancestor::openroadm-devices' || 150           | 26
