...
class NcmpWriteSubJobSpec extends CpsIntegrationSpecBase {
    ...
    def setup() {
        ...
        dmiDispatcher.moduleNamesPerCmHandleId['ch-2'] = ['M2']
        dmiDispatcherForDataJobs.moduleNamesPerCmHandleId['ch-3'] = ['M3']
        registerCmHandle(DMI_URL, 'ch-1', CpsIntegrationSpecBase.NO_MODULE_SET_TAG, '/a/b')
        registerCmHandle(DMI_URL, 'ch-2', CpsIntegrationSpecBase.NO_MODULE_SET_TAG, '/a/b/c')
        registerCmHandle(DMI_DATA_JOB_URL, 'ch-3', CpsIntegrationSpecBase.NO_MODULE_SET_TAG, '/a/b/c/d')
    }
    ...
    def 'Create a sub-job write request.'() {
        ...
        and: 'dmi 1 received the correct job details'
            def receivedSubJobsForDispatcher1 = dmiDispatcher.receivedSubJobs['my-data-job-id']['data'].collect()
