...
class CpsDataServiceLimitsPerfTest extends CpsPerfTestBase {
    ...
    def 'Create 33,000 books (note further tests depend on this running first).'() {
        ...
        then: 'the operation completes within 25 seconds'
            recordAndAssertPerformance("Creating 33,000 books", TimeUnit.SECONDS.toMillis(25), durationInMillis, 200, resourceMeter.getTotalMemoryUsageInMB())
    }
    ...
    def 'Clean up test data.'() {
        ...
        then: 'test data is deleted in 1 second'
            recordAndAssertPerformance("Deleting test data", TimeUnit.SECONDS.toMillis(1), durationInMillis, 200, resourceMeter.getTotalMemoryUsageInMB())
    }
