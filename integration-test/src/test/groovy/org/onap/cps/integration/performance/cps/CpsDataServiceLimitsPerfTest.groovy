...
class CpsDataServiceLimitsPerfTest extends CpsPerfTestBase {
    ...
    def 'Create 40,000 books (note further tests depend on this running first).'() {
        ...
        when: '40,000 books are added'
            stopWatch.start()
            def booksData = '{"books":[' + (1..40_000).collect {'{ "title": "' + it + '" }' }.join(',') + ']}'
        ...
        then: 'the operation completes within 4 minutes'
            recordAndAssertPerformance("Creating 40,000 books", 4 * 60_000, durationInMillis)
    }
