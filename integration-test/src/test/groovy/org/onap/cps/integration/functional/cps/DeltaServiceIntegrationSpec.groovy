...
class DeltaServiceIntegrationSpec extends FunctionalSpecBase {
    ...
    def 'Get delta between anchor and JSON payload'() {
        when: 'attempt to get delta report between anchor and JSON payload'
            def jsonPayload = '{"book-store:bookstore":{"bookstore-name":"Crossword Bookstores"}}'
            def result = objectUnderTest.getDeltaByDataspaceAnchorAndPayload(FUNCTIONAL_TEST_DATASPACE_3, BOOKSTORE_ANCHOR_3, '/bookstore', [:], jsonPayload, NO_GROUPING, JSON)
            result = result.toList().sort { it.xpath }
        then: 'delta report contains a replace action for the bookstore node'
            assert result.any { it.getAction() == 'replace' && it.getXpath() == '/bookstore' }
        and: 'the replace action has expected source and target bookstore name'
            def replaceEntry = result.find { it.getAction() == 'replace' && it.getXpath() == '/bookstore' }
            assert replaceEntry.getSourceData() == ['bookstore': ['bookstore-name': 'Easons-1']]
            assert replaceEntry.getTargetData() == ['bookstore': ['bookstore-name': 'Crossword Bookstores']]
    }
