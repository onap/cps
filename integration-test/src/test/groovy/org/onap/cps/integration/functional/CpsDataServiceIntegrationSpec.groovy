...
class CpsDataServiceIntegrationSpec extends FunctionalSpecBase {
    ...
    //TODO: This test can be removed. Will discuss over a call.
    ...
    def 'Get delta between anchors when leaves of existing data nodes are updated,: #scenario'() {
        ...
        where: 'following data was used'
            ...
            'exiting leaf in source anchor is modified in target anchor' | BOOKSTORE_ANCHOR_3 | BOOKSTORE_ANCHOR_5 | '/bookstore'                                              || ['bookstore-name': 'Easons-1'] | ['bookstore-name': 'Crossword Bookstores']
            'leaf in source anchor is removed in target anchor'          | BOOKSTORE_ANCHOR_3 | BOOKSTORE_ANCHOR_5 | "/bookstore/categories[@code='5']/books[@title='Book 1']" || [price:1]                      | null
            'new leaf is added in target anchor'                         | BOOKSTORE_ANCHOR_5 | BOOKSTORE_ANCHOR_3 | "/bookstore/categories[@code='5']/books[@title='Book 1']" || null                           | [price:1]
    }
    ...
    def 'Get delta between anchors when child data nodes under existing parent data nodes are updated: #scenario'() {
        ...
        where: 'following data was used'
            ...
            'source and target parent nodes have child data nodes'                               | BOOKSTORE_ANCHOR_3 | BOOKSTORE_ANCHOR_5 | '/bookstore/premises' || '/bookstore/premises/addresses[@house-number=\'2\' and @street=\'Main Street\']'
            'source parent data node has child data node, removed in target parent node'         | BOOKSTORE_ANCHOR_5 | BOOKSTORE_ANCHOR_3 | '/bookstore'          || '/bookstore/support-info'
            'source parent data node has no child, target parent data node has a new child node' | BOOKSTORE_ANCHOR_3 | BOOKSTORE_ANCHOR_5 | '/bookstore'          || '/bookstore/support-info'
    }
    ...
    def 'Get delta between anchors where source and target data nodes have leaves and child data nodes'() {
        given: 'parent node xpath and expected data in delta report'
            ...
            def expectedTargetDataInChildNode = [['lang':'English/German'], ['price':200, 'editions':[1988, 2023, 2000]]]
        when: 'attempt to get delta between leaves of existing data nodes'
            def result = objectUnderTest.getDeltaByDataspaceAndAnchors(FUNCTIONAL_TEST_DATASPACE_3, BOOKSTORE_ANCHOR_3, BOOKSTORE_ANCHOR_5, parentNodeXpath, INCLUDE_ALL_DESCENDANTS)
        then: 'expected action is update'
            assert result.first().getAction() == 'update'
        and: 'the payload has expected parent node xpath along with its source and target data'
            def deltaReportEntities = getDeltaReportEntities(result)
            def xpathsInDeltaReport = deltaReportEntities.get('xpaths')
            def sourceDataInDeltaReport = deltaReportEntities.get('sourcePayload')
            def targetDataInDeltaReport = deltaReportEntities.get('targetPayload')
            assert xpathsInDeltaReport.contains(parentNodeXpath)
            assert sourceDataInDeltaReport.contains(expectedSourceDataInParentNode)
            assert targetDataInDeltaReport.contains(expectedTargetDataInParentNode)
        and: 'the delta report also has expected child node xpaths with source and target data from child nodes'
            xpathsInDeltaReport.remove(parentNodeXpath)
            def childNodeXpaths = xpathsInDeltaReport
            assert childNodeXpaths.containsAll(["/bookstore/categories[@code='1']/books[@title='The Gruffalo']", "/bookstore/categories[@code='1']/books[@title='Matilda']"])
            assert sourceDataInDeltaReport.containsAll(expectedSourceDataInChildNode)
            assert targetDataInDeltaReport.containsAll(expectedTargetDataInChildNode)
    }
