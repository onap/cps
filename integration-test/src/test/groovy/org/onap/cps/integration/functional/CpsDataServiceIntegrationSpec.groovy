...
class CpsDataServiceIntegrationSpec extends FunctionalSpecBase {
    ...
    def 'Get delta between 2 anchors for when #scenario'() {
        ...
        and: 'delta report contains expected action'
            assert result[index].getAction() == expectedActions
        and: 'delta report contains expected xpath'
            assert result[index].getXpath() == expectedXpath
        where: 'following data was used'
            scenario            | index || expectedActions | expectedXpath
            'a node is updated' |   0   ||    'update'     | "/bookstore"
            'a node is removed' |   1   ||    'remove'     | "/bookstore-address[@bookstore-name='Easons-1']"
            'a node is added'   |   2   ||     'add'       | "/bookstore-address[@bookstore-name='Crossword Bookstores']"
    }
