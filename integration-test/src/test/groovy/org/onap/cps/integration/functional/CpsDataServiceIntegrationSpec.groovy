...
class CpsDataServiceIntegrationSpec extends FunctionalSpecBase {
    ...
    def 'Get delta between anchors for update operation when source data node is empty #scenario'() {
        when: 'attempt to get delta between existing data nodes'
    ...
    def 'Get delta between anchors for added nodes #scenario'() {
        ...
        where: 'following data was used'
            ...
            'node with only leaves'                      |   26  || "/bookstore/categories[@code='6']"
            'node with leaves and child data node'       |   20  || "/bookstore/categories[@code='1']"
            'node with no leaves and no child data node' |   27  || "/bookstore/container-without-leaves"
