...
class CpsDataServiceIntegrationSpec extends FunctionalSpecBase {
    ...
    def 'Get whole list data' () {
        def matcherXpath = "/bookstore/categories"
        when: 'get data nodes for bookstore container'
            def dataNodes = objectUnderTest.getDataNodes(FUNCTIONAL_TEST_DATASPACE_1, BOOKSTORE_ANCHOR_1, '/bookstore/categories', FetchDescendantsOption.INCLUDE_ALL_DESCENDANTS)
        then: 'the tree consist ouf of #expectNumberOfDataNodes data nodes'
            assert dataNodes.size() == 4
        and: 'each datanode conatains the list node xpath partially in its xpath'
            dataNodes.each {dataNode ->
                def matcher = matcherXpath =~ dataNode.xpath
