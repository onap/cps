...
class CpsDataServiceIntegrationSpec extends FunctionalSpecBase {
    ...
    def 'Get delta returns empty response when #scenario'() {
        when: 'attempt to get delta report'
            def deltaReport = objectUnderTest.getDeltaByDataspaceAndAnchors(FUNCTIONAL_TEST_DATASPACE_3, referenceAnchor, comparandAnchor, '/', INCLUDE_ALL_DESCENDANTS)
    ...
    def 'Get delta between anchors error scenario: #scenario'() {
        when: 'attempt to get delta between anchors'
            objectUnderTest.getDeltaByDataspaceAndAnchors(dataspaceName, referenceAnchor, comparandAnchor, xpath, INCLUDE_ALL_DESCENDANTS)
        then: 'expected exception is thrown'
            thrown(expectedException)
        where: 'following data was used'
                    scenario                               | dataspaceName               | referenceAnchor       | comparandAnchor       | xpath                              | expectedException
    ...
    def 'Get delta between anchors on addition/deletion of leaves of existing data node scenario: #scenario'() {
        when: 'attempt to get delta between leaves of existing data nodes'
            def deltaReport = objectUnderTest.getDeltaByDataspaceAndAnchors(FUNCTIONAL_TEST_DATASPACE_3, referenceAnchor, comparandAnchor, xpath, OMIT_DESCENDANTS)
        then: 'expected action is update'
            assert deltaReport.get(0).getAction() == 'update'
        and: 'the payload has expected values'
            Map<String, Serializable> sourcePayload = deltaReport.get(0).getSourceData() as Map<String, Serializable>
            Map<String, Serializable> targetPayload = deltaReport.get(0).getTargetData() as Map<String, Serializable>
            assert sourcePayload.equals(expectedSourceValue)
            assert targetPayload.equals(expectedTargetValue)
        where: 'following data was used'
            scenario                              | referenceAnchor    | comparandAnchor    | xpath                                                     | expectedSourceValue | expectedTargetValue
    ...
    def 'Get delta between anchors for delete operation #scenario'() {
        ...
        where: 'following data was used'
            ...
            'comparand node is empty'                   | '/bookstore/supportinfo'
    }

    def 'Get delta between anchors for update operation #scenario'() {
        when: 'attempt to get delta between existing data nodes'
            def deltaReport = objectUnderTest.getDeltaByDataspaceAndAnchors(FUNCTIONAL_TEST_DATASPACE_3, referenceAnchor, comparandAnchor, xpath, fetchDescendantsOption)
        ...
        and: 'the payload has expected values'
            ...
            assert targetPayload.equals(expectedTargetValue)
        where: 'following data was used'
            scenario                                                                          | index | xpath                            | referenceAnchor      | comparandAnchor      | expectedSourceValue                     | expectedTargetValue                     | expectedParentNodeXpath                                                          | fetchDescendantsOption  | expectedChildNodeXpaths
    ...
    def 'Get delta between anchors for added nodes #scenario'() {
        when: 'attempt to get delta between leaves of existing data nodes'
            def deltaReport = objectUnderTest.getDeltaByDataspaceAndAnchors(FUNCTIONAL_TEST_DATASPACE_3, referenceAnchor, comparandAnchor, parentNodeXpath, INCLUDE_ALL_DESCENDANTS)
        then: 'the expected action is present in delta report'
            deltaReport.get(index).getAction() == 'add'
        and: 'the delta report has expected number of changes'
            deltaReport.size() == expectedDeltaReportSize
        and: 'the expected payload is present in delta report'
            deltaReport.get(index).getXpath() == expectedChildNodeXpath
        where: 'following data was used'
            scenario                                     | index | referenceAnchor      | comparandAnchor      | expectedDeltaReportSize | parentNodeXpath                    | expectedChildNodeXpath
            'node with only leaves'                      |   1   | EMPTY_BOOKSTORE_DATA | BOOKSTORE_ANCHOR_4   | 32                      | '/bookstore'                       | "/bookstore/categories[@code='6']"
            'node with leaves and child data node'       |   2   | EMPTY_BOOKSTORE_DATA | BOOKSTORE_ANCHOR_4   | 32                      | '/bookstore'                       | "/bookstore/categories[@code='1']"
            'node with no leaves and no child data node' |   19  | EMPTY_BOOKSTORE_DATA | BOOKSTORE_ANCHOR_4   | 32                      | '/bookstore'                       | "/bookstore/container-without-leaves"
            'node with only child data node'             |   25  | EMPTY_BOOKSTORE_DATA | BOOKSTORE_ANCHOR_4   | 32                      | '/bookstore'                       | "/bookstore/premises/addresses[@house-number='2' and @street='Main Street']"
            'empty reference nodes'                      |   0   | BOOKSTORE_ANCHOR_3   | BOOKSTORE_ANCHOR_4   | 1                       | "/bookstore/categories[@code='6']" | "/bookstore/categories[@code='6']"
    }
