...
class CpsDataServiceIntegrationSpec extends FunctionalSpecBase {
    ...
    def 'Attempt to create a top level datanode using root.'() {
        given: 'a new anchor'
    ...
    def 'Delete data nodes with non-existing datanode(s).'() {
    ...
    //TODO add  multiple lists (successful)
    ...
    def 'Update multiple data node leaves.'() {
        ...
        when: 'update is performed for leaves'
            objectUnderTest.updateNodeLeaves(FUNCTIONAL_TEST_DATASPACE_1, BOOKSTORE_ANCHOR_FOR_PATCH, "/bookstore/categories[@code='1']", jsonData, now)
        then: 'the updated data nodes are retrieved'
            def result = cpsDataService.getDataNodes(FUNCTIONAL_TEST_DATASPACE_1, BOOKSTORE_ANCHOR_FOR_PATCH, "/bookstore/categories[@code=1]/books[@title='Matilda']", INCLUDE_ALL_DESCENDANTS)
