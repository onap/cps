...
class CpsDataServiceIntegrationSpec extends FunctionalSpecBase {
    ...
    def 'Get delta between 2 anchors for #scenario'() {
        when: 'attempt to get delta report'
            def deltaReport = objectUnderTest.getDeltaByDataspaceAndAnchors(FUNCTIONAL_TEST_DATASPACE_3, BOOKSTORE_ANCHOR_3, BOOKSTORE_ANCHOR_5, xpath, fetchDescendantOption)
        ...
        where: 'following data was used'
            ...
            'a node is added'   |   1   | '/'   ||     'add'       || "/bookstore-address[@bookstore-name='My New Bookstore']" | OMIT_DESCENDANTS
    }

    def 'Get delta between 2 anchors for nested changes'() {
        def parentNodeXpath = "/bookstore"
        when: 'attempt to get delta report'
            def deltaReport = objectUnderTest.getDeltaByDataspaceAndAnchors(FUNCTIONAL_TEST_DATASPACE_3, BOOKSTORE_ANCHOR_3, BOOKSTORE_ANCHOR_5, parentNodeXpath, INCLUDE_ALL_DESCENDANTS)
        then: 'delta report contains expected number of changes'
            deltaReport.size() == 11
    ...
    def 'Get delta between anchors for remove action #scenario'() {
        when: 'attempt to get delta between leaves of existing data nodes'
            def deltaReport = objectUnderTest.getDeltaByDataspaceAndAnchors(FUNCTIONAL_TEST_DATASPACE_3, BOOKSTORE_ANCHOR_5, BOOKSTORE_ANCHOR_3, parentNodeXpath, INCLUDE_ALL_DESCENDANTS)
        then: 'expected action is present in delta report'
            assert deltaReport.get(0).getAction() == 'remove'
        where: 'following data was used'
            scenario                                       | parentNodeXpath
            'source data node with leaves and child nodes' | "/bookstore/categories[@code='6']"
            ...
            'source data node is empty'                    | "/bookstore/container-without-leaves"
    }

    def 'Get delta between anchors for add action #scenario'() {
        when: 'attempt to get delta between leaves of existing data nodes'
            def deltaReport = objectUnderTest.getDeltaByDataspaceAndAnchors(FUNCTIONAL_TEST_DATASPACE_3, BOOKSTORE_ANCHOR_3, BOOKSTORE_ANCHOR_5, parentNodeXpath, INCLUDE_ALL_DESCENDANTS)
        ...
        and: 'the expected xapth is present in delta report'
            deltaReport.get(0).getXpath() == parentNodeXpath
        where: 'following data was used'
            scenario                                       | parentNodeXpath
            'target data node with leaves and child nodes' | "/bookstore/categories[@code='6']"
            'target data node has only leaves'             | "/bookstore/categories[@code='5']/books[@title='Book 11']"
            'target data node has child data node only'    | "/bookstore/supportinfo/contact-emails"
            'target data node is empty'                    | "/bookstore/container-without-leaves"
    }
