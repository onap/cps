...
class CpsDataServiceIntegrationSpec extends FunctionalSpecBase {
    ...
    def 'Read bookstore top-level container(s) using "root"  variations.'() {
        when: 'get data nodes for bookstore container'
            def result = objectUnderTest.getDataNodes(TEST_DATASPACE_1, TEST_ANCHOR_1, root, OMIT_DESCENDANTS)
    ...
    def countDataNodesInParentlist() {
        return countDataNodesInTree(objectUnderTest.getDataNodes(TEST_DATASPACE_1, TEST_ANCHOR_1, '/', FetchDescendantsOption.INCLUDE_ALL_DESCENDANTS))
    }
