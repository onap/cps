...
For testing purposes with consistent DMI plugin naming, use the 'test' release name:
```bash
helm install test ./cps-charts/
```
...
Example:
```bash
helm install cps cps-charts/ --set cps.replicas=1
```
---
## Chart Components
This Helm chart deploys the following components:
- postgresql: CPS database
- cps-and-ncmp: CPS and NCMP backend services (2 replicas)
