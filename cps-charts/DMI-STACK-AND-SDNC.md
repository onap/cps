...
This stack replicates the environment previously defined in `docker-compose/dmi-services.yml` (removed from the repository) as native Kubernetes resources, reproducing the environment used by the (now disabled) CSIT `dmi-integration` tests:
...
| SDNC cannot reach its database | Startup scripts hardcode `dbhost`, `sdnctldb01`, `sdnctldb02` (originally docker-compose `links:` aliases), which has no K8s equivalent | Chart appends these aliases to `/etc/hosts` at startup — see `templates/onap-dmi-stack-sdnc-deployment.yaml` |
...
- SDNC mount script: `config/sdnc/check_sdnc_mount_node.sh` (ported from the removed `docker-compose` setup into this chart as a ConfigMap + Job)
