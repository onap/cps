...
The Helm chart includes optional monitoring and tracing components, all **disabled by default**:

| Component  | Purpose                                       | Default  |
|------------|-----------------------------------------------|----------|
| Prometheus | Metrics collection from CPS and PostgreSQL    | Disabled |
| Grafana    | Dashboards for JVM, database, REST, inventory | Disabled |
...
### Enable all monitoring and tracing
...
### Enable only Prometheus and Grafana (no tracing)
...
### CPS only (no monitoring or tracing)
...
| Prometheus targets showing "DOWN" | Wrong service name in scrape config | Verify `kubectl get svc` matches `config/prometheus.yaml` targets |
