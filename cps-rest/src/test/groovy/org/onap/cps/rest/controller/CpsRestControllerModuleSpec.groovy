...
class CpsRestControllerModuleSpec extends Specification {
    ...
    def 'Create schema set from file with bad name'() {
        given: 'File with unexpected extension in name'
           def multipartFile = createMultipertFile("filename.doc", "content")
        when:
    ...
    def 'Create schema set from empty file with valid name'() {
        given:
            def multipartFile = createMultipertFile("filename.yang", "")
        when:
            def response = performCreateSchemaSetRequest(multipartFile)
        then:
            response.status == HttpStatus.BAD_REQUEST.value()
    ...
    def 'Create schema set from yang file'() {
        ...
        given:
           def multipartFile = createMultipertFile("filename.yang", "content")
        when:
            def response = performCreateSchemaSetRequest(multipartFile)
    ...
    def createMultipertFile(filename, content){
