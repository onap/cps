...
class MdcServletFilterSpec extends Specification {
    ...
    def 'should set MDC from header if present'(){
        given:
            ...
            request.getHeader("X-ONAP-RequestID") >> "test-request-id"
            request.getHeader("X-ClientID") >> "test-client-id"
        when:
            filter.doFilter(request, response, chain)
        then:
            chain.doFilter(request, response)
            MDC.get("X-ONAP-RequestID") == null
            MDC.get("X-ClientID") == null
    ...
    def 'should generate UUIDs if Headers are missing'(){
        given:
            ...
            def response = Mock(ServletResponse)
            def chain = Mock(FilterChain)
            request.getHeader("X-ONAP-RequestID") >> null
            request.getHeader("X-ClientID") >> ""
        when:
            filter.doFilter(request, response, chain)
        then:
            chain.doFilter(request, response)
