...
class MultipartFileUtilSpec extends Specification {
    ...
    @Unroll
    def 'Extract yang resource from invalid multipart file (#description)'() {
        given:
            def multipartFile = new MockMultipartFile("file", filename, "text/plain", content.getBytes())
        when:
        ...
        where:
            ...
            'filename.doc'  | 'content' | 'invalid filename extension'
            'filename.yang' | ''        | 'empty file'
    }
