    ...
    def 'Query data nodes with composite query payload for given dataspace and anchor with #scenario.'() {
        ...
        when: 'the search query API is invoked with the JSON body'
            def response = mvc.perform(post(searchEndpoint)
                    ...
                    .content(searchBody))
                    .andReturn().response
        then: 'the call is delegated to the cps facade executeCompositeQuery which returns a list with one data node map'
            1 * mockCpsFacade.executeCompositeQuery('my_dataspace', 'my_anchor', searchBody,
