...
class QueryRestControllerSpec extends Specification {
    ...
    def 'Query data node v2 API by cps path for the given dataspace and anchor with #scenario and media type JSON'() {
        ...
        then: 'the response contains the datanode in the expected JSON format'
            ...
            assert response.getContentAsString().contains(expectedJsonResponseContent)
        where: 'the following options for include descendants are provided in the request'
            scenario          | includeDescendantsOptionString || expectedDepth || expectedJsonResponseContent
            'direct children' | 'direct'                       || 1             || '{"xpath":{"leaf":"value","leafList":["leaveListElement1","leaveListElement2"]}}'
            'descendants'     | '2'                            || 2             || '{"xpath":{"leaf":"value","leafList":["leaveListElement1","leaveListElement2"]}}'
    }
    ...
    def 'Query data node v2 API by cps path for the given dataspace and anchor with #scenario and media type XML'() {
        ...
        then: 'the response contains the datanode in the expected XML format'
            assert response.status == HttpStatus.OK.value()
            assert response.getContentAsString().contains(expectedXmlResponseContent)
        where: 'the following options for include descendants are provided in the request'
            scenario          | includeDescendantsOptionString || expectedDepth || expectedXmlResponseContent
            'direct children' | 'direct'                       || 1             || '<xpath><leaf>value</leaf><leafList>leaveListElement1</leafList><leafList>leaveListElement2</leafList></xpath>'
            'descendants'     | '2'                            || 2             || '<xpath><leaf>value</leaf><leafList>leaveListElement1</leafList><leafList>leaveListElement2</leafList></xpath>'
    }
