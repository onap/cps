   ...
   def 'Query data node v2 api by cps path for the given dataspace and anchor with #scenario.'() {
        given: 'service method returns a list containing a data node'
            ...
            mockCpsQueryService.queryDataNodes(dataspaceName, anchorName, cpsPath, { descendantsOption -> {
                    assert descendantsOption.depth == expectedDescendantsOptionDepth}}) >> [dataNode1, dataNode1]
        when: 'query data nodes API is invoked'
            def response =
                mvc.perform(
        ...
        where: 'the following options for include descendants are provided in the request'
            scenario                               | descendants || expectedDescendantsOptionDepth
            'no descendants by default'            | ''          || 0
            'no descendants explicitly using none' | 'none'      || 0
            'no descendants explicitly using 0'    | '0'         || 0
            'all descendants with string'          | 'all'       || -1
            'all descendants with number'          | '-1'        || -1
            'descendants with depth 1'             | '1'         || 1
            'descendants with depth 2'             | '2'         || 2
    }
