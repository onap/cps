    ...
    def 'Get request with no data found CPS exception returns HTTP Status Not Found'() {
        ...
        then: 'an HTTP Not Found response is returned with correct message and details'
            assertTestResponse(response, NOT_FOUND, 'Object not found',
                    'Description does not exist in namespace MyDataSpace.')
    }

    def 'Get request with a schema set already defined exception returns HTTP Status Bad Request'() {

        when: 'no data found CPS exception is thrown by the service'
            def dataspaceName = 'MyDataSpace'
            def schemaSetName = 'MySchemaSet'
            setupTestException(new SchemaSetAlreadyDefinedException(dataspaceName, schemaSetName,
                    null))
            def response = performTestRequest()

        then: 'an HTTP Bad Request response is returned with correct message an details'
            assertTestResponse(response, BAD_REQUEST, 'Duplicate Schema Set',
                    'Schema Set with name MySchemaSet already exists for dataspace' +
                            ' MyDataSpace.')
    }

    def 'Get request with a validation exceptions returns HTTP Status Bad Request'() {
        ...
        where: 'the following exceptions are thrown'
            e << [new ModelValidationException(errorMessage, errorDetails, null),
