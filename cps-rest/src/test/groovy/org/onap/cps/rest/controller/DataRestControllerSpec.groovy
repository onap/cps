...
class DataRestControllerSpec extends Specification {
    ...
    def 'Update data node leaves: #scenario.'() {
        ...
        then: 'the service method is invoked with expected parameters'
            1 * mockCpsDataService.updateNodeLeaves(dataspaceName, anchorName, xpathServiceParameter, expectedData,  noTimestamp, expectedContentType)
        and: 'response status indicates success'
            response.status == HttpStatus.OK.value()
        where:
            scenario                             | inputXpath    | contentType                | expectedContentType || xpathServiceParameter | requestBody     | expectedData
            'JSON content: root node by default' | ''            | MediaType.APPLICATION_JSON | ContentType.JSON    || '/'                   | requestBodyJson | expectedJsonData
            'JSON content: root node by choice'  | '/'           | MediaType.APPLICATION_JSON | ContentType.JSON    || '/'                   | requestBodyJson | expectedJsonData
            'JSON content: some xpath by parent' | '/some/xpath' | MediaType.APPLICATION_JSON | ContentType.JSON    || '/some/xpath'         | requestBodyJson | expectedJsonData
            'XML content: root node by default'  | ''            | MediaType.APPLICATION_XML  | ContentType.XML     || '/'                   | requestBodyXml  | expectedXmlData
            'XML content: root node by choice'   | '/'           | MediaType.APPLICATION_XML  | ContentType.XML     || '/'                   | requestBodyXml  | expectedXmlData
            'XML content: some xpath by parent'  | '/some/xpath' | MediaType.APPLICATION_XML  | ContentType.XML     || '/some/xpath'         | requestBodyXml  | expectedXmlData
    }
    ...
    def 'Update data node leaves with observedTimestamp'() {
        ...
        then: 'the service method is invoked with expected parameters'
            expectedApiCount * mockCpsDataService.updateNodeLeaves(dataspaceName, anchorName, '/', expectedData,
                { it == DateTimeUtility.toOffsetDateTime(observedTimestamp) }, expectedContentType)
        and: 'response status indicates success'
            response.status == expectedHttpStatus.value()
        where:
            scenario                                        | contentType                | expectedContentType | requestBody     | observedTimestamp              || expectedApiCount | expectedHttpStatus     | expectedData
            'JSON content: with observed-timestamp'         | MediaType.APPLICATION_JSON | ContentType.JSON    | requestBodyJson | '2021-03-03T23:59:59.999-0400' || 1                | HttpStatus.OK          | expectedJsonData
            'JSON content: with invalid observed-timestamp' | MediaType.APPLICATION_JSON | ContentType.JSON    | requestBodyJson | 'invalid'                      || 0                | HttpStatus.BAD_REQUEST | expectedJsonData
            'XML content: with observed-timestamp'          | MediaType.APPLICATION_XML  | ContentType.XML     | requestBodyXml  | '2021-03-03T23:59:59.999-0400' || 1                | HttpStatus.OK          | expectedXmlData
            'XML content: with invalid observed-timestamp'  | MediaType.APPLICATION_XML  | ContentType.XML     | requestBodyXml  | 'invalid'                      || 0                | HttpStatus.BAD_REQUEST | expectedXmlData
    }
