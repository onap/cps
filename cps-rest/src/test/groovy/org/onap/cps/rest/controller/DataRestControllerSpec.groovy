    ...
    def 'Get data node using v2 api with #scenario.'() {
        given: 'the service returns data node with #scenario'
            ...
            mockCpsDataService.getDataNode(dataspaceName, anchorName, xpath, {descendantsOption -> {
                assert descendantsOption.depth == expectedDescendantsOptionDepth}}) >> dataNode
        when: 'get request is performed through REST API'
            def response =
                mvc.perform(
                        get(endpoint)
        ...
        and: 'the response contains the root node identifier: #expectedRootidentifier'
            assert response.contentAsString.contains(expectedRootidentifier)
        and: 'the response contains child is #expectChildInResponse'
            assert response.contentAsString.contains('"child"') == expectChildInResponse
        where:
            scenario                    | dataNode                     | descendants || expectedDescendantsOptionDepth | expectChildInResponse   | expectedRootidentifier
            'no descendants by default' | dataNodeWithLeavesNoChildren | ''          || 0                              | false                   | 'xpath'
            'no descendant explicitly'  | dataNodeWithLeavesNoChildren | 'none'      || 0                              | false                   | 'xpath'
            'with all descendants'      | dataNodeWithChild            | 'all'       || -1                             | true                    | 'parent'
            'with one descendants'      | dataNodeWithChild            | '1'         || 1                              | true                    | 'parent'
    }
