...
class DataRestControllerSpec extends Specification {
    ...
    def 'Get delta between two anchors using #fetchDescendantsOption'() {
        given: 'the service returns a list containing delta'
            DeltaReport deltaReport = new DeltaReportBuilder().withXpath('/bookstore').withSourceData('bookstore-name': 'Easons').withTargetData('bookstore-name': 'Easons').build()
        ...
        and: 'the response contains expected value'
            assert response.contentAsString.contains("[{\"action\":null,\"xpath\":\"/bookstore\",\"sourceData\":{\"bookstore-name\":\"Easons\"},\"targetData\":{\"bookstore-name\":\"Easons\"}}]")
    }
