...
class DataRestControllerSpec extends Specification {
    ...
    def 'buildResponseEntity returns XML response'() {
        given: 'a list of dataMaps and XML content type'
            def dataMaps = [["parent-2": ["leaf": "value"]]]
        and: 'XmlFileUtils returns correct XML response'
            xmlFileUtils.convertDataMapsToXml(dataMaps) >> '<parent-2><leaf>value</leaf></parent-2>'
        when: 'perform buildResponseEntity with XML content type'
            def response = DataRestController.buildResponseEntity(dataMaps)
        then: 'a success response is returned with XML format'
            response.statusCode == HttpStatus.OK
        and: 'the response contains the datanode in XML format'
            def responseXml = response.getBody().toString()
            responseXml == '<parent-2><leaf>value</leaf></parent-2>'
    }
