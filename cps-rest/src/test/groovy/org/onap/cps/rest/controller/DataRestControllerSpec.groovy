...
class DataRestControllerSpec extends Specification {
    ...
    def 'Get all the data trees as XML with root node xPath using V2'() {
        ...
        and: 'the response contains the datanode in XML format'
            def xmlResponse = response.getContentAsString()
            xmlResponse.contains('<parent-1>')
            xmlResponse.contains('<leaf>value</leaf>')
            xmlResponse.contains('<leafList>leaveListElement1</leafList>')
            xmlResponse.contains('<leafList>leaveListElement2</leafList>')
            xmlResponse.contains('<parent-2>')
            xmlResponse.contains('<leaf>value</leaf>')
        and: 'the XML response contains expected number of data trees'
            def numberOfDataTrees = xmlResponse.split('<parent-').size() - 1
            assert numberOfDataTrees == 2
    }
