        ...
        when: 'put request is performed'
            def response =
                mvc.perform(
                    ...
                    .andReturn().response
        then: 'the cps data service method is invoked with expected parameters'
            1 * mockCpsDataService.updateDataNodeAndDescendants(dataspaceName, anchorName, xpathServiceParameter, expectedJsonData, noTimestamp, ContentType.JSON) >> hasNewNodes
        and: 'response status indicates success or creation'
            response.status == expectedStatus
        where:
            scenario                                      | inputXpath || xpathServiceParameter | hasNewNodes || expectedStatus
