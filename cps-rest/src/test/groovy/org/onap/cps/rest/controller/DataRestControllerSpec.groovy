...
class DataRestControllerSpec extends Specification {
        ...
        when: 'put request is performed'
            def response =
                mvc.perform(
                    ...
                    .andReturn().response
        then: 'the cps data service method is invoked with expected parameters'
            1 * mockCpsDataService.updateDataNodeAndDescendants(dataspaceName, anchorName, xpathServiceParameter, expectedData, noTimestamp, expectedContentType) >> false
        and: 'response status indicates success'
            response.status == HttpStatus.OK.value()
        where:
            scenario                             | inputXpath    | contentType                || xpathServiceParameter | requestBody     | expectedData     | expectedContentType
    ...
    def 'Replace data node tree on an anchor with no existing data.'() {
        ...
        and: 'the cps data service reports that the data was newly created'
            mockCpsDataService.updateDataNodeAndDescendants(dataspaceName, anchorName, '/', expectedJsonData, noTimestamp, ContentType.JSON) >> true
