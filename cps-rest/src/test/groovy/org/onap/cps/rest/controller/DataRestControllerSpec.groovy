...
class DataRestControllerSpec extends Specification {
    ...
    def 'Replace data node tree hasNewNodes: #scenario.'() {
        given: 'endpoint to replace node'
        ...
        when: 'put request is performed'
            def response =
                mvc.perform(
                    ...
                    .andReturn().response
        then: 'the cps data service method is invoked with expected parameters'
            1 * mockCpsDataService.updateDataNodeAndDescendants(dataspaceName, anchorName, xpathServiceParameter, expectedData, noTimestamp, expectedContentType) >> hasNewNodes
        and: 'response status indicates success or creation'
            response.status == expectedStatus
        where:
            scenario                                      | inputXpath | contentType                || xpathServiceParameter | requestBody     | expectedData     | expectedContentType | hasNewNodes || expectedStatus
            'JSON content: root node updated only'        | ''         | MediaType.APPLICATION_JSON || '/'                   | requestBodyJson | expectedJsonData | ContentType.JSON    | false       || HttpStatus.OK.value()
            'JSON content: root node with new list items' | ''         | MediaType.APPLICATION_JSON || '/'                   | requestBodyJson | expectedJsonData | ContentType.JSON    | true        || HttpStatus.CREATED.value()
    }
