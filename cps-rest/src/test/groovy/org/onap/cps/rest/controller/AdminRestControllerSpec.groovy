...
class AdminRestControllerSpec extends Specification {
    ...
    def static schemaSetNames = ["my-schema-set1","my-schema-set2"].join(',')
    def static pageIndex = 1
    def static pageSize = 4
    def negativePageIndex  = -1
    def negativePageSize = -1
    ...
    def 'Get #scenario in a dataspace.'() {
        given: 'service method returns a list of anchors based on request parameters'
            mockCpsAnchorService.getAnchors(dataspaceName, requestedSchemaSetNames, requestedPageIndex, requestedPageSize) >> [anchor]
        and: 'the endpoint for getting anchors'
            def anchorEndpoint = "$basePath/v1/dataspaces/$dataspaceName/anchors"
            def requestBuilder = get(anchorEndpoint)
            if (requestedSchemaSetNames != null) {
                requestBuilder.param('schema-set-names', requestedSchemaSetNames)
            }
            if (requestedPageIndex != null && requestedPageSize != null) {
                requestBuilder.param('pageIndex', requestedPageIndex.toString())
                requestBuilder.param('pageSize', requestedPageSize.toString())
            }
        ...
        where: 'following scenarios are tested'
            scenario                                        | requestedSchemaSetNames | requestedPageIndex | requestedPageSize
            'all anchors'                                   | null                    | null               | null
            'anchors with pagination'                       | null                    | pageIndex          | pageSize
            'anchors by schema set name without pagination' | schemaSetNames          | null               | null
            'anchors by schema set name and pagination'     | schemaSetNames          | pageIndex          | pageSize
    }

    def 'Attempt to get anchors using unsupported pagination values.'() {
        given: 'the service throws Pagination related Exception for invalid pagination values'
            mockCpsAnchorService.getAnchors(dataspaceName, null, negativePageIndex, negativePageSize) >> {
        ...
        and: 'the endpoint is called with a negative page parameter'
            def requestParams = new LinkedMultiValueMap<>()
            requestParams.add('pageIndex', negativePageIndex.toString())
            requestParams.add('pageSize', negativePageSize.toString())
            def anchorEndpoint = "$basePath/v1/dataspaces/$dataspaceName/anchors"
        when: 'get anchors API is invoked'
