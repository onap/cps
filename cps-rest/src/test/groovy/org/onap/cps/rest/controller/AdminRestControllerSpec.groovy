...
class AdminRestControllerSpec extends Specification {
    ...
    def 'Get all schema sets from a dataspace.'() {
        given: 'service method returns all schema sets from a dataspace'
            def schemaSetList = new ArrayList<SchemaSet>();
            def schemaSet = new SchemaSet(name: schemaSetName, dataspaceName: dataspaceName)
            schemaSetList.add(schemaSet)
            mockCpsModuleService.getSchemaSets(dataspaceName) >> schemaSetList
        and: 'an endpoint'
            def schemaSetEndpoint = "$basePath/v1/dataspaces/$dataspaceName/schema-sets"
