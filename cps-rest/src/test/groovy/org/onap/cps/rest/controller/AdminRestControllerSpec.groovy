...
class AdminRestControllerSpec extends Specification {
    ...
    def 'Create dataspace over existing with same name'() {
        given:
            def expectedException = new DataspaceAlreadyDefinedException("", new RuntimeException())
            mockCpsAdminService.createDataspace("existing-dataspace") >> { throw expectedException }
        when:
