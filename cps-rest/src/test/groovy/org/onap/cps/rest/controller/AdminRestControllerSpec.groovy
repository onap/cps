...
class AdminRestControllerSpec extends Specification {
    ...
    def static sampleSchemaSetNames = ["my-schema-set1","my-schema-set2"]
    ...
    def 'Get #scenario from a dataspace.'() {
        given: 'service method returns a list of anchors based on request parameters'
            mockCpsAnchorService.getAnchors(dataspaceName, schemaSetNames, pageIndex, pageSize) >> [anchor]
        when: 'get anchors is invoked'
            def response = mvc.perform(buildAnchorsRequest(
                "$basePath/v1/dataspaces/$dataspaceName/anchors", schemaSetNames, pageIndex, pageSize)).andReturn().response
        then: 'the correct anchor is returned'
            response.status == HttpStatus.OK.value()
            response.getContentAsString().contains(anchorName)
        where: 'following scenarios are tested'
            scenario                                        | schemaSetNames       | pageIndex | pageSize
            'all anchors'                                   | null                 | null      | null
            'anchors with pagination'                       | null                 | 1         | 4
            'anchors by schema set name without pagination' | sampleSchemaSetNames | null      | null
            'anchors by schema set name and pagination'     | sampleSchemaSetNames | 1         | 4
    }

    def 'Attempt to get anchors using unsupported pagination values.'() {
        given: 'the service throws expected exception for invalid pagination values'
            mockCpsAnchorService.getAnchors(dataspaceName, null, -1, -1) >> {
                throw new DataValidationException('Pagination validation error.', 'Invalid page index or size')
            }
        and: 'the endpoint to get all anchors.'
    ...
    def buildAnchorsRequest(endpoint, schemaSetNames, pageIndex, pageSize) {
        def requestBuilder = get(endpoint)
        if (schemaSetNames != null)
            requestBuilder.param('schema-set-names', sampleSchemaSetNames.join(','))
        if (pageIndex != null && pageSize != null) {
            requestBuilder.param('pageIndex', pageIndex.toString())
