...
class AdminRestControllerSpec extends Specification {
    ...
    def 'when a createAnchor API call is made then the response has status 200 '() {
        given:
        ...
        expect: 'Status is 200 and the response is the name of the created anchor -> my_anchor'
    ...
    def 'when get all anchors for a dataspace is performed, then the response has status 200 '() {
        given:
