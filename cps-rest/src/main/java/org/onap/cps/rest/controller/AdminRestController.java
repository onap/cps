    ...
    public ResponseEntity<List<AnchorDetails>> getAnchors(final String apiVersion,
        ...
        if (StringUtils.isBlank(schemaSetName)) {
            ...
            if (paginationOption == PaginationOption.NO_PAGINATION) {
                anchors = cpsAnchorService.getAnchors(dataspaceName);
            } else {
                anchors = cpsAnchorService.getAnchors(dataspaceName, paginationOption);
            }
            anchorDetails = anchors.stream().map(cpsRestInputMapper::toAnchorDetails).collect(Collectors.toList());
        } else {
            final Collection<Anchor> anchors;
            if (paginationOption == PaginationOption.NO_PAGINATION) {
                anchors = cpsAnchorService.getAnchorsBySchemaSetName(dataspaceName, schemaSetName);
            } else {
                anchors = cpsAnchorService.getAnchorsBySchemaSetName(dataspaceName, schemaSetName, paginationOption);
            }
            anchorDetails = anchors.stream().map(cpsRestInputMapper::toAnchorDetails).collect(Collectors.toList());
        }
        return new ResponseEntity<>(anchorDetails, HttpStatus.OK);
