...
public class AdminRestController implements CpsAdminApi {
    ...
    public ResponseEntity<List<AnchorDetails>> getAnchors(final String dataspaceName) {
        ...
        final List<AnchorDetails> ads = new ArrayList<>();

        anchorDetails.stream().forEach(a -> {
            final AnchorDetails ad = new AnchorDetails();
            ad.setName(a.getName());
            ad.setDataspaceName(a.getDataspaceName());
            ad.setSchemaSetName(a.getSchemaSetName());
            ads.add(ad);
        });
