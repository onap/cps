...
public class DataRestController implements CpsDataApi {
    ...
    public ResponseEntity<Object> getDeltaByDataspaceAnchorAndPayload(final String dataspaceName,
        ...
        final Collection<DeltaReport> deltareports;
        if (multipartFile == null) {
            deltareports = cpsDataService.getDeltaByDataspaceAnchorAndPayload(dataspaceName,
                    sourceAnchorName, xpath, Collections.emptyMap(), jsonPayload.toString(), fetchDescendantsOption);

        } else {
            deltareports = cpsDataService.getDeltaByDataspaceAnchorAndPayload(dataspaceName, sourceAnchorName,
                    xpath, extractYangResourcesMap(multipartFile), jsonPayload.toString(), fetchDescendantsOption);
        }
        deltaReport.addAll(deltareports);
