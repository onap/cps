...
public class CpsDataServiceImpl implements CpsDataService {
    ...
    public List<DeltaReport> getDeltaByDataspaceAnchorAndPayload(final String dataspaceName,
        ...
        if (sourceDataNodes != null) {
            sourceDataNodesRebuilt.addAll(rebuildDataNodes(sourceAnchor, xpath, sourceDataNodes));
        }

        if (!yangResourcesNameToContentMap.isEmpty()) {
            targetDataNodes.addAll(
                    buildDataNodesWithXpath(yangResourcesNameToContentMap, xpath, jsonPayload, ContentType.JSON));
        } else {
            targetDataNodes.addAll(buildDataNodesWithXpath(sourceAnchor, xpath, jsonPayload, ContentType.JSON));
        }
    ...
    private Collection<DataNode> rebuildDataNodes(final Anchor anchor, final String xpath,
        ...
        final String dataNodesAsJsonString = jsonObjectMapper.asJsonString(mergedDataMap);
        return buildDataNodesWithXpath(anchor, xpath, dataNodesAsJsonString, ContentType.JSON);
    }

    private Collection<DataNode> buildDataNodesWithXpath(final Anchor anchor, final String xpath,
                                                         final String jsonPayload,
                                                         final ContentType contentType) {
        ...
        if (!isRootNodeXpath(xpath)) {
            ...
            if (parentNodeXpath.isEmpty()) {
                return buildDataNodes(anchor, ROOT_NODE_XPATH, jsonPayload, contentType);
            }
            return buildDataNodes(anchor, parentNodeXpath, jsonPayload, contentType);
        }
        return buildDataNodes(anchor, xpath, jsonPayload, contentType);
    }

    private Collection<DataNode> buildDataNodesWithXpath(final Map<String, String> yangResourcesNameToContentMap,
                                                         final String xpath, final String jsonPayload,
                                                         final ContentType contentType) {
        if (!isRootNodeXpath(xpath)) {
            final String parentNodeXpath = CpsPathUtil.getNormalizedParentXpath(xpath);
            if (parentNodeXpath.isEmpty()) {
                return buildDataNodes(yangResourcesNameToContentMap, ROOT_NODE_XPATH, jsonPayload, contentType);
            }
            return buildDataNodes(yangResourcesNameToContentMap, parentNodeXpath, jsonPayload, contentType);
        }
        return buildDataNodes(yangResourcesNameToContentMap, xpath, jsonPayload, contentType);
    }
