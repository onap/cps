...
public class CpsDataServiceImpl implements CpsDataService {
    ...
    public void updateNodeLeaves(final String dataspaceName, final String anchorName, final String parentNodeXpath,
        ...
        cpsValidator.validateNameCharacters(dataspaceName, anchorName);
        final Collection<DataNode> dataNodes = buildDataNodes(dataspaceName, anchorName, parentNodeXpath, jsonData,
                ContentType.JSON);
        if (dataNodes.size() > 1) {
            throw new DataValidationException("Operation is not supported for multiple data nodes",
                    "Number of data nodes present: " + dataNodes.size());
        }
        cpsDataPersistenceService.updateDataLeaves(dataspaceName, anchorName, dataNodes.iterator().next().getXpath(),
                dataNodes.iterator().next().getLeaves());
        processDataUpdatedEventAsync(dataspaceName, anchorName, parentNodeXpath, UPDATE, observedTimestamp);
    }
