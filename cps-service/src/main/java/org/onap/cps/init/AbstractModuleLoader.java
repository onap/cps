...
abstract class AbstractModuleLoader implements ModuleLoader {
    ...
    void createSchemaSet(final String dataspaceName, final String schemaSetName, final String... resourceNames) {
        try {
            final Map<String, String> yangResourcesContentMap = mapYangResourcesToContent(resourceNames);
    ...
    void createInitialSubscription(final String dataspaceName, final String anchorName) {
        final Collection<Dataspace> dataSpaces = cpsDataspaceService.getAllDataspaces();
        Optional.ofNullable(dataSpaces).ifPresent(dsList -> dsList.forEach(this::subscribeNotificationForDataspace));
    }
