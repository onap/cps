 ...
 * created per query, so instances are never shared between threads and the state does not need to be passed
 * from method to method.
 */
public class CompositeQueryExecution {
    ...
    /**
     * Validates the composite query and executes it against the given dataspace and anchor.
     * The query is first validated in a single upfront pass so that invalid input fails fast before any
     * database access. A new {@link CompositeQueryExecution} then carries the per-query state through the
     * three phases of the algorithm: fetch the candidate trees, compute the selected xpaths and build the
     * filtered result trees.
     *
     * @param cpsDataPersistenceService the persistence service used to fetch candidate and condition trees
     ...
     */
    public static Collection<DataNode> processCompositeQuery(
            final CpsDataPersistenceService cpsDataPersistenceService,
            final String dataspaceName,
            final String anchorName,
            final CompositeQuery compositeQuery,
            final FetchDescendantsOption fetchDescendantsOption) {
        CompositeQueryValidator.validateCompositeQuery(compositeQuery);
        final CompositeQueryExecution compositeQueryExecution = new CompositeQueryExecution(
            cpsDataPersistenceService, dataspaceName, anchorName);
    ...
    private Set<String> computeMatchedXpaths(final CompositeQuery compositeQuery,
        ...
        final CompositeQueryOperator operator = toCompositeQueryOperator(compositeQuery.getOperator());
        return CompositeQueryEvaluator.evaluate(operator, compositeQueryConditions, candidateXpaths,
    ...
    private Set<String> evaluateCondition(final CompositeQuery compositeQueryCondition,
        ...
        if (nestedMatches.isEmpty()) {
            return switch (toCompositeQueryOperator(compositeQueryCondition.getOperator())) {
    ...
    private Set<String> getXpathsMatchingConditionAcrossAnchor(final String conditionCpsPath) {
        final Set<String> cachedXpaths = xpathsPerConditionCpsPath.get(conditionCpsPath);
        if (cachedXpaths != null) {
            return cachedXpaths;
        }
        final List<DataNode> conditionDataNodes = cpsDataPersistenceService.queryDataNodes(
            dataspaceName, anchorName, conditionCpsPath, OMIT_DESCENDANTS, 0);
        final Set<String> xpathsMatchingCondition = new HashSet<>(conditionDataNodes.size());
        for (final DataNode dataNode : conditionDataNodes) {
     ...
     * When a selected node has a selected descendant, that descendant is not duplicated: the descendant is
     * simply part of the subtree already returned by buildCompleteTree for the selected ancestor, appearing
     * once, nested in its natural position in the result tree.
     */
