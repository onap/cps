...
final class CompositeQueryEvaluator {
     ...
     * according to the given operator. For OR, every condition is evaluated against the same full scope. For
     * AND, each condition is evaluated against the scope as narrowed by the previous conditions, so that
     * later conditions only need to match within the results already selected.
    ...
    /**
     * Intersection: each condition is evaluated against the running result of the previous conditions, so the
     * scope passed to a condition is progressively narrowed rather than the original full scope. Evaluation
     * short-circuits: once the running intersection is empty, remaining conditions are not evaluated (avoiding
     * their database queries) because the result can no longer become non-empty.
     */
