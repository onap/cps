...
public class CompositeQueryProcessor {
    ...
    private Set<String> resolveMatchingXpaths(final CompositeQuery compositeQuery,
        ...
        final Map<String, DataNode> nodeIndex = new HashMap<>();
        dataNodeTree.forEach(node -> traverseTree(node, scopeXpaths, nodeIndex));
        return applyConditions(compositeQuery, scopeXpaths, nodeIndex, dataspaceName, anchorName);
    }

    private Set<String> applyConditions(final CompositeQuery compositeQuery,
        ...
        return normalizedOperator.evaluateOperator(
            ...
            (condition, conditionScopeXpaths, conditionDataspaceName, conditionAnchorName) ->
                evaluateCondition(condition, conditionScopeXpaths, nodeIndex,
    ...
    private Set<String> evaluateCondition(final CompositeQuery compositeQuery,
        ...
        final Set<String> nestedMatches = applyConditions(compositeQuery, nestedScope, nodeIndex,
            dataspaceName, anchorName);
    ...
    private void traverseTree(final DataNode node,
                               final Set<String> xpaths,
                               final Map<String, DataNode> index) {
        xpaths.add(node.getXpath());
        index.put(node.getXpath(), node);
        if (node.getChildDataNodes() != null) {
            node.getChildDataNodes().forEach(child -> traverseTree(child, xpaths, index));
        }
    }
