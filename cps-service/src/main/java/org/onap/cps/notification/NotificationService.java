...
public class NotificationService {
    ...
    /**
     * Delta Updated Event and publishes the notification.
     *
     * @param anchor            anchor
     * @param xpath             xpath of changed data node
     * @param operation         operation
     ...
     */
    @Async("notificationExecutor")
    public Future<Void> deltaUpdatedEvent(final Anchor anchor, final String xpath, final Operation operation,
                                          final OffsetDateTime observedTimestamp) {
        ...
        try {
            if (shouldSendNotification(anchor.getDataspaceName())) {
                final var cpsDeltaUpdatedEvent =
                        cpsDataUpdatedEventFactory.createDeltaUpdatedEvent(anchor,
                                                                           observedTimestamp,
                                                                           getRootNodeOperation(xpath, operation));
