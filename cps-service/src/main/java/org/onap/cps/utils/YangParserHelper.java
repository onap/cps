...
public class YangParserHelper {
    ...
    private ContainerNode parseXmlData(final String xmlData,
                                       ...
                                       final String parentNodeXpath) {
        if (parentNodeXpath.isEmpty()) {
            return parseXmlDataAtRoot(xmlData, schemaContext);
        }
        return parseXmlDataFragment(xmlData, schemaContext, parentNodeXpath);
    }

    private ContainerNode parseXmlDataAtRoot(final String xmlData, final SchemaContext schemaContext) {
        ...
        final String preparedXml;
        try {
            preparedXml = XmlUtils.parsedXmlContent(xmlData, schemaContext);
        ...
        parseXmlIntoWriter(preparedXml, XmlParserStream.create(writer, (EffectiveModelContext) schemaContext));
        final ContainerNode parsed = builder.build();
        return preparedXml.equals(xmlData) ? parsed : removeWrappedRootNode(parsed);
    }

    private ContainerNode parseXmlDataFragment(final String xmlData,
                                               final SchemaContext schemaContext,
                                               final String parentNodeXpath) {
        final DataContainerNodeBuilder<YangInstanceIdentifier.NodeIdentifier, ContainerNode> builder =
                newDataRootBuilder();
        ...
        final EffectiveStatementInference effectiveStatementInference =
                SchemaInferenceStack.of(effectiveModelContext,
                        SchemaNodeIdentifier.Absolute.of(
                                getDataSchemaNodeIdentifiers(schemaContext, parentNodeXpath))).toInference();
        final String preparedXml;
        try {
            preparedXml = XmlUtils.parsedXmlContent(xmlData, parentSchemaNode, parentNodeXpath);
        } catch (final IOException | ParserConfigurationException | TransformerException | SAXException exception) {
            throw new DataValidationException(
                    DATA_VALIDATION_FAILURE_MESSAGE, "Failed to parse xml data: " + exception.getMessage(), exception);
        }
        parseXmlIntoWriter(preparedXml, XmlParserStream.create(writer, effectiveStatementInference));
        return removeWrappedRootNode(builder.build());
    }
