...
public final class CpsValidator {
    ...
    private static  final String UNSUPPORTED_NAME_CHARACTERS_AS_STRING = "!“” #$%&‘()*+,./\\:;<=>?@[]^`{|}~";

    private static final char[] UNSUPPORTED_NAME_CHARACTERS = UNSUPPORTED_NAME_CHARACTERS_AS_STRING.toCharArray();
                ...
                if (charactersOfName.contains(unsupportedCharacter)) {
                    throw new DataValidationException("Name or ID Validation Error.",
                        "'"  + name + "' contains an unsupported character."
                            + "The following characters are not supported for naming purposes within CPS"
                            +  UNSUPPORTED_NAME_CHARACTERS_AS_STRING);
                }
            }
        }
    }
