...
public final class CpsValidator {
     ...
     * @param ids ids
     */
    public static void validateFunctionIds(final String... ids) {
        for (final String id : ids) {
            final String[] nameCharacters = id.split("");
            for (int i = 0; i < id.length(); i++) {
                if (INVALID_NAME_AND_IDENTIFIER_CHARACTERS.contains(nameCharacters[i])) {
                    throw new DataValidationException("Invalid data.", id + "contains an invalid character");
                }
            }
        }
    }
