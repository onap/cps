...
public class DeltaReportExecutor {
     ...
     * @param deltaReportAsString original delta report in XML or JSON string format
    ...
    public void applyChangesInDeltaReport(final String dataspaceName, final String anchorName,
        ...
        final Function<Object, String> dataToStringConverter;
        if (XML.equals(contentType)) {
            deltaReports = xmlObjectMapper.convertToXmlArray(deltaReportAsString, DeltaReport.class);
            dataToStringConverter = xmlObjectMapper::asXmlString;
        } else {
            deltaReports = jsonObjectMapper.convertToJsonArray(deltaReportAsString, DeltaReport.class);
            dataToStringConverter = jsonObjectMapper::asJsonString;
        }
        for (final DeltaReport deltaReport : deltaReports) {
            ...
            switch (action) {
                case REPLACE_ACTION -> {
                    final String dataForUpdate = dataToStringConverter.apply(deltaReport.getTargetData());
                    updateDataNodes(dataspaceName, anchorName, xpath, dataForUpdate, contentType);
                }
                case REMOVE_ACTION -> {
                    final String dataForDelete = dataToStringConverter.apply(deltaReport.getSourceData());
                    deleteDataNodesUsingDelta(dataspaceName, anchorName, xpath, dataForDelete, contentType);
                }
                default -> {
                    final String dataForCreate = dataToStringConverter.apply(deltaReport.getTargetData());
