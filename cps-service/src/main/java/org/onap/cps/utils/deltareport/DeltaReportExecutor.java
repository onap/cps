...
public class DeltaReportExecutor {
    ...
    public void applyChangesInDeltaReport(final String dataspaceName, final String anchorName,
        ...
        for (final DeltaReport deltaReport : deltaReports) {
            ...
            if (REPLACE_ACTION.equals(action)) {
                final String dataForUpdate = getData(deltaReport.getTargetData(), contentType);
                updateDataNodes(dataspaceName, anchorName, xpath, dataForUpdate, contentType);
            } else if (REMOVE_ACTION.equals(action)) {
                final String dataForDelete = getData(deltaReport.getSourceData(), contentType);
                deleteDataNodesUsingDelta(dataspaceName, anchorName, xpath, dataForDelete, contentType);
            } else {
                final String dataForCreate = getData(deltaReport.getTargetData(), contentType);
    ...
    private String getData(final Map<String, Serializable> data, final ContentType contentType) {
        return JSON.equals(contentType) ? jsonObjectMapper.asJsonString(data) : convertDataMapsToXml(data);
    }
