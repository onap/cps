...
class CpsExceptionsSpec extends Specification {
    ...
    def 'Creating an exception that a dataspace already exists.'() {
        expect: 'the exception details contains the correct message with dataspace name.'
            (AlreadyDefinedException.forDataspace(dataspaceName, rootCause)).details
                    == "${dataspaceName} already exists."
    }

    def 'Creating an exception that an anchor already exists.'() {
    ...
    def 'Creating an exception that a data node already exists.'() {
    ...
    def 'Creating an exception that a schema set already exists.'() {
