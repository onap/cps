...
class CpsLoggingAspectServiceSpec extends Specification {
    ...
    def setup() {
        mockMethodSignature.getDeclaringType() >> { return this.getClass() }
        mockMethodSignature.getDeclaringType().getSimpleName() >> { return "CpsLoggingAspectServiceSpec" }
        mockMethodSignature.getName() >> { return "logMethodExecutionTime" }
        mockProceedingJoinPoint.getSignature() >> { return mockMethodSignature }
    }

    def 'Log method execution time.'() {
        given: 'mock valid arguments'
            mockProceedingJoinPoint.getArgs() >> { return "dataspace-name" }
