...
class CpsNotificationSubscriptionModelLoaderSpec extends Specification {
    ...
    def 'Onboard subscription model via application started event.'() {
        ...
        and: 'the dataspace saved for notification subscription'
    ...
    def 'Creating initial subscription when dataspaces list is null'() {
        given: 'there is no dataspaces available'
    ...
    def 'Subscribe notification for available dataspaces.'() {
        given: 'dataspace is already present'
            mockCpsDataspaceService.getAllDataspaces() >> [new Dataspace('test')]
        when: 'subscribed notification for a dataspace'
            objectUnderTest.subscribeNotificationForDataspace(new Dataspace('test'))
        then: 'the dataspace saved for notification subscription'
            1 * mockCpsDataService.saveData(CPS_DATASPACE_NAME, ANCHOR_NAME, '/dataspaces', '{"dataspace":[{"name":"test"}]}', _, _)
    }
