...
class ReadinessHealthIndicatorSpec extends Specification {
    ...
    def "CPS service UP when all model loaders are completed"() {
        given: "no model loaders are in progress"
        ...
        then: "health status is UP with following message"
            ...
            assert cpsHealth.details["Model Loaders"] == "All model loaders completed"
    }

    def "CPS service is DOWN when some model loaders are in progress"() {
        given: "some loaders are still running"
        ...
        then: "cps service is DOWN with loaders listed"
            ...
            def busyModelLoaders = cpsHealth.details["Model Loader(s) Busy"]
            assert busyModelLoaders.contains("someModuleLoader")
    }
    ...
    def "CPS service is UP after loaders complete"() {
        ...
        then: "cps health status flips to UP"
            ...
            assert health.details["Model Loaders"] == "All model loaders completed"
    }
}
