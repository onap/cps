...
class AbstractModelLoaderSpec extends Specification {
    ...
    def mapYangResourcesToContent
    def logger = (Logger) LoggerFactory.getLogger(AbstractModuleLoader)
    def loggingListAppender

    void setup() {
        mapYangResourcesToContent = objectUnderTest.mapYangResourcesToContent('cps-notification-subscriptions@2024-07-03.yang')
    ...
    void cleanup() {
        ((Logger) LoggerFactory.getLogger(AbstractModuleLoader.class)).detachAndStopAllAppenders()
    ...
    def 'Creating initial subscription for already available dataspaces.'() {
        given: 'dataspace is already present'
            mockCpsDataspaceService.getAllDataspaces() >> [new Dataspace('test')]
        when: 'initial subscription is created'
            objectUnderTest.createInitialSubscription('CPS-Admin','cps-notification-subscriptions')
        then: 'the correct JSON is saved using the data service'
            1 * mockCpsDataService.saveData('CPS-Admin','cps-notification-subscriptions', '/dataspaces', '{"dataspace":[{"name":"test"}]}',_,_)
    }

    def 'Creating initial subscription handles already defined exception.'() {
        given: 'dataspace is already present'
            mockCpsDataspaceService.getAllDataspaces() >> [new Dataspace('test')]
        ...
        then: 'the exception is ignored, and a log message is produced'
            noExceptionThrown()
            def logs = loggingListAppender.list.toString()
            logs.contains("Data node for dataspace 'test' already exists under 'dataspaces'")
    }
    ...
    class TestModelLoader extends AbstractModuleLoader {
