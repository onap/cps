...
class DataNodeBuilderSpec extends Specification {
    ...
    def assertLeavesMaps(actualLeavesMap, expectedLeavesMap) {
        assert actualLeavesMap.size() == expectedLeavesMap.size();
        expectedLeavesMap.each {
            key, value -> assert value == expectedLeavesMap[key]
        }
    }
