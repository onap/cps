...
class YangParserHelperSpec extends Specification {
    ...
    def 'Parsing data to validate #scenario String.'() {
        ...
        and: 'a model for that data'
            ...
            def schemaContext = YangTextSchemaSourceSetBuilder.of(yangResourceNameToContent).getSchemaContext()
        when: 'the data is parsed'
            objectUnderTest.parseAndValidateData(schemaContext, fileData,  parentNodeXpath, contentType)
        then: 'the result is a normalized node of the correct type'
            noExceptionThrown()
        where:
    ...
    def 'Validating data fragment by xpath for #scenario.'() {
        given: 'schema context'
            ...
            def schemaContext = YangTextSchemaSourceSetBuilder.of(yangResourcesMap).getSchemaContext()
        when: 'json string is parsed'
            objectUnderTest.parseAndValidateData(schemaContext, nodeData,  parentNodeXpath, contentType)
        then: 'a ContainerNode holding collection of normalized nodes is returned'
            noExceptionThrown()
        where:
    ...
    def 'Validating invalid data by parsing it: #description.'() {
        given: 'a yang model (file)'
            ...
            def schemaContext = YangTextSchemaSourceSetBuilder.of(yangResourceNameToContent).getSchemaContext()
        when: 'invalid data is parsed'
            objectUnderTest.parseAndValidateData(schemaContext, invalidData, '', contentType)
        then: 'an exception is thrown'
            thrown(DataValidationException)
        where: 'the following invalid data is provided'
