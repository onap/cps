...
class GroupedDeltaReportGeneratorSpec extends Specification {
    ...
    def 'Condensed delta report generation with a failing async task'() {
        given: 'source data nodes with an invalid xpath that causes a RuntimeException in the async task'
        ...
        then: 'the original RuntimeException propagates unwrapped'
    ...
    def 'Condensed delta report generation with an unexpected checked exception in an async task'() {
        given: 'a future pre-failed with a checked exception (not a RuntimeException)'
            ...
            failedFuture.completeExceptionally(new Exception('some checked error'))
        ...
        and: 'the original checked exception is preserved as the cause'
            assert thrownException.cause.message == 'some checked error'
    }
}
