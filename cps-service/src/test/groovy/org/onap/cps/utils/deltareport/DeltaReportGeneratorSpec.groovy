...
class DeltaReportGeneratorSpec extends Specification {
    ...
    def 'CompletionException with RuntimeException cause is unwrapped'() {
        ...
        then: 'the original RuntimeException is thrown unwrapped from CompletionException'
            ...
            thrownException.message == 'test error'
    }

    def 'CompletionException with non-RuntimeException cause is rethrown as CompletionException'() {
        ...
        when: 'delta reports are created'
            objectUnderTest.createDeltaReports(source, target)
        then: 'the CompletionException itself is thrown'
