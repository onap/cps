...
class DeltaReportGeneratorSpec extends Specification {
    ...
    def 'Delta report generation with an unexpected checked exception in an async task'() {
        given: 'a mocked helper that throws a CompletionException wrapping a checked exception'
            mockDeltaReportHelper.createDeltaReportsForUpdates(_, _, _) >> { throw new CompletionException(new Exception('checked error')) }
        and: 'source and target data nodes with the same xpath'
            def source = [new DataNode(xpath: '/node', leaves: ['key': 'value1'])]
            def target = [new DataNode(xpath: '/node', leaves: ['key': 'value2'])]
        when: 'delta reports are created'
        ...
        then: 'a CpsException is thrown with details about the operation'
            ...
            assert thrownException.message == 'Failed to generate delta report'
            assert thrownException.cause.message == 'checked error'
    }
}
