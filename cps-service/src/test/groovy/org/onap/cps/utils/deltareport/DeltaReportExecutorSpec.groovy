...
class DeltaReportExecutorSpec extends Specification {
    ...
    def 'Perform create operation on existing data under an anchor using delta report to add a parent list node with #scenario'() {
        ...
        and: 'expected data nodes'
            def expectedDataNodes = [new DataNode(xpath: '/bookstore/categories[@code=\'4\']',leaves: ['code': '4', 'name': 'Maths'],childDataNodes: [new DataNode(xpath: '/bookstore/categories[@code=\'4\']/books[@title=\'A Book with No Language\']',leaves: ['lang': 'en', 'price': '400', 'title': 'A Book with No Language', 'pub_year': '1993'])])]
        when: 'attempt to apply delta using the delta report'
            objectUnderTest.applyChangesInDeltaReport(dataspaceName, ANCHOR_NAME_1, deltaReport, contentType)
        then: 'data nodes are built from the target data of delta report'
            dataNodeFactory.createDataNodesWithAnchorParentXpathAndNodeData(anchor1, '/bookstore', '_', contentType) >> expectedDataNodes
        and: 'appropriate cps data service method is invoked with expected parameters to create data nodes'
            1 * mockCpsDataPersistenceService.addListElements(dataspaceName, ANCHOR_NAME_1, '/bookstore', {
                it*.xpath == expectedDataNodes*.xpath && it*.leaves == expectedDataNodes*.leaves
            })
        where:
            scenario | contentType      | deltaReport
            'JSON'   | ContentType.JSON | '[{"action":"create","xpath":"/bookstore","targetData":{"categories":[{"code":"4","name":"Maths","books":[{"lang":"en","price":"400","title":"A Book with No Language","pub_year":"1993"}]}]}}]'
            'XML'    | ContentType.XML  | '<deltaReports><deltaReport><action>create</action><xpath>/bookstore</xpath><targetData><categories><code>4</code><name>Maths</name><books><lang>en</lang><price>400</price><title>A Book with No Language</title><pub_year>1993</pub_year></books></categories></targetData></deltaReport></deltaReports>'
    }
