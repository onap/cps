...
class DeltaReportExecutorSpec extends Specification {
    ...
    def cpsValidator = Mock(CpsValidator)
    ...
    def objectUnderTest = new DeltaReportExecutor(mockCpsAnchorService, mockCpsDataPersistenceService, cpsValidator, dataNodeFactory, jsonObjectMapper)
    ...
    def 'Perform replace operation on existing data under an anchor using delta report'() {
        given: 'schema mocks and delta report in JSON format'
            setupSchemaSetMocks('bookstore.yang')
            def deltaReportJson = '[{"action":"replace","xpath":"/bookstore/categories[@code=\'1\']/books[@title=\'Matilda\']","sourceData":{"books":[{"price":20,"title":"Matilda"}]},"targetData":{"books":[{"price":30,"title":"Matilda"}]}}]'
        and: 'delta report constructed from JSON'
            def deltaReport = new DeltaReportBuilder().actionReplace().withXpath('/bookstore/categories[@code=\'1\']/books[@title=\'Matilda\']').withSourceData(['books': [['price': 20, 'title': 'Matilda']]]).withTargetData(['books': [['price': 30, 'title': 'Matilda']]]).build()
        ...
        and: 'target data as JSON is fetched from delta report'
            def targetData = jsonObjectMapper.asJsonString(deltaReport.getTargetData())
        and: 'data nodes to be replaced with leaves'
            def dataNodes = [new DataNode(xpath: '/bookstore/categories[@code=\'1\']/books[@title=\'Matilda\']', leaves: ['price': 30, 'title': 'Matilda'])]
        when: 'attempt to apply delta using the delta report'
            objectUnderTest.applyChangesInDeltaReport(dataspaceName, ANCHOR_NAME_1, deltaReportJson)
        then: 'the delta report in JSON format is converted to a list of DeltaReport objects'
            jsonObjectMapper.convertToJsonArray(deltaReportJson, DeltaReport) >> [deltaReport]
        ...
        and: 'cps data service is invoked with expected parameters to delete data nodes by using their xpaths'
            dataNodes.each {
                1 * mockCpsDataPersistenceService.batchUpdateDataLeaves(dataspaceName, ANCHOR_NAME_1, [(it.xpath): it.leaves])
            }
    }
