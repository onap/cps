...
class DeltaReportExecutorSpec extends Specification {
    ...
    def 'Perform create operation on existing data under an anchor using delta report to add a parent list node with XML'() {
        given: 'schema mocks and delta report in XML format'
            setupSchemaSetMocks('bookstore.yang')
        and: 'delta report as XML'
            def deltaReportXml = '<deltaReports><deltaReport><action>create</action><xpath>/bookstore</xpath><targetData><categories><code>4</code><name>Maths</name><books><lang>en</lang><price>400</price><title>A Book with No Language</title><authors>Terry Pratchett</authors><pub_year>1993</pub_year></books></categories></targetData></deltaReport></deltaReports>'
        and: 'delta report constructed'
            def deltaReport = new DeltaReportBuilder().actionCreate().withXpath('/bookstore').withTargetData(['categories': [['code': '4', 'name': 'Maths', 'books': [['lang': 'en', 'price': '400', 'title': 'A Book with No Language', 'authors': 'Terry Pratchett', 'pub_year': '1993']]]]]).build()
        and: 'expected data nodes'
            def expectedDataNodes = [new DataNode(xpath: '/bookstore/categories[@code=\'4\']', leaves: ['code': '4', 'name': 'Maths'],childDataNodes: [new DataNode(xpath: '/bookstore/categories[@code=\'4\']/books[@title=\'A Book with No Language\']',leaves: ['lang': 'en', 'price': '400', 'title': 'A Book with No Language', 'authors': 'Terry Pratchett', 'pub_year': '1993'])])]
        when: 'delta report is applied'
            objectUnderTest.applyChangesInDeltaReport(dataspaceName, ANCHOR_NAME_1, deltaReportXml, ContentType.XML)
        then: 'XML is converted to DeltaReport objects'
            xmlObjectMapper.convertToXmlArray(deltaReportXml, DeltaReport) >> [deltaReport]
        and: 'data nodes are built from target data'
            dataNodeFactory.createDataNodesWithAnchorParentXpathAndNodeData(anchor1, '/bookstore', _, ContentType.XML) >> expectedDataNodes
        and: 'data nodes are stored'
            1 * mockCpsDataPersistenceService.addListElements(dataspaceName, ANCHOR_NAME_1, '/bookstore', { it*.xpath == expectedDataNodes*.xpath && it*.leaves == expectedDataNodes*.leaves})
    }
