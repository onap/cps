...
class DeltaReportExecutorSpec extends Specification {
    ...
    def 'Perform create operation on existing data under an anchor using delta report to add a node with #scenario'() {
        ...
        where: 'the following data was used'
            ...
            'JSON and xpath'           | JSON        | CREATE_JSON      | bookstoreCategoryNode || '/bookstore'
            'JSON and list node xpath' | JSON        | CREATE_LIST_JSON | bookstoreBookNode     || '/bookstore/categories[@code=\'1\']'
            'XML and xpath'            | XML         | CREATE_XML       | bookstoreCategoryNode || '/bookstore'
            'XML and list node xpath'  | XML         | CREATE_LIST_XML  | bookstoreBookNode     || '/bookstore/categories[@code=\'1\']'
    }
    ...
    def 'Perform create operation on existing data under an anchor using delta report to add a parent list node with #scenario'() {
        ...
        where: 'the following content type and delta report was used'
            scenario | contentType | deltaReport
            'JSON'   | JSON        | CREATE_PARENT_LIST_JSON
            'XML'    | XML         | CREATE_PARENT_LIST_XML
    }
    ...
    def 'Perform replace operation on existing data under an anchor using delta report'() {
        ...
        where: 'following content tye and delta report was used'
            scenario | contentType | deltaReport
            'JSON'   | JSON        | REPLACE_JSON
            'XML'    | XML         | REPLACE_XML
    }
