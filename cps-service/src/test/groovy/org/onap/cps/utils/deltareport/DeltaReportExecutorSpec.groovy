...
class DeltaReportExecutorSpec extends Specification {
    ...
    def 'Perform delete operation on existing data under an anchor using delta report'() {
        given: 'schema mocks and data nodes'
        ...
        when: 'attempt to apply delta using the delta report'
            objectUnderTest.applyChangesInDeltaReport(dataspaceName, ANCHOR_NAME_1, deltaReport, contentType)
        then: 'data nodes are built from the source data of delta report'
            dataNodeFactory.createDataNodesWithAnchorParentXpathAndNodeData(anchor1, '/bookstore', 'my-source-data', contentType) >> dataNodes
    ...
    def 'Perform create operation on existing data under an anchor using delta report to add a node with #scenario'() {
        ...
        and: 'data nodes to be created'
            def dataNodes = scenario == 'xpath'? [new DataNode(xpath: '/bookstore/categories[@code=\'1\']', leaves: ['code': '1', 'name': 'Children'])]: [new DataNode(xpath: '/bookstore/categories[@code=\'1\']/books[@title=\'Matilda\']', leaves: ['price': 20, 'title': 'Matilda'])]
        when: 'attempt to apply delta using the delta report'
            objectUnderTest.applyChangesInDeltaReport(dataspaceName, ANCHOR_NAME_1, deltaReport, contentType)
        then: 'data nodes are built from the target data of delta report'
            dataNodeFactory.createDataNodesWithAnchorParentXpathAndNodeData(anchor1, expectedXpath, '_', contentType) >> dataNodes
        ...
        where:
            contentType       | scenario          | deltaReport                                                                                                                                                                                                    || expectedXpath
            ContentType.JSON  | 'xpath'           | '[{"action":"create","xpath":"/bookstore","targetData":{"categories":[{"code":"1","name":"Children"}]}}]'                                                                                                       || '/bookstore'
            ContentType.JSON  | 'list node xpath' | '[{"action":"create","xpath":"/bookstore/categories[@code=\'1\']","targetData":{"books":[{"price":20,"title":"Matilda"}]}}]'                                                                                   || '/bookstore/categories[@code=\'1\']'
            ContentType.XML   | 'xpath'           | '<deltaReports><deltaReport><action>create</action><xpath>/bookstore</xpath><targetData><categories><code>1</code><name>Children</name></categories></targetData></deltaReport></deltaReports>'                || '/bookstore'
            ContentType.XML   | 'list node xpath' | '<deltaReports><deltaReport><action>create</action><xpath>/bookstore/categories[@code=\'1\']</xpath><targetData><books><price>20</price><title>Matilda</title></books></targetData></deltaReport></deltaReports>' || '/bookstore/categories[@code=\'1\']'
    }

    def 'Perform create operation on existing data under an anchor using delta report to add a parent list node with #scenario'() {
        given: 'schema mocks and data nodes created from delta report'
            setupSchemaSetMocks('bookstore.yang')
        ...
        when: 'attempt to apply delta using the delta report'
            objectUnderTest.applyChangesInDeltaReport(dataspaceName, ANCHOR_NAME_1, deltaReport, contentType)
        then: 'data nodes are built from the target data of delta report'
            dataNodeFactory.createDataNodesWithAnchorParentXpathAndNodeData(anchor1, '/', '_', contentType) >> dataNodesCreatedFromDeltaReport
        and: 'appropriate cps data service method is invoked with expected parameters to create data nodes'
        ...
        where:
            scenario | contentType      | deltaReport
            'JSON'   | ContentType.JSON | '[{"action":"create","xpath":"/bookstore-address[@bookstore-name=\'Easons\']","targetData":{"bookstore-address":[{"bookstore-name":"Easons"}]}}]'
            'XML'    | ContentType.XML  | '<deltaReports><deltaReport><action>create</action><xpath>/bookstore-address[@bookstore-name=\'Easons\']</xpath><targetData><bookstore-address><bookstore-name>Easons</bookstore-name></bookstore-address></targetData></deltaReport></deltaReports>'
    }

    def 'Perform replace operation on existing data under an anchor using delta report'() {
        given: 'schema mocks data nodes'
            setupSchemaSetMocks('bookstore.yang')
        ...
        when: 'attempt to apply delta using the delta report'
            objectUnderTest.applyChangesInDeltaReport(dataspaceName, ANCHOR_NAME_1, deltaReport, contentType)
        then: 'data nodes are built from the target data of delta report'
            dataNodeFactory.createDataNodesWithAnchorParentXpathAndNodeData(anchor1, '/bookstore', '_', contentType) >> dataNodesCreatedFromDeltaReport
        and: 'cps data service is invoked with expected parameters to update data nodes'
            1 * mockCpsDataPersistenceService.updateDataNodesAndDescendants(dataspaceName, ANCHOR_NAME_1,{
                dataNodesToBeStored -> dataNodesToBeStored*.xpath == dataNodesCreatedFromDeltaReport*.xpath && dataNodesToBeStored*.leaves == dataNodesCreatedFromDeltaReport*.leaves })
        where:
            scenario | contentType       | deltaReport
