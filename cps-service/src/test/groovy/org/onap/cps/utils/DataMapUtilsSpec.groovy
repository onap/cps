...
class DataMapUtilsSpec extends Specification {
    ...
    def 'Data node structure conversion for to map them as a grouped dataNodes'() {
        given: 'an empty map to collect data'
            def groupedDataNodes = [:]
        when: 'dataNodes are converted to map'
            bookstore.each { dataNode ->
                DataMapUtils.toDataMapsWithIdentifier(dataNode, "book-store", groupedDataNodes)
            }
        then: 'the root node should contain prefix book-store:books'
            groupedDataNodes.containsKey("book-store:books")
        and: 'list entries are populated correctly'
            groupedDataNodes."book-store:books".size() == 2
        and: 'first list populated with correct attributes'
            def firstBook = groupedDataNodes."book-store:books"[0]
        ...
        and: 'second list populated with correct attributes'
            def secondBook = groupedDataNodes."book-store:books"[1]
