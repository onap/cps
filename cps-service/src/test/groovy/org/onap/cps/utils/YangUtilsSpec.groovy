...
class YangUtilsSpec extends Specification {
    def 'Parsing a valid Json String.'() {
        ...
        then: 'the children in result are of type normalized node'
            DataContainerChild dataContainerChild = result.getValue()[0]
            dataContainerChild instanceof NormalizedNode == true
        then: 'qualified name of children created is as expected'
            dataContainerChild.nodeType == QName.create('org:onap:ccsdk:sample', '2020-09-15', 'bookstore')
    }
    ...
    def 'Parsing json data fragment by xpath for #scenario.'() {
        ...
        then: 'normalized nodes are returned'
