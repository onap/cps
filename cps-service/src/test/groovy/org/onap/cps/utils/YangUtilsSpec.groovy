...
class YangUtilsSpec extends Specification {
    def 'Parsing a valid Json String.'() {
        ...
        then: 'result is a ContainerNode holding collection of normalized nodes is returned'
            result.getValue()[index] instanceof NormalizedNode == true
        then: 'qualified name of children created is as expected'
            result.getValue()[index].nodeType == QName.create('org:onap:ccsdk:multiDataTree', '2020-09-15', nodeName)
        where:
            index   | nodeName
            0       | 'first-container'
            1       | 'last-container'
    ...
    def 'Parsing json data fragment by xpath for #scenario.'() {
        ...
        then: 'result is a collection of normalized nodes is returned'
            result.getValue()[0] instanceof NormalizedNode == true
        then: 'result represents a node of expected type'
            result.getValue()[0].nodeType == QName.create('org:onap:cps:test:test-tree', '2020-02-02', nodeName)
        where:
