...
class DeltaReportUtilsSpec extends Specification {
    ...
    def 'Converting delta report with nested map in sourceData'() {
        given:'A delta report with nested map inside sourceData'
            def deltaReports = [
                new DeltaReport(
                        action:'replace',
                        xpath:'/bookstore',
                        sourceData: ['details': ['name':'Bookstore','location':'City']],
                        targetData: null
                )
        ]
        when:'deltaReports are converted to XML'
            def result = buildXml(deltaReports)
        then:'the result contains nested map elements'
            assert result.contains('<details>')
            assert result.contains('<name>Bookstore</name>')
            assert result.contains('<location>City</location>')
    }

    def 'Converting delta report with collection of maps in sourceData'() {
        given:'A delta report with collection containing maps'
             def deltaReports = [
                new DeltaReport(
                        action:'replace',
                        xpath:'/bookstore',
        ...
        when:'deltaReports are converted to XML'
            def result = buildXml(deltaReports)
        then:'the result contains nested elements for each map in the collection'
            assert result.contains('<categories>')
            assert result.contains('<code>1</code>')
            assert result.contains('<name>Fiction</name>')
            assert result.contains('<code>2</code>')
            assert result.contains('<name>Science</name>')
    }
}