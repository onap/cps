...
import static org.onap.cps.utils.deltareport.DeltaReportUtils.buildXmlUsingDom
...
class DeltaReportUtilsSpec extends Specification {
    ...
    def 'Converting delta report to XML scenario'() {
        ...
        where:
            scenario                                  | deltaReports || expectedXmlOutput
                    ...
                    'delta report with null lists'     | [new DeltaReport(action: 'Add', xpath: '/bookstore', sourceData: null, targetData: null)]  || '<deltaReports><deltaReport id="1"><action>Add</action><xpath>/bookstore</xpath></deltaReport></deltaReports>'
    }

    def 'Converting delta report objects to XML'() {
        given: 'A delta report with nested collections in sourceData and targetData'
             def deltaReports = createSampleDeltaReports()
        when: 'deltaReports are converted to XML'
            def result = buildXmlUsingDom(deltaReports)
        then: 'the result contains the expected XML with nested collection values as comma-separated strings'
        result == '<deltaReports><deltaReport id="1"><action>Update</action><xpath>/bookstore</xpath><source-data><books>Book-1, Book-2, Book-3</books></source-data><target-data><authors>Author-1, Author-2</authors></target-data></deltaReport></deltaReports>'
    }
    private List<DeltaReport> createSampleDeltaReports() {
        return [
                new DeltaReport(
                        id: 1,
                        action: 'Update',
    ...
    def 'delta report with all fields null'() {
        given: 'A delta report with all fields set to null'
            def deltaReports = [new DeltaReport(action: null, xpath: null, sourceData: null, targetData: null)]
        when: 'buildXmlUsingDom is called'
            def result = buildXmlUsingDom(deltaReports)
        then: 'the result contains a delta report element with only an id'
