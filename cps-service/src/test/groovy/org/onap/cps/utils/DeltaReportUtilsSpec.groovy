...
class DeltaReportUtilsSpec extends Specification {
    ...
    def 'Converting delta reports when buildXml is called then sourceData elements are omitted'() {
        ...
        when:'deltaReports are converted to XM'
            def result = buildXml(deltaReports)
    ...
    def 'Given a nested map in sourceData when buildXml is called then nested elements are created'() {
    ...
    def 'Given a collection of maps in sourceData when buildXml is called then createXmlElement processes each map item'() {
