...
class XmlFileUtilsSpec extends Specification {
    ...
    def 'Convert data maps to XML #scenario'() {
        ...
        where:
            ...
            'XML with mixed content in list'      | [['branch': ['name': 'Left', 'nest': ['name': 'Small', 'birds': ['', 'Sparrow']]]]]                                                      || '<branch><name>Left</name><nest><name>Small</name><birds>Sparrow</birds></nest></branch>'
    }
    ...
    def 'Convert data maps to XML should throw exception for invalid data'() {
        when: 'data maps with empty content are converted to XML'
            XmlFileUtils.convertDataMapsToXml(dataMaps)
        then:
            thrown(RuntimeException)
        where:
            scenario                  | dataMaps
            'null entry'              | [null]
            'null map entry'          | [['branch': ['': ' ', nest: [name: 'Small', birds: []]]]]
            'non-string element name' | [['123': [name: 'Left', nest: [name: 'Small', birds: 'Sparrow']]]]
    }
}
