...
class CpsDataUpdateEventsProducerSpec extends Specification {
    ...
    def 'Send CPS event when delta report notifications is enabled'() {
        ...
        where: 'the following values are used'
            ...
            'delta report with both source and target data'  | [new DeltaReportBuilder().actionReplace().withXpath('/bookstore').withSourceData(['categories': ['code': '4', 'name': 'Computing']]).withTargetData(['categories': [['code': '4', 'name': 'Funny']]]).build()] || 1                       | REPLACE_ACTION
    }
    ...
    def 'Attempt to send CPS event fails when no cloud event is generated'() {
        given: 'an anchor'
            def anchor = new Anchor('anchor01', 'dataspace01', 'schema01');
            def deltaReport = [new DeltaReportBuilder().actionCreate().withXpath('/bookstore').build()]
        and: 'notificationsEnabled is true'
            objectUnderTest.notificationsEnabled = true
        and: 'cpsChangeEventNotificationsEnabled is also true'
            objectUnderTest.cpsChangeEventNotificationsEnabled = true
        and: 'deltaNotificationEnabled is also true'
            objectUnderTest.deltaNotificationEnabled = true
        when: 'service is called to send data update event'
