...
class CpsDataUpdateEventsProducerSpec extends Specification {
    ...
    def 'Send CPS event when delta report is null'() {
        given: 'an anchor and null delta report'
            def anchor = new Anchor('anchor01', 'dataspace01', 'schema01');
        and: 'notificationsEnabled is true'
            objectUnderTest.notificationsEnabled = true
        and: 'cpsChangeEventNotificationsEnabled is false'
        ...
        and: 'cpsDeltaReportEventNotificationsEnabled is true'
            objectUnderTest.deltaNotificationEnabled = true
        when: 'service is called to send data event'
            objectUnderTest.sendCpsDataUpdateEvent(anchor, '/', REPLACE.value(), null, OffsetDateTime.now())
        then: 'the event is not sent'
            0 * mockEventsProducer.sendCloudEvent(*_)
    }

    def 'Send CPS event when delta report is empty'() {
        ...
        and: 'cpsDeltaReportEventNotificationsEnabled is true'
            objectUnderTest.deltaNotificationEnabled = true
        when: 'service is called to send data event'
            objectUnderTest.sendCpsDataUpdateEvent(anchor, '/', REPLACE.value(), [], OffsetDateTime.now())
        then: 'the event is not sent'
            0 * mockEventsProducer.sendCloudEvent(*_)
    }

    def 'Send CPS event when delta report has null source and target data'() {
        given: 'an anchor and delta report with null source and target data'
            def anchor = new Anchor('anchor01', 'dataspace01', 'schema01');
            def deltaReports = [new DeltaReportBuilder().actionReplace().withXpath('/bookstore').withSourceData(null).withTargetData(null).build()]
        and: 'notificationsEnabled is true'
            objectUnderTest.notificationsEnabled = true
        and: 'cpsChangeEventNotificationsEnabled is false'
            objectUnderTest.cpsChangeEventNotificationsEnabled = true
    ...
    def 'Send CPS event when delta report has empty source and target data'() {
        given: 'an anchor and delta report with empty source and target data'
            def anchor = new Anchor('anchor01', 'dataspace01', 'schema01');
            def deltaReports = [new DeltaReportBuilder().actionReplace().withXpath('/bookstore').withSourceData([:]).withTargetData([:]).build()]
        and: 'notificationsEnabled is true'
            objectUnderTest.notificationsEnabled = true
        and: 'cpsChangeEventNotificationsEnabled is false'
            objectUnderTest.cpsChangeEventNotificationsEnabled = true
