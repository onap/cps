...
class CpsDataUpdateEventsProducerSpec extends Specification {
    ...
    def 'Create and send cps REPLACE event where events are #scenario.'() {
            ...
            args ->
                {
                    ...
                    def actualEventOperation = CloudEventUtils.mapData(cpsDataUpdatedEvent, PojoCloudEventDataMapper.from(objectMapper, CpsDataUpdatedEvent.class)).getValue().data.action
                    assert actualEventOperation == expectedOperation
                }
            }
        where: 'the following values are used'
            scenario                                    | xpath        | actionInRequest || expectedOperation
            'empty xpath'                               | ''           | CREATE          || CREATE
            'root xpath and create operation'           | '/'          | CREATE          || CREATE
            'root xpath and REPLACE operation'          | '/'          | REPLACE         || REPLACE
            'root xpath and REMOVE operation'           | '/'          | REMOVE          || REMOVE
            'not root xpath and REPLACE operation'      | 'test'       | REPLACE         || REPLACE
            'root node xpath and create operation'      | '/test'      | CREATE          || CREATE
            'non root node xpath and REPLACE operation' | '/test/path' | CREATE          || CREATE
            'non root node xpath and REMOVE operation'  | '/test/path' | REMOVE          || REMOVE
    }

    def 'Send cps REPLACE event when no timestamp provided.'() {
    ...
    def 'Enabling and disabling sending cps REPLACE events.'() {
