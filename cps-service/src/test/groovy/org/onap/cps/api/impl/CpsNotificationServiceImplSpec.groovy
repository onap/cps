...
class CpsNotificationServiceImplSpec extends Specification {
    ...
    def  prefixResolver = Mock(PrefixResolver)
    def objectUnderTest = new CpsNotificationServiceImpl(mockCpsAnchorService, mockCpsDataPersistenceService, yangParser, prefixResolver)
    ...
    def 'get notification subscription for given xpath'() {
        given: 'details for notification subscription'
            def xpath = '/dataspaces/dataspace[@name="ds01]'
        and: 'persistance service returns data nodes for subscribed data'
            mockCpsDataPersistenceService.getDataNodes(dataspaceName, anchorName,
                    xpath, FetchDescendantsOption.INCLUDE_ALL_DESCENDANTS) >>
                    [new DataNodeBuilder().withXpath('/dataspaces').withLeaves([leaf: 'dataspace', leafList: ['ds01', 'ds02']]).build()]
        when: 'delete notification subscription is called'
           def notificationSubscription =  objectUnderTest.getNotificationSubscription(xpath)
        then: 'the persistence service is called once with the correct parameters'
            notificationSubscription.get(0).toString() == '{dataspaces={leaf=dataspace, leafList=[ds01, ds02]}}'
    }
