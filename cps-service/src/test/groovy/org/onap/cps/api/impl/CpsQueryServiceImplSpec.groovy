...
class CpsQueryServiceImplSpec extends Specification {
    ...
    def 'Query total anchors for dataspace and cps path.'() {
        when: 'query total anchors is invoked'
            objectUnderTest.queryTotalAnchorsForDataspaceAndCpsPath("some-dataspace", "/cps-path")
        then: 'the persistence service is called once with the correct parameters'
            1 * mockCpsDataPersistenceService.queryTotalAnchorsForDataspaceAndCpsPath("some-dataspace", "/cps-path")
    }
}
