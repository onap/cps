...
class CpsAdminServiceImplSpec extends Specification {
    ...
    def 'Create anchor from new modules and existing modules.'() {
        ...
        when: 'create anchor from new modules and existing modules method is invoked'
            objectUnderTest.createAnchorFromModules("someAnchor", [newModule: "newContent"], listOfExistingModulesModuleReference)
        then: 'the persistence service method is invoked with the same parameters'
            1 * mockCpsAdminPersistenceService.createAnchorFromModules("someAnchor", [newModule: "newContent"], listOfExistingModulesModuleReference)
    }
