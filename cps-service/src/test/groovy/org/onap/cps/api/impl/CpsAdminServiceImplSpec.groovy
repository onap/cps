...
class CpsAdminServiceImplSpec extends Specification {
    ...
    def 'Retrieve dataspace.'() {
        ...
        expect: 'the dataspace provided by persistence service is returned as result'
            objectUnderTest.getDataspace('someDataspace') == dataspace
    }

    def 'Retrieve all dataspace.'() {
        given: 'that all given dataspaces are already created'
            def dataspaces = new ArrayList()
            mockCpsAdminPersistenceService.getAllDataspace() >> dataspaces
        expect: 'the dataspace provided by persistence service is returned as result'
            objectUnderTest.getAllDataspaces() == dataspaces
    }
