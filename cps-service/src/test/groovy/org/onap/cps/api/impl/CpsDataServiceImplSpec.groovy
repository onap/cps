...
class CpsDataServiceImplSpec extends Specification {
    ...
    def 'Delete with empty list-node XPath.'() {
        given: 'schema set for given anchor and dataspace references test-tree model'
            setupSchemaSetMocks('test-tree.yang')
        when: 'delete list data method is invoked with empty list-node XPath'
            objectUnderTest.deleteListNodeData(dataspaceName, anchorName, "")
        then: 'invalid data exception is thrown'
            thrown(CpsPathException)
    }

    def 'Delete with invalid list-node XPath.'() {
        given: 'schema set for given anchor and dataspace references test-tree model'
            setupSchemaSetMocks('test-tree.yang')
        when: 'delete list data method is invoked with empty list-node XPath'
            objectUnderTest.deleteListNodeData(dataspaceName, anchorName, "/unknown")
        then: 'invalid data exception is thrown'
            thrown(CpsPathException)
    }
