...
class CpsDataServiceImplSpec extends Specification {
    ...
    def 'Get all data nodes (for V2 of Get API) #scenario.'() {
        given: 'persistence service returns data for GET request'
            mockCpsDataPersistenceService.getMultipleDataNodes(dataspaceName, anchorName, xpath, fetchDescendantsOption) >> dataNode
        expect: 'service returns same data if using same parameters'
            objectUnderTest.getMultipleDataNodes(dataspaceName, anchorName, xpath, fetchDescendantsOption) == dataNode
