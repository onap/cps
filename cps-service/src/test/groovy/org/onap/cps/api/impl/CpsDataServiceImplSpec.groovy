...
import org.onap.cps.spi.model.DeltaReportCustomBuilder
...
class CpsDataServiceImplSpec extends Specification {
    ...
    def 'Get delta between 2 anchors'() {
        ...
        def deltaReport = [new DeltaReportCustomBuilder().withXpath(xpath).build(), new DeltaReportCustomBuilder().withXpath(xpath).build()]
        when: 'attempt to get delta between 2 anchors and it returns delta report'
            objectUnderTest.getDeltaByDataspaceAndAnchors(dataspaceName, ANCHOR_NAME_1, ANCHOR_NAME_2, xpath, FetchDescendantsOption.INCLUDE_ALL_DESCENDANTS) == deltaReport
