...
class CpsDataServiceImplSpec extends Specification {
    ...
    def 'Get all data nodes using V2 #scenario.'() {
        ...
        where: 'all fetch options are supported'
            fetchDescendantsOption << [FetchDescendantsOption.OMIT_DESCENDANTS, FetchDescendantsOption.INCLUDE_ALL_DESCENDANTS]
        and: 'following parameters were used'
            scenario                | xpath   |   dataNode
            'with root node xpath'  | '/'     | [new DataNodeBuilder().withXpath('/xpath-1').build(), new DataNodeBuilder().withXpath('/xpath-2').build()]
            'with valid xpath'      | '/xpath'| [new DataNodeBuilder().withXpath('/xpath').build()]
    }
    ...
    def 'Get data nodes V2 error scenario: #scenario'() {
        ...
        where: 'following parameters are used'
            scenario             | xpath                 | dataNode         | expectedException
            'non existing xpath' | '/not-existing-xpath' | []               | DataNodeNotFoundException
            'invalid xpath'      | 'INVALID xPath'       | [new DataNode()] | PathParsingException
        and: 'fetch decendants option is set as following'
            fetchDescendantsOption << [FetchDescendantsOption.OMIT_DESCENDANTS, FetchDescendantsOption.INCLUDE_ALL_DESCENDANTS]
    }
