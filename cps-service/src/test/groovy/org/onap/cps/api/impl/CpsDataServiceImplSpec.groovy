...
class CpsDataServiceImplSpec extends Specification {
    ...
    def 'Update data node leaves: #scenario.'() {
        ...
        then: 'the persistence service method is invoked with correct parameters'
            1 * mockCpsDataPersistenceService.updateMultipleDataLeaves(dataspaceName, anchorName, {dataNode -> dataNode.xpath[0] == expectedNodeXpath})
        and: 'the CpsValidator is called on the dataspaceName and AnchorName'
            1 * mockCpsValidator.validateNameCharacters(dataspaceName, anchorName)
        and: 'data updated event is sent to notification service'
    ...
    def 'Update multiple data nodes' () {
        ...
        then:
            1 * mockCpsDataPersistenceService.updateMultipleDataLeaves(dataspaceName, anchorName, {dataNode -> dataNode.xpath[index] == expectedNodeXpath})
        and: 'the CpsValidator is called on the dataspaceName and AnchorName'
            1 * mockCpsValidator.validateNameCharacters(dataspaceName, anchorName)
        and: 'data updated event is sent to notification service'
            1 * mockNotificationService.processDataUpdatedEvent(anchor, parentNodeXpath, Operation.UPDATE, observedTimestamp)
