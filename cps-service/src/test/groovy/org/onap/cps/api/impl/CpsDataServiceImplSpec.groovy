...
class CpsDataServiceImplSpec extends Specification {
    ...
    def "Validating data when dry run is enabled. #scenario"() {
        ...
        then: 'appropriate parsing method is invoked with correct parameters'
            yangParser.parseAndValidateData(anchor, xpath, data, contentType)
        ...
        'XML data with root node xpath'  | '/'             | ''           | ContentType.XML  | '<stores><bookstore xmlns="org:onap:ccsdk:sample"><bookstore-name>Chapters</bookstore-name></bookstore></stores>'
        'XML data with xpath'            | '/bookstore'    | '/bookstore' | ContentType.XML  | '<bookstore-name>Chapters</bookstore-name>'
    }
