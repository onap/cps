...
class CompositeQueryValidatorSpec extends Specification {
    ...
    def 'Composite query with a #scenario cps path on a condition.'() {
        given: 'a query whose condition has a #scenario cps path'
            def condition = CompositeQuery.builder().cpsPath(conditionCpsPath).build()
            def query = CompositeQuery.builder().cpsPath('//categories').conditions([condition]).build()
        when: 'the composite query is validated'
            CompositeQueryValidator.validateCompositeQuery(query)
        then: 'a data validation exception is thrown'
            thrown(DataValidationException)
        where: 'the following cps path values are used'
            scenario  | conditionCpsPath
            'missing' | null
            'blank'   | '   '
    }
