...
class DataNodeBuilderServiceSpec extends Specification {
    ...
    def objectUnderTest = new DataNodeBuilderServiceImpl(yangParser)
    ...
    void cleanup() {
        ((Logger) LoggerFactory.getLogger(DataNodeBuilderServiceImpl.class)).detachAndStopAllAppenders()
    ...
    def 'Build data nodes using anchor and map of xpath to #scenario'() {
        given:'schema set for given anchor and dataspace references test-tree model'
            setupSchemaSetMocks('test-tree.yang')
        when: 'attempt to build data nodes'
            def dataNodes = objectUnderTest.buildDataNodesWithAnchorAndXpathToNodeData(anchor, xpathToNodeData, contentType)
    ...
    def 'Build data nodes using anchor, xpath and #scenario string'() {
        given:'xpath, json string and schema set for given anchor and dataspace references test-tree model'
            ...
            setupSchemaSetMocks('test-tree.yang')
        when: 'attempt to build data nodes'
            def dataNodes = objectUnderTest.buildDataNodesWithAnchorXpathAndNodeData(anchor, xpath, nodeData, contentType)
        then: 'expected number of data nodes are created'
            dataNodes.size() == 1
        and: 'data nodes have expected xpaths'
    ...
    def 'Building data nodes using anchor, xpath and invalid json string'() {
        given:'xpath, invalid json string and schema set for given anchor and dataspace references test-tree model'
            setupSchemaSetMocks('test-tree.yang')
        when: 'attempt to build data nodes'
            objectUnderTest.buildDataNodesWithAnchorXpathAndNodeData(anchor, '/test-tree', invalidData, contentType)
        then: 'expected number of data nodes are created'
    ...
    def 'Build data nodes using anchor, parent node xpath and #scenario string'() {
        given:'parent node xpath, json string and schema set for given anchor and dataspace references test-tree model'
            ...
            setupSchemaSetMocks('test-tree.yang')
        when: 'attempt to build data nodes'
            def dataNodes = objectUnderTest.buildDataNodesWithAnchorParentXpathAndNodeData(anchor, parentXpath, nodeData, contentType)
        then: 'expected number of data nodes are created'
            dataNodes.size() == 1
        and: 'data nodes have expected xpaths'
    ...
    def 'Build data nodes using anchor, parent node xpath and invalid #scenario string'() {
        given:'parent node xpath, invalid json string and schema set for given anchor and dataspace references test-tree model'
            def parentXpath = "/test-tree"
            setupSchemaSetMocks('test-tree.yang')
        when: 'attempt to build data nodes'
            objectUnderTest.buildDataNodesWithAnchorParentXpathAndNodeData(anchor, parentXpath, invalidData, contentType)
        then: 'expected number of data nodes are created'
            def exceptionThrown = thrown(DataValidationException)
            assert exceptionThrown.message.startsWith(expectedMessage)
        where:
    ...
    def 'Build data nodes using schema, xpath and #scenario string'() {
        ...
        when: 'attempt to build data nodes'
            def dataNodes = objectUnderTest.buildDataNodesWithYangResourceXpathAndNodeData(yangResourcesNameToContentMap, '/', nodeData, contentType)
        then: 'expected number of data nodes are created'
            dataNodes.size() == 1
    ...
    def 'Build data nodes using schema, xpath and invalid #scenario string'() {
        given:'xpath, invalid json string and schema set for given anchor and dataspace references bookstore model'
            def yangResourcesNameToContentMap = TestUtils.getYangResourcesAsMap('bookstore.yang')
            setupSchemaSetMocksForDelta(yangResourcesNameToContentMap)
        when: 'attempt to build data nodes'
            objectUnderTest.buildDataNodesWithYangResourceXpathAndNodeData(yangResourcesNameToContentMap, '/', invalidData, contentType)
        then: 'expected number of data nodes are created'
            def exceptionThrown = thrown(DataValidationException)
            assert exceptionThrown.message.startsWith(expectedMessage)
        where:
            ...
            'no xml nnodes' | '"<bookstore xmlns=\"org:onap:ccsdk:sample\"/>' | ContentType.XML  || 'Data Validation Failed'
