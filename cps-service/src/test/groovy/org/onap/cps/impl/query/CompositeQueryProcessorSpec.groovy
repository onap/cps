...
class CompositeQueryProcessorSpec extends Specification {
    ...
    def objectUnderTest = new CompositeQueryProcessor(mockCpsDataPersistenceService, new CompositeQueryEvaluator())
    ...
    def 'Composite query where matched node has null childDataNodes during nested scope collection.'() {
        ...
        and: 'a nested AND query so that collectDescendantXpaths recurses into the child with null children'
    ...
    def 'Composite query where a node has duplicate child xpaths during nested scope collection.'() {
        ...
        and: 'a nested AND query so that collectDescendantXpaths iterates the duplicate child twice'
            def subCondition = CompositeQuery.builder().cpsPath('/ns/parent/child[@x=1]').build()
            def condition    = CompositeQuery.builder().cpsPath('/ns/parent').operator('and').conditions([subCondition]).build()
            def query        = CompositeQuery.builder().cpsPath('/ns/parent').operator('and').conditions([condition]).build()
    ...
    def 'Composite query with an invalid operator on #scenario fails before any database access.'() {
        given: 'a query carrying an unsupported operator'
            ...
            def query = CompositeQuery.builder().cpsPath('//categories').operator(queryOperator)
                .conditions([condition]).build()
        when: 'the composite query is processed'
            objectUnderTest.processCompositeQuery(DATASPACE, ANCHOR, query,
                FetchDescendantsOption.INCLUDE_ALL_DESCENDANTS)
        then: 'a data validation exception is thrown'
            thrown(DataValidationException)
        and: 'the persistence service is never queried'
            0 * mockCpsDataPersistenceService.queryDataNodes(*_)
        where: 'the invalid operator appears on the following level'
            scenario                | queryOperator | conditionOperator
            'the top-level query'   | 'xor'         | 'and'
            'a nested condition'    | 'and'         | 'nand'
    }

    def 'Composite query with a missing cps path on a condition fails before any database access.'() {
    ...
    def 'Composite query exceeding the maximum nesting depth fails before any database access.'() {
    ...
    def 'Composite query where two conditions use the same cps path issues only one condition query.'() {
    ...
    def 'Composite query with AND operator skips remaining conditions once a condition selects nothing.'() {
