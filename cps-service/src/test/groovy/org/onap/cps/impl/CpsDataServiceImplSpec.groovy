...
class CpsDataServiceImplSpec extends Specification {
    ...
    def objectUnderTest = new CpsDataServiceImpl(mockCpsDataPersistenceService, mockDataUpdateEventsService, mockCpsAnchorService,
            dataNodeBuilderService, mockCpsValidator, yangParser, mockCpsDeltaService, jsonObjectMapper, mockPrefixResolver)
    ...
    def 'Saving #scenario data.'() {
        ...
        then: 'the data node builder service is invoked with correct paramters and returns data nodes'
            dataNodeBuilderService.buildDataNodesWithAnchorXpathAndNodeData(anchor, '/', data, contentType) >> [new DataNodeBuilder().withXpath('/test-tree').build()]
    ...
    def 'Saving data with error: #scenario.'() {
        ...
        then: 'the data node builder service is invoked and a data validation exception is thrown with the correct message'
            dataNodeBuilderService.buildDataNodesWithAnchorXpathAndNodeData(anchor, '/', invalidData, contentType) >> { throw new DataValidationException(expectedMessage, 'Exception details') }
    ...
    def 'Saving list element data fragment under Root node.'() {
        ...
        then: 'the data node builder service is invoked with correct paramters and returns data nodes'
            dataNodeBuilderService.buildDataNodesWithAnchorParentXpathAndNodeData(anchor, '/', jsonData, ContentType.JSON) >> [new DataNodeBuilder().withXpath('/bookstore-address[@bookstore-name=\'Easons\']').build()]
        and: 'the persistence service method is invoked with correct parameters'
            1 * mockCpsDataPersistenceService.storeDataNodes(dataspaceName, anchorName,
                { dataNodeCollection ->
                    {
    ...
    def 'Saving child data fragment under existing node.'() {
        ...
        then: 'the data node builder service is invoked with correct paramters and returns data nodes'
            dataNodeBuilderService.buildDataNodesWithAnchorParentXpathAndNodeData(anchor, '/test-tree', jsonData, ContentType.JSON) >> [new DataNodeBuilder().withXpath('/test-tree/branch[@name=\'New\']').build()]
        and: 'the persistence service method is invoked with correct parameters'
    ...
    def 'Saving list element data fragment under existing #scenario .'() {
        ...
        then: 'the data node builder service is invoked with correct paramters and returns data nodes'
            dataNodeBuilderService.buildDataNodesWithAnchorParentXpathAndNodeData(anchor, '/test-tree', data, contentType) >> [new DataNodeBuilder().withXpath('/test-tree/branch[@name=\'A\']').build(), new DataNodeBuilder().withXpath('/test-tree/branch[@name=\'B\']').build()]
        and: 'the persistence service method is invoked with correct parameters'
    ...
    def 'Saving empty list element data fragment for #scenario.'() {
        ...
        when: 'save data method is invoked with an empty list'
            objectUnderTest.saveListElements(dataspaceName, anchorName, '/test-tree', data, observedTimestamp, contentType)
        then: 'the data node builder service is invoked with correct paramters and returns data nodes'
            dataNodeBuilderService.buildDataNodesWithAnchorParentXpathAndNodeData(anchor, '/test-tree', data, contentType) >> { throw new DataValidationException('Exception message', 'Exception details') }
        and: 'invalid data exception is thrown'
    ...
    def 'Get delta between anchor and payload with user provided schema #scenario'() {
        ...
        and: 'the data node builder service is invoked with correct paramters and returns data nodes'
            dataNodeBuilderService.buildDataNodesWithAnchorParentXpathAndNodeData(anchor, xpath, jsonData, ContentType.JSON) >> [new DataNodeBuilder().withXpath('/bookstore').build()]
