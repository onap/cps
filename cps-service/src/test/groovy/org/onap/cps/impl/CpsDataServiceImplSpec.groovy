...
class CpsDataServiceImplSpec extends Specification {
    ...
    def 'Replace data node when delta notification is enabled'() {
        ...
        and: 'delta service returns delta report for updated data'
            def dataNodes = [new DataNodeBuilder().withAnchor(anchorName).withXpath('/xpath').build()]
            mockCpsDataPersistenceService.getDataNodes(dataspaceName, anchorName, '/', FetchDescendantsOption.INCLUDE_ALL_DESCENDANTS) >> dataNodes
        when: 'replace data method is invoked with json data #jsonData and parent node xpath #parentNodeXpath'
        ...
        then: 'the persistence service method is invoked with correct parameters'
            1 * mockCpsDataUpdateEventsProducer.sendCpsDataUpdateEvent(_, dataNodes, '/', REPLACE, observedTimestamp)
    }
