...
class CompositeQueryOperatorSpec extends Specification {
    ...
    static def AND = CompositeQueryOperator.AND
    static def OR = CompositeQueryOperator.OR
    static final String XPATH_1 = '/bookstore/categories[@code=1]'
    static final String XPATH_2 = '/bookstore/categories[@code=2]'
    static final String XPATH_3 = '/bookstore/categories[@code=3]'

    static def objectUnderTest = CompositeQueryOperator

    def dataNodes = [(XPATH_1): new DataNode(xpath: XPATH_1), (XPATH_2): new DataNode(xpath: XPATH_2), (XPATH_3): new DataNode(xpath: XPATH_3)]
    ...
    def '#scenario operator with empty conditions.'() {
        ...
        and: 'a condition evaluator'
            def evaluator = Mock(CompositeQueryOperator.ConditionEvaluator)
        when: 'the operator is evaluated'
            def result = operator.evaluateOperator(conditions, dataNodes, 'ds', 'anchor', evaluator)
        then: 'all xpaths from the flat map are returned'
            result == dataNodes.keySet()
        and: 'the evaluator is never called'
            0 * evaluator.evaluate(_, _, _, _)
        where: 'the following operators are used'
            scenario | operator
    ...
    def 'AND operator with overlapping condition results.'() {
        ...
        and: 'evaluator returns overlapping sets'
            def evaluator = Mock(CompositeQueryOperator.ConditionEvaluator)
            evaluator.evaluate(condition1, dataNodes, 'ds', 'anchor') >> ([XPATH_1, XPATH_2] as Set)
            evaluator.evaluate(condition2, dataNodes, 'ds', 'anchor') >> ([XPATH_2, XPATH_3] as Set)
        when: 'AND operator is evaluated'
            def result = objectUnderTest.AND.evaluateOperator(conditions, dataNodes, 'ds', 'anchor', evaluator)
    ...
    def 'AND operator when #scenario.'() {
        given: 'two conditions'
            ...
            def condition2 = CompositeQuery.builder().cpsPath('//NodeB').build()
            def conditions = [condition1, condition2]
        and: 'evaluator returns results for each condition'
            def evaluator = Mock(CompositeQueryOperator.ConditionEvaluator)
            evaluator.evaluate(condition1, dataNodes, 'ds', 'anchor') >> resultOfCondition1
            evaluator.evaluate(condition2, dataNodes, 'ds', 'anchor') >> resultOfCondition2
        when: 'AND operator is evaluated'
            def result = objectUnderTest.AND.evaluateOperator(conditions, dataNodes, 'ds', 'anchor', evaluator)
    ...
    def 'OR operator with #scenario.'() {
        ...
        when: 'OR operator is evaluated'
            def result = objectUnderTest.OR.evaluateOperator(conditions, dataNodes, 'ds', 'anchor', evaluator)
