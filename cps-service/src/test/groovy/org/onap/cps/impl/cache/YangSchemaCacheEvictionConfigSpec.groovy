...
class YangSchemaCacheEvictionConfigSpec extends Specification {
    ...
    def 'Yang schema cache eviction topic'() {
        given: 'retrieving the map config'
            ...
            def topicConfig = hazelcastInstance.getConfig().findTopicConfig(yangSchemaCacheEvictionTopic.getName())
        expect: 'the topic config is the default topic'
            assert topicConfig.name == 'default'
    }
