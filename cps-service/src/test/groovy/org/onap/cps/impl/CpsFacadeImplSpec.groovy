...
class CpsFacadeImplSpec extends Specification {
    ...
    def 'Execute composite search query.'() {
        given: 'a JSON search body representing a composite query'
            def compositeQueryBody = '{"cpsPath":"/my/path","operator":"and","conditions":[]}'
        ...
        when: 'composite search query is invoked'
            def result = objectUnderTest.searchDataNodes('my dataspace', 'my anchor', compositeQueryBody, myFetchDescendantsOption)
        then: 'data nodes from query service are returned'
            assert result.size() == 2
            assert result[0].keySet()[0] == 'prefix1:path1'
            assert result[1].keySet()[0] == 'prefix2:path2'
    }
    ...
    def 'Execute search query deserializes the JSON body into a Composite Query.'() {
        ...
        when: 'composite query search is invoked'
            objectUnderTest.searchDataNodes('my dataspace', 'my anchor', compositeQueryBody, myFetchDescendantsOption)
