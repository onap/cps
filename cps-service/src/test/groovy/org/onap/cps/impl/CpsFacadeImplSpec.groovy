...
class CpsFacadeImplSpec extends Specification {
    ...
    def 'Execute search query.'() {
        ...
        when: 'executeSearchQuery is invoked'
            def result = objectUnderTest.executeSearchQuery('my dataspace', 'my anchor', searchBody, myFetchDescendantsOption)
        then: 'all nodes (from the query service result) are returned'
            assert result.size() == 2
            assert result[0].keySet()[0] == 'prefix1:path1'
            assert result[1].keySet()[0] == 'prefix2:path2'
    ...
    def 'Execute search query deserializes the JSON body into a CompositeQuery.'() {
        given: 'a JSON search body with a specific cps path'
            def searchBody = '{"cpsPath":"/search/path","operator":"or","conditions":[]}'
        when: 'executeSearchQuery is invoked'
            objectUnderTest.executeSearchQuery('my dataspace', 'my anchor', searchBody, myFetchDescendantsOption)
