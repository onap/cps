...
class CpsDeltaServiceImplSpec extends Specification{
    ...
    def 'Delta report between leaves, #scenario'() {
        ...
        where: 'the following data was used'
            scenario                                           | sourceDataNode                   | targetDataNode                   | groupingEnabled || expectedSourceData                                             | expectedTargetData
            'source and target data nodes have leaves'         | sourceDataNodeWithLeafData       | targetDataNodeWithLeafData       | false           || [['parent-leaf': 'parent-payload-in-source']]                  | [['parent-leaf': 'parent-payload-in-target']]
            'only source data node has leaves'                 | sourceDataNodeWithLeafData       | targetDataNodeWithoutLeafData    | false           || [['parent-leaf': 'parent-payload-in-source']]                  | null
            'only target data node has leaves'                 | sourceDataNodeWithoutLeafData    | targetDataNodeWithLeafData       | false           || null                                                           | [['parent-leaf': 'parent-payload-in-target']]
            'source and target dsta node with multiple leaves' | sourceDataNodeWithMultipleLeaves | targetDataNodeWithMultipleLeaves | false           || [['leaf-1': 'leaf-1-in-source', 'leaf-2': 'leaf-2-in-source']] | [['leaf-1': 'leaf-1-in-target', 'leaf-2': 'leaf-2-in-target']]
    }
    ...
    def 'Get delta between data nodes for updated data,  '() {
        ...
        where:
            ...
            'where target data node is empty'                   | sourceDataNodeWithoutLeafData | []                            | true
            'source and target have data'                       | sourceDataNodeWithLeafData    | targetDataNodeWithLeafData    | true
    }
}
