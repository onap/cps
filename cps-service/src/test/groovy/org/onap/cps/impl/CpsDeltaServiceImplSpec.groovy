...
class CpsDeltaServiceImplSpec extends Specification {
    ...
    def 'Get delta between 2 anchors with an unexpected checked exception during async data fetch'() {
        given: 'xpath to get delta'
            def xpath = '/'
        ...
        and: 'the expected CpsException is thrown with details about the operation'
            def thrownException = thrown(CpsException)
            assert thrownException.message == 'Failed to compute delta between anchors'
            assert thrownException.details.contains(dataspaceName)
