...
class CpsDeltaServiceImplSpec extends Specification {
    ...
    def 'Get delta between 2 anchors where data node with children is #scenario without grouping'() {
        given: 'xpath to get delta'
            def xpath = '/'
        when: 'attempt to get delta between 2 anchors'
        ...
        and: 'cps data service is invoked again to return target data nodes'
            mockCpsDataService.getDataNodesForMultipleXpaths(dataspaceName, ANCHOR_NAME_2, [xpath], OMIT_DESCENDANTS) >> targetDataNodes
        and: 'the delta report contains expected number of entries for parent and child'
            deltaReport.size() == expectedSize
        and: 'the delta report entries have expected actions'
            deltaReport[0].action == expectedParentAction
            deltaReport[0].xpath == '/parent'
        where: 'following data was used'
            scenario  | sourceDataNodes                                                                                                                                                                             | targetDataNodes                                                                                                                                                                             || expectedSize | expectedParentAction
            'removed' | [new DataNode(xpath: '/parent', leaves: ['parent-leaf': 'parent-leaf-as-source-data'], childDataNodes: [new DataNode(xpath: '/parent/child', leaves: ['child-leaf': 'child-leaf-as-source-data'])])] | []                                                                                                                                                                                          || 2            | 'remove'
            'added'   | []                                                                                                                                                                                          | [new DataNode(xpath: '/parent', leaves: ['parent-leaf': 'parent-leaf-as-target-data'], childDataNodes: [new DataNode(xpath: '/parent/child', leaves: ['child-leaf': 'child-leaf-as-target-data'])])] || 2            | 'create'
    }
    ...
    def 'Delta Report between parent nodes with children where data node is #scenario without grouping of data nodes'() {
        ...
        then: 'the delta persistence service is invoked and returns delta reports'
            1 * mockCpsDeltaPersistenceService.getDeltaByDataspaceAndAnchors(dataspaceName, ANCHOR_NAME_1, ANCHOR_NAME_2, xpath) >> expectedDeltaReports
    ...
    when: 'attempt to get delta between 2 anchors'
        def deltaReport = objectUnderTest.getDeltaByDataspaceAndAnchors(dataspaceName, ANCHOR_NAME_1, ANCHOR_NAME_2, xpath, INCLUDE_ALL_DESCENDANTS, GROUPING_DISABLED)
    then: 'the delta persistence service is invoked and returns delta reports'
        1 * mockCpsDeltaPersistenceService.getDeltaByDataspaceAndAnchors(dataspaceName, ANCHOR_NAME_1, ANCHOR_NAME_2, xpath) >> [createDeltaReport('replace', '/parent', expectedSourceData, expectedTargetData)]
    ...
    def 'Get delta between data nodes for updated data, where source and target data nodes have no leaves '() {
        ...
        then: 'the delta persistence service is invoked and returns empty delta reports'
            1 * mockCpsDeltaPersistenceService.getDeltaByDataspaceAndAnchors(dataspaceName, ANCHOR_NAME_1, ANCHOR_NAME_2, xpath) >> []
