...
class CpsDeltaServiceImplSpec extends Specification {
    ...
    def groupDataNodesTrue = true
    def groupDataNodesFalse = false
    ...
    def 'Get Delta between 2 anchors for #scenario with grouping disabled'() {
        given: 'xpath to get delta'
            def xpath = '/'
        when: 'attempt to get delta between 2 anchors'
            def deltaReport = objectUnderTest.getDeltaByDataspaceAndAnchors(dataspaceName, ANCHOR_NAME_1, ANCHOR_NAME_2, xpath, OMIT_DESCENDANTS, groupDataNodesFalse)
    ...
    def 'Delta Report between parent nodes containing child nodes where #scenario with grouping disabled'() {
        given: 'root node xpath'
            def xpath = '/'
        when: 'attempt to get delta between 2 anchors'
            def deltaReport = objectUnderTest.getDeltaByDataspaceAndAnchors(dataspaceName, ANCHOR_NAME_1, ANCHOR_NAME_2, xpath, INCLUDE_ALL_DESCENDANTS, groupDataNodesFalse)
        then: 'cps data service is invoked and returns source data nodes'
    ...
    def 'Get Delta Report between with grouping enabled between #scenario'() {
        given: 'xpath and source data node'
            ...
            def sourceDataNodes  = []
        when: 'attempt to get delta between 2 anchors'
            def deltaReport = objectUnderTest.getDeltaByDataspaceAndAnchors(dataspaceName, ANCHOR_NAME_1, ANCHOR_NAME_2, xpath, INCLUDE_ALL_DESCENDANTS, groupDataNodesTrue)
        then: 'cps data service is invoked and returns source data nodes'
            mockCpsDataService.getDataNodesForMultipleXpaths(dataspaceName, ANCHOR_NAME_1, [xpath], INCLUDE_ALL_DESCENDANTS) >> sourceDataNodes
        and: 'cps data service is invoked again to return target data nodes'
            mockCpsDataService.getDataNodesForMultipleXpaths(dataspaceName, ANCHOR_NAME_2, [xpath], INCLUDE_ALL_DESCENDANTS) >> targetDataNodes
        ...
        where: 'following data was used'
            ...
            'Data node with parent node xpath' | targetDataNodeWithChild                                                                     || ['parent':['parent-leaf': 'parent-leaf-as-target-data', 'child':['child-leaf': 'child-leaf-as-target-data']]]
            'Data node with xpath'             | [new DataNode(xpath: '/parent/child', leaves: ['child-leaf': 'child-leaf-as-target-data'])] || ['child':['child-leaf': 'child-leaf-as-target-data']]
    }
    ...
    def 'Get delta between anchor and payload with user provided schema #scenario'() {
        ...
        when: 'attempt to get delta between an anchor and a JSON payload'
            def deltaReport = objectUnderTest.getDeltaByDataspaceAnchorAndPayload(dataspaceName, ANCHOR_NAME_1, xpath, yangResourceContentPerName, jsonData, INCLUDE_ALL_DESCENDANTS, groupDataNodesFalse)
        then: 'cps data service is invoked and returns source data nodes'
            mockCpsDataService.getDataNodesForMultipleXpaths(dataspaceName, ANCHOR_NAME_1, [xpath], INCLUDE_ALL_DESCENDANTS) >> sourceDataNodes
    ...
    def 'Get delta between anchor and payload by using schema from anchor #scenario'() {
        ...
        when: 'attempt to get delta between an anchor and a JSON payload'
            def deltaReport = objectUnderTest.getDeltaByDataspaceAnchorAndPayload(dataspaceName, ANCHOR_NAME_1, xpath, [:], jsonData, INCLUDE_ALL_DESCENDANTS, groupDataNodesFalse)
        then: 'cps data service is invoked and returns source data nodes'
            mockCpsDataService.getDataNodesForMultipleXpaths(dataspaceName, ANCHOR_NAME_1, [xpath], INCLUDE_ALL_DESCENDANTS) >> sourceDataNodes
        and: 'source data nodes are rebuilt (to match the data type with target data nodes)'
            dataNodeFactory.createDataNodesWithAnchorXpathAndNodeData(anchor1, xpath, jsonObjectMapper.asJsonString(sourceDataNodesAsMap), ContentType.JSON)
    ...
    def 'Delta between anchor and payload error scenario #scenario'() {
        ...
        when: 'attempt to get delta between anchor and payload'
            objectUnderTest.getDeltaByDataspaceAnchorAndPayload(dataspaceName, ANCHOR_NAME_1, xpath, yangResourceContentPerName, jsonData, INCLUDE_ALL_DESCENDANTS, groupDataNodesFalse)
