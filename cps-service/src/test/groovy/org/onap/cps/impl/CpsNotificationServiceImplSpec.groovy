...
class CpsNotificationServiceImplSpec extends Specification {
    ...
    def 'get notification subscription for given xpath'() {
        ...
        when: 'delete notification subscription is called'
            def notificationSubscriptionList =  objectUnderTest.getNotificationSubscription(xpath)
        then: 'the cps notification service return list of subscribed data'
            assert notificationSubscriptionList.get(0).toString() == '{path={leaf=dataspace, leafList=[ds01, ds02]}}'
    }
