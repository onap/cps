...
class CpsDataUpdateEventFactorySpec extends Specification {
    ...
    def cpsDataUpdatedEventFactory = new CpsDataUpdatedEventFactory(mockCpsDataService, mockCpsAdminService)
    ...
    def 'create a CPSDataUpdatedEvent successfully.'() {
        ...
        when: 'CPSDataUpdatedEvent is created'
            def cpsDataUpdatedEvent = cpsDataUpdatedEventFactory.createCpsDataUpdatedEvent(myDataspaceName, myAnchorName)

        then: 'CPSDataUpdatedEvent is created with expected values'
            ...
            with(cpsDataUpdatedEvent.content) {
                assert isValidDateTimeFormat(observedTimestamp): "$observedTimestamp is not in $dateTimeFormat format"
