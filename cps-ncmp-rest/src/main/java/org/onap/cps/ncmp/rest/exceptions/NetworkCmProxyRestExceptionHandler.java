...
public class NetworkCmProxyRestExceptionHandler {
    ...
    public static ResponseEntity<Object> handleBadGatewayExceptions(final Exception exception) {
        return buildErrorResponse(HttpStatus.BAD_GATEWAY, exception);
    }
    ...
    private static ResponseEntity<Object> buildErrorResponse(final HttpStatus status, final Exception exception) {
        ...
        if (exception instanceof HttpRequestException) {
            return wrapDmiErrorResponse(status, exception);
        }
    ...
    private static ResponseEntity<Object> wrapDmiErrorResponse(final HttpStatus status, final Exception exception) {
        final var dmiErrorMessage = new DmiErrorMessage();
        final var dmiErrorResponse = new DmiErrorMessageDmiresponse();
        if (exception instanceof HttpRequestException) {
            dmiErrorResponse.setHttpCode(((HttpRequestException) exception).getHttpStatus());
            dmiErrorResponse.setBody(((HttpRequestException) exception).getBody());
        }
        dmiErrorMessage.setMessage(exception.getMessage());
        dmiErrorMessage.setDmiResponse(dmiErrorResponse);
        return new ResponseEntity<>(dmiErrorMessage, status);
    }
}
