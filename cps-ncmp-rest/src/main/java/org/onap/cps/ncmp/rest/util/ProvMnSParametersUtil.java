...
public class ProvMnSParametersUtil {
    ...
    public ResponseEntity<Object> checkDataProducerIdentifierAndReadyState(final YangModelCmHandle yangModelCmHandle,
        ...
        if (yangModelCmHandle.getDataProducerIdentifier() == null
            ...
            errorResponseMessage = provMnSErrorResponseBuilder.buildNotReadyStateMessage(yangModelCmHandle);
        }

        if (errorResponseHttpStatus != HttpStatus.OK) {
            return switch (type) {
                case "GET" -> provMnSErrorResponseBuilder.buildErrorResponseGet(errorResponseHttpStatus,
                    errorResponseMessage);
                case "PATCH" -> provMnSErrorResponseBuilder.buildErrorResponsePatch(errorResponseHttpStatus,
                    errorResponseMessage);
                default -> provMnSErrorResponseBuilder.buildErrorResponseDefault(errorResponseHttpStatus,
                    errorResponseMessage);
            };
        }
        return null;
     ...
     * @param provMnsRequestParameters        request parameters including uri-ldn-first-part, className and id
     * @param resource                        provided request resource
     * @return JSON string
     */
    public String policyExecutorOperationToJson(final String operation,
        ...
        policyExecutorOperationDetails.setTargetIdentifier(provMnsRequestParameters.getAlternateId());
        ...
        try {
            ...
            policyExecutorOperationEntry.setId(provMnsRequestParameters.getId());
            policyExecutorOperationEntry.setAttributes(fullValue.get("attributes"));
        } catch (final JsonProcessingException exception) {
            log.debug("JSON processing error: {}", exception);
        }

        changeRequest.put(provMnsRequestParameters.getClassName(), List.of(policyExecutorOperationEntry));
