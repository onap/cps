...
public class ProvMnSController implements ProvMnS {
    ...
    private void checkPermissionForEachPatchItem(final String baseFdn,
        ...
        for (final PatchItem patchItem : patchItems) {
            ...
            try {
                final String resourceIdentifier = ParameterHelper.removeTrailingHash(requestParameters.fdn());
                checkPermission(yangModelCmHandle, resourceIdentifier, operationDetails);
                patchItemCounter++;
            } catch (final Exception exception) {
