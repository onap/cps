    ...
    public void executeTask(final Supplier<Object> taskSupplier) {
        CompletableFuture.supplyAsync(taskSupplier::get)
            ...
            .whenCompleteAsync((responseAsJson, error) -> {
                if (error == null) {
                    log.info("Request acknowledgement received {}", responseAsJson);
                } else {
                    throw new CpsTaskExecutionException("Error receiving acknowledgement.", error.getMessage(), error);
                }
            });
    }
}
