...
public class NetworkCmProxyRestExceptionHandler {
    ...
    public static ResponseEntity<ErrorResponseDefault> handleTypeMismatchExceptions(
        ...
        final ErrorResponseDefault errorResponse = new ErrorResponseDefault()
                .status(String.valueOf(HttpStatus.BAD_REQUEST.value()))
                .type("VALIDATION_ERROR")
                .title(exception.getMessage())
                .addBadAttributesItem(exception.getName());

        return new ResponseEntity<>(errorResponse, HttpStatus.BAD_REQUEST);
    }

    private static ResponseEntity<ErrorResponseDefault> buildValidationErrorResponse(
            final MethodArgumentNotValidException exception) {
        ...
        exception.getBindingResult().getFieldErrors()
                .forEach(err -> errorResponse.addBadAttributesItem(err.getField()));

        exception.getBindingResult().getFieldErrors()
                .stream()
                .findFirst()
                .ifPresent(err -> errorResponse.title(exception.getMessage()));
