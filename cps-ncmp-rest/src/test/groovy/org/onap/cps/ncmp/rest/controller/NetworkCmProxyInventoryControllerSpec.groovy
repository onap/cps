...
class NetworkCmProxyInventoryControllerSpec extends Specification {
    ...
    def 'Get all cm handles by DMI plugin identifier.'() {
        given: 'an endpoint for returning CM-Handles for a registered dmi plugin'
            def getUrl = "$ncmpBasePathV1/ch/cmHandles?dmi-plugin-identifier=some-dmi-plugin-identifier"
        and: 'a collection of CM-Handles are returned'
            1 * mockNetworkCmProxyDataService.getAllCmHandlesByDmiPluginIdentifier('some-dmi-plugin-identifier')
                    >> [new NcmpServiceCmHandle(cmHandleId: 'cm-handle-1'),new NcmpServiceCmHandle(cmHandleId: 'cm-handle-2')]
        ...
        then: 'the response matches the result returned by the service layer'
            assert response.contentAsString.contains('cm-handle-1')
            assert response.contentAsString.contains('cm-handle-2')
