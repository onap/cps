...
class NetworkCmProxyInventoryControllerSpec extends Specification {
    ...
    def 'Get a cm handle by dmi service name.'() {
        given: 'an endpoint for returning cm handles by dmi service name'
            def postUrl = "$ncmpBasePathV1/ch/cmHandlesByDmiService?includePrivateProperties=true&limit=0"
        ...
        and: 'a cm handle is returned'
            ...
            mockNetworkCmProxyInventoryFacade.executeCmHandleSearch(_) >> Flux.fromIterable([ncmpServiceCmHandle])
        and: 'the mapper is requested to convert the object with private properties'
            restOutputCmHandleMapper.toRestOutputCmHandle(ncmpServiceCmHandle, true) >> new RestOutputCmHandle(privateCmHandleProperties: ['someName': 'my dmi'])
        when: 'the endpoint is invoked'
            def response = mvc.perform(post(postUrl).contentType(MediaType.APPLICATION_JSON).content(jsonString)).andReturn().response
        then: 'the response contains the private property'
            assert response.contentAsString.contains('my dmi')
    }
