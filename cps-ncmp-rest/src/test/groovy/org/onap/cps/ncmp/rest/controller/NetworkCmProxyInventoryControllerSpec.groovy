...
class NetworkCmProxyInventoryControllerSpec extends Specification {
    ...
    def 'Dmi plugin registration with valid json but undefined data label is accepted (unknown fields are ignored).'() {
        given: 'a dmi plugin registration with a valid json but undefined data label'
        ...
        then: 'response status is ok since valid json with undefined data label is accepted'
    ...
    def 'Dmi plugin registration with badly formed json.'() {
        ...
        when: 'post request is performed'
            def response = mvc.perform(
                post("$ncmpBasePathV1/ch")
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(badlyFormedJson)
            ).andReturn().response
        then: 'response status is BAD_REQUEST'
            assert response.status == HttpStatus.BAD_REQUEST.value()
    }

    def 'Dmi plugin registration with null response from service.'() {
        given: 'a valid dmi plugin registration'
            def jsonData = '{"dmiPlugin":"my-dmi"}'
        and: 'the registration service returns null'
            mockNetworkCmProxyInventoryFacade.updateDmiRegistration(_) >> null
        when: 'post request is performed'
            def response = mvc.perform(
                post("$ncmpBasePathV1/ch")
                    .contentType(MediaType.APPLICATION_JSON)
    ...
    def 'Get cm handle details by reference (distinguished name) when #scenario.'() {
        ...
        when: 'the endpoint is invoked'
            def response = mvc.perform(get("$ncmpBasePathV1/ch?distinguishedName=$cmHandleReference$additionalQueryParameter").accept(MediaType.APPLICATION_JSON_VALUE)).andReturn().response
        ...
        where: 'the following additional query parameters are used'
            scenario                                    | additionalQueryParameter     || expectedIncludeAdditionalProperties
