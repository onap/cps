...
class ProvMnSParametersMapperSpec extends Specification{
    ...
    def objectUnderTest = new ProvMnSParametersMapper(new JsonObjectMapper(new ObjectMapper()), new ObjectMapper())
    ...
    def 'State validation with Advised state.'() {
        given:'a yangModelCmHandle'
    ...
    def 'Convert a configurationManagementOperation to json string'() {
        ...
        when: 'a configurationManagementOperation is created and converted to JSON'
            def result = objectUnderTest.configurationManagementOperationToJson('create', path, resource)
        then:
