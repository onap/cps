...
import org.onap.cps.ncmp.rest.executor.CpsTaskExecutor
...
class NetworkCmProxyControllerSpec extends Specification {
    ...
    @SpringBean
    CpsTaskExecutor cpsTaskExecutor = Spy()
    ...
    def 'Get Resource Data from #datastoreInUrl with #scenario.'() {
        ...
        then: 'the NCMP data service is called with operational data for cm handle'
                expectedNumberOfMethodExecutions
                    * mockNetworkCmProxyDataService."${expectedMethodName}"(
                'testCmHandle', 'parent/child',
                '(a=1,b=2)', expectedTopicName, _)
        and: 'task executor is called appropriate number of times'
            expectedNumberOfExecutorExecutions * cpsTaskExecutor.executeTask(_)
        then: 'response status is expected'
            response.status == expectedHttpStatus
        where: 'the following parameters are used'
            scenario                               | datastoreInUrl            | topicQueryParam        || expectedTopicName | expectedMethodName                             | expectedNumberOfExecutorExecutions | expectedNumberOfMethodExecutions | expectedHttpStatus
            'url with valid topic'                 | 'passthrough-operational' | '&topic=my-topic-name' || 'my-topic-name'   | 'getResourceDataOperationalForCmHandle'        | 1                                | 1                                | HttpStatus.OK.value()
            'no topic in url'                      | 'passthrough-operational' | ''                     || NO_TOPIC          | 'getResourceDataOperationalForCmHandle'        | 0                                | 1                                | HttpStatus.OK.value()
            'null topic in url'                    | 'passthrough-operational' | '&topic=null'          || 'null'            | 'getResourceDataOperationalForCmHandle'        | 0                                | 0                                | HttpStatus.BAD_REQUEST.value()
            'empty topic in url'                   | 'passthrough-operational' | '&topic=\"\"'          || null              | 'getResourceDataOperationalForCmHandle'        | 0                                | 0                                | HttpStatus.BAD_REQUEST.value()
            'missing topic in url'                 | 'passthrough-operational' | '&topic='              || null              | 'getResourceDataOperationalForCmHandle'        | 0                                | 0                                | HttpStatus.BAD_REQUEST.value()
            'blank topic value in url'             | 'passthrough-operational' | '&topic=\" \"'         || null              | 'getResourceDataOperationalForCmHandle'        | 0                                | 0                                | HttpStatus.BAD_REQUEST.value()
            'invalid non-empty topic value in url' | 'passthrough-operational' | '&topic=1_5_*_#'       || null              | 'getResourceDataOperationalForCmHandle'        | 0                                | 0                                | HttpStatus.BAD_REQUEST.value()
            'url with valid topic'                 | 'passthrough-running'     | '&topic=my-topic-name' || 'my-topic-name'   | 'getResourceDataPassThroughRunningForCmHandle' | 1                                | 1                                | HttpStatus.OK.value()
            'no topic in url'                      | 'passthrough-running'     | ''                     || NO_TOPIC          | 'getResourceDataPassThroughRunningForCmHandle' | 0                                | 1                                | HttpStatus.OK.value()
            'null topic in url'                    | 'passthrough-running'     | '&topic=null'          || 'null'            | 'getResourceDataPassThroughRunningForCmHandle' | 0                                | 0                                | HttpStatus.BAD_REQUEST.value()
            'empty topic in url'                   | 'passthrough-running'     | '&topic=\"\"'          || null              | 'getResourceDataPassThroughRunningForCmHandle' | 0                                | 0                                | HttpStatus.BAD_REQUEST.value()
            'missing topic in url'                 | 'passthrough-running'     | '&topic='              || null              | 'getResourceDataPassThroughRunningForCmHandle' | 0                                | 0                                | HttpStatus.BAD_REQUEST.value()
            'blank topic value in url'             | 'passthrough-running'     | '&topic=\" \"'         || null              | 'getResourceDataPassThroughRunningForCmHandle' | 0                                | 0                                | HttpStatus.BAD_REQUEST.value()
            'invalid non-empty topic value in url' | 'passthrough-running'     | '&topic=1_5_*_#'       || null              | 'getResourceDataPassThroughRunningForCmHandle' | 0                                | 0                                | HttpStatus.BAD_REQUEST.value()
    }
