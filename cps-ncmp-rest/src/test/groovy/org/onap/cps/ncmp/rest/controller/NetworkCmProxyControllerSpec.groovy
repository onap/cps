...
class NetworkCmProxyControllerSpec extends Specification {
    ...
    def jsonData = groovy.json.JsonOutput.toJson(jsonString)
        ...
        when: 'update data resource request is performed'
            def response = mvc.perform(
                put(updateUrl)
                    ...
                    .accept(MediaType.APPLICATION_JSON_VALUE).content(jsonData)
            ).andReturn().response
    ...
    def 'Create Resource Data from pass-through running with #scenario.' () {
        ...
        when: 'create resource request is performed'
            def response = mvc.perform(
                    post(url)
                            .contentType(MediaType.APPLICATION_JSON_VALUE)
                            .accept(MediaType.APPLICATION_JSON_VALUE).content(groovy.json.JsonOutput.toJson(requestBody))
            ).andReturn().response
        ...
        where: 'given request body'
            ...
            'body contains " and new line'  |  '{"some-"key" : "/nsome-value"}'
