...
class NetworkCmProxyControllerSpec extends Specification {
    ...
    def testCompositeState = new CompositeState(cmHandleState: CmHandleState.ADVISED,
    ...
    def 'Execute (async) data operation to read data from dmi service.'() {
        ...
        when: 'post data operation request is performed'
            def response = mvc.perform(post(getUrl).contentType(MediaType.APPLICATION_JSON).content(dataOperationRequestAsJsonString)).andReturn().response
    ...
    def 'Update resource data from pass-through running.'() {
        ...
        when: 'update data resource request is performed'
            def response = mvc.perform(put(updateUrl).contentType(MediaType.APPLICATION_JSON_VALUE).content(validRequestBody)).andReturn().response
    ...
    def 'Create Resource Data from pass-through running with #scenario.'() {
        ...
        when: 'create resource request is performed'
            def response = mvc.perform(post(url).contentType(MediaType.APPLICATION_JSON_VALUE).content(validRequestBody)).andReturn().response
    ...
    def 'Execute cm handle search.'() {
        ...
        when: 'the search endpoint is invoked'
            def response = mvc.perform(post(searchesEndpoint).contentType(MediaType.APPLICATION_JSON).content(validSearchRequest)).andReturn().response
        then: 'the response status is OK'
            assert response.status == HttpStatus.OK.value()
    ...
    def 'Query for cm handles with invalid request payload'() {
        when: 'the searches api is invoked'
            def searchesEndpoint = "$ncmpBasePathV1/ch/id-searches"
            def response = mvc.perform(post(searchesEndpoint).contentType(MediaType.APPLICATION_JSON).content('{invalidJson}')).andReturn().response
        then: 'the response status is Bad Request'
            assert response.status == HttpStatus.BAD_REQUEST.value()
    }
    ...
    def 'Patch resource data in pass-through running datastore.'() {
        ...
        when: 'patch data resource request is performed'
            def response = mvc.perform(patch(url).contentType(MediaType.APPLICATION_JSON).accept(MediaType.APPLICATION_JSON).content(validRequestBody)).andReturn().response
    ...
    def executeRestOperation(operation, url) {
        if (operation == 'POST') {
            return post(url).contentType(MediaType.APPLICATION_JSON_VALUE).content(validRequestBody)
        }
        if (operation == 'PUT') {
            return put(url).contentType(MediaType.APPLICATION_JSON_VALUE).content(validRequestBody)
        }
        if (operation == 'PATCH') {
            return patch(url).contentType(MediaType.APPLICATION_JSON_VALUE).content(validRequestBody)
        }
        if (operation == 'DELETE') {
