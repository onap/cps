...
class NetworkCmProxyControllerSpec extends Specification {
    ...
    def 'Set the data sync enabled based on the cm handle id and the enabled flag is #scenario' () {
        when: 'the set data sync enabled request is invoked'
            def response = mvc.perform(put("$ncmpBasePathV1/ch/some-cm-handle-id/data-sync?enabled=" + dataSyncEnabledFlag))
                    .andReturn().response
        then: 'ncmp service method to set data sync enabled is called'
