...
class NetworkCmProxyControllerSpec extends Specification {
    ...
    def 'Get Cm Handle details by Cm Handle id.'() {
        ...
        and: 'the response returns public properties and the correct cm handle states'
            ...
            response.contentAsString.contains('state')
    ...
    def 'Get Cm Handle composite state by Cm Handle id.' () {
        ...
        then: 'the correct response is returned'
            response.status == HttpStatus.OK.value()
        and: 'the response returns state with the correct properties'
            response.contentAsString.contains('{"state":{')
            response.contentAsString.contains('"cmHandleState":"ADVISED"')
            response.contentAsString.contains('"reason":"LOCKED_MISBEHAVING"')
            response.contentAsString.contains('"details":"lock misbehaving details"')
            response.contentAsString.contains('"lastUpdateTime":"some-timestamp"')
            response.contentAsString.contains('"dataSyncEnabled":false')
            response.contentAsString.contains('"dataSyncState":')
            response.contentAsString.contains('"operational":{"state":"NONE_REQUESTED"')
            response.contentAsString.contains('"lastSyncTime":"2022-12-31T20:30:40.000+0000"')
            response.contentAsString.contains('"running":null')
    }
