...
class NetworkCmProxyControllerSpec extends Specification {
    ...
    def 'Get Cm Handle details by Cm Handle name.' () {
        given: 'an endpoint and a cm handle'
            ...
            ncmpServiceCmHandle.publicProperties = [Book:'Romance Novel']
            def yangModelCmHandle = YangModelCmHandle.toYangModelCmHandle('','','', ncmpServiceCmHandle)
            def cmHandleDetailsEndpoint = "$ncmpBasePathV1/ch/testCmHandle"
        and: 'the service method is invoked with the cm handle name'
            1 * mockNetworkCmProxyDataService.getCmHandleDetails('testCmHandle') >>  yangModelCmHandle
