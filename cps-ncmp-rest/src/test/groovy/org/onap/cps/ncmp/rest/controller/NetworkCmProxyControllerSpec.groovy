...
class NetworkCmProxyControllerSpec extends Specification {
    ...
    def 'Get Resource Data from pass-through operational.'() {
        ...
        and: 'the expected request target'
            def expectedRequestTarget = new RequestTarget(PASSTHROUGH_OPERATIONAL.datastoreName, 'testCmHandle', 'parent/child')
        ...
        then: 'the NCMP data service is called with getResourceDataOperationalForCmHandle'
            1 * mockNetworkCmProxyDataService.getResourceDataForCmHandle(expectedRequestTarget, '(a=1,b=2)', NO_TOPIC, NO_REQUEST_ID, NO_AUTH_HEADER)
    ...
    def 'Get Resource Data from ncmp-datastore:operational (cached) parameters handling with #scenario.'() {
        ...
        and: 'the expected request target'
            def expectedRequestTarget = new RequestTarget('ncmp-datastore:operational', 'h123', 'parent/child')
        when: 'get data resource request is performed'
            def response = mvc.perform(get(getUrl).contentType(MediaType.APPLICATION_JSON)).andReturn().response
        then: 'task executor is called appropriate number of times'
            1 * mockNetworkCmProxyDataService.getResourceDataForCmHandle(expectedRequestTarget, expectedIncludeDescendants)
        and: 'response status is OK'
            assert response.status == HttpStatus.OK.value()
    ...
    def 'Get Resource Data from pass-through running with #scenario value in resource identifier param.'() {
        ...
        and: 'ncmp service returns json object'
            def expectedRequestTarget = new RequestTarget(PASSTHROUGH_RUNNING.datastoreName, 'testCmHandle', resourceIdentifier)
            mockNetworkCmProxyDataService.getResourceDataForCmHandle(expectedRequestTarget,'(a=1,b=2)', NO_TOPIC, NO_REQUEST_ID, NO_AUTH_HEADER) >> '{valid-json}'
        when: 'get data resource request is performed'
            def response = mvc.perform(get(getUrl).contentType(MediaType.APPLICATION_JSON)).andReturn().response
        then: 'response status is Ok'
    ...
    def 'Get Resource Data from operational with or without descendants'() {
        ...
        and: 'the expected target'
            def expectedRequestTarget = new RequestTarget(OPERATIONAL.datastoreName, 'testCmHandle', 'parent/child')
        when: 'get data resource request is performed'
            def response = mvc.perform(get(getUrl).contentType(MediaType.APPLICATION_JSON)).andReturn().response
        then: 'the NCMP data service is called with getResourceDataOperational with #descendantsOption'
            1 * mockNetworkCmProxyDataService.getResourceDataForCmHandle(expectedRequestTarget, descendantsOption)
        and: 'response status is Ok'
            assert response.status == HttpStatus.OK.value()
        where: 'the following parameters are used'
