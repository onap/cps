...
class NetworkCmProxyControllerSpec extends Specification {
    ...
    @SpringBean
    NetworkCmProxyDataService mockNCmProxyDataService = Mock()
    ...
    def 'Query data node by cps path for the given cm handle with #scenario.'() {
        given: 'service method returns a list containing a data node'
            ...
            mockNCmProxyDataService.queryDataNodes(cmHandle, cpsPath, expectedCpsDataServiceOption) >> [dataNode]
    ...
    def 'Update data node leaves.'() {
        ...
        then: 'the service method is invoked once with expected parameters'
            1 * mockNCmProxyDataService.updateNodeLeaves(cmHandle, xpath, jsonData)
    ...
    def 'Replace data node tree.'() {
        ...
        then: 'the service method is invoked once with expected parameters'
            1 * mockNCmProxyDataService.replaceNodeTree(cmHandle, xpath, jsonData)
        and: 'response status indicates success'
            response.status == HttpStatus.OK.value()
    }
    ...
    def 'Get data node.'() {
        given: 'the service returns a data node'
            ...
            mockNCmProxyDataService.getDataNode(cmHandle, xpath, OMIT_DESCENDANTS) >> dataNode
        and: 'the query endpoint'
