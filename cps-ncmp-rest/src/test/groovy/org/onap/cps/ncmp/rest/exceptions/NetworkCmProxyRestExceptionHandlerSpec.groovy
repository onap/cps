...
import static org.onap.cps.ncmp.rest.exceptions.NetworkCmProxyRestExceptionHandlerSpec.MethodType.NCMP
import static org.onap.cps.ncmp.rest.exceptions.NetworkCmProxyRestExceptionHandlerSpec.MethodType.NCMPINVENTORY
...
class NetworkCmProxyRestExceptionHandlerSpec extends Specification {
    ...
    def 'Post request with generic #scenario exception returns correct HTTP Status.'() {
        when: 'an exception is thrown by the service'
        ...
        where:
            scenario         | exception                                               || expectedErrorDetails | expectedErrorCode
            'NCMP-inventory' | new DataValidationException(errorMessage, errorDetails) || errorDetails         | BAD_REQUEST
    }

    def setupTestException(exception, methodType) {
        if (NCMP.equals(methodType)) {
            mockNetworkCmProxyDataService.getYangResourcesModuleReferences('testCmHandle') >> { throw exception }
        } else if (NCMPINVENTORY.equals(methodType)) {
            mockNetworkCmProxyDataService.updateDmiRegistrationAndSyncModule(*_) >> { throw exception }
        }
    }

    def performTestRequest(methodType) {
        if (NCMP.equals(methodType)) {
