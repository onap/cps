...
class NetworkCmProxyRestExceptionHandlerSpec extends Specification {
    ...
    def 'Request returns correct HTTP status and body for NCMP-passthrough scenarios'() {
        when: 'an exception is thrown by the service'
            mockNetworkCmProxyDataService.getResourceDataPassThroughRunningForCmHandle(*_) >> { throw new HttpRequestException('Error Message Details NCMP', 400, 'Bad Request from DMI') }
            def response = mvc.perform(get("$dataNodeBaseEndpointNcmp/ch/testCmHandle/data/ds/ncmp-datastore:passthrough-running?resourceIdentifier=stores:bookstore/categories=100"))
                .andReturn().response
        then: 'NCMP service responds with correct HTTP status'
            response.status.equals(HttpStatus.BAD_GATEWAY.value())
        and: 'DMI responds with correct status code and body'
