...
class NcmpDatastoreRequestHandlerSpec extends Specification {
    ...
    def 'Attempt to execute async get request with #scenario.'() {
        ...
        and: 'wait a little for async execution'
            Thread.sleep(200)
    ...
    def 'Attempt to execute async data operation request with #scenario datastore.'() {
        ...
        and: 'wait a little for async execution'
            Thread.sleep(200)
        then: 'the task is executed or not'
            expectedCalls * spiedCpsNcmpTaskExecutor.executeTask(*_)
            expectedCalls * mockNetworkCmProxyDataService.executeDataOperationForCmHandles('myTopic', _, _)
        and: 'the correct error is thrown or not'
           if (expectedErrorMessage == null) {
               assert thrown == null
           } else {
               thrown instanceof InvalidDatastoreException
               assert thrown.message.contains(expectedErrorMessage)
           }
        where: 'the following datastores are used'
            scenario      | datastore                               || expectedCalls | expectedErrorMessage
            'supported'   | 'ncmp-datastore:passthrough-running'    || 1             | null
            'supported'   | 'ncmp-datastore:passthrough-operational'|| 1             | null
            'unsupported' | 'ncmp-datastore:operational'            || 0             | 'not supported'
            'invalid'     | 'invalid'                               || 0             | 'invalid datastore name'
    }
