...
class ProvMnsControllerSpec extends Specification {
    ...
    def 'Put Resource Data from provmns interface.'() {
        given: 'resource data url'
            def putUrl = "$provMnSBasePath/v1/someurl"
        and: 'a valid jason body containing a valid Resource instance'
            def jsonBody = '{ "id": "some-resource", "objectClass": "SomeClass" }'
    ...
    def 'Patch Resource Data from provmns interface.'() {
        given: 'resource data url'
            def patchUrl = "$provMnSBasePath/v1/someurl"
        and: 'a valid jason body containing a valid Resource instance'
    ...
    def 'Delete Resource Data from provmns interface.'() {
        given: 'resource data url'
            def deleteUrl = "$provMnSBasePath/v1/ldnFirstPart/someClass=someId"
        ...
        and: 'the policy executor invoked'
            1 * policyExecutor.checkPermission(_, OperationType.DELETE, null, 'ldnFirstPart', _)
