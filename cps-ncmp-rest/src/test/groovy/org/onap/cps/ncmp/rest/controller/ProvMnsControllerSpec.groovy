...
class ProvMnsControllerSpec extends Specification {
    ...
    def 'Get Resource Data from provmns interface #scenario.'() {
        given: 'resource data url'
            def getUrl = "$provMnSBasePath/v1/UriLdnFirstPart/ClassName=id"+path
        and: 'request classes return correct information'
            inventoryPersistence.getYangModelCmHandle("cm-1") >> new YangModelCmHandle(dmiServiceName: "someDmiService", dataProducerIdentifier: 'UriLdnFirstPart/ClassName=id')
            alternateIdMatcher.getCmHandleId("UriLdnFirstPart/ClassName=id") >> "cm-1"
    ...
    def 'Patch Resource Data from provmns interface.'() {
        given: 'resource data url'
            def patchUrl = "$provMnSBasePath/v1/UriLdnFirstPart/ClassName=id"
    ...
    def 'Put Resource Data from provmns interface.'() {
        given: 'resource data url'
        def putUrl = "$provMnSBasePath/v1/UriLdnFirstPart/ClassName=id"
        and: 'an example resource json object'
        def jsonBody = jsonObjectMapper.asJsonString(new ResourceOneOf('test'))
    ...
    def 'Delete Resource Data from provmns interface.'() {
        given: 'resource data url'
            def deleteUrl = "$provMnSBasePath/v1/UriLdnFirstPart/ClassName=id"
