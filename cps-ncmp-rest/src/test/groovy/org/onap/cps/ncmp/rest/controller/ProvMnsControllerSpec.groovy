...
class ProvMnsControllerSpec extends Specification {
    ...
    @SpringBean
    RequestValidator requestValidator = new RequestValidator(new ErrorResponseBuilder())
    ...
    def 'Get resource data request where #scenario'() {
        ...
        where:
            ...
            'dataProducerId is empty'                                | ''                   | CmHandleState.READY   || HttpStatus.UNPROCESSABLE_ENTITY
            'dataProducerId is null'                                 | null                 | CmHandleState.READY   || HttpStatus.UNPROCESSABLE_ENTITY
            'cmHandle state is Advised'                              | 'someDataProducerId' | CmHandleState.ADVISED || HttpStatus.NOT_ACCEPTABLE
            'cmHandle state is Ready with populated dataProducerId'  | 'someDataProducerId' | CmHandleState.READY   || HttpStatus.OK
    }
    ...
    def 'Get resource data request with no match for alternate id'() {
        ...
        and: 'persistence service returns yangModelCmHandle'
            inventoryPersistence.getYangModelCmHandle('cm-1') >> new YangModelCmHandle(id:'cm-1', dmiServiceName: 'someDmiService', dataProducerIdentifier: 'someDataProducerId', compositeState: new CompositeState(cmHandleState: CmHandleState.ADVISED))
        when: 'get data resource request is performed'
            def response = mvc.perform(get(getUrl).contentType(MediaType.APPLICATION_JSON)).andReturn().response
    ...
    def 'Put resource data request with no permission from coordination management'() {
        ...
        and: 'alternate Id can be matched'
            alternateIdMatcher.getCmHandleIdByLongestMatchingAlternateId('/someUriLdnFirstPart/someClassName=someId', "/") >> 'cm-1'
        and: 'persistence service returns valid yangModelCmHandle'
            inventoryPersistence.getYangModelCmHandle('cm-1') >> new YangModelCmHandle(id:'cm-1', dmiServiceName: 'someDmiService', dataProducerIdentifier: 'someDataProducerId', compositeState: new CompositeState(cmHandleState: CmHandleState.READY))
