...
public class NfProxyController implements NfProxyApi {
    ...
    @Override
    public ResponseEntity<Object> getNodeByCmHandleIdAndXpath(final String cmHandleId, @Valid final String xpath,
                                                              @Valid final Boolean includeDescendants) {
        if (ROOT.equals(xpath)) {
            return new ResponseEntity<>(HttpStatus.NOT_IMPLEMENTED);
        }
        final FetchDescendantsOption fetchDescendantsOption = Boolean.TRUE.equals(includeDescendants)
            ? FetchDescendantsOption.INCLUDE_ALL_DESCENDANTS : FetchDescendantsOption.OMIT_DESCENDANTS;
        final DataNode dataNode = nfProxyDataService.getDataNode(cmHandleId, xpath, fetchDescendantsOption);
